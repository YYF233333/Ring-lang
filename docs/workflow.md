...
1. 启动前依据同类历史耗时、当前范围和机器负载给出保守预估。需要后续分析的完整输出一次性重定向到临时文件；命令只启动一次。
2. 预计耗时达到 **5 分钟**时，启动后不得用短间隔 `wait`、进程查询或日志读取反复探测。若没有可安全补位的独立工作，直接按预计完成时间进入一次可中断的 dormant wait / sleep；首次完成检查只能发生在该计划等待结束后。不得用连续短 `wait` 模拟这次 sleep。
3. 每条命令在当前执行上下文中维护等待时长和轮询计数，不写 Inbox。任何仅为确认“是否结束”而进行的 `wait` 返回、进程/任务状态查询或增量日志读取都算一次轮询；同一轮同时查状态和日志仍按多次工具调用计，不得换工具规避计数。
4. 第 3 次完成检查后命令仍未结束，说明原预估已经失效。第 4 次检查前必须重估剩余时间，下一次 sleep 至少为上一次实际 sleep 的 2 倍；以后每次仍未结束都继续至少 2 倍退避，禁止固定频率轮询。只有明确的新证据表明命令将在更早的确定时点完成，才可缩短一次等待，并记录该证据。
5. 平台有单次等待上限时，优先使用可中断的事件完成通知、deferred wait 或定时唤醒。只能分段等待时，每段使用平台允许的最大时长，退避到该上限后保持上限；不得在段间追加状态/日志查询，也不得把平台上限变成高频轮询。用户明确询问、命令转为全局阻塞或结果改变结论时才报告状态。
