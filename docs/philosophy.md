...
> 唯一真值源（2026-06-12 由四条扩为六条：可判定性与资源语义两条隐性地基成文；同日全面审视后重构为三层结构，⑦ 自 ⑥ 的 GC 记录、⑧⑨ 自「语法原则」升格成文）。design.md/CLAUDE.md/README 只放指针或速查，不重复全文。**编号是标识符不是优先级**——永不复用、永不重排，层级是元数据。

### 层级与仲裁

| 层 | 性质 | 条目 | 检验方式 |
|----|------|------|---------|
| 0 | 目标函数（北极星） | ④ 无人回路 × 全场景 | 测量（B-111 / B-089 类可证伪锚点） |
| 1 | 硬约束（预承诺） | ⑤ 可判定终止 ⑥ 确定性资源 ⑦ 场景不可堵死 | 当场可裁定（可判定性/确定性/架构事实，无需测量） |
| 2 | 设计策略（服务层 0） | ① 类型即模型 ② 效果即可见性 ③ 推断为王 ⑧ 一种写法 ⑨ 语法借用 | 可被层 0 的测量推翻 |

分层依据（认识论不对称）：约束今天就可检验，目标只能渐近测量。若层 0 词法优先（冲突时自动赢），每场特性辩论都会退化为不可证伪的「这有助于 agent」断言——⑤⑥⑦ 的存在意义正是**预承诺**：推导一次、永久生效，抵抗「为了 agent 通融一次」的逐案侵蚀。目标不压倒约束，目标在约束内优化。

仲裁规则（记录入 design.md 附录「公理仲裁决策表」）：
1. **策略 vs 约束** → 约束赢，无需讨论。
2. **策略 vs 策略** → 用层 0 判据（对 agent 错误率/迭代轮数的影响）仲裁；无测量时凭判断，记入决策表。
...
对 ④ 具名场景集的部署路径，设计决策不得不可逆地堵死。当前兑现形式：LLVM native、零强制 runtime、C ABI FFI。与 ⑤⑥ 同为当场可检验——「这个设计需要强制 runtime 吗」是今天即可裁定的架构事实，无需等测量。不可逆性是本条成立的根基：堵死是单向门（与 ⑥ GC 记录的「不可逆性不对称」同一逻辑）。
...
每当一种操作出现第二种写法，必须删掉一种；两种都有道理时，删语法更重的那个。动机：LLM 在多写法语言（TS 的 interface/type/class/literal 四种数据定义）中每次选不同的，大型代码库风格混乱、上下文压缩失效。实战否决记录为全体公理最强：`|>`、`?`、`try`、`or`、`|x|` lambda、`++`（实例库见下文「语法原则」与 design.md 已落地决策表）。2026-06-12 D-6 自「语法原则」升格成文。

### 9. 语法借用，最大化知识迁移（层 2 · 策略）

不发明新关键字，除非语义确实是新的——Ring 特有语法仅 `handle...with` 与 `catch { arms }`（algebraic effect handler 在主流语言无对应物）。这是核心赌注最大风险（零训练数据）的主要缓解：LLM 的先验知识直接可用，错误率不因语法陌生而上升。借用清单见下文「语法原则」。2026-06-12 D-6 自「语法原则」升格成文。

### 性能的地位（非公理，2026-06-12 D-6 拍板）

没有任何公理承诺速度——这是显式决定，不是疏漏。性能是工程目标：与任何公理冲突时性能让步；但受 ⑥（确定性资源 = RC 无 GC headroom、FBIP 路径）与 ⑦（native、零强制 runtime）间接保护——按公理推导出的架构天然不慢。perf 项（B-079/B-080 类）的优先级锚点 = 层 0 判据（agent 工作流的实际瓶颈），不独立成轴。
