...
- **`while` / `for..in` / `break` / `continue`**：Phase 3a Batch 1 实现。while 条件必须为 Bool，`..` 为 Rust 风格中缀运算符（Prec.Range），for..in 仅支持 Range 迭代器，codegen 缓存 end 表达式避免重复求值
- **Checker scope 安全**：所有 `push_scope`/`pop_scope` 用 try/finally 保护，type_error 抛出时自动清理

### 未实现特性优先级

**高（阻塞实际程序）**：模块系统、字符串方法、List\<T\>、Map\<K,V\>
