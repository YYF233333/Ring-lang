...
**A2 设计边界已锁（2026-06-07 Discussion，贴码核定，B-102 Phase 2）**：A2 = hash-cons Type 去重达 G-a，但**只在 `apply_subst` 的复合构造 arm intern**（洪流唯一来源，167 处 `let x=apply_subst`），intern 表塞进 `UnionFind`（已穿到每个调用，零线程化）。**⚠️ 订正本节 / backlog 旧述「apply_subst 输出已 resolve」——不成立**：`apply_subst`（`env.ring:515` TypeVar arm）会原样穿出未绑定 var，上层复合 arm 无条件重建 spine → 输出可含残留 var。故 intern 边界 = **含未解析 TypeVar 就不入表**（只 ground/resolved 入表，仍吃洪流主体）。intern key = 专门的 `type_intern_key(t) -> Str`（Ring Map key 走 `===` 不能拿 Type 当 key），逐字对齐 `types_equal`（struct/enum nominal-shallow 只编码 name+type_params、无序集字段先排序、不复用有损的 `type_to_string`）。**前置不变量门**：Type 须为不可变共享 DAG（无就地 mutate、无 `===` 身份比较），动手前实跑 + 搜码确认。「全构造点 intern 拿 O(1) 类型相等」与内存门解耦、另立项。详见 backlog B-102 Phase 2 D1–D4。
...
| A2 Type hash-cons 边界（2026-06-07，B-102 Phase 2）| 只在 `apply_subst` 复合构造 arm intern（洪流唯一源，167 处），表塞 `UnionFind`（零线程化）；**含未解析 TypeVar 不入表**（订正旧述「apply_subst 输出已 resolve」，`env.ring:515` 证未绑定 var 会穿出）；intern key = 专门 `type_intern_key`，逐字对齐 `types_equal`（struct/enum nominal-shallow、无序集排序、不复用有损 `type_to_string`）；前置不变量门：Type 须不可变（无就地 mutate / 无 `===` 身份比较）；O(1) 类型相等彩蛋与内存门解耦另立项 | apply_subst 每次无条件重建 spine = 2.51亿洪流；intern 去重达 G-a（A1 never-drop 不去重仍 ~25.9GB）。边界全取最保守侧绕开 var 时序坑；key 不对齐 types_equal 会漏 dedup 或破坏推断 |
