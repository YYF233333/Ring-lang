...
#### Refinement × Linear（正交）

Refinement 是值级谓词——描述值本身的性质（`Int where it > 0`），不引用外部可变状态。Linear 是使用计数——值必须恰好消耗一次。两者正交。
...
#### Linear × Effect（RAII / Drop trait）

Linear 值在 fail-capable 作用域内必须有确定的释放路径。采用 RAII 模型：
...
编译器自动插入 drop 调用的三个路径：
1. **作用域退出**（正常流）
2. **fail abort 路径**（unwind）
3. **async cancel 路径**（`Cancelled` fail）

Linear 值与 Drop 的关系：

| 情况 | 行为 |
|------|------|
| Linear 值 + 有 Drop | 正常路径必须显式消耗，abort 路径自动 drop（实质 affine + Drop 兜底） |
| Linear 值 + 无 Drop | 必须恰好消耗一次；fail-capable 作用域内无 Drop 覆盖 → 编译错误 |

**不引入 `defer` 关键字**——RAII 覆盖了资源清理的所有场景。`Drop::drop` 禁止 `fail` effect，避免"清理时抛异常"问题。

#### Linear × async（Move 语义）

`spawn` 闭包捕获 linear 值 = ownership 转移。原作用域失去访问权（编译错误）。

- 一个 linear 值不能被两个 `spawn` 闭包同时捕获（违反 linearity，编译错误）
- 子任务取消时，被取消的任务走 `Cancelled` fail 路径 → 触发 Drop（同 Linear × Effect 规则）
...
| Refinement × Linear × Effects 交互 | 详见 1.5 交互矩阵 | 三系统正交 + RAII（Drop trait）处理 linear 值在 abort 路径的释放 |
| Linear 值资源释放 | RAII（Drop trait），不引入 defer | Drop 自动触发（不可忘记）、跟类型走（写一次）、泛型友好；Drop::drop 禁止 fail effect |
...
**中（增强表达力）**：关联类型（~~`delegate`~~ ✅、~~supertrait 继承~~ ✅ 已实现）
