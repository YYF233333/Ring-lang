...
**双层策略**：日常开发用 `Int`/`Float`，精确宽度类型用于 FFI/布局/性能关键场景。

| 层 | 类型 | 用途 |
|----|------|------|
| 语义层 | `Int`（= I64 别名）、`Float`（= F64 别名） | 日常开发，字面量默认类型 |
| FFI/布局层 | I8, I16, I32, I64, I128, U8, U16, U32, U64, U128, F32, F64, ISize, USize | C FFI、协议头、密码学、内存布局控制 |

**字面量规则**：
- `42` → `Int`，`3.14` → `Float`，永远确定，无歧义
...
- `u8_val + int_val` → 编译错误，要求 `u8_val.to_int() + int_val`
- 算术运算只在同类型之间
- 理由：日常代码只用 Int/Float 不碰宽度问题；碰到窄类型说明在 FFI/性能层，显式转换是预期的
...
| 维度 | Ring 语义 | 备注 |
|------|----------|------|
| **整数范围** | Int = I64 = ±2^63，其余各类型固定宽度 | JS 后端只有 Int（受限于 ±2^53），已知差异 |
...
**值表示**：Uniform boxing——所有 Ring 值在 LLVM 层面统一为 `void*`（含 Int/Float/Bool）。极简 codegen，容器天然类型擦除，闭包捕获统一，递归类型自然工作。性能不是 bootstrap 阶段的目标。Perceus RC 阶段再引入 unboxing 优化。
...
1. 声明 LLVM FFI 层（`extern fn` + `extern type` 包装 LLVM-C 核心 API，~50-80 个函数）
2. 编写 Ring LLVM codegen 模块（消费 HProgram，调用 LLVM builder API）
3. 本地编译 N-API addon（~300 行 C++，纯机械转发，不入仓库）
