...
function load_config(path, __h_io, __h_fail) {
    const raw = __h_io.read(path);
    const data = __h_fail.try_(() => json_parse(raw));
    return data;
}
...
- 基础 LSP server（hover 显示推断类型和 effect）
...
**高（阻塞实际程序）**：`loop`/`for`/range、模块系统、LSP server

**中（增强表达力）**：enum named fields、`map_fail`、关联类型、`delegate`、Checker 多错误恢复
