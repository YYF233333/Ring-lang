...
- **Str 内置方法**：Phase 3a Batch 2 实现。16 个纯方法（len/contains/starts_with/ends_with/slice/trim/to_upper/to_lower/replace/split/char_at/index_of/byte_at/pad_start/pad_end/repeat/char_code_at），通过 `impl_methods.set("Str", ...)` 注册，infer_method_call 扩展支持 primitive type 方法查找。**`replace` 是全替换语义**（对应 JS `replaceAll`，非 `replace`），即替换所有匹配项而非仅第一个——这是有意设计，JS 用户应注意此差异
...
- **Map\<K,V\> 内置类型**：Phase 3a Batch 3 实现。StructType 表示、JS Map 底层、不可变值语义。`map_from([(k,v)])` / `map_new()` 构造。13 个方法：读取 7（len/get/contains_key/is_empty/keys/values/entries）+ 变换 2（insert/remove）+ HOF 4（map_values/filter/fold/any，effect 多态，IIFE 内联生成）。key 使用 JS `===`，待 Eq trait 升级
- **Set\<T\> 内置类型**：Phase 3a Batch 3 实现。StructType 表示、JS Set 底层、不可变值语义。`set_from(list)` / `set_new()` 构造。13 个方法：读取 4（len/contains/is_empty/to_list）+ 变换 5（insert/remove/union/intersect/difference）+ HOF 4（filter/fold/any/all，effect 多态）。for..in 迭代编译为 `for (const x of set)`
