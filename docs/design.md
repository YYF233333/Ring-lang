...
**Type-DAG 所有权特例 = intern + 永不 drop（B-101，2026-06-05 锁定方案 A）**：上面「实现订正」的 crash-free 仅经 GATE 1（a_empty，不碰 prelude）+ GATE 2（JS/llvm_diff 用 node 编译器）验证；**Wave A 用 native 编真实程序证伪了 Type-DAG 的 crash-free**——`ring_dup` 浅 +1 最外层 vs `Type` `drop_T` 深递归 drop（type_params/fields/variants）+ `apply_subst` 故意共享子结构（不 dup）→ owner-bearing / fresh-constructor 的 Type 绑定 scope-end-drop 时**深递归 over-free DAG 共享子结构** → apply_subst 后续 `uf_lookup` 读悬垂 Type 崩（native prelude 全空、self-compile 自 L0/B-012 从未成功）。**方案 (A)**：Type **不走通用 clone-all-escape RC**，特例化为 **intern (hash-cons) + 永不 drop**——构造走全局 intern 表（结构相同共享一份指针），Type typeid 的 drop = no-op。永不 drop → 结构性消除 UAF；intern 去重 → 2.51亿棵 apply_subst Type 塌缩成有界类型集 → G-a（附赠 O(1) 类型相等）。这是 GHC/OCaml/LLVM 对不可变共享类型表示的标准做法（不可变 → intern 安全）。通用 clone-all-escape RC 仍管其余值；其余共享返回（subst/UnionFind/HIR pass-through）继续保守不 drop（小泄漏，留 B-096/L3）。两步：A1 排除 drop 解锁 native 正确性 → A2 hash-cons 达内存门。详见 backlog B-101。

**A1 已落地（2026-06-05 Worker）**：runtime `ring_register_never_drop(typeid)` + `never_drop_table`（`ring_dup`/`ring_drop` 对标记 typeid no-op，永不 free）；codegen `emit_drop_functions`/`emit_drop_registrations` 对 Type-DAG 类型集（`Type`/`Effect`/`EffectRow`/`StructField`/`EnumVariant`/`RecordField`/`RowMergeResult`，判据 `hir::is_type_dag_type_name`）不生成递归 `drop_T`、改注册 never-drop；perceus 对 Type-typed 绑定不发 Clone/Drop（`is_type_dag_type`）。撤 Wave A 的 `is_fresh_owned_returning_call` 空 hook（保留 in-code DEAD ROAD 注释）。**实证**：Type-DAG over-free 段错误消除（real_program 不再因 Type drop 崩）。**A1 暴露的后续 over-free（pre-existing，被空 prelude 早先掩盖）**：(a) 上面订正 #3 的 enum 变体构造器 call-arg over-free——**已修**（解锁空 prelude，std prelude 现可加载并进入类型解析）；(b) `resolve_type_expr` 读 `TypeExpr` 的间歇堆损坏——**已修（B-102 Phase 1，git `1a6b1d7`）**：根因 = codegen 同名 local 共享 function-entry alloca，旧的「整块 block_locals 一次性纳入 owned + 全 scope-drop」在「该名未构造的分支」over-free（layer-3 `resolve_type_expr` 读 freed `TypeExpr` / layer-4 owned 别名污染 sibling 分支 / layer-5 `ring_list_extend` 共享元素双释放）；修法 = perceus `rc_block_inner` owned 集改增量可见（binding 只从 `let` 起进 visible_owned，新 `stmt_droppable_locals`）+ runtime extend 元素 escape-dup。**over-free（UAF）链终结，ASan-clean ×3**；余 self-compile OOM = G-a 泄漏（归 A2）。

**A2 设计边界已锁（2026-06-07 Discussion + 实现，贴码核定，B-102 Phase 2）**：A2 = hash-cons Type 去重达 G-a，**只在 `apply_subst` 的 5 个复合 arm intern**（FnType/GenericType/RecordType/EffectRowType/TupleType；洪流主体 167 处 `let x=apply_subst`），intern 表塞进 `UnionFind`（已穿到每个调用，零线程化）。**⚠️ Struct/Enum 排除（实现中发现的健全性洞，方案 A）**：原计划 nominal-shallow（key 只 `name+type_params`，忽略 variants/fields）**不健全**——`apply_subst` 不代换 variants/fields（`variants: variants`，即下面「实现订正」/#45 的故意行为），但 `exhaustive.ring:128` **结构化读 variants** 拿 payload 类型做嵌套穷尽检查；intern 按 `name+type_params` 任选一份缓存 → stale 泛型 `T` payload 泄漏 → 回归 `tuple_option_sugar.ring`。「key 不在被读字段上、apply_subst 又不代换该字段」= 健全性洞（非身份比较问题）。排除后 731 全绿，dedup 损失边际（Struct/Enum 的 `type_params` 深层子树仍经递归子节点 intern，只外壳不共享）。否决「让 apply_subst 代换 variants/fields」（撞决策表「fields 遍历致递归类型栈溢出」一条）+「deep key」（违反 key==types_equal + O(size²) 无限递归）。**⚠️ 另订正旧述「apply_subst 输出已 resolve」——不成立**（`env.ring:515` TypeVar arm 会原样穿出未绑定 var）→ intern 边界 = **含未解析 TypeVar 就不入表**（`type_intern_key(t) -> Str?` 含 var 返回 none）。key 逐字对齐 `types_equal`：Record/EffectRowType 无序集先排序，**FnType effects 有序**（`effects_list_equal` 按位，非无序），不复用有损 `type_to_string`。**前置不变量门已过**（两轮只读核查）：Type 是不可变共享 DAG（无就地 mutate 键内字段、无 `===` 身份比较、三处注册期 in-place push 对本设计良性）。「全构造点 intern 拿 O(1) 类型相等」与内存门解耦、另立项。详见 backlog B-102 Phase 2 D1–D4。
