...
16 个固定宽度类型，无 `Int`/`Float` 别名：`I8, I16, I32, I64, I128, U8, U16, U32, U64, U128, F32, F64, ISize, USize`。

字面量默认推断：`42` → `I64`，`3.14` → `F64`。上下文需要固定宽度时推断为对应类型。

隐式转换仅限安全 widening：同族窄→宽（`I8→I64`、`U8→U64`、`F32→F64`）+ 跨族 `U{N}→I{M}`（M>N）。其余显式 `.to_xxx()`。
...
| 维度 | Ring 语义 | 备注 |
|------|----------|------|
| **整数范围** | 各类型固定宽度（I64 = ±2^63，I32 = ±2^31 等） | JS 后端只有 I64（受限于 ±2^53），已知差异 |
