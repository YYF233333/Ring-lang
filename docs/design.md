...
**Type-DAG RC 试错结论（2026-06-05→06-07，never-drop / intern 全套已撤销；过程细节见 git history + backlog B-102 归档块）**：Wave A 用 native 编真实程序证伪了 Type 走通用 RC 的首版落地——`ring_dup` 浅 +1 最外层 vs `drop_T` 深递归 + `apply_subst` 故意共享子结构（不 dup）→ scope-end-drop 深递归 over-free DAG 共享子树 → `uf_lookup` 读悬垂 Type 崩（native prelude 全空的根因之一）。此后四条路先后失败：① **A1 never-drop**（Type-DAG typeid 的 drop = no-op，曾落地）——结构性消除 UAF 但永不回收 → native 自编译线性爬 15GB；② **A2 intern wrap-after-build**（父节点先构造再查表，git `68a56e9`）——never-drop 下命中时丢弃的副本永不释放 → intern 零内存收益；③ **A2 intern lookup-before-build**（先算 key 查表、命中零分配，git `574b92d`）——非-interned Type（含-var transient + 非-apply_subst 构造）仍无界泄漏，根因 = never-drop 本身不回收内存；④ **`let x = <Call 结果>` 保守不 drop**（实现订正 #2 的通用 Call arm）——瞬态不回收即 G-a 泄漏，激进放开又误 drop borrow-返回 call → UAF，正解 = B-103 return-mode 分类（见下段）。**现行真值 = R-clean pure Perceus RC：撤 A1/A2 全套 hook，Type 回 clone-all-escape（B-102 Phase 2 rev，git `27fe62d`，见下段）；intern 留作后续纯性能优化。** 日后重启 intern 的三条硬约束（本轮实证）：key 逐字对齐 `types_equal`（FnType effects 有序、Record/EffectRow 无序集先排序，不复用有损 `type_to_string`）；含未解析 TypeVar 不入表（`env.ring:515` 未绑定 var 会原样穿出，「apply_subst 输出已 resolve」不成立）；Struct/Enum 不可 nominal-shallow key（`apply_subst` 不代换 variants/fields 而 `exhaustive.ring:128` 结构化读 variants → stale 泛型 payload 泄漏，「key 不在被读字段上、apply_subst 又不代换该字段」= 健全性洞）。
