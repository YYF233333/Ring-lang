...
> **2026-06-05 Worker Wave A：Phase 1 续做中。** 在 head-start worktree `worktree-agent-a33863947e108b385`（commit `dabccab`，含完整 native 工具链 + addon）resume——重验半成品修复（perceus.ring +81 增量可见 owned 集 / ring_runtime.cpp +22 ring_list_extend escape-dup）+ 续 ASan 链猎杀至 native `real_program.ring` EXIT 0 ×3。Phase 2（A2 hash-cons）gate 在 native 正确性打通后，本轮不做。B-089 阻塞跳过。
...
- **native 正确性打通后做**（否则无法实测 peak RSS）。全局 intern 表，Type 构造点走 `intern_type`（env.ring apply_subst 各 arm / types.ring / infer.ring）；TypeKey = 结构哈希 + 相等。设计见 design.md §7.11「Type-DAG 所有权特例」。
- **⚠️ unification var 边界**：只 intern ground/resolved Type（apply_subst 输出已 resolve，2.51亿洪流主体）；含未解析 TypeVar 的 transient 不 intern。非 trivial 边界写 `worker_feedback` [决策]。
- 2.51亿 Type 塌缩成有界类型集 → peak RSS << 25.9GB（A1 永不 drop 不去重、内存仍 ~25.9GB，靠 A2 去重才达 G-a）。附赠 O(1) 类型相等。

**涉及修改**：
1. native 构建脚本：ASan 变体（`-fsanitize=address`）。
2. `compiler/perceus.ring` + 相关：逐层 over-free 修（escape-分类 / ownership-mismatch）。
3. `compiler/env.ring` / `types.ring` / `infer.ring`：Type intern 表 + `intern_type`（Phase 2）。
4. `tests/`：每层 over-free 的 native 回归。
