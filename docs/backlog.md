...
| **B-101** | **Type-DAG 所有权 = intern + 永不 drop — native 自举真 blocker：永不 drop 杀 pre-existing L0 prelude UAF（shallow-dup/deep-drop 不对称）+ hash-cons 去重杀 apply_subst 无界泄漏（2.51亿→有界）；A1 解锁正确性→A2 达 G-a；native E2E×3 已建。A1 + enum 构造器 call-arg over-free 已落地（解锁空 prelude），但 native self-compile 卡第三层独立 `TypeExpr` over-free** | **G-a/c** | **P1（native-working 硬前置，2026-06-05 锁方案 A，A1 落地，waiting-feedback）** |
...
### B-101 Type-DAG 所有权 = intern + 永不 drop（native-working 硬前置）[bugfix] [P1] [L] [judgment] [waiting-feedback]

> 2026-06-05 立项（Discussion，两个诊断 subagent 实证）。**native 自举的真 blocker**——解锁 B-089 G-a/G-c。Wave A 证伪务实白/黑名单后，**2026-06-05 Discussion 锁定方案 (A) Type intern/hash-cons + 永不 drop**（见下「方案 (A)」节）。

> **⚠️ A1 已落地 + fix-forward 修了 enum 构造器 call-arg over-free，但 native self-compile 卡在第三层独立 over-free（`TypeExpr`）→ 转 `waiting-feedback`（2026-06-05 Worker）。** A1（Type-DAG never-drop runtime/codegen/perceus 三处）已消除 Type-DAG over-free 段错误；fix-forward 修了 `some(x)`/变体构造器 call 语法的 owned-arg over-free（**这是空 prelude 的真根因**——`find_std_dir` 的 `some(dir)` 载荷被提前释放 → std prelude 静默全失败），解锁 prelude 加载推进进类型解析。**但 native 编任何用 prelude 的程序现崩在 `resolve_type_expr` 读 `TypeExpr`（十亿级垃圾 tag / 栈溢出 / 间歇）= 第三类 pre-existing over-free，已证伪非 Type-DAG/非构造器类，需 native 调试逐点定位。** 验证全绿：JS 731×3、llvm_diff 51×3（含新回归 `variant_ctor_owned_arg.ring`）、double-bootstrap 字节一致、对 JS/llvm_diff 零回归。**A2（hash-cons / G-a 内存门）尚未动**（A1 内存仍 ~25.9GB）。决策点（继续 native UAF 链猎杀 vs 收口拆 B-102 + A2）详见 `worker_feedback.md` [决策] B-101。
...
- **⚠️ A2 主设计风险 = unification var**：HM 推断期 Type 含未解析 `TypeVar`（会 resolve），intern 可变结构有讲究。务实边界：**只 intern ground/resolved Type**（apply_subst 输出已 resolve，正是 2.51亿洪流主体）；含未解析 TypeVar 的 transient type 不 intern（量少）。或 TypeVar identity = union-find root id。实现时定，遇非 trivial 边界写 `worker_feedback` [决策]。
