...
| **B-104** | **完整 Perceus RC（total drop pass + 静态 verifier）= 当前里程碑、G-a 真解；当前步 D4** | G-a | **P1 doing** |
...
**Phase 2 (rev) — R-clean（✅ 已落地，git `27fe62d`；spec 细节缩编 2026-06-12，全文见 git history + design.md §7.11「Type-DAG RC 试错结论」）**：撤 A1 never-drop 全部 hook + 撤 A2 intern，Type 回 clone-all-escape（codegen 生成完整递归 `drop_T`）；命门 = dup-on-share 完整性，由 clone-all-escape 自动盖全 + ASan 驱动逐点验证。内存模型：peak = 同时存活 Type 集（非累积）。intern 留作后续纯性能优化另立项（重启三条硬约束见 design.md §7.11）。
...
**既有现象备忘（2026-06-12，worker [观察] 归档）**：dist-llvm 自编译现有 970 条 `unknown function 'LLVM*'` warning（base 即有、非回归——llvm_ffi extern fn 在 JS-host 编译路径下的输出形态）。本项落地（LLVM-C extern 真实 declare）后应消失，作回归观察点。
