...
### B-049 Codegen 声明遍历提取递归 helper [refactor] [P1] [M] [queued]
`codegen.ring` 中 `local_names` 收集、`struct_field_order` 收集、`impl_methods` 注册、`local_fn_effects` 收集等多处有几乎相同的 top-level → ModBlock → sub-ModBlock 三层嵌套 match。每次新增声明类型需在 3+ 处同步添加分支，遗漏导致 #87（impl 方法被跳过）。

**修复方向**：提取通用递归 helper（如 `for_each_decl(decls, callback)` 或 visitor 模式），统一遍历所有层级的声明。各收集逻辑只需提供回调，不再关心 ModBlock 嵌套结构。

**涉及修改**：
1. `codegen.ring`：新增递归声明遍历 helper 函数
2. `codegen.ring`：`local_names`（line 74-142）改用 helper
3. `codegen.ring`：`local_fn_effects` / `collect_local_calls`（line 147-180）改用 helper
4. `codegen.ring`：`struct_field_order` / `impl_methods` 注册改用 helper
5. 确认覆盖所有使用 3 层嵌套 match 的位置

**验收标准**：
- 所有原有的 3 层嵌套 match 替换为 helper 调用
- #87 的根因消除（impl 方法不再因嵌套遗漏被忽略）
- 新增声明类型只需在 helper 的回调中处理一次
- 全部 E2E 测试通过
- 自举编译器正常编译自身

### B-050 `insert_mod_aliases` 增量化 [refactor] [P1] [M] [queued]
`insert_mod_aliases` 当前在 `register_phase1` 的 pass 1b **之后** batch 运行，导致 pass 1b 内部的跨声明引用（如 supertrait 查找）无法使用短名 alias。这是 #83（supertrait 在 mod 内查找失败）的根本原因，也是观察 1（模块交互 bug 热点）中多个 bug 的系统性成因。

**修复方向**：将 `insert_mod_aliases` 从 batch 执行改为增量执行——在每个注册子步骤（struct/enum 注册、trait 注册、effect 注册等）完成后立即插入对应的短名 alias，而非等全部注册完成后统一插入。

**涉及修改**：
1. `infer_register.ring`：拆分 `insert_mod_aliases` 为按类别的增量版本（`insert_mod_aliases_types`、`insert_mod_aliases_traits`、`insert_mod_aliases_effects` 等），或改为每注册一个声明就立即插入对应 alias
2. `infer_register.ring`：调整 `register_phase1` 中各 pass 的调用顺序，确保 alias 在消费方之前可见
3. 验证 #83 修复：mod 内 supertrait 查找使用短名正常工作
4. 验证不引入新的注册顺序问题（alias 指向的定义必须已注册）

**验收标准**：
- #83（mod 内 supertrait 短名查找失败）修复
- mod 内 trait/effect/struct 之间的跨声明引用使用短名正常工作
- 不依赖注册完成后的 batch alias 插入
- 全部 E2E 测试通过
- 自举编译器正常编译自身
