...
### B-080 标量 Unboxing (L4) — G-a 内存墙真解 [feature] [P1] [XL] [judgment] [queued]
Int/Float/Bool 从 uniform-boxed 堆 ptr 改为寄存器内联值，消除标量的 alloc + RC 流量。当前 uniform boxing 一切皆 `void*`（含标量：Int=boxed i64、Bool=boxed i1、Float=boxed f64，各带 typeid header）。

> **2026-06-08 提 P1（用户拍板 native G-a 攻坚），并入 condition-Bool 漏的修复**。**精确-RC 四波（B-104 W1/W2/W3a/W4）已尽，数据确证 B-080 是 G-a 内存墙的真解、非性能优化**：原"非内存墙解法"注被实测推翻。残留 leak 绝对主导 = INT 63M（@402M，四波纹丝不动）= ② 合法存活装箱标量（HIR/AST 的 Span.start/end、def_id、type-var-id 等 Int 字段，随程序结构存活，不可由任何 RC 波回收）。BOOL 14.6M 次之。W4 隔离测试证明标量重赋值 drop 真工作但全自编译 INT ~flat → ① 在编译器本就小。**唯 unboxing 让 Int/Bool 堆对象整类消失 → 根除 ② + 装箱 alloc/RC 流量 → peak 真下来**。

**核心设计挑战 = uniform-boxing 边界**：containers（List/Map/Option/tuple）、泛型函数、trait dict 槽位都按 `void*`（uniform）。标量 unbox 后无法直接进 void* 槽 → 需**多态边界 box / 单态代码 unbox**（标准做法，参考 OCaml/MLton）：
- 标量进入泛型/容器/Option/dict 位置 → box（在边界处 ring_box_*）
- 标量从这些位置读出 → unbox
- 纯标量单态代码（算术、比较、condition、scalar let/mut）→ 全程寄存器值，零 box

**涉及修改**：
1. `compiler/codegen_llvm_expr.ring` / `_stmt.ring`：类型导向的 repr 选择——scalar-typed HExpr 生成寄存器值（i64/f64/i1）；算术/比较/condition 不 box；多态边界（容器 push、Option 构造、泛型 call arg、dict 派发）插 box，读出插 unbox。
2. `compiler/perceus.ring`：scalar-typed 值不参与 RC（无 dup/drop/Clone）——`is_scalar_type` 已有（W4），扩展到全 RC 决策；W4 的 scalar-reassign-drop 在 unbox 后变 no-op（标量无 box 可 drop，自然失效，无需删）。
3. `ring_runtime.cpp`：box/unbox 在边界仍需（ring_box_int/bool/float 保留供边界用）；容器/Option 存取标量时的 box/unbox 约定。
4. `compiler/codegen.ring`（JS 后端）：JS 标量本就是原生值，需确保双后端 parity（JS 无 box 概念，LLVM 新 repr 须行为一致）。
5. condition lowering：`if`/`while` 条件直接用 i1（comparison 产 i1，不经 box）→ **condition-Bool 泄漏随之消失**（折入本项，见验收）。

**验收标准**：
- 标量（Int/Bool/Float）在单态算术/比较/condition/scalar 变量位置**不再堆分配**（counter：tid0 INT / tid2 BOOL 大幅下落）
- 算术热路径无 box/unbox 指令
- **condition-Bool 泄漏消失**：本 session 的隔离测试（`while i<N` 10M 循环，原 tid2 BOOL 线性爬 10M）改为 BOOL 恒定 / 不爬
- 多态边界（List<Int>/Option<Int>/泛型/dict）box/unbox 正确，行为不变
- **native 自编译 peak RSS << 25.9GB 且本机可运行**（G-a 达成的判据，counter live plateau）
- 全 E2E + `llvm_diff` 不回归（≥53 例）；double-bootstrap 字节一致；全程 ASan-clean
- **本机可做，native-worktree-unfriendly**（缺 gitignored addon，worktree 无法自验）→ 在主仓做，长任务尽早 commit 半成品
