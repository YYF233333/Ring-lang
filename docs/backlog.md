...
### B-101 alias-aware ownership（perceus，native-working 硬前置）[bugfix] [P1] [L] [judgment] [doing]

> 2026-06-05 立项（Discussion，两个诊断 subagent 实证）。**native 自举的真 blocker**——解锁 B-089 G-a/G-c。**采用务实白/黑名单路线**（用户 2026-06-05 拍板，fix-forward；原则化 alias 分析留 B-096/L3）。
