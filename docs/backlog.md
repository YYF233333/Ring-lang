...
### B-103 Perceus return-mode（borrow）推断 → G-a 内存回收 [feature] [P1] [XL] [judgment] [planning]

> 2026-06-07 立项（用户拍板做完整机制，非补丁循环）。**G-a 内存门的最终 unblock**——解锁 B-102 G-a + B-089。**正在设计调研中**（return-mode 算法 + builtin 分类 + perceus 集成 + Koka 参考），调研结论回来后补全本 spec 的算法/集成细节。

**问题**：R-clean（B-102，git `27fe62d`）让 Type-DAG RC *正确*（ASan-clean），但 `perceus.ring:is_droppable_init` 通用 `Call` arm 保守 `=> false` → `let x = call()`（含 167 处 apply_subst）结果不 drop → 瞬态不回收 → G-a 仍 FAIL。激进放开 = native UAF（borrow-返回 call 如 `ring_unbox_int` 结果被提前 drop）。**根本缺口 = 每个函数返回值是 owned（fresh，该 drop）还是 borrowed（alias 参数/接收者，不该 drop）的知识。** 这是贯穿 B-098/B-101/B-102 的「fresh vs alias」根问题——never-drop/intern/保守-不-drop/激进-drop 四条路皆败，唯 return-mode 推断正解。参考 Koka borrowing 推断。

**机制（待设计调研定细节）**：
- **return-mode 格**：函数返回 `owned`（fresh 构造/owned-call）或 `borrowed-of-param-i`（alias 入参）。
- **builtin/runtime/extern 叶子分类表**（手工标，不可内省）：owned（list_clone/map_values/构造器/apply_subst/字符串拼接…）vs borrowed（list_get/map_get/unbox_int/str 索引/`.unwrap` 载荷…）。
- **Ring 函数推断**：从 return 表达式反推（字面量/构造器/owned-call→owned；参数本身/参数 field-index/borrow-call→borrowed），调用图上 fixpoint（SCC/worklist）。
- **perceus 集成**：`is_droppable_init` 的 Call arm 据 callee return-mode 决定——owned→droppable（回收）/ borrowed→不 drop（避 UAF）；borrow 结果逃逸时 clone-all-escape 已 escape→Clone。

**涉及修改**（待调研细化）：perceus.ring（is_droppable_init / is_borrow_returning_call）、builtins.ring（return-mode 标注）、新增 return-mode 推断 pass（调用图 fixpoint）、相关数据结构。

**验收标准**：
- native 自编译 peak RSS **<< 25.9GB**（瞬态回收，G-a 达成；本机监督实测，kill 护机器）
- 全程 ASan-clean（无 over-free/UAF）—— return-mode 正确则 owned 回收、borrowed 不误 drop
- JS 731×3 + llvm_diff 51×3 零回归；double-bootstrap 字节一致
- native 自编译推进/完成

**依赖关系**：B-102 R-clean 正确性（✅ done）是前提；本项解锁 B-102 G-a + B-089 capstone。可能 subsume B-068/B-096 的 borrow 建模一部分（调研评估）。
