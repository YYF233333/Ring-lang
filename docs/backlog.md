...
### B-103 完成 borrow-returning builtin 分类 + 开放 call droppability → G-a 内存回收 [feature] [P1] [L] [judgment] [doing]
...
- **D1 — total return-mode drop pass**：每个 **fresh-owned 临时**（子表达式产 owned 且父节点借用/丢弃）在 last-use 后 drop，**统一覆盖所有子表达式位置**（arg / scrutinee / condition / receiver / subexpr / control-flow-init），不再一波一位置。地基 = **B-103 完整 return-mode 分类**（owned vs borrow 叶子，硬前置）。临时天然单次使用 → "何时 drop" 是 per-subexpr 局部判定（父消费→不 drop，父借用→用完 drop），**不需 owned-everywhere 的 per-path 平衡 → 不重蹈 #134**。
...
**执行序（worker 接棒）**：B-103 完整 return-mode 分类（硬前置，现 [planning]）→ D1 total drop pass（收编 W1-W4，统一覆盖所有子表达式位置）→ D2 leak verifier（挂 `npm test`）→ native re-measure（verifier 全绿 + live plateau + peak << 25.9GB）。每改一类 → JS + llvm_diff 53×3 + ASan + verifier。**native-worktree-unfriendly（主仓做，长任务尽早 commit）**。
...
**依赖**：B-102 R-clean（✅ Type 可 drop）+ **B-103 完整 return-mode 分类（硬前置——total drop pass 的 drop 决策地基；现 [planning]，需先完成）**。解锁 B-102 G-a + B-089。这是 Perceus L0/L1 核心的**完整**实现（total drop pass + garbage-free verifier）。
