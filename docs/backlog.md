...
### B-123 #137 根修：List.reverse/sort LLVM runtime 改原地（G-b 地雷）[bugfix] [P1] [M] [judgment] [queued]

> 2026-06-12 立项（audit #137 [medium] 拍板 = 案 ① runtime 原地化）。`std/list.ring` 声明 `reverse`/`sort` 返回 Unit（原地语义），JS 后端真原地；LLVM runtime `ring_list_reverse`/`ring_list_sort_default` 构造 fresh 副本返回、不动 receiver——语句位调用 native 等于 no-op。编译器自身 8 处 `.sort(` → **native 自编译产出与 node 版顺序可能不一致 = G-b（double-bootstrap 字节一致）地雷**。否决案 ②（codegen 写回 receiver alloca）：换指针致别名者看不到变化、与 JS oracle 不一致，parity 洞换形态仍在。
>
> **时序：B-104 落地后执行**（D5 归因测量期间不动 runtime 分配行为——原地化消除 fresh-copy churn 会动 alloc 计数器基线，污染归因对比）。

**涉及修改**：
1. `ring_runtime.cpp`：`ring_list_reverse` / `ring_list_sort` / `ring_list_sort_default` 改为直接 mutate receiver vector（原地 permute 不改元素所有权——B-103 为 fresh-copy 补的元素 dup 一并退役）。
2. **runtime 同族审计**：排查其余「声明 Unit 原地、实现 fresh 丢弃」的方法（audit 只实测了这两个，防漏网）。
3. codegen 调用形态核查：语句位/表达式位调用的返回值处理与 RC 记账一致（Unit 返回与 B-118 约定不冲突）。
4. 测试：`tests/cases/llvm/list_share_ops_rc.ring` reverse 断言从 len 升级为顺序断言；新增 sort 顺序 + 别名可见性（`let ys = xs; xs.sort()` 后 ys 可见）差分用例。

**验收标准**：
- `xs.reverse()` / `xs.sort()` 语句位调用两后端行为一致（顺序断言）；别名可见性一致
- 同族审计结论成文（条目或 commit message），漏网项一并修复或立案
- JS 全量 + llvm_diff 全量 ×3 零回归（动 RC 邻域纪律）；ASan gating clean；自举一致
- audit #137 删除（留墓碑）

### B-124 #148 根修：`join` 签名收紧为 `List<Str>`（消 oracle 盲区堆溢出）[bugfix] [P2] [S] [mechanical] [queued]

> 2026-06-12 立项（audit #148 [medium] 拍板 = 案 ① checker 收紧签名）。`std/list.ring` 的 `join(self: List<T>, ...)` 对 T 无约束：`List<Int>.join` 过 checker，JS 后端靠隐式 coercion 失真输出、native `ring_list_join` 按 `std::string*` 读元素 → heap-buffer-overflow（ASan 实证，oracle 盲区类）。否决案 ②（runtime typeid 分派 stringify）：用户类型仍需 Debug dict 约束绕回类型层、JS/native 输出难严格一致、隐式 stringify 违公理⑧（同族决策 = 字符串无 `+` 拼接）。纯 checker/std 签名改动不碰 runtime 分配，可在 B-104 间隙随任意小波顺手做。

**涉及修改**：
1. `std/list.ring`：`join` 签名 `self: List<T>` → `self: List<Str>`。
2. 负面测试：`List<Int>.join(",")` 报编译错误，错误信息可读（receiver 类型不匹配方向，`--error-format=llm` 单轮可修——如指向 `.map(...)` 先转 Str）。
3. 正常路径回归：`List<Str>.join` 两后端不回归（既有用例核查）。
4. 全库 grep 非 Str join 站点迁移（预期 ≈0——native 自编译已工作说明编译器自身全 Str）。

**验收标准**：
- `List<Int>.join` 编译错误（负面测试锁定）；`List<Str>.join` 全量不回归
- JS 全量 + llvm_diff 通过；自举一致
- audit #148 删除（留墓碑）
