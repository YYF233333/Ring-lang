...
- **注（2026-06-08 更正，原"非内存墙解法"被 B-104 W1/W2/W3a 实测推翻）**：精确-RC 三波后残留 leak 绝对主导 = INT 64M 装箱标量（三波纹丝不动），其中②合法存活装箱标量唯 unboxing 可根除 → **unboxing 是 G-a 内存墙剩余部分的解法，不只是性能优化**。排序：B-104 W4（scalar-reassign-drop 子波，回收①）之后叠加 B-080（消②+装箱开销）。优先级（当前 P3）待 W4 后按实测峰值重评是否提到 P1
...
**⛔ 剩余**：W1+W2+W3a 后 leak ~33.5%@268M、~30%@大规模，轨迹强平稳，但 peak 仍 OOM（>15GB）。**残留绝对主导 = INT 64M+（精确-RC 三波未动），其位置 = 标量装箱**：
...
2. **结论（数据支撑）+ 下一波决策（2026-06-08 用户拍板）**：精确-RC 临时回收已近收益上限（INT 不动证明）。INT 64M 拆两块——①mut-var 重赋值泄漏（精确-RC 可回收，标量值语义不共享→drop 旧值安全）②合法存活装箱标量（唯 B-080 unboxing 可解）。**下一波 = scalar-reassign-drop 子波（W4，回收①）**，理由 = 补全精确 RC、不掩盖 bug（延续 A-via-materialize 原则，与"否决 B-080 先"同源）。**B-080 排在 W4 之后**：②+装箱开销仍需它，且 peak 已知 OOM → W4 多半是中间步非 G-a 终点，B-080 大概率仍需叠加（数据结论，已向用户诚实上报）。

**多波建议**：~~(W1) Call-arg~~✅ → ~~(W2) scrutinee~~✅ → ~~(W3a) control-flow-init~~ ✅（branch-value 分析，INT 不动证明触顶）→ **(W4) scalar-reassign-drop 子波（2026-06-08 用户拍板的下一波，spec 见下）** → **B-080 标量 unboxing（W4 后叠加，②合法装箱标量唯此可解）**；W3b receiver 收口对 INT 贡献小、按需。

**W4 spec — scalar-reassign-drop 子波（2026-06-08 用户拍板的下一波）**：
- **目标**：回收 INT 64M 的①——`mut` 变量持标量（Int/Bool/Float）被重赋值（`x = x + 1`、loop 计数器 `i = i + 1`）时，旧的装箱标量 box 当前直接被覆盖、无人 drop → 泄漏。每次重赋值在写入前 drop 旧 box。
- **安全性论证**：标量值语义不共享——`mut x: Int` 的旧 box 无别名持有者（标量的赋值/传参是值拷贝语义，不像 struct/list 可被借用引用），故重赋值前 drop 旧值安全、不会 UAF。这是精确-RC 的合法收口（区别于 B-080 把症状藏掉）。
- **范围**：只对**标量类型**（Int/Bool/Float）的 mut-var 重赋值（Assign 到已绑定的 `let mut`，RHS 求值后、store 旧覆盖前 drop 旧值）。非标量（struct/list/string/Option）的 mut-var 重赋值**不在本波**——它们可能被借用别名，drop 旧值需更强分析（留后续，不在 W4 误伤）。
- **涉及修改**：`perceus.ring`（Assign 语句对标量-typed mut-var 目标插入"drop 旧值"——RHS 临时 materialize 后、写入前）；可能 `codegen_llvm_*.ring`（生成 drop-old-then-store 序列）。
- **验证**：alloc/free 计数器看 INT 是否下落（①占比）；ASan-clean（drop 旧标量不误伤别名——标量无别名故应天然 clean）；JS 731×3 + llvm_diff 不回归；double-bootstrap 一致；native self-compile peak 监督（15GB kill）。
- **预期**：回收①后 INT 部分下落，但②（合法存活装箱标量）仍在 → 大概率仍 OOM、需 B-080 叠加（已是数据预期，本波是精确-RC 的完整收口而非 G-a 终点）。每波 ASan + alloc/free 计数器 + peak RSS 监督。**工具**：alloc/free 计数器（`ring_runtime.cpp` 已带 `-DRING_ALLOC_STATS` 守卫的 `g_allocs`/`g_frees`/`g_live_tid[4096]` 在 ring_alloc/ring_drop，每 32M 打印 top-typeid + atexit）+ ASan 配方（B-102：`clang -fsanitize=address -D_DISABLE_STRING_ANNOTATION -D_DISABLE_VECTOR_ANNOTATION`，runtime DLL 在 `<llvm>/lib/clang/22/lib/windows` 入 PATH）+ native 链接 `clang dist-llvm/main.o ring_runtime.o -o ring.exe -lmsvcrt` + 监督 self-compile 15GB kill 护机器。typeid 速查：0=INT 2=BOOL 3=STR 4=LIST 7=CLOSURE 8=OPTION 10=TUPLE 64+=user。**工具**：alloc/free 计数器（`ring_runtime.cpp` 已带 `-DRING_ALLOC_STATS` 守卫的 `g_allocs`/`g_frees`/`g_live_tid[4096]` 在 ring_alloc/ring_drop，每 32M 打印 top-typeid + atexit）+ ASan 配方（B-102：`clang -fsanitize=address -D_DISABLE_STRING_ANNOTATION -D_DISABLE_VECTOR_ANNOTATION`，runtime DLL 在 `<llvm>/lib/clang/22/lib/windows` 入 PATH）+ native 链接 `clang dist-llvm/main.o ring_runtime.o -o ring.exe -lmsvcrt` + 监督 self-compile 15GB kill 护机器。typeid 速查：0=INT 2=BOOL 3=STR 4=LIST 7=CLOSURE 8=OPTION 10=TUPLE 64+=user。
