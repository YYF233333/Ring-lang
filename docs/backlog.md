...
### B-104 精确 Perceus RC：drop owned 临时值（last-use）+ 开放 BinOp/UnaryOp/控制流 droppable → G-a 真解 [feature] [P1] [XL] [judgment] [planning]

> 2026-06-07 立项（orchestrator 监督诊断后定位，用户拍板方案 A）。**G-a 内存墙的真正根因 + 真解**——推翻此前归因。
...
**涉及修改**：`perceus.ring`（backward-liveness 临时 drop 插入 + is_droppable_init 开放 BinOp/UnaryOp/控制流 + 临时值 last-use drop）；codegen_llvm（如需新 drop 点）；ASan + 计数器验证。
