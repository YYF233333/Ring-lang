...
**新发现待定（W4 顺带，非本 item）**：while/if condition-result 的 Bool box 未 drop（隔离测试 tid2 随循环次数线性爬）——独立于 W4（reassignment）的 precise-RC gap。可回收（condition 值是 fresh-owned 临时，branch 消费后 unbox 到 i1 即死）但需在 codegen 条件 lowering 处 drop box。已记 worker_feedback [观察]。注：BOOL @402M=14.6M < INT 63M，即便修也到不了 G-a，仍需 B-080。
