...
### B-102 native over-free 链终结（ASan 驱动）+ A2 hash-cons [bugfix] [P1] [L] [judgment] [queued]
...
- **设计决策已锁（2026-06-07 Discussion，贴码核定）**：
  ...
  - **D2 intern key = 规范化字符串**：Ring Map key 走 JS `===`（不能拿 Type 当 key，否则废掉 intern）→ 新增 `type_intern_key(t) -> Str`，满足 `key(a)==key(b) ⟺ types_equal(a,b)`，配 `Map<Str, Type>`。key 须逐字对齐 `types_equal`：① struct/enum **nominal-shallow**（只编码 `name+type_params`，忽略 fields/variants，见 `types.ring:319-328` 的 `..`，避免把巨型 variants 列进 key）② RecordType / EffectRow / FnType effects 是**无序集**（`.all/.any` 比较）→ key 先按名/kind 排序 ③ **不复用 `type_to_string`**（有损、非 injective）。
  - **D3 范围 = 只在 apply_subst 的构造 arm intern**：洪流唯一来源（167 处 `let x=apply_subst`），只包它的 7 个复合 arm（Fn/Struct/Enum/Generic/Record/EffectRow/Tuple）。「全 Type 构造点 intern」（拿 O(1) 类型相等推断提速彩蛋）**另立项、与内存门解耦**——O(1) 相等要指针相等需全面铺开，G-a 不需要。
  ...
  - **🚦 前置不变量门（动手前必验，否则 intern 引新 bug）**：代码库不得有 (1) 就地 mutate 一个 Type，或 (2) 用 `===` 身份比较 Type——intern 让等价类型共享同一指针，违反 (1) 会串改别名、违反 (2) 行为变。实跑 + 搜码确认「Type 是不可变共享 DAG」这一硬前提成立后再 intern。
...
4. `compiler/types.ring`：`type_intern_key(t) -> Str`（D2，对齐 `types_equal`：nominal-shallow + 无序集排序）+ `intern_type(uf, t)`（含 D1 的「含 var 不入表」递归检查）。
5. `compiler/env.ring`：`apply_subst` 的 7 个复合构造 arm 走 `intern_type`（D3）。
