...
### B-114 HOF 闭包捕获 `let mut` 与普通闭包对齐 [bugfix] [P2] [M] [judgment] [queued]

> 2026-06-11 立项（Discussion，公理 1 违例簇）。普通闭包捕 `let mut` 自动 box 正常工作（§1.6 明确支持）；同样的闭包传给 `.map()` 等 HOF 报错——**同一语法两种行为**，LLM 无法从规则预测，比单纯限制更伤。根因待查：疑似 builtin HOF inline codegen 路径不走 boxed-capture，或 HOF fn 类型 effect row 拒绝 `mut<T>`。

**涉及修改**：
1. 先查根因（checker 拒绝 vs codegen 缺路径），按根因修：HOF 闭包捕 let mut 与普通闭包完全一致（auto-box + 写穿）。
2. 与 B-056（闭包捕获注入 `mut<T>` effect）相邻：若根因在 effect row，两项合并实现。
3. e2e：`.map`/`.filter` 闭包捕 let mut 读写 + LLVM 后端 mut-cell 写穿（B-091 机制）各一例。

**验收标准**：
- design.md/CLAUDE.md「已知限制」删除该条（`.map()` 闭包不能捕获 let mut）；双后端 e2e 通过
...
- **基础设施注记（2026-06-11 Discussion）**：total drop pass 与 D2 leak verifier 都是全 HIR 遍历——动工前评估先补 audit #20（hir-visitor 扩展），避免第 6/7 个重复 HExpr match；是否先修由 milestone session 自行判断。
...
**执行序（worker 接棒）**：~~B-103 完整 return-mode 分类~~（✅ 2026-06-11 done，条目已删，成果见上 D1 地基注）→ **D1 total drop pass（推进中：内置规则①② ✅ 2026-06-11 Stage 1，git `bfd4fe6`..`b62423b`，#139 → resolved + 9 mutator 名退役；剩 total pass 收编 + 规则③④）**（收编 W1-W4，统一覆盖所有子表达式位置）→ D2 leak verifier（挂 `npm test`）→ native re-measure（verifier 全绿 + live plateau + peak << 25.9GB）。每改一类 → JS + llvm_diff 53×3 + ASan + verifier。**native-worktree-unfriendly（主仓做，长任务尽早 commit）**。
