...
**⛔ 剩余（难类，需 precise-Perceus 分析，专门 session）**：W1+W2 后 leak ~39%（@268M）、轨迹平稳，但 peak 仍 OOM（self-compile >15GB）。剩余主导 = **INT（tid0 64M@402M，现最大）+ BOOL 27M** —— 标量装箱放大：
1. **control-flow-as-let-init（`let x = if/match{...}`）= 需 branch-value 分析**：`is_droppable_init` 仍 `_ => false`；分支值 owned 性混合，需判每分支 tail 是否 droppable。
2. `f(x).field` 的 owned receiver `f(x)`、And/Or、EffectOp args（handler resume 别名）—— 各需对应所有权分析。
3. **标量 unboxing（B-080）= 放大器**：INT/BOOL 装箱使每个标量都是堆对象，剩余 INT 64M/BOOL 27M 即便精确 RC 也部分是「合法存活的装箱标量」。**待评估**：W3 精确 RC 能否单独达 G-a，还是必须叠加 B-080。

**多波建议**：~~(W1) Call-arg materialize~~ **✅ DONE** → ~~(W2) scrutinee materialize~~ **✅ DONE（无需 match-arm 分析）** → **(W3) control-flow-init branch-value + receiver/And-Or/EffectOp 收口**；之后评估 B-080 标量 unboxing 是否为达 G-a 的必需叠加（INT 现为最大泄漏 typeid）。每波 ASan + alloc/free 计数器（live plateau）+ peak RSS 监督实测。**工具**：alloc/free 计数器（`ring_runtime.cpp` 已带 `-DRING_ALLOC_STATS` 守卫的 `g_allocs`/`g_frees`/`g_live_tid[4096]` 在 ring_alloc/ring_drop，每 32M 打印 top-typeid + atexit）+ ASan 配方（B-102：`clang -fsanitize=address -D_DISABLE_STRING_ANNOTATION -D_DISABLE_VECTOR_ANNOTATION`，runtime DLL 在 `<llvm>/lib/clang/22/lib/windows` 入 PATH）+ native 链接 `clang dist-llvm/main.o ring_runtime.o -o ring.exe -lmsvcrt` + 监督 self-compile 15GB kill 护机器。typeid 速查：0=INT 2=BOOL 3=STR 4=LIST 7=CLOSURE 8=OPTION 10=TUPLE 64+=user。
