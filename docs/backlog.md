...
| **B-101** | **Type-DAG 所有权 = intern + 永不 drop — native 自举真 blocker：永不 drop 杀 pre-existing L0 prelude UAF（shallow-dup/deep-drop 不对称）+ hash-cons 去重杀 apply_subst 无界泄漏（2.51亿→有界）；A1 解锁正确性→A2 达 G-a；native E2E×3 已建** | **G-a/c** | **P1（native-working 硬前置，2026-06-05 锁方案 A，doing）** |
...
### B-101 Type-DAG 所有权 = intern + 永不 drop（native-working 硬前置）[bugfix] [P1] [L] [judgment] [doing]
