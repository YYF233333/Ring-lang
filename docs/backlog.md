...
**⛔ 剩余（难类，需 precise-Perceus 分析，专门 session）**：W1 后 leak 仍 ~52%（@268M）且单调爬（尾部被以下位置主导）：
1. **scrutinee 位 fresh-owned 临时（确认最大，OPTION ~66M 主体）= 需 match-arm return-value 分析**：编译器海量 `match map.get(k)/find(...) {...}` 的 fresh Option scrutinee 未 materialize。难点 = `match scrut { _ => scrut }` arm 把 scrutinee verbatim 返回 → materialize+drop = double-free。需判每个 arm 是否返回 scrutinee（或其投影绑定）。**W2 主目标。** EffectOp args 同类（handler resume 别名）。
2. **control-flow-as-let-init（`let x = if/match{...}`）= 需 branch-value 分析**：`is_droppable_init` 仍 `_ => false`；分支值 owned 性混合，需判每分支 tail 是否 droppable。
3. `f(x).field` 的 owned receiver `f(x)`、And/Or —— 各需对应所有权分析。

**多波建议**：~~(W1) 返-arg builtin 分类 + 开 arg materialize~~ **✅ DONE（Call-arg 半）** → **(W2) scrutinee materialize + match-arm return-value 分析（OPTION 大块，最大头，ASan 驱动）** → (W3) control-flow-init branch-value + 余类收口。每波 ASan + alloc/free 计数器（live plateau）+ peak RSS 监督实测。**工具**：alloc/free 计数器（`ring_runtime.cpp` 已带 `-DRING_ALLOC_STATS` 守卫的 `g_allocs`/`g_frees`/`g_live_tid[4096]` 在 ring_alloc/ring_drop，每 32M 打印 top-typeid + atexit）+ ASan 配方（B-102：`clang -fsanitize=address -D_DISABLE_STRING_ANNOTATION -D_DISABLE_VECTOR_ANNOTATION`，runtime DLL 在 `<llvm>/lib/clang/22/lib/windows` 入 PATH）+ native 链接 `clang dist-llvm/main.o ring_runtime.o -o ring.exe -lmsvcrt` + 监督 self-compile 15GB kill 护机器。typeid 速查：0=INT 2=BOOL 3=STR 4=LIST 7=CLOSURE 8=OPTION 10=TUPLE 64+=user。
