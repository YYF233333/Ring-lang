...
| B-089 | Native 终验 capstone（G-a/b/c + B-098 暴露的 native print bug / 内存实测 / B-099）| 全部 | P1（B-098 已落地，next P1 wave）|
...
> **本机可做**：依赖 B-083/B-084/B-085/B-086/B-087/B-088 + #133 + **B-098（✅ 2026-06-04 done）** 全部落地。
...
> 1. **native `print` builtin bug（最关键）**：native ring.exe 编用 `print` 等 builtin 的程序报 `Undefined variable: print`（node 编译器正常；a_empty.ring 无 builtin 故 GATE 1 没覆盖、llvm_diff 用 node 编译器也没覆盖）。**根因未定**——可能是 **B-098 残留 RC over-free 了 builtins 环境**（同 #134 类、不崩而误编），也可能是 native 前端 pre-existing 限制。**这是 native 自举的真 blocker**，G-a 内存实测也卡在它（native 自编跑不通）。本项重加诊断仪表（或 -g + windbg）定位 RC vs 前端，是 RC 则修，是前端 parity 则归 G-c。
> 2. **`is_droppable_init` 保守泄漏**：clone-all-escape 假设「Call 结果是 fresh owned」对编译器共享 DAG 不成立（Call 结果常 alias 共享态，drop 会 UAF），B-098 已收紧为仅 fresh 构造器 / owner-bearing 绑定才 drop，Call/控制流结果一律不 drop（泄漏 crash-free）。内存收益部分打折——精确 DAG-aware ownership 在本项或 L3/B-096 做（影响 G-a 是否达 <<25.9GB）。
