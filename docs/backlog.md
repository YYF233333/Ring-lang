...
### B-049 Delegate 冲突检测 + 泛型测试 [bugfix] [P2] [S] [queued]
B-010 delegate 遗留的两项 spec 未完成项：

**1. 冲突检测**：同一 trait 不能既 delegate 又手写 impl，当前两者共存时行为未定义（可能重复注册 ImplEntry）。

**2. 泛型 delegate 测试**：`impl<T: Eq> Wrapper<T> { delegate inner: SomeTrait }` 基础设施已有但未 E2E 验证。

**涉及修改**：
1. `infer_register.ring`：phase 3 delegate 注册时，检查该 trait 是否已有手写 impl → 报编译错误（新错误码）
2. `tests/cases/`：添加冲突检测负面测试 + 泛型 delegate 正向测试

**验收标准**：
- `delegate field: Trait` + 手写 `impl Trait for Struct` → 编译错误
- `impl<T: Eq> Wrapper<T> { delegate inner: Eq }` → 正常工作
- 全部 E2E 测试通过
- 自举编译器正常编译自身
