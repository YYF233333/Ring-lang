...
> **暂不执行**（2026-06-13）：① 并发会话占主仓工作树（B-126/B-080 未提交 codegen_llvm/ffi/addon）→ 须先收口/隔离；② 用户暂停。**✅ 已同步（2026-06-13 Discussion）**：design.md §7.11 premise 修正已落账（codegen-drop 已记为首选/原则修法）；本 spec Part 1 已 A 化（见下）。
