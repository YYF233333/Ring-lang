...
>
> abort handler 方向已拍板（2026-07-27，audit #251）——采用方案 A：捕获 payload 后退出当前 catch/evidence 作用域，将 payload 绑定到 op 参数并恰好执行一次 arm body，arm 结果成为整个 `handle` 结果；无 resume，arm 内 re-raise 必须逃向外层。checker 恒等限制方案否决；静态上还须把 abort arm 结果类型与 handle body 统一，并将 arm effects 合回外层。实现前 probe 同时发现 `infer_handle` 对所有 tail-resumptive arm 也缺结果/effect 契约，通用半边另立 audit #258，避免借 #251 静默扩面。#251 现已解除 judgment 阻塞、可进入独立实现 wave；本段只记录决策，代码尚未修改。
>
