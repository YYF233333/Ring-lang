...
- **实测**：leak 88% → ~50%（ANF 易类）；JS 733×3 + llvm_diff 52×3 + real_program ASan ×3 clean。

**⛔ 剩余（难类，需 precise-Perceus 分析，专门 session）**：即便 materialize 全部 temp 也只到 ~50% leak —— 剩余主体非简单 temp：
1. **arg/scrutinee 位临时（最大，含 OPTION 87M/INT 大块）= 需 return-mode 分析**：callee 可能把 arg verbatim alias 进返回（builtin `fold` 返 init、`unwrap_or` 返 default、Ring `_ => scrut`）→ materialize+drop = double-free（list_fold 实证）。安全 drop 需知每个 callee 是否 arg-aliasing。**关键认识**：Ring 函数靠 clone-all-escape return-clone 恒返 owned（arg 可安全 drop）；**仅"返-arg builtin"不安全**（有界分类，同 B-103 borrow-builtin 套路 + ASan 收敛）。先验此前提再批量开 arg materialize。
...
3. EffectOp/HandleExpr/TryCatch init、`f(x).field` 的 owned receiver `f(x)`、And/Or —— 各需对应所有权分析。

**多波建议**：(W1) 返-arg builtin 分类 + 开 arg materialize（最大头，ASan 驱动）→ (W2) control-flow-init branch-value 分析 → (W3) 余类收口。每波 ASan + alloc/free 计数器（live plateau）+ peak RSS 监督实测。**工具**：alloc/free 计数器配方（`g_allocs`/`g_live`/`g_live_tid[4096]` 在 ring_alloc/ring_drop，每 268M 打印 top-typeid）+ ASan 配方（B-102）+ 监督 self-compile 15GB kill 护机器。typeid 速查：0=INT 2=BOOL 3=STR 4=LIST 7=CLOSURE 8=OPTION 10=TUPLE 64+=user。
