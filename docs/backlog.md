...
### B-066 Checker 热路径索引化 [refactor] [P2] [M] [queued]
自举编译 ~20s，瓶颈在 checker。三个核心数据结构使用 List 线性扫描，改为 Map/Set 索引可预估减少 30-50% checker 时间。

**修改项**：

1. **`trait_impls: List<ImplEntry>` → `Map<Str, Map<Str, ImplEntry>>`**（`env.ring`）
   - 键为 `(target_type_name, trait_name)`，O(1) 查找
   - 消除 13+ 处重复的 `.any(fn(i) { i.target_type_name == X && i.trait_name == Y })` 模式
   - 影响文件：`env.ring`、`infer.ring`、`infer_ctx.ring`、`infer_register.ring`

2. **`current_fn_bounds: List<FnBoundsEntry>` → `Map<Int, Map<Str, FnBoundsEntry>>`**（`infer_ctx.ring`）
   - 键为 `(resolved_var_id, trait_name)`，O(1) 查找
   - 消除 predicate 内部 `apply_subst` 调用
   - 影响文件：`infer_ctx.ring`、`infer.ring`

3. **`free_type_vars_in_env` 跳过函数内部 generalize**（`infer_ctx.ring`）
   - Ring 函数内的 `let` 绑定实际是单态的，无 let-polymorphism
   - 跳过函数体内的 generalize 调用，消除每个 `let` 绑定触发的全环境扫描
   - 影响文件：`infer_ctx.ring`

**验收标准**：
- 自举编译时间显著下降（目标 < 12s）
- 所有查找语义不变——相同 trait dispatch 结果
- 全部 E2E 测试通过
- 自举编译器正常编译自身（double bootstrap 验证）

### B-067 Checker 代码质量清理 [refactor] [P3] [S] [queued]
