...
2. ~~**#150 修复（拍板 = runtime dup + fold 退役）**~~（✅ 2026-06-12 done，git `0f80b42`/`735a669`：`ring_list_fold` 空表 dup-on-share + `is_arg_returning_call` 谓词删除 + `anf_arg` 删除 + verifier x-fold-arg 豁免类退役（自校验 1292→1289 exempt）；回归 `fold_empty_owner_init.ring`；「map/set fold 同形」核实不成立 → audit #138 精化 + 新发现 runtime HOF 内部泄漏类 → audit #152）。
