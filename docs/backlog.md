...
**涉及修改**：
1. lexer：`${` 进入表达式模式栈（brace 深度 + 字符串状态），外层字符串终止判定仅在深度 0 生效；支持任意层嵌套（插值内字符串内再插值）。
2. 未闭合插值的诊断（span 指向 `${`）。
3. e2e：嵌套引号 / 双层嵌套插值 / 与 raw string 交互 / 插值内字符串含 `}` 等边角。

**验收标准**：
- `"${fn("arg")}"`、`"${m.get("k").unwrap_or("d")}"` 双后端正常编译运行；既有字符串测试零回归
...
### B-104 精确 Perceus RC：drop owned 临时值 → G-a 真解 [feature] [P1] [XL] [judgment] [milestone-dedicated-session] [planning]
...
**执行序（worker 接棒）**：~~B-103 完整 return-mode 分类~~（✅ 2026-06-11 done，条目已删，成果见上 D1 地基注）→ **D1 total drop pass（下一步）**（收编 W1-W4，统一覆盖所有子表达式位置，含上方内置规则①-④）→ D2 leak verifier（挂 `npm test`）→ native re-measure（verifier 全绿 + live plateau + peak << 25.9GB）。每改一类 → JS + llvm_diff 53×3 + ASan + verifier。**native-worktree-unfriendly（主仓做，长任务尽早 commit）**。
