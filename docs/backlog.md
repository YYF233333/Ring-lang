...
>
> **G-b 根因定位（2026-06-15，深度调查 + 攻坚）**：B-136 修 Ring 层 Set 排序后 G-b 从 32→5→8 文件差异（Wave A/B/C 改了更多模块）。攻坚发现：(1) **旧 ring.exe segfault 已消除**——重编 dist-llvm 后 native 可编任何多文件项目。(2) **LLVM codegen 缺 transitive effect closure 已修**（codegen_llvm.ring 加 `compute_transitive_effect_closure` + `collect_local_names_rec` 补 impl 方法名）。(3) **简单多文件 + 3 模块链验证一致**——小规模 effect 传播 native/node 完全相同。(4) **编译器自编译仍有 8/44 文件差异**——全是 `__ring_ev_fail` evidence 缺失，问题只在编译器规模（~30 模块 / ~27 亿次分配）。**新发现 blocker = B-141**：LLVM verify error `parse_expr` vs `parse_expr_bp` 方法名解析混淆——native 运行时 `parse_expr` 收到 PREC_NONE 当 evidence，可能级联污染 evidence 链。原假说「数据布局 bug」已被推翻——简单跨模块正常工作证明布局无问题。
>
> **G-b 根因深化（2026-06-16，worker feedback 消化）**：**真正根因 = `gen_try_catch` 的 closure capture 值拷贝**——TryCatch body 编为 closure 后 captured `let mut` 变量是值拷贝（load outer alloca → store local alloca），闭包内赋值不回传外层。JS 后端 IIFE + try/catch 引用 capture 所以正确。影响编译器内所有 `some({...}) catch` 块的外层 mut 赋值。最小复现 = `tests/cases/b089_repro7.ring`。write-back/read-back 原型小文件验证通过但全量 heap corruption（env RC 不平衡）。**修复方向拍板（2026-06-16 Discussion）= (B) inline setjmp**：不用 closure 实现 TryCatch，直接在当前函数 emit setjmp/longjmp IR，彻底消除 capture 语义问题；解耦 B-096 闭包 RC 依赖。方向 (A)（修 RC 平衡 write-back）留观。
>
> **G-b inline setjmp 落地（2026-06-16）**：inline setjmp 实现完成——LLVM codegen 直接调平台 `_setjmp`（经 `ring_catch_get_buf` 获取 heap jmp_buf）+ `returns_twice` 属性，86/87 llvm_diff pass。唯一失败 = `receiver_temp_drop.ring` JS 端 hang（60s timeout，pre-existing，与 inline setjmp 无关，native 端正常）——JS 后端归档前不单独修。`ring_catch_setjmp` 成 dead code（B-142）。
