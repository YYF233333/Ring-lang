...
> **I′ H+T COFF object-identity verdict（2026-08-21 用户批准 A）**：fresh `ffd6a416` gen1 construction 只建立construction claim；sealed candidate structural v3中四个RC case与off/on1/on2 child/audit均成功，off/on C字节一致、on1/on2 ledger字节一致，candidate hash不变。on1/on2 AMD64 COFF object等长，raw差异仅落在标准COFF header的4-byte `TimeDateStamp`字段（offset 4..7，实测唯一差异为offset 4）。object equality oracle必须保留raw objects及hashes、先验证AMD64 COFF结构，再仅对offset 4..7归一化比较，并额外要求全部raw diff offsets严格包含于这4字节；C、ledger与stderr继续literal byte identity。该裁决只修正验收oracle，不接受sealed v3、不授权丢弃object比较，也不修改Ring的clang invocation或引入确定性构建专项；harness-only变更须独立review后才能形成新的单次candidate gate，之后原focused、gen2、RC/ASan/self-host/12 GiB fixed-point门均不降低。
