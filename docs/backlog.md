...
- 层 3 在 native 自举 + 归档后启动。**排序（2026-06-05 Discussion 定）**：B-002 Drop/RAII(L2) → **async(B-007) 先**（设计已锁、自包含、风险低、宣发价值大）→ Refinement(B-001) 随后（需 Z3 + 先做 B-070 const generics）；M 项（B-072 Union / B-069 默认参数 / B-070 固定数组）当 XL 间换气穿插；P3 研究（GADTs/dyn/GATs）最后。B-002 的 abort-unwind 子集可能因 G-a 内存提前（待 B-101 内存实测定）
...
| **B-101** | **alias-aware ownership（务实白/黑名单）— native 自举真 blocker：修 pre-existing L0 prelude UAF（别名读取误 drop）+ apply_subst 族无界泄漏（fresh 漏 drop）；加 native 真实程序 E2E×3** | **G-a/c** | **P1（native-working 硬前置，2026-06-05 立项，doing）** |
| B-089 | Native 终验 capstone（G-a/b/c + 内存实测 + B-099）| 全部 | P1（依赖 B-101，B-101 后启动）|
...
### B-101 alias-aware ownership（perceus，native-working 硬前置）[bugfix] [P1] [L] [judgment] [waiting-feedback]

> 2026-06-05 立项（Discussion，两个诊断 subagent 实证）。**native 自举的真 blocker**——解锁 B-089 G-a/G-c。~~采用务实白/黑名单路线~~（下方 Wave A 已证伪此路线）。
...
> - **待 /discussion 锁方案**：(A) Type intern/hash-cons+永不 drop〔同杀 UAF+泄漏，最看好〕/ (B) 全子结构 RC / (C) 逃逸深 clone / (D) 原则化 escape 分析（L3）。
...
**根因**：perceus 的 clone-all-escape（及 L0 always-own）分不清两类值，导致同一缺陷的两面：
- **别名读取/Call 结果**（list/map 元素读取 `ring_list_get`/`map_get`/IndexExpr 结果 alias 容器内部；`path_*`/字符串 Call 返回）被当 owner 在 scope-end drop → **over-free / UAF**（pre-existing L0 bug，自 B-012；B-098 把硬崩软化成静默 corruption，native prelude 全空、自编译从未成功）。
- **fresh unshared Call 结果**（`apply_subst` 族新建 Type 树，167 处 `let x = apply_subst(...)`）被保守「一律不 drop」→ **无界泄漏**，G-a 主因未回收。

**务实白/黑名单修法**：
1. **drop-白名单 `is_fresh_owned_returning_call`**（`is_borrow_returning_call` 的对偶）：列已知「返回 unshared fresh」的纯函数——`apply_subst` / `apply_subst_row` / `apply_subst_map` / `zonk` 等——其 Call-结果绑定恢复 droppable（修泄漏）。**⚠️ 生效前逐 arm 验别名安全**：`apply_subst` 的 `TypeVar` 透传 arm 把 subst-map value 原样返回（可能 alias），list_clone 路径同理——这些 arm 不满足「fresh unshared」，需排除或确认。误列 → UAF（与白名单风险反向对称）。
2. **borrow-黑名单（不 drop）**：list/map 元素读取（`ring_list_get`/`_opt`/`map_get(_opt)`/`map_int_get(_opt)`/IndexExpr 结果）+ alias 性 `path_*`/字符串 Call 返回，按 borrow 处理、scope-end 不 drop（修 UAF）。B-098 已对裸 Call 收紧但**漏了元素读取经 `let` 重绑定后的投影 drop 路径**（subagent 1 定位）——补上。
3. **native 真实程序 E2E（回归缺口，必做）**：新增 native E2E——native ring.exe 编含 `print`/std 调用（list/map/str）的真实程序，断言 EXIT 0 + 输出正确，按间歇 bug 经验**跑 3×**。这是本修复的验收兜底 + 永久回归网（补 a_empty 漏检的洞）。

**涉及修改**：
1. `compiler/perceus.ring`：`is_droppable_init`（377-402）精确化——加 `is_fresh_owned_returning_call` 白名单 + 元素读取/alias-Call 黑名单；`is_borrow_returning_call`（203-208）对照扩展。
2. `compiler/env.ring`：逐 arm 核 `apply_subst`（506-586）/ `apply_subst_row` / `apply_subst_map` 的 TypeVar 透传 + list_clone 别名性，确认白名单安全边界。
3. `tests/`：native 真实程序 E2E harness（3×）。

**验收标准**：
- native ring.exe 编含 std 调用的真实程序（`print`、list/map/str）EXIT 0 + 输出正确，**3× 稳定**（间歇 bug 防假绿）
- native 自编译推进到不再撞 prelude UAF（理想：完整自编译，否则报告卡点）
- G-a 内存实测 << 25.9GB（apply_subst 族恢复 drop 后；本机实跑）
- JS 731×3 + llvm_diff 49×3 零回归；double-bootstrap 字节一致
- 全程无 UAF（白名单别名安全验证通过）

**注**：白名单是务实解，会累积「漏列 = 泄漏 / 误列 = UAF」的维护债；原则化 alias/escape 分析（系统区分 alias vs fresh）留 **B-096/L3**。

### B-089 Native 自举终验 capstone [bugfix] [P1] [L] [judgment] [queued]

> **本机可做**：依赖 B-083/B-084/B-085/B-086/B-087/B-088 + #133 + **B-098（✅ 2026-06-04 done）** + **B-101（alias-aware ownership，native-working 真 blocker）** 全部落地。
...
> **根因已查实（2026-06-05 两个诊断 subagent，决定性 revert 实验 + 静态 167 调用点核查）**：#1 与 #2 是**同一根因的两面** = perceus 分不清「Call/读取结果 alias 共享态」（不该 drop，否则 UAF）vs「fresh unshared」（该 drop，否则泄漏）。**修复 = alias-aware ownership，已立项 B-101（P1，native-working 硬前置），B-089 依赖 B-101。**
...
> 2. **`is_droppable_init` 无界泄漏（同根因另一面）**：clone-all-escape「Call 结果 fresh owned」对共享 DAG 不成立。subagent 2 核查 25.9GB 主因 `apply_subst` 的 **167 处调用全是 `let x = apply_subst(...)`**（Call 结果）→ 全落「永不 drop」→ **无界泄漏（正比编译步数，结构同 no-GC）**。RC 在主内存路径省到 ≈ 0 → **G-a（<<25.9GB）近乎注定失败**。修法见 B-101（apply_subst 族 drop 白名单）。
