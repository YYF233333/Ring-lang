...
### B-055 Match 表达式统一 labeled block 替代 IIFE [refactor] [P2] [M] [judgment] [doing]
不含 `return` 的 match 表达式用 IIFE `(function() { ... })()`，含 `return` 的已用 labeled block + temp variable。统一为后者，避免闭包分配。

**已知阻塞及解法方向（2026-06-15 Discussion 更新）**：temp variable 方案在 lambda body 中使用 match 时失效——`emit` 语句被写到 lambda 外部作用域（如 `.any(fn(d) { match d.severity { ... } })`）。根因疑为 emit target 缓冲区指错（lambda body 本身是函数体，labeled block 在其内部应合法）。**第一步 = 确认精确根因**：worker 需实际读 codegen 的 emit 路径，确认是 buffer 指错还是上下文栈缺失，再定修法。

**涉及修改**：
1. `codegen_expr.ring`：确认 lambda body 内 match emit 失败的精确根因（emit target buffer 指向 / 上下文栈缺失）
2. 修复 emit target 或加上下文栈，使 lambda body 内 temp variable 声明/赋值 emit 到正确作用域
3. match 表达式统一使用 `__ring_blkN` temp variable 方案，消除所有 IIFE

**验收标准**：
- 生成的 JS 中 match 表达式不再出现 IIFE
- lambda 内部的 match 表达式仍正常工作（`.any(fn(d) { match d.severity { ... } })` 类场景）
- 全部 E2E 测试通过
- 自举编译器正常编译自身
...
### B-136 G-b 残余：native vs node effect evidence 传参差异（5 文件）[bugfix] [P1] [M] [judgment] [queued]

> 2026-06-15 立项（B-089 G-b 攻坚实测）。SCC 排序确定化后 G-b 从 32/44 差异收敛到 5/44。剩余 5 文件（cli.js, compiler_mod.js, infer.js, infer_ctx.js, infer_decl.js）差异全是 **effect evidence 参数** `__ring_ev_fail` / `__ring_ev_io`——node 版传了但 native 版没传。根因：effect 推断结果在两后端不一致，可能与 impl effect 传播的已知限制相关（CLAUDE.md "Impl 方法 effect 传播有 `__ring_raise_fail` workaround"）。

**涉及的 5 个差异函数**：
- `compile_project_llvm`：`__ring_ev_io` 参数差异
- `infer_unary_op`：`__ring_ev_fail` 参数差异
- `merge_effects`：`__ring_ev_fail` 参数差异
- `check_extern_fn_decl`：`__ring_ev_fail` 参数差异
- `check_fn_body` / `check_fn_decl`：`__ring_ev_fail` 参数差异

**调查方向**：
1. 比对两后端的 effect 推断中间结果——哪些函数的 effect row 不同
2. 检查 checker Pass 2 中 effect 传播是否依赖 Map/Set 迭代序
3. 检查 `update_fn_effects` / effect back-propagation 路径的确定性

**验收标准**：
- native 自编译产出与 node 版字节级一致（44/44 文件 match = G-b 通过）
- 全部 E2E + llvm_diff 通过
