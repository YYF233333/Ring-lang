...
### #148 泛型 `join` 对非 Str 元素 native 堆溢出（oracle 盲区类）[medium] [judgment] [open] [queued: B-124]

> 2026-06-12 拍板：案 ① checker 收紧签名 `List<Str>`，立项 **B-124 [P2] [S] [mechanical]**（B-104 间隙可做）。本条留作发现记录，B-124 落地后删除。
...
> **D5 定量更新（2026-06-12，git `7323ee6`/`deab122` 直接计数器实测）**：原文「编译器自编译重度使用 filter/any/find/for_each → 疑似 BOOL 67.8M / STR 86.5M residual 未归因部分」**被证伪**——自编译 @2.382B 三类站点合计份额 ≈ 0（pred_bool=5,921 绝对值 = BOOL residual 的 0.008%；fold_acc=0；foreach_key_str/int=0；编译器源码以 for 循环为主，HOF 走得极少）。**本条对 G-a 零杠杆**，但泄漏类对用户面 HOF-heavy 程序仍真实——worker_feedback「B-104 D5」[决策] 建议降级为用户面收口项排期，待拍板。BOOL/STR residual 真身见 #153/#154 + And/Or-cond 类（D1 保守保留清单）。

### #153 LLVM 后端 `none` 构造 per-eval fresh 分配且 RC 从不 drop（JS=frozen 单例的系统性偏离，#151 dict 同构）[high] [judgment] [open]

B-104 D5 box-profile 实测（@2.382B 自编译，git `deab122` 仪表）：`ring_Option_none`（codegen 合成的零参 helper，每次求值 `ring_alloc` 16B fresh none）**live=born=100%——所有 none 构造无人 drop，≈64.2M = residual 第二大类（29%）**；对照组 `ring_Option_some` born≈39.5M 仅 0.09% live（some 处处正常回收），runtime helper 路径（find/get_opt 经 enum_some）零 live 样本（D1/D2 已盖住）。差别待遇是结构性的：JS 后端 `none` 是 `Object.freeze({_tag:"none"})` 模块级单例（runtime.ring:208），LLVM 是 per-eval fresh；perceus.ring 对 Option_none 零特判 → 疑 zero-arg ctor 调用路径（codegen_llvm_expr.ring:341）在所有权记账外，机制待深挖。波及所有用户程序的每个 none 求值（泄漏 + churn 双重）。修复方向（需拍板，见 worker_feedback「B-104 D5」[决策]）：none 模块级单例化（D4 DICT_STATIC never-drop + lazy getter 基建同构可复用）vs 进所有权记账正常 drop。发现者：B-104 D5。

### #154 `HDecl::Const` 的 Str 常量 lower 成 zero-arg getter，每次访问 fresh 物化且调用位不 drop [medium] [judgment] [open]

codegen_llvm_decl.ring:88「Generate const as a zero-arg function that returns the init value」——Str 常量（如 types.ring `BUILTIN_OPTION`）每次访问调 getter → `ring_str_from_cstr` fresh 分配；D5 box-profile 实测 getter 结果在调用位（BinOp 操作数/cond 位）100% 不被 drop：`BUILTIN_OPTION` 18.0M + `BUILTIN_STR` 6.3M + `BUILTIN_INT` 4.7M + FLOAT/BOOL 0.7M ≈ **29.6M live @2.382B（STR residual 的 51%）**。JS 后端 const = 模块级单例——又一个 #151/#153 同构的 JS/LLVM 偏离。热度驱动者 = 穷尽检查/type_to_string 把 BUILTIN_* 当比较常量高频访问（exhaustive.ring:121/245/332/573 在非错误路径拿 type_to_string 当类型 identity key）。修复方向（需拍板，同 #153 一并）：const 单例化（lazy memoised getter，D4 基建）；注意同站点的 `&&` 双臂 BOOL 泄漏属 And/Or-cond 类（D1 保守保留清单），两类在 types.ring:386 一行叠加。发现者：B-104 D5。
...
### #155 llvm_diff harness：case 名含 "update" 等 UAC 关键词时 exe 启动失败（ERROR_ELEVATION_REQUIRED），假阴/挂死 [medium] [judgment] [open]

`tests/llvm_diff.test.mjs` 把 exe 链接成 `<case 名>.exe`——Windows UAC installer-detection 启发式对名含 update/setup/install/patch 的无 manifest exe 要求提权 → `struct_update_enum.exe` 自 2026-06-12 起在本机 100% 启动失败（昨日 D4 ×3 还全绿 = 机器策略状态变化，非代码回归；同字节改名 `sue_plain.exe` 即正常，HEAD/d1f10ff 两 runtime A/A 同失败）。**失败形态多变且极易误判为堆损坏**：经 cmd.exe/execSync 时或为空 stdout + exit 0（assert 实际输出为空）、或 consent 路径卡死 60s ETIMEDOUT。当前影响：`npm run test:llvm` 恒 71/72，×3 纪律判读须排除该例。修复方向（需拍板）：① harness 链接到中性临时名（如 `case_N.exe`）② 链接时嵌 asInvoker manifest ③ 改 case 文件名避开关键词——①最稳（对未来 case 名免疫）。发现者：B-104 D5（测量并行验证时撞出）。
