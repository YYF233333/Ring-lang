...
### #139 llvm_ffi extern handle（非 RC 外来指针）游离于 RC 分类之外 [high] [judgment] [resolved] [B-104 D1 内置规则①]

> **已修复（2026-06-11，B-104 D1 Stage 1，commits `bfd4fe6`/`dcb57d2`/`3d3e747`/`b62423b`）**：类型级排除落地——perceus 对直接 extern-handle 值不 Clone（escape=move）/不 Drop/不入 owned/不 materialise（`hir.ring: is_extern_handle_type`），对 transitively-contains 类型（List/Option/Map/struct 字段嵌套）不 Drop/不 materialise（`type_contains_extern_handle`）；codegen `emit_drop_functions` 对 contains-extern 的 struct/enum 字段跳过 ring_drop（`field_rc_skip`）。IR 实证（probe + dist-llvm 自编译）：`ring_drop_LlvmCtx` 跳过全部 22 个含 handle 字段、handle 进容器 sink 不再 ring_dup、`fn_ty`/`fn_val` 绑定不再 scope-end-drop。回归 `tests/cases/llvm/extern_handle_rc_exclusion.ring`。runtime 级终验依赖 B-099（native 自跑 --target=llvm），届时本条目可删。原文保留如下供终验对照。
