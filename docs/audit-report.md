...
### #69 Tuple `==`/`!=` 使用 JS `===` 引用比较 [critical] [open]

**运行时错误**：`(1,2) == (1,2)` 永远 false。`infer_bin_op`（infer.ring:882）把 tuple 标记为 `is_builtin`，codegen 发出 `===`。Tuple 编译为 JS Array，`===` 是引用比较。应移除 `is_tuple_type` 从 `is_builtin` 检查，对 tuple 生成逐元素比较。

发现者：Opus

### #70 Int 除法产生 Float [critical] [open]

**类型不安全**：`7/2` 类型推断为 `Int` 但运行时结果为 `3.5`。codegen（codegen_expr.ring:188）发出 JS `/`（浮点除法）。应对 Int `/` 发出 `Math.trunc(a/b)`。

发现者：Opus

### #71 Struct match 模式生成错误 `_tag` 检查 [critical] [open]

**运行时错误**：`gen_pattern_condition`（codegen_stmt.ring:380）对 `NamedConstructor` 统一生成 `_tag` 检查，但 struct 实例是 JS class（无 `_tag`），条件永远 false → `__match_fail`。`gen_catch_pattern_condition`（codegen_expr.ring:876-895）已有正确的 `instanceof` 区分，需将同样逻辑应用到 `gen_pattern_condition`。此外 `bind_named_constructor_pattern`（infer_ctx.ring:883）不处理 struct 类型，静默跳过不报错。
