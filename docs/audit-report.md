...
### #115 外部限定 struct pattern（`inner::Pair { a, b }`）报 E0201 [medium] [doing]
...
### #100 `register_fn` 与 `register_extern_fn` 近乎相同 [medium] [doing]
...
### #101 ModBlock 注册逻辑在 `register_phase1` 和 `register_decl` 中重复 [medium] [doing]
...
### #77 Delegate forwarding：effect 标注 / evidence 未传递 [medium] [open]
...
### #106 `check_one_decl` 在 delegate expansion 前将 impl HIR 推入 hdecls [medium] [doing]
