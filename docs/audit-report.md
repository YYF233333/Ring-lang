...
当时顺序曾改为 `I′ -> A′ atomic transfer authority -> S′ cleanup -> final acceptance`。A′ 仍按既有用户批准一次贯通全 executable-HDecl inventory、transitive may-Drop、callable ownership modes、CFG/显式 `Take`/source-clear，并由 checker→HIR→Perceus→verifier→codegen 共用；不能缩成 Option ctor 特判、删除 ANF Drop、runtime dup、fresh-only ANF sink、旧 `56a` 双遍历、unity 或资源加码。transfer canary 先覆盖 literal/named Resource，struct/variant/list/tuple/range/slot generic owning sink、non-Drop control，以及删除 Take/错 DefId/重复 Drop mutations；Trait-default executable-HDecl 必须原子纳入。只有该矩阵绿，才恢复 exact-none W4/exit、tail negatives、shadow/loop/catch 与 S′ fixed-point/full acceptance。该重排只恢复已批准 ownership/safety 契约，不改变公开语义或保证，也不申请豁免；其实现分层现已由下述 FinalHIR 架构取代。

**Final-HIR single Resource Planner architecture approval（2026-08-22）**：用户在完整讲解current pipeline、ResolvedHIR/FinalHIR/RcHIR边界、隐式行为展开时点、SymbolRef identity、统一slot-state machine与certificate proof后明确表示理解并批准执行。本段supersede `I′ -> A′ -> S′` 的实现分层：I′ identity-only durable claim保持；A′ source transfer与S′ exact-none/overwrite/exit cleanup由FinalHIR上的唯一project-wide ResourcePlanner同一次完成。FinalHIR前必须完成全部semantic lowering、typed identity、ownership-neutral ANF、pattern projection与scope-result造槽，并冻结所有binder/node/edge；Planner按Logical OwnershipShape、Physical RcShape、callable least fixed point、per-body CFG的固定顺序输出binder不变且Clone/Take/Drop/Cleanup全显式的RcHIR与ranked certificate。Verifier只核验frozen constraints、有限derivation与路径守恒，codegen只接受verified RcHIR；planner后无独立Perceus、无producer/sink猜测、无新增HIR binder。

Proof-audit已逐项覆盖dict/ANF/rc-scope binder、Trait/Effect default、default/delegate、DerivedImpl与codegen-only body、constructor/slot intrinsic、HOF/factory、reexport/diamond、extern replay/bridge、recursive SCC、loop/catch/handler、partial move、generic resource以及single/project parity，未发现架构反证。不可删契约包括evaluation-region内左到右NeutralANF、scope值先转交父result再cleanup、source/member/anonymous identity分域、extern bridge显式graph edge、logical/physical双shape、RHS完全求值后Drop-old的Assign顺序、所有exit逆词法cleanup、ranked certificate双向证明least fixed point，以及single-file单节点Project入口。

旧eager checker graph（d5/17）、representation/counter与extern replay候选（df1/37a4/a7）全部被新架构supersede；其commits、review反例、construction-only与sealed evidence保留，但不得作实现seed、normalization理由或自动重跑。a7 one-shot按用户指令安全终止，无gen1/receipt/verdict，记录为`user-directed-superseded/inconclusive`。新实施必须从最新main另建唯一authority，禁止恢复checker hydration、shared counter member identity、old Perceus ownership heuristics、独立S′ producer lattice、name/FnType fallback或第二resolver/solver。该架构恢复既有ownership/safety保证，不改变公开语义/ABI，也不申请验证豁免。
