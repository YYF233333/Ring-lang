...
B-104 D5 box-profile 实测（@2.382B 自编译，git `deab122` 仪表）：`ring_Option_none`（codegen 合成的零参 helper，每次求值 `ring_alloc` 16B fresh none）**live=born=100%——所有 none 构造无人 drop，≈64.2M = residual 第二大类（29%）**；对照组 `ring_Option_some` born≈39.5M 仅 0.09% live（some 处处正常回收），runtime helper 路径（find/get_opt 经 enum_some）零 live 样本（D1/D2 已盖住）。差别待遇是结构性的：JS 后端 `none` 是 `Object.freeze({_tag:"none"})` 模块级单例（runtime.ring:208），LLVM 是 per-eval fresh；perceus.ring 对 Option_none 零特判 → 疑 zero-arg ctor 调用路径（codegen_llvm_expr.ring:341）在所有权记账外，机制待深挖。波及所有用户程序的每个 none 求值（泄漏 + churn 双重）。**已拍板（2026-06-12 Discussion）= none 模块级单例化，收口归 B-104 D6**（spec 见 backlog D6 块；D4 DICT_STATIC never-drop + lazy getter 基建复用），落地后删除本条。发现者：B-104 D5。

### #154 `HDecl::Const` 的 Str 常量 lower 成 zero-arg getter，每次访问 fresh 物化且调用位不 drop [medium] [judgment] [open]

codegen_llvm_decl.ring:88「Generate const as a zero-arg function that returns the init value」——Str 常量（如 types.ring `BUILTIN_OPTION`）每次访问调 getter → `ring_str_from_cstr` fresh 分配；D5 box-profile 实测 getter 结果在调用位（BinOp 操作数/cond 位）100% 不被 drop：`BUILTIN_OPTION` 18.0M + `BUILTIN_STR` 6.3M + `BUILTIN_INT` 4.7M + FLOAT/BOOL 0.7M ≈ **29.6M live @2.382B（STR residual 的 51%）**。JS 后端 const = 模块级单例——又一个 #151/#153 同构的 JS/LLVM 偏离。热度驱动者 = 穷尽检查/type_to_string 把 BUILTIN_* 当比较常量高频访问（exhaustive.ring:121/245/332/573 在非错误路径拿 type_to_string 当类型 identity key）。**已拍板（2026-06-12 Discussion）= const 单例化（lazy memoised getter，D4 基建），收口归 B-104 D6**（spec 见 backlog D6 块），落地后删除本条；注意同站点的 `&&` 双臂 BOOL 泄漏属 And/Or-cond 类 → B-104 D7（lower 成 if-else）收口，两类在 types.ring:386 一行叠加。发现者：B-104 D5。
