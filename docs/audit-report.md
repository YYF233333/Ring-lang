...
### #162 Match string literal pattern 泄漏临时字符串 [medium] [mechanical] [doing]
...
### #164 Dict struct 类型硬编码 4 方法 slot — trait 5+ 方法时 LLVM UB [medium] [mechanical] [doing]
...
### #174 Wrapped dict thunk evidence 参数与 inner dict 参数顺序交叉 [medium] [mechanical] [doing]
...
### #176 LLVM codegen match/if-let catch-all 将未识别 pattern 类型视为 irrefutable [low] [mechanical] [doing]
...
### #181 Int literal or-pattern 在 LLVM codegen crash [medium] [mechanical] [doing]
