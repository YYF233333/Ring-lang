...
### #7 `infer.ring` 3016 行单文件 [low] [judgment] [open] [blocked: circular-import]

编译器最大单文件，从 2565→2763→2826→3016 行持续增长。三路互递归（`infer_block` ↔ `infer_stmt` ↔ `infer_expr`）阻塞拆分——Ring 不支持循环模块 import（E0704）。备选：(A) 提取非互递归 helper ~600 行；(B) 回调参数打破环；(C) nominal 重构后重评（选定）。
...
### #16 StructType/EnumType 携带冗余 fields/variants 数据 [low] [judgment] [open] → **即将修复**
...
**2026-06-25**：用户拍板启动。影响面调研完成——159 构造点 + 132 访问点 / 18 文件。registry 已存在（TypeEnv.types.structs/enums）。
