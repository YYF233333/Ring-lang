...
**I′ H+T candidate object-identity verdict（2026-08-21 用户批准 A）**：exact fresh snapshot `ffd6a4160365e5c78682601592ed0301b038f217` 已在A-env下完成一次gen0→gen1 construction；gen1 SHA-256为`9D786546E6BBCE32332C6A7225A25F2888B7635EC9F480F76942E5A17C350668`，该结论只证明construction。sealed structural v3随后使四个candidate `--verify-rc` case以及off/on1/on2 child与audit全部成功；off/on C literal byte-identical、on1/on2 ledger literal byte-identical且candidate hash稳定。唯一失败是两个102,071-byte AMD64 COFF objects的raw identity：差异集合仅含header offset 4，属于标准4-byte `TimeDateStamp`（offset 4..7）；archive `bench/check/results/iprime-78382c51-candidate-structural-v3-failure.7z` SHA-256 `621EB7F98709C6E1F470340B72643202D7326895DE8457D3A3C2A620B0D7E766`。用户裁决验收oracle保留raw objects与hashes并验证AMD64 COFF结构，只为equality assertion归一化offset 4..7，同时要求全部raw diff offsets严格包含于该字段；C、ledger、stderr仍逐字节一致。不得删除object比较、不得改Ring clang invocation或为此重建gen1；v3继续sealed且不构成I′ acceptance，只有harness-only改动独立review并形成新单次packet后才可继续，其余focused/gen2/RC/ASan/self-host/12 GiB fixed-point门不变。
