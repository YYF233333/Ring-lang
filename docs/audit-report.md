...
测试复现：`match (a, b) { (some(x), none) => x, (none, none) => 0 }` → 最后一个 arm 生成 `const none = __ring_m[0]; const none = __ring_m[1];`
...
### #80 `gen_try_catch` 中 `body_has_fail` 死代码遍历 [medium] [open]

`gen_try_catch`（codegen_expr.ring:911-912）调用 `has_fail_effect(body)` 计算 `body_has_fail`，但该变量从未被引用。`has_fail_effect`（25 行 HExpr match，行 976-1001）和 `check_fail` helper 仍存在且每次 `gen_try_catch` 调用都执行无用的全 HExpr 遍历。CLAUDE.md 已注明"gen_try_catch 已去除 has_fail_effect 前置检查"，应删除残留死代码。

发现者：DS

### #83 `resolve_field_action` 对 NeverType/AnyType/ErrorType 静默返回 none [medium] [open]
