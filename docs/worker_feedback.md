...
## [通知] B-098 CAPSTONE STRETCH 结果（native 自编 `--target=llvm`）— 停在 B-099 边界，非 B-098 根因

按 spec 试跑 `./ring.exe build compiler/main.ring --target=llvm`（native 编译器产 native 代码）。**不通**，但根因 = **B-099（native 自托管 LLVM 后端）缺失，与 B-098 RC 模型无关**，故按 spec「确实不同根因 → 记录并停 STRETCH，不硬凑」处理：

1. **`--target=llvm` 在 native 下结构性不可能**：native ring.exe 跑 `--target=llvm` 立即 `ring panic: LLVM: missing function 'LLVMInitializeX86TargetInfo'`——native 二进制没链接 LLVM-C 库（JS 路径靠 N-API addon 桥接，native 路径需直接链 LLVM）。这正是 B-099「最后一公里」的定义（backlog: 「native 二进制自己产 native 代码、Node 从工具链彻底消除」），明确 deferred 在 B-098+B-089 **之后**。3.2s 后的 access violation = 调到缺失/空的 LLVM 函数指针，非 RC over-free。
