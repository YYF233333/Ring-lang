...
| 特性 | 原因 | 预估 |
|------|------|------|
| ✅ **Module system**（`use`/`pub`/`pub use`，多文件编译） | 编译器不可能是单文件 | Phase 3a Batch 4 |
| ✅ **File I/O**（通过 extern fn，读源码写输出） | CLI 需要文件操作 | Phase 3a Batch 6 |

Map/Set/Tuple 已就绪。Auto-derive（Phase 3a Batch 8 完成）、Refinement types、Linear types 等是自举**之后**的特性，不阻塞。
...
每替换一个模块后跑全量 E2E（当前 315 个）。全部通过才进入下一个模块。
