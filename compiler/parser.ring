...
impl Parser {
    ...
    fn parse_decl(mut self) -> Decl? {
        ...
        while self.check(TokenKind::TkAt) {
            ...
            let attr_end = rparen.span.end
            for trait_name in trait_names {
                let trait_span_start = Position {
                    line: attr_start.line,
                    column: attr_start.column,
                    offset: attr_start.offset
                }
                let trait_span_end = Position {
                    line: attr_end.line,
                    column: attr_end.column,
                    offset: attr_end.offset
                }
                derive_attrs.push(DeriveAttribute {
                    trait_name: trait_name,
                    span: self.make_span(trait_span_start, trait_span_end)
                })
            }
        }
