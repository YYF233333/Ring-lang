...
function empty_fn_bounds() {
  const dummy = new FnBoundsEntry(0, "", "");
  const x = [dummy];
  List_clear(x);
  return x;
}
...
function new_infer_ctx(sink) {
  return new InferCtx(env$new_type_env(), unify$empty_subst(), sink, map_new(), Option_none, empty_fn_bounds(), empty_fn_bounds_stack(), 0);
}
...
function generalize(env, t, subst) {
  ...
  let bounds = empty_scheme_bounds();
...
function empty_record_fields() {
  const dummy = new types$RecordField("", types$UNIT());
  const x = [dummy];
  List_clear(x);
  return x;
}
