...
import { infer_block as infer$infer_block, infer_stmt as infer$infer_stmt, infer_expr as infer$infer_expr, check as infer$check, resolve_type_expr_public as infer$resolve_type_expr_public } from "./infer.js";
    ...
    if (__ring_m0._tag === "some") {
      ...
      for (const file of STD_FILES()) {
        ...
        if (file_exists(file_path)) {
          ...
          const ast = parser$parse(source, file_path);
...
function new_infer_ctx(sink) {
  ...
  let fn_bounds_stack = empty_fn_bounds_stack();
  return new infer_ctx$InferCtx(env, unify$empty_subst(), sink, map_new(), Option_none, infer_ctx$empty_fn_bounds(), fn_bounds_stack, 0);
}

function empty_fn_bounds_stack() {
  const x = [infer_ctx$empty_fn_bounds()];
  List_clear(x);
  return x;
}
...
function check(program, sink) {
  ...
  load_prelude(ctx);
  const hprogram = infer$check(ctx, program);
...
function inject_module_exports(ctx, exports) {
  for (const mod_ of exports) {
    for (const entry of _Map_entries(mod_.types)) {
      ...
      __ring_match1: {
        ...
        if (__ring_m1._tag === "StructDef_") {
          ...
          _Map_insert(ctx.env.structs, name, sdef);
          break __ring_match1;
        }
        if (__ring_m1._tag === "EnumDef_") {
          const edef = __ring_m1._0;
          _Map_insert(ctx.env.enums, name, edef);
          for (const v of edef.variants) {
            _Map_insert(ctx.env.variant_to_enum, v.name, name);
          }
          break __ring_match1;
        }
    ...
    for (const entry of _Map_entries(mod_.impl_methods)) {
      ...
      __ring_match2: {
        const __ring_m2 = _Map_get(ctx.env.impl_methods, type_name);
        ...
        if (__ring_m2._tag === "none") {
          _Map_insert(ctx.env.impl_methods, type_name, map_clone(methods));
          break __ring_match2;
        }
