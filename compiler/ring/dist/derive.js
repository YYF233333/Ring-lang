...
function run_derive_pass(env) {
  let derived_impls = empty_derived_impls();
...
function derive_trait(env, all_types, trait_name, derived_impls) {
  let known = set_new();
  for (const imp of env.trait_impls) {
  ...
  while (changed) {
    ...
    for (const ut of all_types) {
      if (_Set_contains(known, ut.name)) {
      } else {
...
function try_derive(env, ut, trait_name, known) {
  let bounds = empty_trait_bounds();
  __ring_match5: {
    const __ring_m5 = ut.type_kind;
    if (__ring_m5._tag === "StructKind") {
      __ring_match6: {
        const __ring_m6 = ut.struct_def;
        if (__ring_m6._tag === "some") {
          const def = __ring_m6._0;
          ...
          __ring_match7: {
            const __ring_m7 = fields;
            if (__ring_m7._tag === "some") {
              const fs = __ring_m7._0;
    ...
    if (__ring_m5._tag === "EnumKind") {
      __ring_match8: {
        const __ring_m8 = ut.enum_def;
        if (__ring_m8._tag === "some") {
          const def = __ring_m8._0;
          let variants = empty_derived_variants();
          let ok = true;
  ...
  __match_fail(__ring_m);
              ...
              let field_entries = empty_field_entries();
  ...
  __match_fail(__ring_m);
              ...
              const fields = try_derive_fields(env, field_entries, def.type_param_vars, def.type_params, trait_name, known, ut.name, bounds);
              __ring_match9: {
                const __ring_m9 = fields;
                if (__ring_m9._tag === "some") {
                  const fs = __ring_m9._0;
                  let final_fields = fs;
                  if ((has_named_fields === false)) {
                    let updated = empty_derived_fields();
...
function try_derive_fields(env, fields, type_param_vars, type_param_names, trait_name, known, self_type_name, bounds) {
  let result = empty_derived_fields();
  for (const field of fields) {
    ...
    __ring_match10: {
      const __ring_m10 = action;
      if (__ring_m10._tag === "some") {
        const a = __ring_m10._0;
...
function resolve_field_action(env, field_type, type_param_vars, type_param_names, trait_name, known, self_type_name, bounds) {
  __ring_match11: {
    ...
    if (__ring_m11._tag === "TypeVar") {
      ...
      return Option_some(hir$FieldAction_Call(hir$trait_bound_param_name(param_name, trait_name), empty_strs()));
      break __ring_match11;
    }
    if (__ring_m11._tag === "StructType") {
      const name = __ring_m11.name; const type_params = __ring_m11.type_params;
      if ((name === self_type_name)) {
        const extra = resolve_extra_dicts(type_params, type_param_vars, type_param_names, trait_name, known, self_type_name, bounds);
        __ring_match12: {
          const __ring_m12 = extra;
        ...
        if (_Set_contains(known, name)) {
          const extra = resolve_extra_dicts(type_params, type_param_vars, type_param_names, trait_name, known, self_type_name, bounds);
          __ring_match13: {
            const __ring_m13 = extra;
...
function register_derived_impl(env, di, trait_name) {
  List_push(env.trait_impls, new env$ImplEntry(trait_name, di.type_name, di.type_params, get_method_names(trait_name)));
  let methods = (function() {
  const __ring_m = _Map_get(env.impl_methods, di.type_name);
  if (__ring_m._tag === "some") { const m = __ring_m._0; return m; }
  if (__ring_m._tag === "none") { return map_new(); }
  __match_fail(__ring_m);
})();
  let type_var_ids = empty_ints();
  let self_type_params = empty_type_list();
  ...
  return _Map_insert(env.impl_methods, di.type_name, methods);
}
...
function empty_derived_impls() {
  const x = [0];
  List_clear(x);
  return x.map((function(i) { return panic("unreachable"); }));
}

function empty_user_types() {
  const x = [0];
  List_clear(x);
  return x.map((function(i) { return panic("unreachable"); }));
}
...
function empty_derived_variants() {
  const x = [0];
  List_clear(x);
  return x.map((function(i) { return panic("unreachable"); }));
}

function empty_field_entries() {
  const x = [0];
  List_clear(x);
  return x.map((function(i) { return panic("unreachable"); }));
}

function empty_derived_fields() {
  const x = [0];
  List_clear(x);
  return x.map((function(i) { return panic("unreachable"); }));
}

function empty_type_list() {
  const x = [0];
  List_clear(x);
  return x.map((function(i) { return panic("unreachable"); }));
}
