...
fn duplicate_c_derived_bounds(bounds: List<TraitBound>) -> List<TraitBound> {
    let mut duplicated: List<TraitBound> = []
    for bound in bounds {
        duplicated.push(TraitBound {
...
fn predeclare_c_derived_fn(
    ...
    let c_name = if is_runtime_symbol(mangled) { "${mangled}__ring" } else { mangled }
    let mangled_for_function = mangled
    let mangled_for_predeclared = mangled
    let mangled_for_evidence = mangled
    let mangled_for_bounds = mangled
    let c_name_for_function = c_name
    let c_name_for_proto = c_name
    ctx.functions.insert(mangled_for_function, CFnInfo {
        c_name: c_name_for_function,
        total_params: total
    })
    ctx.predeclared_derived_fns.insert(mangled_for_predeclared)
    let mut no_ev: List<Str> = []
    ctx.fn_evidence_params.insert(mangled_for_evidence, no_ev)
    ctx.fn_trait_bounds.insert(mangled_for_bounds, bounds)
    let mut ps: List<Str> = []
    for _i in 0..total { ps.push("void*") }
    ctx.fn_protos.push("void* ${c_name_for_proto}(${ps.join(", ")});")
}
...
fn begin_c_derived_fn(mut ctx: CCtx, type_name: Str, method_name: Str, trait_name: Str, is_binary: Bool, bounds: List<TraitBound>) -> CDerivedFn? {
    ...
    if !ctx.predeclared_derived_fns.contains(mangled) {
        ...
        let predeclared_bounds = duplicate_c_derived_bounds(bounds)
        predeclare_c_derived_fn(
            ctx, type_name, method_name, is_binary, predeclared_bounds)
    }
...
fn predeclare_c_derived_trait_dict(
    ...
    let dict_name = trait_dict_name(target_type, trait_name)
    let dict_name_for_insert = dict_name
    let dict_name_for_proto = dict_name
    if !ctx.dict_build_fns.contains(dict_name) {
        ctx.dict_build_fns.insert(dict_name_for_insert)
        ctx.fn_protos.push(
            "void* ring_dict_build_${c_symbol_fragment(dict_name_for_proto)}(void);")
    }
}
