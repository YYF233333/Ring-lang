    ...
    __ring_match16: {
      ...
      if (__ring_m16._tag === "some") {
        ...
        if (_Set_contains(expanding, eff.name, __Str_Eq)) {
          ...
          const __ring_iter_12 = __List_Iterable.iter(saved_scope);
          while (true) {
          ...
          const __ring_iter_13 = __List_Iterable.iter(expanded);
          while (true) {
      ...
      if (__ring_m16._tag === "none") {
        List_push(effects, infer_ctx$resolve_effect_expr(ctx, eff));
        break __ring_match16;
      }
...
function prefix_decl_name(mod_name, decl) {
  __ring_match27: {
    const __ring_m27 = decl;
    if (__ring_m27._tag === "Fn") {
      const name = __ring_m27.name; const type_params = __ring_m27.type_params; const params = __ring_m27.params; const return_type = __ring_m27.return_type; const declared_effects = __ring_m27.declared_effects; const body = __ring_m27.body; const is_pub = __ring_m27.is_pub; const is_abstract = __ring_m27.is_abstract; const span = __ring_m27.span;
    ...
    if (__ring_m27._tag === "Struct") {
      const name = __ring_m27.name; const type_params = __ring_m27.type_params; const fields = __ring_m27.fields; const is_pub = __ring_m27.is_pub; const span = __ring_m27.span;
      return ast$Decl_Struct(`${mod_name}::${name}`, type_params, fields, is_pub, span);
      break __ring_match27;
    }
    if (__ring_m27._tag === "Enum") {
      const name = __ring_m27.name; const type_params = __ring_m27.type_params; const variants = __ring_m27.variants; const is_pub = __ring_m27.is_pub; const span = __ring_m27.span;
      return ast$Decl_Enum(`${mod_name}::${name}`, type_params, variants, is_pub, span);
      break __ring_match27;
    }
    if (__ring_m27._tag === "ExternFn") {
      const name = __ring_m27.name; const type_params = __ring_m27.type_params; const params = __ring_m27.params; const return_type = __ring_m27.return_type; const declared_effects = __ring_m27.declared_effects; const is_pub = __ring_m27.is_pub; const span = __ring_m27.span;
      return ast$Decl_ExternFn(`${mod_name}::${name}`, type_params, params, return_type, declared_effects, is_pub, span);
      break __ring_match27;
    }
    if (__ring_m27._tag === "Const") {
      const name = __ring_m27.name; const type_annotation = __ring_m27.type_annotation; const init = __ring_m27.init; const is_pub = __ring_m27.is_pub; const span = __ring_m27.span;
      return ast$Decl_Const(`${mod_name}::${name}`, type_annotation, init, is_pub, span);
      break __ring_match27;
    }
    if (__ring_m27._tag === "Sig") {
      const name = __ring_m27.name; const members = __ring_m27.members; const is_pub = __ring_m27.is_pub; const span = __ring_m27.span;
      return ast$Decl_Sig(`${mod_name}::${name}`, members, is_pub, span);
      break __ring_match27;
    }
    if (__ring_m27._tag === "Impl") {
      const target_type = __ring_m27.target_type; const type_params = __ring_m27.type_params; const trait_name = __ring_m27.trait_name; const methods = __ring_m27.methods; const span = __ring_m27.span;
    ...
    if (__ring_m27._tag === "Trait") {
      const name = __ring_m27.name; const type_params = __ring_m27.type_params; const supertraits = __ring_m27.supertraits; const methods = __ring_m27.methods; const is_pub = __ring_m27.is_pub; const span = __ring_m27.span;
      return ast$Decl_Trait(`${mod_name}::${name}`, type_params, supertraits, methods, is_pub, span);
      break __ring_match27;
    }
    if (__ring_m27._tag === "Effect") {
      const name = __ring_m27.name; const type_params = __ring_m27.type_params; const ops = __ring_m27.ops; const is_pub = __ring_m27.is_pub; const span = __ring_m27.span;
      return ast$Decl_Effect(`${mod_name}::${name}`, type_params, ops, is_pub, span);
      break __ring_match27;
    }
    if (__ring_m27._tag === "ExternType") {
      const name = __ring_m27.name; const type_params = __ring_m27.type_params; const is_pub = __ring_m27.is_pub; const span = __ring_m27.span;
      return ast$Decl_ExternType(`${mod_name}::${name}`, type_params, is_pub, span);
      break __ring_match27;
    }
    if (__ring_m27._tag === "TypeAlias") {
      const name = __ring_m27.name; const type_params = __ring_m27.type_params; const type_expr = __ring_m27.type_expr; const is_pub = __ring_m27.is_pub; const span = __ring_m27.span;
      return ast$Decl_TypeAlias(`${mod_name}::${name}`, type_params, type_expr, is_pub, span);
      break __ring_match27;
    }
    if (__ring_m27._tag === "EffectAlias") {
      const name = __ring_m27.name; const type_params = __ring_m27.type_params; const effects = __ring_m27.effects; const is_pub = __ring_m27.is_pub; const span = __ring_m27.span;
      return ast$Decl_EffectAlias(`${mod_name}::${name}`, type_params, effects, is_pub, span);
      break __ring_match27;
    }
    if (__ring_m27._tag === "ModBlock") {
      const name = __ring_m27.name; const uses = __ring_m27.uses; const decls = __ring_m27.decls; const required_effects = __ring_m27.required_effects; const is_pub = __ring_m27.is_pub; const span = __ring_m27.span;
      return ast$Decl_ModBlock(`${mod_name}::${name}`, uses, decls, required_effects, is_pub, span);
      break __ring_match27;
    }
    if (__ring_m27._tag === "AssocType") {
      return decl;
      break __ring_match27;
    }
    return decl;
...
function register_const(ctx, name, type_annotation, span) {
  ...
  __ring_match30: {
    const __ring_m30 = type_annotation;
    if (__ring_m30._tag === "some") {
      const texpr = __ring_m30._0;
...
function register_effect(ctx, name, type_params, ops) {
  ...
  while (true) {
    ...
    let param_types = [];
    const __ring_iter_23 = __List_Iterable.iter(op.params);
    while (true) {
...
function register_fn_common(ctx, name, type_params, params, return_type, declared_effects, span, check_dup, track_mut_params, track_fn_bounds) {
  ...
  if (track_mut_params) {
    ...
    while (true) {
      ...
      let __ring_blk0;
      __ring_match37: {
        const __ring_m37 = p.type_annotation;
        if (__ring_m37._tag === "some") {
          const ta = __ring_m37._0;
          __ring_blk0 = infer_ctx$resolve_type_expr(ctx, ta);
  ...
  __ring_match39: {
    ...
    if (__ring_m39._tag === "some") {
      ...
      __ring_blk1 = infer_ctx$resolve_type_expr(ctx, rt);
  ...
  sorted_tp_scope3.sort((function(__ring_a0, __ring_a1) { return hir$compare_by_first(__ring_a0, __ring_a1); }));
  const __ring_iter_31 = __List_Iterable.iter(sorted_tp_scope3);
  while (true) {
  ...
  let __ring_blk2;
  __ring_match41: {
    const __ring_m41 = declared_effects;
    if (__ring_m41._tag === "some") {
      const de = __ring_m41._0;
      __ring_blk2 = resolve_declared_effects(ctx, de);
      break __ring_match41;
    }
    if (__ring_m41._tag === "none") {
      __ring_blk2 = types$EMPTY_ROW;
      break __ring_match41;
    }
    __match_fail(__ring_m41);
  }
  const effects = __ring_blk2;
...
function resolve_impl_self_type(ctx, target_type, impl_type_params) {
  ...
  let impl_tp_types = [];
  const __ring_iter_38 = __List_Iterable.iter(impl_type_params);
  while (true) {
  ...
  __ring_match52: {
    const __ring_m52 = _Map_get(ctx.env.types.structs, target_type);
    ...
    if (__ring_m52._tag === "none") {
      __ring_match53: {
        const __ring_m53 = _Map_get(ctx.env.types.enums, target_type);
...
function register_impl_method(ctx, methods_map, impl_tv_ids, target_type, mname, mtps, params, return_type, declared_effects, impl_scheme_bounds, outer_saved, impl_type_params, is_extern) {
  ...
  let param_types = [];
  const __ring_iter_40 = __List_Iterable.iter(params);
  while (true) {
  ...
  let __ring_blk3;
  __ring_match56: {
    const __ring_m56 = return_type;
    if (__ring_m56._tag === "some") {
      const rt = __ring_m56._0;
  ...
  const __ring_iter_41 = __List_Iterable.iter(method_tv_ids);
  while (true) {
    const __ring_next_41 = __ListIterator_Iterator.next(__ring_iter_41);
    if (__ring_next_41._tag === "none") break;
    const mtv = __ring_next_41._0;
    List_push(all_tvs, mtv);
  }
  if ((!is_extern)) {
    let declared_names = set_new();
    let sorted_tp_scope = _Map_entries(ctx.type_param_scope);
    sorted_tp_scope.sort((function(__ring_a0, __ring_a1) { return hir$compare_by_first(__ring_a0, __ring_a1); }));
    const __ring_iter_42 = __List_Iterable.iter(sorted_tp_scope);
    while (true) {
  ...
  let __ring_blk4;
  __ring_match58: {
    const __ring_m58 = declared_effects;
    if (__ring_m58._tag === "some") {
      const de = __ring_m58._0;
  ...
  if ((List_len(params) > 0)) {
    __ring_match59: {
      const __ring_m59 = List_first(params);
      if (__ring_m59._tag === "some") {
        const first_p = __ring_m59._0;
        if (((first_p.name === "self") ? first_p.is_mutable : false)) {
          let __ring_blk5;
          __ring_match60: {
            const __ring_m60 = _Map_get(ctx.env.trait_reg.mut_methods, target_type);
...
function register_impl(ctx, target_type, type_params, trait_name, methods, span) {
  let __ring_blk6;
  __ring_match61: {
    const __ring_m61 = _Map_get(ctx.env.trait_reg.impl_methods, target_type);
    if (__ring_m61._tag === "some") {
      const m = __ring_m61._0;
      __ring_blk6 = m;
  ...
  const __ring_iter_48 = __List_Iterable.iter(methods);
  while (true) {
    const __ring_next_48 = __ListIterator_Iterator.next(__ring_iter_48);
    if (__ring_next_48._tag === "none") break;
    const method = __ring_next_48._0;
    __ring_match63: {
      const __ring_m63 = method;
      if (__ring_m63._tag === "AssocType") {
        const aname = __ring_m63.name; const avalue = __ring_m63.value; const aspan = __ring_m63.span;
        __ring_match64: {
          const __ring_m64 = avalue;
          if (__ring_m64._tag === "some") {
            const v = __ring_m64._0;
  ...
  sorted_assoc_map.sort((function(__ring_a0, __ring_a1) { return hir$compare_by_first(__ring_a0, __ring_a1); }));
  const __ring_iter_49 = __List_Iterable.iter(sorted_assoc_map);
  while (true) {
  ...
  const __ring_iter_50 = __List_Iterable.iter(methods);
  while (true) {
    ...
    __ring_match65: {
      ...
      if (__ring_m65._tag === "Fn") {
        const mname = __ring_m65.name; const mtps = __ring_m65.type_params; const params = __ring_m65.params; const return_type = __ring_m65.return_type; const declared_effects = __ring_m65.declared_effects;
  ...
  __ring_match66: {
    const __ring_m66 = trait_name;
    if (__ring_m66._tag === "some") {
      const tname = __ring_m66._0;
      __ring_match67: {
        const __ring_m67 = _Map_get(ctx.env.trait_reg.traits, tname);
        if (__ring_m67._tag === "some") {
          const trait_def = __ring_m67._0;
          ...
          sorted_assoc_map2.sort((function(__ring_a0, __ring_a1) { return hir$compare_by_first(__ring_a0, __ring_a1); }));
          const __ring_iter_53 = __List_Iterable.iter(sorted_assoc_map2);
          while (true) {
          ...
          while (true) {
            ...
            if ((!_Set_contains(impl_assoc_names, atdef.name, __Str_Eq))) {
              __ring_match69: {
                const __ring_m69 = atdef.default_type;
                if (__ring_m69._tag === "some") {
                  const dt = __ring_m69._0;
          ...
          sorted_assoc_map3.sort((function(__ring_a0, __ring_a1) { return hir$compare_by_first(__ring_a0, __ring_a1); }));
          const __ring_iter_56 = __List_Iterable.iter(sorted_assoc_map3);
          while (true) {
...
function register_sig(ctx, name, members, is_pub) {
  ...
  const __ring_iter_62 = __List_Iterable.iter(members);
  while (true) {
    ...
    let param_types = [];
    const __ring_iter_64 = __List_Iterable.iter(m.params);
    while (true) {
      const __ring_next_64 = __ListIterator_Iterator.next(__ring_iter_64);
      if (__ring_next_64._tag === "none") break;
      const p = __ring_next_64._0;
      __ring_match74: {
        const __ring_m74 = p.type_annotation;
        if (__ring_m74._tag === "some") {
          const ta = __ring_m74._0;
          List_push(param_types, infer_ctx$resolve_type_expr(ctx, ta));
    ...
    let __ring_blk7;
    __ring_match75: {
      const __ring_m75 = m.return_type;
      if (__ring_m75._tag === "some") {
        const rt = __ring_m75._0;
...
function register_trait(ctx, name, type_params, supertraits, methods, span) {
  ...
  while (true) {
    ...
    while ((List_len(stack) > 0)) {
      ...
      _Set_insert(visited, current);
      __ring_match77: {
        const __ring_m77 = _Map_get(ctx.env.trait_reg.traits, current);
        if (__ring_m77._tag === "some") {
          const parent_def = __ring_m77._0;
          const __ring_iter_68 = __List_Iterable.iter(parent_def.supertraits);
          while (true) {
            const __ring_next_68 = __ListIterator_Iterator.next(__ring_iter_68);
            if (__ring_next_68._tag === "none") break;
            const parent_st = __ring_next_68._0;
            List_push(stack, parent_st);
          }
  ...
  while (true) {
    ...
    __ring_match78: {
      ...
      if (__ring_m78._tag === "AssocType") {
        const aname = __ring_m78.name; const abounds = __ring_m78.bounds; const avalue = __ring_m78.value;
  ...
  const __ring_iter_71 = __List_Iterable.iter(assoc_type_defs);
  while (true) {
    const __ring_next_71 = __ListIterator_Iterator.next(__ring_iter_71);
    if (__ring_next_71._tag === "none") break;
    const atd = __ring_next_71._0;
    __ring_match80: {
      const __ring_m80 = _Map_get(ctx.type_param_scope, atd.name);
      if (__ring_m80._tag === "some") {
        const at_ty = __ring_m80._0;
  ...
  while (true) {
    ...
    __ring_match81: {
      const __ring_m81 = method;
      if (__ring_m81._tag === "Fn") {
        const mname = __ring_m81.name; const method_tps = __ring_m81.type_params; const params = __ring_m81.params; const return_type = __ring_m81.return_type; const declared_effects = __ring_m81.declared_effects; const is_abstract = __ring_m81.is_abstract;
        let param_types = [];
        let param_muts = [];
        const __ring_iter_73 = __List_Iterable.iter(params);
        while (true) {
          ...
          if ((p.name === "self")) {
            ...
            __ring_match82: {
              const __ring_m82 = p.type_annotation;
              if (__ring_m82._tag === "some") {
                const ta = __ring_m82._0;
                List_push(param_types, infer_ctx$resolve_type_expr(ctx, ta));
        ...
        let __ring_blk9;
        __ring_match83: {
          const __ring_m83 = return_type;
          if (__ring_m83._tag === "some") {
            const rt = __ring_m83._0;
            __ring_blk9 = infer_ctx$resolve_type_expr(ctx, rt);
            break __ring_match83;
          }
          if (__ring_m83._tag === "none") {
            __ring_blk9 = env$TypeEnv_fresh_var(ctx.env);
            break __ring_match83;
          }
          __match_fail(__ring_m83);
        }
        const ret = __ring_blk9;
        let __ring_blk10;
        __ring_match84: {
          const __ring_m84 = declared_effects;
          if (__ring_m84._tag === "some") {
            const de = __ring_m84._0;
            __ring_blk10 = resolve_declared_effects(ctx, de);
            break __ring_match84;
          }
          if (__ring_m84._tag === "none") {
            __ring_blk10 = types$EMPTY_ROW;
            break __ring_match84;
          }
          __match_fail(__ring_m84);
        }
        const method_effects = __ring_blk10;
...
function register_phase1(ctx, decl, deferred_struct_names, deferred_enum_names) {
  __ring_match86: {
    const __ring_m86 = decl;
    if (__ring_m86._tag === "Struct") {
      const name = __ring_m86.name; const type_params = __ring_m86.type_params; const fields = __ring_m86.fields; const span = __ring_m86.span;
      preregister_struct(ctx, name, type_params);
      return List_push(deferred_struct_names, name);
      break __ring_match86;
    }
    if (__ring_m86._tag === "Enum") {
      const name = __ring_m86.name; const type_params = __ring_m86.type_params; const variants = __ring_m86.variants; const span = __ring_m86.span;
      preregister_enum(ctx, name, type_params);
      return List_push(deferred_enum_names, name);
      break __ring_match86;
    }
    if (__ring_m86._tag === "ModBlock") {
      const mod_name = __ring_m86.name; const mod_decls = __ring_m86.decls;
...
function register_mod_item(ctx, decl, deferred_struct_names, deferred_enum_names) {
  __ring_match87: {
    const __ring_m87 = deferred_struct_names;
    if (__ring_m87._tag === "some") {
      const dsn = __ring_m87._0;
      __ring_match88: {
        const __ring_m88 = deferred_enum_names;
        if (__ring_m88._tag === "some") {
          const den = __ring_m88._0;
...
function register_mod_block_items(ctx, mod_name, mod_decls, deferred_struct_names, deferred_enum_names) {
  const __ring_iter_76 = __List_Iterable.iter(mod_decls);
  while (true) {
  ...
  while (true) {
    ...
    __ring_match91: {
      const __ring_m91 = d;
      if (__ring_m91._tag === "Effect") {
        const prefixed = prefix_decl_name(mod_name, d);
        register_mod_item(ctx, prefixed, deferred_struct_names, deferred_enum_names);
        break __ring_match91;
      }
      if (__ring_m91._tag === "EffectAlias") {
        const prefixed = prefix_decl_name(mod_name, d);
        register_mod_item(ctx, prefixed, deferred_struct_names, deferred_enum_names);
        break __ring_match91;
      }
      if (__ring_m91._tag === "ExternType") {
        const prefixed = prefix_decl_name(mod_name, d);
        register_mod_item(ctx, prefixed, deferred_struct_names, deferred_enum_names);
        break __ring_match91;
      }
  ...
  while (true) {
    ...
    __ring_match92: {
      const __ring_m92 = d;
      if (__ring_m92._tag === "Struct") {
        break __ring_match92;
      }
      if (__ring_m92._tag === "Enum") {
        break __ring_match92;
      }
      if (__ring_m92._tag === "Trait") {
        break __ring_match92;
      }
      if (__ring_m92._tag === "Effect") {
        break __ring_match92;
      }
      if (__ring_m92._tag === "EffectAlias") {
        break __ring_match92;
      }
      if (__ring_m92._tag === "ExternType") {
        break __ring_match92;
      }
      const prefixed = prefix_decl_name(mod_name, d);
      register_mod_item(ctx, prefixed, deferred_struct_names, deferred_enum_names);
...
function register_decl(ctx, decl) {
  __ring_match93: {
    ...
    if (__ring_m93._tag === "Struct") {
      const name = __ring_m93.name; const type_params = __ring_m93.type_params; const fields = __ring_m93.fields; const span = __ring_m93.span;
      preregister_struct(ctx, name, type_params);
      return complete_struct_fields(ctx, name, fields);
      break __ring_match93;
    }
    if (__ring_m93._tag === "Enum") {
      const name = __ring_m93.name; const type_params = __ring_m93.type_params; const variants = __ring_m93.variants; const span = __ring_m93.span;
      preregister_enum(ctx, name, type_params);
      return complete_enum_variants(ctx, name, type_params, variants);
      break __ring_match93;
    }
    if (__ring_m93._tag === "Effect") {
      const name = __ring_m93.name; const type_params = __ring_m93.type_params; const ops = __ring_m93.ops;
      return register_effect(ctx, name, type_params, ops);
      break __ring_match93;
    }
    if (__ring_m93._tag === "Impl") {
      const target_type = __ring_m93.target_type; const type_params = __ring_m93.type_params; const trait_name = __ring_m93.trait_name; const methods = __ring_m93.methods; const span = __ring_m93.span;
      return register_impl(ctx, target_type, type_params, trait_name, methods, span);
      break __ring_match93;
    }
    if (__ring_m93._tag === "Fn") {
      const name = __ring_m93.name; const type_params = __ring_m93.type_params; const params = __ring_m93.params; const return_type = __ring_m93.return_type; const declared_effects = __ring_m93.declared_effects; const span = __ring_m93.span;
      return register_fn(ctx, name, type_params, params, return_type, declared_effects, span);
      break __ring_match93;
    }
    if (__ring_m93._tag === "Test") {
      break __ring_match93;
    }
    if (__ring_m93._tag === "Trait") {
      const name = __ring_m93.name; const type_params = __ring_m93.type_params; const supertraits = __ring_m93.supertraits; const methods = __ring_m93.methods; const span = __ring_m93.span;
      return register_trait(ctx, name, type_params, supertraits, methods, span);
      break __ring_match93;
    }
    if (__ring_m93._tag === "ExternFn") {
      const name = __ring_m93.name; const type_params = __ring_m93.type_params; const params = __ring_m93.params; const return_type = __ring_m93.return_type; const declared_effects = __ring_m93.declared_effects; const span = __ring_m93.span;
      return register_extern_fn(ctx, name, type_params, params, return_type, declared_effects, span);
      break __ring_match93;
    }
    if (__ring_m93._tag === "ExternType") {
      const name = __ring_m93.name; const type_params = __ring_m93.type_params;
      return register_extern_type(ctx, name, type_params);
      break __ring_match93;
    }
    if (__ring_m93._tag === "TypeAlias") {
      const name = __ring_m93.name; const type_params = __ring_m93.type_params; const type_expr = __ring_m93.type_expr;
      return register_type_alias(ctx, name, type_params, type_expr);
      break __ring_match93;
    }
    if (__ring_m93._tag === "Const") {
      const name = __ring_m93.name; const type_annotation = __ring_m93.type_annotation; const span = __ring_m93.span;
      return register_const(ctx, name, type_annotation, span);
      break __ring_match93;
    }
    if (__ring_m93._tag === "Sig") {
      const name = __ring_m93.name; const members = __ring_m93.members; const is_pub = __ring_m93.is_pub;
      return register_sig(ctx, name, members, is_pub);
      break __ring_match93;
    }
    if (__ring_m93._tag === "EffectAlias") {
      const name = __ring_m93.name; const type_params = __ring_m93.type_params; const effects = __ring_m93.effects; const span = __ring_m93.span;
    ...
    if (__ring_m93._tag === "AssocType") {
      break __ring_match93;
    }
    if (__ring_m93._tag === "ModBlock") {
      const mod_name = __ring_m93.name; const mod_decls = __ring_m93.decls;
...
function register_phase2_enum(ctx, decl) {
  __ring_match94: {
    ...
    if (__ring_m94._tag === "Enum") {
      const name = __ring_m94.name; const type_params = __ring_m94.type_params; const variants = __ring_m94.variants; const span = __ring_m94.span;
      return complete_enum_variants(ctx, name, type_params, variants);
      break __ring_match94;
    }
    if (__ring_m94._tag === "ModBlock") {
      const mod_name = __ring_m94.name; const mod_decls = __ring_m94.decls;
...
function register_delegate_traits(ctx, methods_map, impl_tv_ids, target_type, field, trait_names, span, impl_scheme_bounds, impl_type_params, field_type_name, ft) {
  const __ring_iter_82 = __List_Iterable.iter(trait_names);
  while (true) {
    ...
    __ring_match96: {
      const __ring_m96 = _Map_get(ctx.env.trait_reg.traits, tname);
      if (__ring_m96._tag === "none") {
        const _ = infer_ctx$type_error(ctx.sink, codes$E0501, `Unknown trait: ${tname}`, span, diagnostics$DiagnosticContext_TraitError(`unknown trait '${tname}'`));
        break __ring_match96;
      }
      if (__ring_m96._tag === "some") {
        const trait_def = __ring_m96._0;
        if ((!env$has_impl(ctx.env.trait_reg, field_type_name, tname))) {
          ...
          while (true) {
            ...
            __ring_match97: {
              const __ring_m97 = _Map_get(ctx.env.trait_reg.traits, reg_tname);
              if (__ring_m97._tag === "none") {
                break __ring_match97;
              }
              if (__ring_m97._tag === "some") {
                const reg_trait_def = __ring_m97._0;
                let field_assoc_types = map_new();
                __ring_match98: {
                  const __ring_m98 = env$find_impl(ctx.env.trait_reg, field_type_name, reg_tname);
                  if (__ring_m98._tag === "some") {
                    const field_impl = __ring_m98._0;
                ...
                while (true) {
                  ...
                  const tp = __ring_next_85._0;
                  List_push(tp_names, tp.name);
                }
                let method_names = [];
                const __ring_iter_86 = __List_Iterable.iter(reg_trait_def.methods);
                while (true) {
                  const __ring_next_86 = __ListIterator_Iterator.next(__ring_iter_86);
                  if (__ring_next_86._tag === "none") break;
                  const tm = __ring_next_86._0;
                ...
                while (true) {
                  ...
                  __ring_match99: {
                    const __ring_m99 = tm.ty;
                    if (__ring_m99._tag === "FnType") {
                      const trait_params = __ring_m99.params; const trait_ret_ty = __ring_m99.return_type; const trait_eff = __ring_m99.effects;
                      let __ring_blk11;
                      __ring_match100: {
                        const __ring_m100 = field_methods;
                        if (__ring_m100._tag === "some") {
                          const fm_map = __ring_m100._0;
                          __ring_blk11 = _Map_get(fm_map, tm.name);
                      ...
                      const resolved_method_scheme = __ring_blk11;
                      let __ring_blk12;
                      __ring_match101: {
                        const __ring_m101 = resolved_method_scheme;
                        if (__ring_m101._tag === "some") {
                          const rs = __ring_m101._0;
                          let __ring_blk13;
                          __ring_match102: {
                            const __ring_m102 = rs.ty;
                            if (__ring_m102._tag === "FnType") {
                              const rr = __ring_m102.return_type;
                              __ring_blk13 = rr;
                              break __ring_match102;
                            }
                            __ring_blk13 = trait_ret_ty;
                            break __ring_match102;
                          }
                        ...
                        if (__ring_m101._tag === "none") {
                          __ring_blk12 = trait_ret_ty;
                          break __ring_match101;
                        }
                        __match_fail(__ring_m101);
                      }
                      const ret_ty = __ring_blk12;
                      let __ring_blk14;
                      __ring_match103: {
                        ...
                        if (__ring_m103._tag === "some") {
                          ...
                          __ring_match104: {
                            ...
                            if (__ring_m104._tag === "FnType") {
                              const re = __ring_m104.effects;
                              __ring_blk15 = re;
                              break __ring_match104;
                            }
                            __ring_blk15 = trait_eff;
                            break __ring_match104;
                          }
                        ...
                        if (__ring_m103._tag === "none") {
                          __ring_blk14 = trait_eff;
                          break __ring_match103;
                        }
                        __match_fail(__ring_m103);
                      }
                      const eff = __ring_blk14;
                      let param_types = [];
                      let first = true;
...
function register_delegate(ctx, methods_map, impl_tv_ids, target_type, field, trait_names, span, impl_scheme_bounds, outer_saved, impl_type_params) {
  __ring_match105: {
    const __ring_m105 = _Map_get(ctx.env.types.structs, target_type);
    ...
    if (__ring_m105._tag === "some") {
      const struct_def = __ring_m105._0;
...
function register_phase3_delegate(ctx, decl) {
  __ring_match109: {
    const __ring_m109 = decl;
    if (__ring_m109._tag === "Impl") {
      const target_type = __ring_m109.target_type; const type_params = __ring_m109.type_params; const methods = __ring_m109.methods; const span = __ring_m109.span;
      ...
      if (has_delegates) {
        const saved = map_clone(ctx.type_param_scope);
        let impl_tv_ids = [];
        const __ring_iter_91 = __List_Iterable.iter(type_params);
        while (true) {
          ...
          const tv = env$TypeEnv_fresh_var(ctx.env);
          __ring_match111: {
            const __ring_m111 = tv;
            if (__ring_m111._tag === "TypeVar") {
              const id = __ring_m111.id;
              List_push(impl_tv_ids, id);
        ...
        let __ring_blk16;
        __ring_match112: {
          const __ring_m112 = _Map_get(ctx.env.trait_reg.impl_methods, target_type);
          if (__ring_m112._tag === "some") {
            const m = __ring_m112._0;
            __ring_blk16 = m;
          ...
          if (__ring_m112._tag === "none") {
            let new_map = map_new();
            _Map_insert(ctx.env.trait_reg.impl_methods, target_type, new_map);
            __ring_blk16 = new_map;
            break __ring_match112;
          }
          __match_fail(__ring_m112);
        }
        let impl_methods_map = __ring_blk16;
        ...
        while (true) {
          ...
          __ring_match113: {
            const __ring_m113 = m;
            if (__ring_m113._tag === "Delegate") {
              const field = __ring_m113.field; const trait_names = __ring_m113.trait_names; const dspan = __ring_m113.span;
    ...
    if (__ring_m109._tag === "ModBlock") {
      const mod_name = __ring_m109.name; const mod_decls = __ring_m109.decls;
...
export { register_decl_public, insert_mod_aliases, prefix_decl_name, register_decls_two_phase, collect_all_supertraits, resolve_declared_effects, inject_assoc_types_from_bounds };