...
function register_decls_two_phase(ctx, decls, __ring_ev_fail) {
  ...
  for (const decl of decls) {
    const result = Option_some(register_phase1(ctx, decl, deferred_struct_names, deferred_enum_names, __ring_ev_fail));
  }
  for (const decl of decls) {
    const result = Option_some(register_phase2_struct(ctx, decl));
  }
  for (const decl of decls) {
    const result = Option_some(register_phase2_enum(ctx, decl));
  }
}
