...
function register_decl(ctx, decl) {
  __ring_match53: {
    const __ring_m53 = decl;
    if (__ring_m53._tag === "Struct") {
      const name = __ring_m53.name; const type_params = __ring_m53.type_params; const fields = __ring_m53.fields; const span = __ring_m53.span;
      preregister_struct(ctx, name, type_params);
      return complete_struct_fields(ctx, name, fields);
      break __ring_match53;
    }
    if (__ring_m53._tag === "Enum") {
      const name = __ring_m53.name; const type_params = __ring_m53.type_params; const variants = __ring_m53.variants; const span = __ring_m53.span;
      preregister_enum(ctx, name, type_params);
      return complete_enum_variants(ctx, name, type_params, variants);
      break __ring_match53;
    }
    if (__ring_m53._tag === "Effect") {
      const name = __ring_m53.name; const type_params = __ring_m53.type_params; const ops = __ring_m53.ops;
      return register_effect(ctx, name, type_params, ops);
      break __ring_match53;
    }
    if (__ring_m53._tag === "Impl") {
      const target_type = __ring_m53.target_type; const type_params = __ring_m53.type_params; const trait_name = __ring_m53.trait_name; const methods = __ring_m53.methods; const span = __ring_m53.span;
      return register_impl(ctx, target_type, type_params, trait_name, methods, span);
      break __ring_match53;
    }
    if (__ring_m53._tag === "Fn") {
      const name = __ring_m53.name; const type_params = __ring_m53.type_params; const params = __ring_m53.params; const return_type = __ring_m53.return_type; const declared_effects = __ring_m53.declared_effects; const span = __ring_m53.span;
      return register_fn(ctx, name, type_params, params, return_type, declared_effects, span);
      break __ring_match53;
    }
    if (__ring_m53._tag === "Test") {
      break __ring_match53;
    }
    if (__ring_m53._tag === "Trait") {
      const name = __ring_m53.name; const type_params = __ring_m53.type_params; const methods = __ring_m53.methods;
    ...
    if (__ring_m53._tag === "ExternType") {
      const name = __ring_m53.name; const type_params = __ring_m53.type_params;
      return register_extern_type(ctx, name, type_params);
      break __ring_match53;
    }
    if (__ring_m53._tag === "TypeAlias") {
      const name = __ring_m53.name; const type_params = __ring_m53.type_params; const type_expr = __ring_m53.type_expr;
      return register_type_alias(ctx, name, type_params, type_expr);
      break __ring_match53;
    }
    if (__ring_m53._tag === "Const") {
      const name = __ring_m53.name; const type_annotation = __ring_m53.type_annotation; const span = __ring_m53.span;
      return register_const(ctx, name, type_annotation, span);
      break __ring_match53;
    }
    if (__ring_m53._tag === "ModBlock") {
      const mod_name = __ring_m53.name; const mod_decls = __ring_m53.decls;
      for (const d of mod_decls) {
        __ring_match54: {
          const __ring_m54 = d;
          if (__ring_m54._tag === "Struct") {
            const prefixed = prefix_decl_name(mod_name, d);
            register_decl(ctx, prefixed);
            break __ring_match54;
          }
          if (__ring_m54._tag === "Enum") {
            const prefixed = prefix_decl_name(mod_name, d);
            register_decl(ctx, prefixed);
            break __ring_match54;
          }
      ...
      for (const d of mod_decls) {
        __ring_match55: {
          ...
          if (__ring_m55._tag === "Struct") {
            const name = __ring_m55.name;
            const qualified = `${mod_name}::${name}`;
            __ring_match56: {
              const __ring_m56 = _Map_get(ctx.env.types.structs, qualified);
          ...
          if (__ring_m55._tag === "Enum") {
            ...
            const qualified = `${mod_name}::${name}`;
            __ring_match57: {
              const __ring_m57 = _Map_get(ctx.env.types.enums, qualified);
      ...
      for (const d of mod_decls) {
        __ring_match58: {
          const __ring_m58 = d;
          if (__ring_m58._tag === "Struct") {
            break __ring_match58;
          }
          if (__ring_m58._tag === "Enum") {
            break __ring_match58;
          }
          const prefixed = prefix_decl_name(mod_name, d);
          register_decl(ctx, prefixed);
