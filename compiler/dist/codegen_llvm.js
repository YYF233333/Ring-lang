...
function emit_drop_functions(ctx) {
  ...
  while (true) {
    ...
    if (hir$is_type_dag_type_name(sname)) {
      continue;
    }
    __ring_match15: {
  ...
  while (true) {
    ...
    if (hir$is_type_dag_type_name(ename)) {
      continue;
    }
    __ring_match16: {
...
function emit_drop_registrations(ctx) {
  ...
  const register_fn = codegen_llvm_ctx$get_or_declare_runtime_fn(ctx, "ring_register_drop", [i64, ptr], _void);
  const register_ty = codegen_llvm_ctx$get_rt_fn_type(ctx, "ring_register_drop");
  const never_fn = codegen_llvm_ctx$get_or_declare_runtime_fn(ctx, "ring_register_never_drop", [i64], _void);
  const never_ty = codegen_llvm_ctx$get_rt_fn_type(ctx, "ring_register_never_drop");
  const struct_names = _Map_keys(ctx.struct_types);
