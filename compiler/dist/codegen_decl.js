...
function collect_all_supertraits_codegen(ctx, trait_name) {
  ...
  let stack = [];
  __ring_match6: {
    const __ring_m6 = _Map_get(ctx.trait_decls, trait_name);
    if (__ring_m6._tag === "some") {
      const tinfo = __ring_m6._0;
      const __ring_iter_2 = __List_Iterable.iter(tinfo.supertraits);
      while (true) {
  ...
  while ((List_len(stack) > 0)) {
    ...
    __ring_match7: {
      const __ring_m7 = _Map_get(ctx.trait_decls, current);
      if (__ring_m7._tag === "some") {
        const parent_info = __ring_m7._0;
        const __ring_iter_3 = __List_Iterable.iter(parent_info.supertraits);
        while (true) {
...
function emit_effect_decl(ctx, name, ops) {
  ...
  const __ring_iter_5 = __List_Iterable.iter(ops);
  while (true) {
    const __ring_next_5 = __ListIterator_Iterator.next(__ring_iter_5);
    if (__ring_next_5._tag === "none") break;
    const op = __ring_next_5._0;
    __ring_match8: {
      const __ring_m8 = op.default_body;
      if (__ring_m8._tag === "some") {
        const body = __ring_m8._0;
        const body_effs = hir$hexpr_effects(body);
        const eff_names = effect_analysis$extract_effect_names(body_effs);
        const __ring_iter_6 = __List_Iterable.iter(eff_names);
        while (true) {
  ...
  List_sort(body_effect_names, __Str_Ord);
  ...
  if ((List_len(body_ev_params) > 0)) {
    ...
    const __ring_iter_7 = __List_Iterable.iter(ops);
    while (true) {
      const __ring_next_7 = __ListIterator_Iterator.next(__ring_iter_7);
      if (__ring_next_7._tag === "none") break;
      const op = __ring_next_7._0;
      __ring_match9: {
        const __ring_m9 = op.default_body;
        if (__ring_m9._tag === "some") {
          const body = __ring_m9._0;
          let params = [];
          const __ring_iter_8 = __List_Iterable.iter(op.params);
          while (true) {
    ...
    const __ring_iter_9 = __List_Iterable.iter(ops);
    while (true) {
      const __ring_next_9 = __ListIterator_Iterator.next(__ring_iter_9);
      if (__ring_next_9._tag === "none") break;
      const op = __ring_next_9._0;
      __ring_match10: {
        const __ring_m10 = op.default_body;
        if (__ring_m10._tag === "some") {
          const body = __ring_m10._0;
          let params = [];
...
function emit_enum_decl(ctx, name, variants) {
  ...
  const __ring_iter_11 = __List_Iterable.iter(variants);
  while (true) {
    ...
    if ((List_len(v.fields) === 0)) {
      ...
      __ring_match11: {
        const __ring_m11 = v.field_names;
        if (__ring_m11._tag === "some") {
          const fnames = __ring_m11._0;
          let sparams = [];
          const __ring_iter_12 = __List_Iterable.iter(fnames);
          while (true) {
            const __ring_next_12 = __ListIterator_Iterator.next(__ring_iter_12);
            if (__ring_next_12._tag === "none") break;
            const n = __ring_next_12._0;
...
function emit_extern_fn_decl(ctx, name) {
  __ring_match12: {
    const __ring_m12 = ctx.module_prefix;
    if (__ring_m12._tag === "some") {
      const prefix = __ring_m12._0;
...
function emit_fn_decl(ctx, name, params, effects, body, trait_bounds, prefix) {
  let __ring_blk0;
  __ring_match13: {
    const __ring_m13 = prefix;
    if (__ring_m13._tag === "some") {
      const p = __ring_m13._0;
      const sn = codegen_ctx$safe_ident(name);
      __ring_blk0 = `${p}_${sn}`;
  ...
  let dict_params = [];
  const __ring_iter_15 = __List_Iterable.iter(trait_bounds);
  while (true) {
  ...
  let __ring_blk1;
  __ring_match14: {
    const __ring_m14 = _Map_get(ctx.local_fn_effects, name);
    if (__ring_m14._tag === "some") {
      const eff = __ring_m14._0;
      __ring_blk1 = eff;
  ...
  const effective_effects = __ring_blk1;
      ...
      while (true) {
        ...
        if (tm.has_default) {
          if ((_Set_contains(impl_method_names, tm.name, __Str_Eq) === false)) {
            ...
            let param_names = [];
            const __ring_iter_19 = __List_Iterable.iter(tm.params);
            while (true) {
...
function emit_impl_decl(ctx, target_type, trait_name, methods) {
  ...
  __ring_match18: {
    ...
    if (__ring_m18._tag === "some") {
      const tn = __ring_m18._0;
      __ring_blk2 = hir$trait_dict_name(codegen_ctx$qualify(ctx, target_type), codegen_ctx$safe_ident(tn));
      break __ring_match18;
    }
    if (__ring_m18._tag === "none") {
      __ring_blk2 = codegen_ctx$qualify(ctx, target_type);
      break __ring_match18;
    }
    __match_fail(__ring_m18);
  }
  const prefix = __ring_blk2;
  ...
  while (true) {
    ...
    __ring_match19: {
      const __ring_m19 = method;
      if (__ring_m19._tag === "ExternFn") {
        break __ring_match19;
      }
      if (__ring_m19._tag === "Fn") {
        const name = __ring_m19.name; const params = __ring_m19.params; const effects = __ring_m19.effects; const body = __ring_m19.body; const trait_bounds = __ring_m19.trait_bounds;
...
function emit_struct_decl(ctx, name, fields) {
  ...
  codegen_ctx$push_indent(ctx);
  const __ring_end23 = List_len(raw_fields);
  for (let i = 0; i < __ring_end23; i++) {
    __ring_match21: {
      const __ring_m21 = [List_get(raw_fields, i), List_get(safe_fields, i)];
      if (Array.isArray(__ring_m21) && __ring_m21.length === 2 && __ring_m21[0]._tag === "some" && __ring_m21[1]._tag === "some") {
        const raw = __ring_m21[0]._0; const safe = __ring_m21[1]._0;
...
function emit_decl(ctx, decl) {
  __ring_match23: {
    ...
    if (__ring_m23._tag === "Fn") {
      const name = __ring_m23.name; const params = __ring_m23.params; const effects = __ring_m23.effects; const body = __ring_m23.body; const trait_bounds = __ring_m23.trait_bounds;
      return emit_fn_decl(ctx, name, params, effects, body, trait_bounds, Option_none);
      break __ring_match23;
    }
    if (__ring_m23._tag === "Struct") {
      const name = __ring_m23.name; const fields = __ring_m23.fields;
      return emit_struct_decl(ctx, name, fields);
      break __ring_match23;
    }
    if (__ring_m23._tag === "Enum") {
      const name = __ring_m23.name; const variants = __ring_m23.variants;
      return emit_enum_decl(ctx, name, variants);
      break __ring_match23;
    }
    if (__ring_m23._tag === "Impl") {
      const target_type = __ring_m23.target_type; const trait_name = __ring_m23.trait_name; const methods = __ring_m23.methods; const assoc_types = __ring_m23.assoc_types;
      return emit_impl_decl(ctx, target_type, trait_name, methods);
      break __ring_match23;
    }
    if (__ring_m23._tag === "Effect") {
      const name = __ring_m23.name; const ops = __ring_m23.ops;
      return emit_effect_decl(ctx, name, ops);
      break __ring_match23;
    }
    if (__ring_m23._tag === "Test") {
      const description = __ring_m23.description; const body = __ring_m23.body;
      return emit_test_decl(ctx, description, body);
      break __ring_match23;
    }
    if (__ring_m23._tag === "Trait") {
      const name = __ring_m23.name; const methods = __ring_m23.methods; const supertraits = __ring_m23.supertraits; const assoc_types = __ring_m23.assoc_types;
    ...
    if (__ring_m23._tag === "ExternType") {
      const name = __ring_m23.name; const is_pub = __ring_m23.is_pub;
      if (is_pub) {
        const sn = codegen_ctx$safe_ident(name);
...
function emit_single_evidence(ctx, name, emitted) {
  ...
  if (_Set_contains(ctx.default_evidence_effects, name, __Str_Eq)) {
    ...
    __ring_match24: {
      const __ring_m24 = _Map_get(ctx.default_evidence_params, name);
      if (__ring_m24._tag === "some") {
        const factory_params = __ring_m24._0;
        const __ring_iter_28 = __List_Iterable.iter(factory_params);
        while (true) {
...
function topo_sort_defaults(ctx, defaults) {
  ...
  const __ring_iter_29 = __List_Iterable.iter(defaults);
  while (true) {
  ...
  let deps = map_new();
  const __ring_iter_30 = __List_Iterable.iter(defaults);
  while (true) {
    const __ring_next_30 = __ListIterator_Iterator.next(__ring_iter_30);
    if (__ring_next_30._tag === "none") break;
    const name = __ring_next_30._0;
    let my_deps = [];
    __ring_match25: {
      const __ring_m25 = _Map_get(ctx.default_evidence_params, name);
      if (__ring_m25._tag === "some") {
        const factory_params = __ring_m25._0;
  ...
  let sorted_deps = _Map_entries(deps);
  sorted_deps.sort((function(__ring_a0, __ring_a1) { return hir$compare_by_first(__ring_a0, __ring_a1); }));
  const __ring_iter_33 = __List_Iterable.iter(sorted_deps);
  while (true) {
    ...
    const __ring_iter_34 = __List_Iterable.iter(dep_list);
    while (true) {
      const __ring_next_34 = __ListIterator_Iterator.next(__ring_iter_34);
      if (__ring_next_34._tag === "none") break;
      const dep = __ring_next_34._0;
      __ring_match26: {
        const __ring_m26 = _Map_get(rev_deps, dep);
        if (__ring_m26._tag === "some") {
          const rev_list = __ring_m26._0;
  ...
  let queue = [];
  let sorted_in_deg = _Map_entries(in_deg);
  sorted_in_deg.sort((function(__ring_a0, __ring_a1) { return hir$compare_by_first(__ring_a0, __ring_a1); }));
  const __ring_iter_35 = __List_Iterable.iter(sorted_in_deg);
  while (true) {
    const __ring_next_35 = __ListIterator_Iterator.next(__ring_iter_35);
    if (__ring_next_35._tag === "none") break;
    const entry = __ring_next_35._0;
  ...
  List_sort(queue, __Str_Ord);
  let sorted = [];
  while ((List_len(queue) > 0)) {
    const cur = Option_unwrap(List_shift(queue));
    List_push(sorted, cur);
    __ring_match27: {
      const __ring_m27 = _Map_get(rev_deps, cur);
      if (__ring_m27._tag === "some") {
        const dependents = __ring_m27._0;
        const __ring_iter_36 = __List_Iterable.iter(dependents);
        while (true) {
          const __ring_next_36 = __ListIterator_Iterator.next(__ring_iter_36);
          if (__ring_next_36._tag === "none") break;
          const dependent = __ring_next_36._0;
          let __ring_blk3;
          __ring_match28: {
            const __ring_m28 = _Map_get(in_deg, dependent);
  ...
  if ((List_len(sorted) < List_len(defaults))) {
    const __ring_iter_37 = __List_Iterable.iter(defaults);
    while (true) {
...
function emit_toplevel_evidence(ctx, effects) {
  const effect_names = effect_analysis$extract_effect_names(effects);
  let emitted = set_new();
  let deferred_defaults = [];
  const __ring_iter_38 = __List_Iterable.iter(effect_names);
  while (true) {
    const __ring_next_38 = __ListIterator_Iterator.next(__ring_iter_38);
    if (__ring_next_38._tag === "none") break;
    const name = __ring_next_38._0;
    const ev_name = hir$evidence_param_name(name);
    if ((name === "io")) {
      _Set_insert(emitted, name);
    } else {
  ...
  const __ring_iter_39 = __List_Iterable.iter(sorted_defaults);
  while (true) {
    ...
    const def_ev = hir$default_evidence_name(name);
    __ring_match29: {
      const __ring_m29 = _Map_get(ctx.default_evidence_params, name);
      if (__ring_m29._tag === "some") {
        const factory_params = __ring_m29._0;
        const __ring_iter_40 = __List_Iterable.iter(factory_params);
        while (true) {
          const __ring_next_40 = __ListIterator_Iterator.next(__ring_iter_40);
          if (__ring_next_40._tag === "none") break;
          const dp = __ring_next_40._0;
          const dep_name = Str_slice(dp, 10, Str_len(dp));
          if ((!_Set_contains(emitted, dep_name, __Str_Eq))) {
            emit_single_evidence(ctx, dep_name, emitted);
          }
