...
function emit_decl(ctx, decl) {
  __ring_match0: {
    ...
    if (__ring_m0._tag === "Fn") {
      const name = __ring_m0.name; const params = __ring_m0.params; const effects = __ring_m0.effects; const body = __ring_m0.body; const trait_bounds = __ring_m0.trait_bounds;
      return emit_fn_decl(ctx, name, params, effects, body, trait_bounds, Option_none);
      break __ring_match0;
    }
    if (__ring_m0._tag === "Struct") {
      const name = __ring_m0.name; const fields = __ring_m0.fields;
      return emit_struct_decl(ctx, name, fields);
      break __ring_match0;
    }
    if (__ring_m0._tag === "Enum") {
      const name = __ring_m0.name; const variants = __ring_m0.variants;
    ...
    if (__ring_m0._tag === "Test") {
      const description = __ring_m0.description; const body = __ring_m0.body;
      return emit_test_decl(ctx, description, body);
      break __ring_match0;
    }
    if (__ring_m0._tag === "Trait") {
      const name = __ring_m0.name; const methods = __ring_m0.methods;
    ...
    if (__ring_m0._tag === "Const") {
      const name = __ring_m0.name; const init = __ring_m0.init;
      return emit_const_decl(ctx, name, init);
      break __ring_match0;
    }
    if (__ring_m0._tag === "ModBlock") {
      const mod_decls = __ring_m0.decls;
...
function emit_extern_fn_decl(ctx, name) {
  __ring_match1: {
    const __ring_m1 = ctx.module_prefix;
    if (__ring_m1._tag === "some") {
      const prefix = __ring_m1._0;
...
function emit_struct_decl(ctx, name, fields) {
  ...
  for (let i = 0; i < __ring_end0; i++) {
    __ring_match2: {
      const __ring_m2 = [List_get(raw_fields, i), List_get(safe_fields, i)];
      if (Array.isArray(__ring_m2) && __ring_m2.length === 2 && __ring_m2[0]._tag === "some" && __ring_m2[1]._tag === "some") {
        const raw = __ring_m2[0]._0; const safe = __ring_m2[1]._0;
...
function emit_enum_decl(ctx, name, variants) {
  ...
  for (const v of variants) {
    ...
    if ((List_len(v.fields) === 0)) {
      ...
      __ring_match3: {
        const __ring_m3 = v.field_names;
        if (__ring_m3._tag === "some") {
          const fnames = __ring_m3._0;
          let sparams = [""];
          List_clear(sparams);
...
function emit_impl_decl(ctx, target_type, trait_name, methods) {
  ...
  for (const method of methods) {
    __ring_match4: {
      const __ring_m4 = method;
      if (__ring_m4._tag === "ExternFn") {
        break __ring_match4;
      }
      if (__ring_m4._tag === "Fn") {
        const name = __ring_m4.name; const params = __ring_m4.params; const effects = __ring_m4.effects; const body = __ring_m4.body; const trait_bounds = __ring_m4.trait_bounds;
  ...
  __ring_match5: {
    const __ring_m5 = trait_name;
    if (__ring_m5._tag === "some") {
      const tn = __ring_m5._0;
...
function emit_trait_dictionary(ctx, target_type, trait_name, methods) {
  ...
  __ring_match8: {
    ...
    if (__ring_m8._tag === "some") {
      const trait_decl = __ring_m8._0;
      for (const tm of trait_decl.methods) {
        if (tm.has_default) {
          if ((_Set_contains(impl_method_names, tm.name) === false)) {
...
function emit_toplevel_evidence(ctx, effects) {
  ...
  for (const name of effect_names) {
    ...
    if ((name === "io")) {
      codegen_ctx$emit(ctx, `const ${ev_name} = { read: (p) => __require("fs").readFileSync(p, "utf-8"), write: (p, d) => __require("fs").writeFileSync(p, d, "utf-8") };`);
    } else {
      if ((name === "fail")) {
