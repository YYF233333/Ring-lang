...
function cli_main() {
  ...
  if (diagnostics$CollectingSink_has_errors(parse_sink)) {
    ...
    if ((parsed.error_format === "llm")) {
      print(formatter$format_llm(diagnostics, file_path));
    } else {
  ...
  if ((parsed.command === "check")) {
    ...
    if ((parsed.command === "build")) {
      ...
      return print(`Compiled: ${out_path}`);
    } else {
      if ((parsed.command === "run")) {
...
function usage() {
  ...
  return print("  --out-dir=<path>          Output directory (default: dist)");
}
