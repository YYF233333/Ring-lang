...
function emit_derived_impl(ctx, impl_) {
  __ring_match2: {
    const __ring_m2 = impl_.trait_name;
    if (__ring_m2 === "Eq") {
...
function emit_derived_eq(ctx, impl_) {
  ...
  __ring_match3: {
    const __ring_m3 = impl_.type_kind;
    if (__ring_m3._tag === "StructKind") {
      __ring_match4: {
        const __ring_m4 = impl_.struct_fields;
        if (__ring_m4._tag === "some") {
          const fields = __ring_m4._0;
    ...
    if (__ring_m3._tag === "EnumKind") {
      __ring_match5: {
        const __ring_m5 = impl_.enum_variants;
        if (__ring_m5._tag === "some") {
          const variants = __ring_m5._0;
...
function emit_derived_clone(ctx, impl_) {
  ...
  codegen_ctx$push_indent(ctx);
  __ring_match7: {
    const __ring_m7 = impl_.type_kind;
    if (__ring_m7._tag === "StructKind") {
      __ring_match8: {
        const __ring_m8 = impl_.struct_fields;
        if (__ring_m8._tag === "some") {
          const fields = __ring_m8._0;
          let args = [""];
          List_clear(args);
          for (const f of fields) {
...
function gen_field_clone(expr, field) {
  __ring_match10: {
    const __ring_m10 = field.action;
    if (__ring_m10._tag === "Identity") {
      return expr;
      break __ring_match10;
    }
    if (__ring_m10._tag === "Call") {
      const dict_name = __ring_m10.dict_name; const extra_dicts = __ring_m10.extra_dicts;
      const extra = extra_dicts_str(extra_dicts);
      ...
      __ring_match12: {
        ...
        if (__ring_m12._tag === "some") {
          ...
          for (let i = 0; i < __ring_end0; i++) {
            __ring_match13: {
              const __ring_m13 = List_get(variants, i);
            ...
            for (const v of variants) {
              if ((List_len(v.fields) === 0)) {
                ...
                if ((List_len(v.fields) === 1)) {
                  __ring_match18: {
                    const __ring_m18 = List_get(v.fields, 0);
                  ...
                  for (let i = 0; i < __ring_end2; i++) {
                    __ring_match19: {
                      const __ring_m19 = List_get(v.fields, i);
                      if (__ring_m19._tag === "some") {
                        const f = __ring_m19._0;
                        const accessor = field_accessor(v, f);
                        const cmp = gen_field_cmp(`self.${accessor}`, `other.${accessor}`, f);
...
function gen_field_cmp(left, right, field) {
  __ring_match20: {
    const __ring_m20 = field.action;
    if (__ring_m20._tag === "Identity") {
      return `(${left} < ${right} ? -1 : ${left} > ${right} ? 1 : 0)`;
      break __ring_match20;
    }
    if (__ring_m20._tag === "Call") {
      const dict_name = __ring_m20.dict_name; const extra_dicts = __ring_m20.extra_dicts;
      const extra = extra_dicts_str(extra_dicts);
...
function emit_derived_debug(ctx, impl_) {
  ...
  __ring_match21: {
    ...
    if (__ring_m21._tag === "EnumKind") {
      __ring_match23: {
        const __ring_m23 = impl_.enum_variants;
        if (__ring_m23._tag === "some") {
          const variants = __ring_m23._0;
          codegen_ctx$emit(ctx, `switch (self.${hir$ENUM_TAG_FIELD}) {`);
          codegen_ctx$push_indent(ctx);
          for (const v of variants) {
            if ((List_len(v.fields) === 0)) {
...
function gen_field_debug(expr, field) {
  __ring_match24: {
    const __ring_m24 = field.action;
    if (__ring_m24._tag === "Identity") {
      return `String(${expr})`;
      break __ring_match24;
    }
    if (__ring_m24._tag === "Call") {
      const dict_name = __ring_m24.dict_name; const extra_dicts = __ring_m24.extra_dicts;
      const extra = extra_dicts_str(extra_dicts);
...
function field_accessor(v, f) {
  if (v.has_named_fields) {
    ...
    __ring_match25: {
      const __ring_m25 = f.positional_index;
      if (__ring_m25._tag === "some") {
        const pi = __ring_m25._0;
        return `_${pi}`;
