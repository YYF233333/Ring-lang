...
function inject_module_exports(ctx, exports) {
  for (const mod_ of exports) {
    ...
    for (const entry of _Map_entries(mod_.impl_methods)) {
      ...
      __ring_match6: {
        const __ring_m6 = _Map_get(ctx.env.trait_reg.impl_methods, type_name);
...
function resolve_uses(ctx, uses, available_modules) {
  ...
  for (const use_decl of uses) {
    ...
    __ring_match7: {
      const __ring_m7 = _Map_get(module_map, mod_key);
  ...
  __match_fail(__ring_m);
              ...
              let found = false;
              __ring_match9: {
                const __ring_m9 = _Map_get(mod_.values, item.name);
              ...
              __ring_match10: {
                const __ring_m10 = _Map_get(mod_.types, item.name);
                if (__ring_m10._tag === "some") {
                  const tdef = __ring_m10._0;
                  found = true;
                  __ring_match11: {
                    ...
                    if (__ring_m11._tag === "EnumDef_") {
                      ...
                      for (const v of edef.variants) {
                        __ring_match12: {
                          const __ring_m12 = _Map_get(mod_.values, v.name);
                          if (__ring_m12._tag === "some") {
                            const vscheme = __ring_m12._0;
            ...
            for (const entry of _Map_entries(mod_.types)) {
              ...
              __ring_match13: {
                ...
                if (__ring_m13._tag === "EnumDef_") {
                  ...
                  for (const v of edef.variants) {
                    __ring_match14: {
                      const __ring_m14 = _Map_get(mod_.values, v.name);
                      if (__ring_m14._tag === "some") {
                        const vscheme = __ring_m14._0;
                        env$TypeEnv_bind(ctx.env, v.name, vscheme);
