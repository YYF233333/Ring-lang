...
import { check as infer_decl$check, resolve_type_expr_public as infer_decl$resolve_type_expr_public } from "./infer_decl.js";
...
function new_infer_ctx(sink) {
  ...
  return new infer_ctx$InferCtx(env, unify$empty_subst(), sink, map_new(), Option_none, [], [], 0);
}
...
function check(program, sink) {
  ...
  return new CheckResult(hprogram, ctx.env);
}
...
function inject_module_exports(ctx, exports) {
  for (const mod_ of exports) {
    for (const entry of _Map_entries(mod_.types)) {
      ...
      __ring_match1: {
        const __ring_m1 = def;
        if (__ring_m1._tag === "StructDef_") {
          const sdef = __ring_m1._0;
          _Map_insert(ctx.env.types.structs, name, sdef);
          break __ring_match1;
        }
        if (__ring_m1._tag === "EnumDef_") {
          const edef = __ring_m1._0;
    ...
    for (const entry of _Map_entries(mod_.impl_methods)) {
      ...
      __ring_match2: {
        const __ring_m2 = _Map_get(ctx.env.trait_reg.impl_methods, type_name);
        if (__ring_m2._tag === "some") {
          const existing = __ring_m2._0;
...
function resolve_uses(ctx, uses, available_modules) {
  ...
  for (const use_decl of uses) {
    ...
    __ring_match3: {
      ...
      if (__ring_m3._tag === "some") {
        const mod_ = __ring_m3._0;
        __ring_match4: {
          const __ring_m4 = use_decl.imports;
          if (__ring_m4._tag === "NamedItems") {
            const names = __ring_m4.names;
  ...
  __match_fail(__ring_m);
              ...
              let found = false;
              __ring_match5: {
                const __ring_m5 = _Map_get(mod_.values, item.name);
                if (__ring_m5._tag === "some") {
                  const scheme = __ring_m5._0;
              ...
              __ring_match6: {
                const __ring_m6 = _Map_get(mod_.types, item.name);
                if (__ring_m6._tag === "some") {
                  ...
                  __ring_match7: {
                    ...
                    if (__ring_m7._tag === "EnumDef_") {
                      ...
                      for (const v of edef.variants) {
                        __ring_match8: {
                          const __ring_m8 = _Map_get(mod_.values, v.name);
                          if (__ring_m8._tag === "some") {
                            const vscheme = __ring_m8._0;
          ...
          if (__ring_m4._tag === "Module") {
            ...
            for (const entry of _Map_entries(mod_.types)) {
              ...
              __ring_match9: {
                const __ring_m9 = tdef;
                if (__ring_m9._tag === "EnumDef_") {
                  const edef = __ring_m9._0;
                  for (const v of edef.variants) {
                    __ring_match10: {
                      const __ring_m10 = _Map_get(mod_.values, v.name);
