...
function HExpr_OptionUnwrap(expr, ty, effects, span) {
  return { _tag: "OptionUnwrap", expr, ty, effects, span };
}
function HExpr_TryBlock(body, ty, effects, span) {
  return { _tag: "TryBlock", body, ty, effects, span };
}
function HExpr_OptionOr(expr, default_value, ty, effects, span) {
  return { _tag: "OptionOr", expr, default_value, ty, effects, span };
}
...
function hexpr_span(e) {
  __ring_match2: {
    ...
    if (__ring_m2._tag === "OptionUnwrap") {
      const span = __ring_m2.span;
      return span;
      break __ring_match2;
    }
    if (__ring_m2._tag === "TryBlock") {
      const span = __ring_m2.span;
      return span;
      break __ring_match2;
    }
    if (__ring_m2._tag === "OptionOr") {
      const span = __ring_m2.span;
      return span;
      break __ring_match2;
    }
