...
function check_decl(ctx, decl, __ring_ev_fail) {
  __ring_match0: {
    ...
    if (__ring_m0._tag === "Fn") {
      const name = __ring_m0.name; const type_params = __ring_m0.type_params; const params = __ring_m0.params; const return_type = __ring_m0.return_type; const declared_effects = __ring_m0.declared_effects; const body = __ring_m0.body; const is_pub = __ring_m0.is_pub; const span = __ring_m0.span;
  ...
  __match_fail(__ring_m);
    ...
    if (__ring_m0._tag === "ModBlock") {
      const name = __ring_m0.name; const uses = __ring_m0.uses; const decls = __ring_m0.decls; const required_effects = __ring_m0.required_effects; const is_pub = __ring_m0.is_pub; const span = __ring_m0.span;
...
function check_mod_decl(ctx, mod_name, uses, decls, required_effects, is_pub, span, __ring_ev_fail) {
  ...
  __ring_match1: {
    const __ring_m1 = required_effects;
    if (__ring_m1._tag === "some") {
      const req_effs = __ring_m1._0;
  ...
  for (const decl of decls) {
    ...
    __ring_match2: {
      const __ring_m2 = result;
      if (__ring_m2._tag === "some") {
        const hd = __ring_m2._0;
        __ring_match3: {
          const __ring_m3 = cap_row;
          if (__ring_m3._tag === "some") {
            const cap = __ring_m3._0;
...
function check_capability(ctx, decl, cap, mod_span) {
  __ring_match4: {
    ...
    if (__ring_m4._tag === "Fn") {
      const name = __ring_m4.name; const effects = __ring_m4.effects; const span = __ring_m4.span;
...
function check_sig_decl(ctx, name, members, is_pub, span) {
  let hmembers = [];
  __ring_match5: {
    const __ring_m5 = _Map_get(ctx.env.types.sigs, name);
    if (__ring_m5._tag === "some") {
      const sig_def = __ring_m5._0;
      for (const m of members) {
        __ring_match6: {
          const __ring_m6 = _Map_get(sig_def.members, m.name);
          if (__ring_m6._tag === "some") {
            const scheme = __ring_m6._0;
...
function resolve_mod_uses(ctx, uses) {
  for (const use_decl of uses) {
    ...
    __ring_match7: {
      ...
      if (__ring_m7._tag === "some") {
        ...
        __ring_match8: {
          const __ring_m8 = use_decl.imports;
          if (__ring_m8._tag === "NamedItems") {
            const names = __ring_m8.names;
  ...
  __match_fail(__ring_m);
              ...
              __ring_match9: {
                const __ring_m9 = env$TypeEnv_lookup(ctx.env, qualified_name);
...
function check_const_decl(ctx, name, type_annotation, init, is_pub, span, __ring_ev_fail) {
  ...
  __ring_match12: {
    const __ring_m12 = expected_ty;
    if (__ring_m12._tag === "some") {
      const ann_ty = __ring_m12._0;
...
function update_impl_method_effects(ctx, target_type, method_name, effects) {
  __ring_match13: {
    const __ring_m13 = _Map_get(ctx.env.trait_reg.impl_methods, target_type);
    if (__ring_m13._tag === "some") {
      const methods_map = __ring_m13._0;
      __ring_match14: {
        const __ring_m14 = _Map_get(methods_map, method_name);
...
function check_impl_decl(ctx, target_type, type_params, trait_name, methods, span, __ring_ev_fail) {
  ...
  for (const tp of type_params) {
    __ring_match16: {
      const __ring_m16 = _Map_get(ctx.type_param_scope, tp.name);
      if (__ring_m16._tag === "some") {
        const tv = __ring_m16._0;
  ...
  __ring_match17: {
    const __ring_m17 = _Map_get(ctx.env.types.structs, target_type);
    ...
    if (__ring_m17._tag === "none") {
      __ring_match18: {
        const __ring_m18 = _Map_get(ctx.env.types.enums, target_type);
  ...
  for (const tp of type_params) {
    __ring_match19: {
      const __ring_m19 = _Map_get(ctx.type_param_scope, tp.name);
      if (__ring_m19._tag === "some") {
        const tv = __ring_m19._0;
        __ring_match20: {
          const __ring_m20 = tv;
  ...
  for (const method of methods) {
    __ring_match21: {
      ...
      if (__ring_m21._tag === "ExternFn") {
        const name = __ring_m21.name; const mtps = __ring_m21.type_params; const params = __ring_m21.params; const return_type = __ring_m21.return_type; const declared_effects = __ring_m21.declared_effects; const is_pub = __ring_m21.is_pub; const mspan = __ring_m21.span;
      ...
      if (__ring_m21._tag === "Fn") {
        const name = __ring_m21.name; const mtps = __ring_m21.type_params; const params = __ring_m21.params; const return_type = __ring_m21.return_type; const declared_effects = __ring_m21.declared_effects; const body = __ring_m21.body; const is_pub = __ring_m21.is_pub; const mspan = __ring_m21.span;
        ...
        __ring_match22: {
          const __ring_m22 = hdecl;
          if (__ring_m22._tag === "Fn") {
            const mname = __ring_m22.name; const inferred_effects = __ring_m22.effects;
...
function check_trait_decl(ctx, name, type_params, ast_methods, is_pub, span, __ring_ev_fail) {
  ...
  if ((List_len(trait_def.methods) > 0)) {
    __ring_match23: {
      const __ring_m23 = List_first(trait_def.methods);
      if (__ring_m23._tag === "some") {
        const first_method = __ring_m23._0;
        __ring_match24: {
          const __ring_m24 = first_method.ty;
          if (__ring_m24._tag === "FnType") {
            const fps = __ring_m24.params;
            if ((List_len(fps) > 0)) {
              __ring_match25: {
                const __ring_m25 = List_first(fps);
                if (__ring_m25._tag === "some") {
                  const fp = __ring_m25._0;
                  self_var = fp;
  ...
  __match_fail(__ring_m);
    ...
    if (m.has_default) {
      __ring_match26: {
        const __ring_m26 = ast_method;
        if (__ring_m26._tag === "some") {
          const am = __ring_m26._0;
          __ring_match27: {
            const __ring_m27 = am;
            if (__ring_m27._tag === "Fn") {
              const abody = __ring_m27.body;
...
function check_trait_default_body(ctx, trait_name, self_var, hparams, body) {
  ...
  for (const p of hparams) {
    ...
    if (p.is_mutable) {
      __ring_match29: {
        const __ring_m29 = env$TypeEnv_lookup(ctx.env, p.name);
        if (__ring_m29._tag === "some") {
          const ps = __ring_m29._0;
          __ring_match30: {
            const __ring_m30 = ps.def_id;
            if (__ring_m30._tag === "some") {
              const did = __ring_m30._0;
      ...
      if (__ring_m31._tag === "some") {
        ...
        __ring_match32: {
          ...
          if (__ring_m32._tag === "TypeVar") {
            ...
            __ring_match33: {
              const __ring_m33 = resolved;
              if (__ring_m33._tag === "TypeVar") {
                const rid = __ring_m33.id;
    ...
    if (((!_Map_contains_key(saved_tp_scope, tpname)) && (!_Set_contains(declared_names, tpname)))) {
      __ring_match34: {
        ...
        if (__ring_m34._tag === "TypeVar") {
          const resolved = env$apply_subst(ctx.subst, tv);
          __ring_match35: {
            const __ring_m35 = resolved;
            if (__ring_m35._tag === "TypeVar") {
              const rid = __ring_m35.id;
...
function check_fn_decl(ctx, name, type_params, params, return_type, declared_effects, body, is_pub, span, self_type, __ring_ev_fail) {
  ...
  __match_fail(__ring_m);
    ...
    __ring_match38: {
      const __ring_m38 = param_scheme;
      if (__ring_m38._tag === "some") {
        const ps = __ring_m38._0;
        __ring_match39: {
          const __ring_m39 = ps.def_id;
