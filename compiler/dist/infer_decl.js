  ...
  for (const method of methods) {
    __ring_match36: {
      ...
      if (__ring_m36._tag === "ExternFn") {
        const name = __ring_m36.name; const mtps = __ring_m36.type_params; const params = __ring_m36.params; const return_type = __ring_m36.return_type; const declared_effects = __ring_m36.declared_effects; const is_pub = __ring_m36.is_pub; const mspan = __ring_m36.span;
      ...
      if (__ring_m36._tag === "Fn") {
        const name = __ring_m36.name; const mtps = __ring_m36.type_params; const params = __ring_m36.params; const return_type = __ring_m36.return_type; const declared_effects = __ring_m36.declared_effects; const body = __ring_m36.body; const is_pub = __ring_m36.is_pub; const mspan = __ring_m36.span;
        ...
        __ring_match37: {
          const __ring_m37 = hdecl;
          if (__ring_m37._tag === "Fn") {
            const mname = __ring_m37.name; const inferred_effects = __ring_m37.effects;
...
function expand_delegate_impls(ctx, target_type, type_params, field, trait_names, span) {
  let result = [];
  __ring_match38: {
    const __ring_m38 = _Map_get(ctx.env.types.structs, target_type);
    if (__ring_m38._tag === "none") {
      return result;
      break __ring_match38;
    }
    if (__ring_m38._tag === "some") {
      const struct_def = __ring_m38._0;
  ...
  __ring_match41: {
    ...
    __match_fail(__ring_m41);
          ...
          for (const tname of all_traits) {
            __ring_match43: {
              const __ring_m43 = _Map_get(ctx.env.trait_reg.traits, tname);
              if (__ring_m43._tag === "none") {
                break __ring_match43;
              }
              if (__ring_m43._tag === "some") {
                const trait_def = __ring_m43._0;
                let trait_hmethods = [];
                for (const tm of trait_def.methods) {
                  __ring_match44: {
                    const __ring_m44 = tm.ty;
  ...
  return Option_none;
                        ...
                        __ring_match45: {
                          const __ring_m45 = ftn;
                          if (__ring_m45._tag === "some") {
                            const field_tn = __ring_m45._0;
                            let has_explicit = false;
                            __ring_match46: {
                              const __ring_m46 = _Map_get(ctx.env.trait_reg.impl_methods, field_tn);
                              if (__ring_m46._tag === "some") {
                                const methods_map = __ring_m46._0;
...
function check_trait_decl(ctx, name, type_params, ast_methods, is_pub, span, __ring_ev_fail) {
  ...
  if ((List_len(trait_def.methods) > 0)) {
    __ring_match47: {
      const __ring_m47 = List_first(trait_def.methods);
      if (__ring_m47._tag === "some") {
        const first_method = __ring_m47._0;
        __ring_match48: {
          const __ring_m48 = first_method.ty;
          if (__ring_m48._tag === "FnType") {
            const fps = __ring_m48.params;
            if ((List_len(fps) > 0)) {
              __ring_match49: {
                const __ring_m49 = List_first(fps);
  ...
  __match_fail(__ring_m);
    ...
    if (m.has_default) {
      __ring_match50: {
        const __ring_m50 = ast_method;
        if (__ring_m50._tag === "some") {
          const am = __ring_m50._0;
          __ring_match51: {
            const __ring_m51 = am;
            if (__ring_m51._tag === "Fn") {
              const abody = __ring_m51.body;
...
function check_trait_default_body(ctx, trait_name, self_var, hparams, body) {
  ...
  for (const p of hparams) {
    ...
    if (p.is_mutable) {
      __ring_match53: {
        const __ring_m53 = env$TypeEnv_lookup(ctx.env, p.name);
...
function check_fn_decl(ctx, name, type_params, params, return_type, declared_effects, body, is_pub, span, self_type, __ring_ev_fail) {
  ...
  __match_fail(__ring_m);
    ...
    __ring_match63: {
      const __ring_m63 = param_scheme;
      if (__ring_m63._tag === "some") {
        const ps = __ring_m63._0;
        __ring_match64: {
          const __ring_m64 = ps.def_id;
  ...
  for (const inferred_eff of inferred_effects.effects) {
    ...
    for (const declared_eff of declared_row.effects) {
      if (types$effects_match_kind(inferred_eff, declared_eff)) {
        found = true;
        __ring_match65: {
          const __ring_m65 = [inferred_eff, declared_eff];
          if (Array.isArray(__ring_m65) && __ring_m65.length === 2 && __ring_m65[0]._tag === "FailEffect" && __ring_m65[1]._tag === "FailEffect") {
            const ie = __ring_m65[0].error_type; const de = __ring_m65[1].error_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, ie, de, ctx.subst, span);
            break __ring_match65;
          }
          if (Array.isArray(__ring_m65) && __ring_m65.length === 2 && __ring_m65[0]._tag === "MutEffect" && __ring_m65[1]._tag === "MutEffect") {
            const is = __ring_m65[0].state_type; const ds = __ring_m65[1].state_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, is, ds, ctx.subst, span);
            break __ring_match65;
          }
          if (Array.isArray(__ring_m65) && __ring_m65.length === 2 && __ring_m65[0]._tag === "CustomEffect" && __ring_m65[1]._tag === "CustomEffect") {
            const ia = __ring_m65[0].type_args; const da = __ring_m65[1].type_args;
            let i = 0;
  ...
  if ((name === "main")) {
    for (const eff of final_effects.effects) {
      __ring_match66: {
        const __ring_m66 = eff;
        if (__ring_m66._tag === "CustomEffect") {
          const eff_name = __ring_m66.name;
          let skip = false;
          __ring_match67: {
            const __ring_m67 = _Map_get(ctx.env.types.effects, eff_name);
            if (__ring_m67._tag === "some") {
              const edef = __ring_m67._0;
  ...
  for (const p of params) {
    if (((p.name === "self") || (!p.is_mutable))) {
      ...
      __ring_match69: {
        const __ring_m69 = List_get(final_params, fi);
        if (__ring_m69._tag === "some") {
          const fp = __ring_m69._0;
...
function check_one_decl(ctx, decl, hdecls, __ring_ev_fail) {
  ...
  __ring_match70: {
    ...
    if (__ring_m70._tag === "Fn") {
      const name = __ring_m70.name; const effects = __ring_m70.effects;
      if ((List_len(effects.effects) > 0)) {
        infer_ctx$update_fn_effects(ctx.env, name, effects);
      }
  ...
  __ring_match71: {
    ...
    if (__ring_m71._tag === "Impl") {
      const target_type = __ring_m71.target_type; const type_params = __ring_m71.type_params; const methods = __ring_m71.methods; const span = __ring_m71.span;
      for (const m of methods) {
        __ring_match72: {
          const __ring_m72 = m;
          if (__ring_m72._tag === "Delegate") {
  ...
  for (const decl of decls) {
    __ring_match73: {
      ...
      if (__ring_m73._tag === "Effect") {
        const name = __ring_m73.name; const span = __ring_m73.span;
        _Map_insert(spans, name, span);
        break __ring_match73;
      }
      if (__ring_m73._tag === "ModBlock") {
        const mod_decls = __ring_m73.decls;
...
function dfs_detect_cycle(ctx, name, state, path, effect_spans) {
  ...
  List_push(path, name);
  __ring_match74: {
    const __ring_m74 = _Map_get(ctx.effect_default_deps, name);
    if (__ring_m74._tag === "some") {
      const deps = __ring_m74._0;
      for (const dep of deps) {
        __ring_match75: {
          const __ring_m75 = _Map_get(state, dep);
  ...
  for (const decl of program.decls) {
    __ring_match76: {
      ...
      if (__ring_m76._tag === "Impl") {
        const target_type = __ring_m76.target_type; const type_params = __ring_m76.type_params; const trait_name = __ring_m76.trait_name; const methods = __ring_m76.methods; const span = __ring_m76.span;
