...
function check_effect_decl(ctx, name, type_params, ast_ops, is_pub, span, __ring_ev_fail) {
  ...
  __match_fail(__ring_m);
    ...
    const ast_op = (function() {
  const __ring_m = List_get(ast_ops, oi);
  if (__ring_m._tag === "some") { const a = __ring_m._0; return a; }
  if (__ring_m._tag === "none") { return Option_unwrap(List_get(ast_ops, 0)); }
  __match_fail(__ring_m);
})();
...
function update_impl_method_effects(ctx, target_type, method_name, effects) {
  __ring_match26: {
    const __ring_m26 = _Map_get(ctx.env.trait_reg.impl_methods, target_type);
    if (__ring_m26._tag === "some") {
      const methods_map = __ring_m26._0;
      __ring_match27: {
        const __ring_m27 = _Map_get(methods_map, method_name);
        if (__ring_m27._tag === "some") {
          const scheme = __ring_m27._0;
          __ring_match28: {
            const __ring_m28 = scheme.ty;
...
function check_impl_decl(ctx, target_type, type_params, trait_name, methods, span, __ring_ev_fail) {
  ...
  for (const tp of type_params) {
    __ring_match29: {
      const __ring_m29 = _Map_get(ctx.type_param_scope, tp.name);
      if (__ring_m29._tag === "some") {
        const tv = __ring_m29._0;
  ...
  __ring_match30: {
    const __ring_m30 = _Map_get(ctx.env.types.structs, target_type);
    ...
    if (__ring_m30._tag === "none") {
      __ring_match31: {
        const __ring_m31 = _Map_get(ctx.env.types.enums, target_type);
  ...
  for (const method of methods) {
    __ring_match34: {
      const __ring_m34 = method;
      if (__ring_m34._tag === "ExternFn") {
        const name = __ring_m34.name; const mtps = __ring_m34.type_params; const params = __ring_m34.params; const return_type = __ring_m34.return_type; const declared_effects = __ring_m34.declared_effects; const is_pub = __ring_m34.is_pub; const mspan = __ring_m34.span;
      ...
      if (__ring_m34._tag === "Fn") {
        const name = __ring_m34.name; const mtps = __ring_m34.type_params; const params = __ring_m34.params; const return_type = __ring_m34.return_type; const declared_effects = __ring_m34.declared_effects; const body = __ring_m34.body; const is_pub = __ring_m34.is_pub; const mspan = __ring_m34.span;
        ...
        __ring_match35: {
          const __ring_m35 = hdecl;
          if (__ring_m35._tag === "Fn") {
            const mname = __ring_m35.name; const inferred_effects = __ring_m35.effects;
...
function expand_delegate_impls(ctx, target_type, type_params, field, trait_names, span) {
  let result = [];
  __ring_match36: {
    const __ring_m36 = _Map_get(ctx.env.types.structs, target_type);
    if (__ring_m36._tag === "none") {
      return result;
      break __ring_match36;
    }
    if (__ring_m36._tag === "some") {
      const struct_def = __ring_m36._0;
  ...
  __ring_match39: {
    ...
    if (__ring_m39._tag === "some") {
      const def = __ring_m39._0;
      return types$Type_StructType(def.name, impl_tp_types, def.fields);
      break __ring_match39;
    }
    if (__ring_m39._tag === "none") {
      __ring_match40: {
        const __ring_m40 = _Map_get(ctx.env.types.enums, target_type);
            ...
            __ring_match41: {
              ...
              if (__ring_m41._tag === "some") {
                ...
                for (const tm of trait_def.methods) {
                  __ring_match42: {
                    const __ring_m42 = tm.ty;
                    if (__ring_m42._tag === "FnType") {
                      const trait_params = __ring_m42.params; const ret_ty = __ring_m42.return_type; const eff = __ring_m42.effects;
                      let hparams = [];
  ...
  return Option_none;
                        ...
                        __ring_match43: {
                          const __ring_m43 = ftn;
                          if (__ring_m43._tag === "some") {
                            const field_tn = __ring_m43._0;
                            let has_explicit = false;
                            __ring_match44: {
                              const __ring_m44 = _Map_get(ctx.env.trait_reg.impl_methods, field_tn);
                              if (__ring_m44._tag === "some") {
                                const methods_map = __ring_m44._0;
...
function check_trait_decl(ctx, name, type_params, ast_methods, is_pub, span, __ring_ev_fail) {
  ...
  if ((List_len(trait_def.methods) > 0)) {
    __ring_match45: {
      const __ring_m45 = List_first(trait_def.methods);
      if (__ring_m45._tag === "some") {
        const first_method = __ring_m45._0;
        __ring_match46: {
          const __ring_m46 = first_method.ty;
          if (__ring_m46._tag === "FnType") {
            const fps = __ring_m46.params;
            if ((List_len(fps) > 0)) {
              __ring_match47: {
                const __ring_m47 = List_first(fps);
  ...
  __match_fail(__ring_m);
    ...
    if (m.has_default) {
      __ring_match48: {
        const __ring_m48 = ast_method;
        if (__ring_m48._tag === "some") {
          const am = __ring_m48._0;
          __ring_match49: {
            const __ring_m49 = am;
            if (__ring_m49._tag === "Fn") {
              const abody = __ring_m49.body;
...
function check_trait_default_body(ctx, trait_name, self_var, hparams, body) {
  ...
  for (const p of hparams) {
    ...
    if (p.is_mutable) {
      __ring_match51: {
        const __ring_m51 = env$TypeEnv_lookup(ctx.env, p.name);
        if (__ring_m51._tag === "some") {
          const ps = __ring_m51._0;
          __ring_match52: {
            const __ring_m52 = ps.def_id;
...
function check_fn_decl(ctx, name, type_params, params, return_type, declared_effects, body, is_pub, span, self_type, __ring_ev_fail) {
  ...
  __match_fail(__ring_m);
    ...
    __ring_match61: {
      const __ring_m61 = param_scheme;
      if (__ring_m61._tag === "some") {
        const ps = __ring_m61._0;
        __ring_match62: {
          const __ring_m62 = ps.def_id;
  ...
  for (const inferred_eff of inferred_effects.effects) {
    ...
    for (const declared_eff of declared_row.effects) {
      if (types$effects_match_kind(inferred_eff, declared_eff)) {
        found = true;
        __ring_match63: {
          const __ring_m63 = [inferred_eff, declared_eff];
          if (Array.isArray(__ring_m63) && __ring_m63.length === 2 && __ring_m63[0]._tag === "FailEffect" && __ring_m63[1]._tag === "FailEffect") {
            const ie = __ring_m63[0].error_type; const de = __ring_m63[1].error_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, ie, de, ctx.subst, span);
            break __ring_match63;
          }
          if (Array.isArray(__ring_m63) && __ring_m63.length === 2 && __ring_m63[0]._tag === "MutEffect" && __ring_m63[1]._tag === "MutEffect") {
            const is = __ring_m63[0].state_type; const ds = __ring_m63[1].state_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, is, ds, ctx.subst, span);
            break __ring_match63;
          }
          if (Array.isArray(__ring_m63) && __ring_m63.length === 2 && __ring_m63[0]._tag === "CustomEffect" && __ring_m63[1]._tag === "CustomEffect") {
            const ia = __ring_m63[0].type_args; const da = __ring_m63[1].type_args;
            let i = 0;
  ...
  if ((name === "main")) {
    for (const eff of final_effects.effects) {
      __ring_match64: {
        const __ring_m64 = eff;
        if (__ring_m64._tag === "CustomEffect") {
          const eff_name = __ring_m64.name;
          let skip = false;
          __ring_match65: {
            const __ring_m65 = _Map_get(ctx.env.types.effects, eff_name);
  ...
  for (const p of params) {
    if (((p.name === "self") || (!p.is_mutable))) {
      ...
      __ring_match67: {
        const __ring_m67 = List_get(final_params, fi);
        if (__ring_m67._tag === "some") {
          const fp = __ring_m67._0;
...
function check_one_decl(ctx, decl, hdecls, __ring_ev_fail) {
  ...
  __ring_match68: {
    ...
    if (__ring_m68._tag === "Fn") {
      const name = __ring_m68.name; const effects = __ring_m68.effects;
      if ((List_len(effects.effects) > 0)) {
        infer_ctx$update_fn_effects(ctx.env, name, effects);
      }
  ...
  __ring_match69: {
    ...
    if (__ring_m69._tag === "Impl") {
      const target_type = __ring_m69.target_type; const type_params = __ring_m69.type_params; const methods = __ring_m69.methods; const span = __ring_m69.span;
      for (const m of methods) {
        __ring_match70: {
          const __ring_m70 = m;
          if (__ring_m70._tag === "Delegate") {
            const field = __ring_m70.field; const trait_names = __ring_m70.trait_names; const dspan = __ring_m70.span;
  ...
  for (const decl of program.decls) {
    __ring_match71: {
      ...
      if (__ring_m71._tag === "Impl") {
        const target_type = __ring_m71.target_type; const type_params = __ring_m71.type_params; const trait_name = __ring_m71.trait_name; const methods = __ring_m71.methods; const span = __ring_m71.span;
