  ...
  for (const decl of decls) {
    ...
    __ring_match8: {
      ...
      if (__ring_m8._tag === "some") {
        ...
        __ring_match9: {
          const __ring_m9 = cap_row;
          if (__ring_m9._tag === "some") {
            const cap = __ring_m9._0;
...
function check_capability(ctx, decl, cap, mod_span) {
  __ring_match10: {
    ...
    if (__ring_m10._tag === "Fn") {
      const name = __ring_m10.name; const effects = __ring_m10.effects; const span = __ring_m10.span;
      return check_effects_capability(ctx, name, effects, cap, span);
      break __ring_match10;
    }
    if (__ring_m10._tag === "Impl") {
      const methods = __ring_m10.methods;
      for (const method of methods) {
        __ring_match11: {
          const __ring_m11 = method;
          if (__ring_m11._tag === "Fn") {
            const name = __ring_m11.name; const effects = __ring_m11.effects; const span = __ring_m11.span;
...
function check_sig_decl(ctx, name, members, is_pub, span) {
  let hmembers = [];
  __ring_match12: {
    const __ring_m12 = _Map_get(ctx.env.types.sigs, name);
    if (__ring_m12._tag === "some") {
      const sig_def = __ring_m12._0;
      for (const m of members) {
        __ring_match13: {
          const __ring_m13 = _Map_get(sig_def.members, m.name);
...
function resolve_mod_uses(ctx, uses) {
  for (const use_decl of uses) {
    ...
    __ring_match14: {
      ...
      if (__ring_m14._tag === "some") {
        ...
        __ring_match15: {
          const __ring_m15 = use_decl.imports;
          if (__ring_m15._tag === "NamedItems") {
            const names = __ring_m15.names;
  ...
  __match_fail(__ring_m);
              ...
              __ring_match16: {
                const __ring_m16 = env$TypeEnv_lookup(ctx.env, qualified_name);
          ...
          if (__ring_m15._tag === "Module") {
            if ((name_start_idx < List_len(segments))) {
              ...
              __ring_match17: {
                const __ring_m17 = env$TypeEnv_lookup(ctx.env, qualified_name);
...
function check_const_decl(ctx, name, type_annotation, init, is_pub, span, __ring_ev_fail) {
  ...
  __ring_match19: {
    const __ring_m19 = expected_ty;
    if (__ring_m19._tag === "some") {
      const ann_ty = __ring_m19._0;
...
function check_effect_decl(ctx, name, type_params, ast_ops, is_pub, span, __ring_ev_fail) {
  ...
  __match_fail(__ring_m);
    ...
    __ring_match20: {
      const __ring_m20 = ast_op.body;
      if (__ring_m20._tag === "some") {
        const body_expr = __ring_m20._0;
...
function update_impl_method_effects(ctx, target_type, method_name, effects) {
  __ring_match21: {
    const __ring_m21 = _Map_get(ctx.env.trait_reg.impl_methods, target_type);
    if (__ring_m21._tag === "some") {
      const methods_map = __ring_m21._0;
      __ring_match22: {
        const __ring_m22 = _Map_get(methods_map, method_name);
        if (__ring_m22._tag === "some") {
          const scheme = __ring_m22._0;
          __ring_match23: {
            const __ring_m23 = scheme.ty;
            if (__ring_m23._tag === "FnType") {
              const params = __ring_m23.params; const return_type = __ring_m23.return_type;
...
function check_impl_decl(ctx, target_type, type_params, trait_name, methods, span, __ring_ev_fail) {
  ...
  __ring_match25: {
    const __ring_m25 = _Map_get(ctx.env.types.structs, target_type);
    ...
    if (__ring_m25._tag === "none") {
      __ring_match26: {
        const __ring_m26 = _Map_get(ctx.env.types.enums, target_type);
  ...
  for (const method of methods) {
    __ring_match29: {
      ...
      if (__ring_m29._tag === "ExternFn") {
        const name = __ring_m29.name; const mtps = __ring_m29.type_params; const params = __ring_m29.params; const return_type = __ring_m29.return_type; const declared_effects = __ring_m29.declared_effects; const is_pub = __ring_m29.is_pub; const mspan = __ring_m29.span;
      ...
      if (__ring_m29._tag === "Fn") {
        const name = __ring_m29.name; const mtps = __ring_m29.type_params; const params = __ring_m29.params; const return_type = __ring_m29.return_type; const declared_effects = __ring_m29.declared_effects; const body = __ring_m29.body; const is_pub = __ring_m29.is_pub; const mspan = __ring_m29.span;
        ...
        __ring_match30: {
          const __ring_m30 = hdecl;
          if (__ring_m30._tag === "Fn") {
            const mname = __ring_m30.name; const inferred_effects = __ring_m30.effects;
...
function expand_delegate_impls(ctx, target_type, type_params, field, trait_names, span) {
  ...
  __ring_match31: {
    ...
    if (__ring_m31._tag === "some") {
      const struct_def = __ring_m31._0;
  ...
  __ring_match34: {
    const __ring_m34 = _Map_get(ctx.env.types.structs, target_type);
    ...
    if (__ring_m34._tag === "none") {
      __ring_match35: {
        const __ring_m35 = _Map_get(ctx.env.types.enums, target_type);
    ...
    __match_fail(__ring_m34);
          ...
          for (const tname of trait_names) {
            __ring_match36: {
              const __ring_m36 = _Map_get(ctx.env.trait_reg.traits, tname);
              if (__ring_m36._tag === "none") {
                break __ring_match36;
              }
              if (__ring_m36._tag === "some") {
                const trait_def = __ring_m36._0;
                let trait_hmethods = [];
                for (const tm of trait_def.methods) {
                  __ring_match37: {
                    const __ring_m37 = tm.ty;
                    if (__ring_m37._tag === "FnType") {
                      const trait_params = __ring_m37.params; const ret_ty = __ring_m37.return_type; const eff = __ring_m37.effects;
                      let hparams = [];
  ...
  __match_fail(__ring_m);
                      ...
                      const call_expr = hir$HExpr_Call(method_access, forward_args, [], [], Option_none, ret_ty, eff, span);
...
function check_trait_decl(ctx, name, type_params, ast_methods, is_pub, span, __ring_ev_fail) {
  ...
  if ((List_len(trait_def.methods) > 0)) {
    __ring_match38: {
      const __ring_m38 = List_first(trait_def.methods);
      if (__ring_m38._tag === "some") {
        const first_method = __ring_m38._0;
        __ring_match39: {
          const __ring_m39 = first_method.ty;
          if (__ring_m39._tag === "FnType") {
            const fps = __ring_m39.params;
            if ((List_len(fps) > 0)) {
              __ring_match40: {
                const __ring_m40 = List_first(fps);
                if (__ring_m40._tag === "some") {
                  const fp = __ring_m40._0;
                  self_var = fp;
  ...
  __match_fail(__ring_m);
    ...
    if (m.has_default) {
      __ring_match41: {
        const __ring_m41 = ast_method;
        if (__ring_m41._tag === "some") {
          const am = __ring_m41._0;
          __ring_match42: {
            const __ring_m42 = am;
            if (__ring_m42._tag === "Fn") {
              const abody = __ring_m42.body;
...
function check_trait_default_body(ctx, trait_name, self_var, hparams, body) {
  ...
  for (const p of hparams) {
    ...
    if (p.is_mutable) {
      __ring_match44: {
        const __ring_m44 = env$TypeEnv_lookup(ctx.env, p.name);
        if (__ring_m44._tag === "some") {
          const ps = __ring_m44._0;
          __ring_match45: {
            const __ring_m45 = ps.def_id;
...
function check_fn_body(ctx, type_params, hparams, expected_ret, body, saved_tp_scope, span, __ring_ev_fail) {
  ...
  for (const tp of type_params) {
    __ring_match46: {
      const __ring_m46 = _Map_get(ctx.type_param_scope, tp.name);
      if (__ring_m46._tag === "some") {
        const tv = __ring_m46._0;
        __ring_match47: {
          const __ring_m47 = tv;
...
function check_fn_decl(ctx, name, type_params, params, return_type, declared_effects, body, is_pub, span, self_type, __ring_ev_fail) {
  ...
  __match_fail(__ring_m);
    ...
    __ring_match53: {
      const __ring_m53 = param_scheme;
      if (__ring_m53._tag === "some") {
        const ps = __ring_m53._0;
        __ring_match54: {
          const __ring_m54 = ps.def_id;
  ...
  for (const inferred_eff of inferred_effects.effects) {
    ...
    for (const declared_eff of declared_row.effects) {
      if (types$effects_match_kind(inferred_eff, declared_eff)) {
        found = true;
        __ring_match55: {
          const __ring_m55 = [inferred_eff, declared_eff];
          if (Array.isArray(__ring_m55) && __ring_m55.length === 2 && __ring_m55[0]._tag === "FailEffect" && __ring_m55[1]._tag === "FailEffect") {
            const ie = __ring_m55[0].error_type; const de = __ring_m55[1].error_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, ie, de, ctx.subst, span);
            break __ring_match55;
          }
          if (Array.isArray(__ring_m55) && __ring_m55.length === 2 && __ring_m55[0]._tag === "MutEffect" && __ring_m55[1]._tag === "MutEffect") {
            const is = __ring_m55[0].state_type; const ds = __ring_m55[1].state_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, is, ds, ctx.subst, span);
            break __ring_match55;
          }
          if (Array.isArray(__ring_m55) && __ring_m55.length === 2 && __ring_m55[0]._tag === "CustomEffect" && __ring_m55[1]._tag === "CustomEffect") {
            const ia = __ring_m55[0].type_args; const da = __ring_m55[1].type_args;
            let i = 0;
  ...
  if ((name === "main")) {
    for (const eff of final_effects.effects) {
      __ring_match56: {
        const __ring_m56 = eff;
        if (__ring_m56._tag === "CustomEffect") {
          const eff_name = __ring_m56.name;
          let skip = false;
          __ring_match57: {
            const __ring_m57 = _Map_get(ctx.env.types.effects, eff_name);
...
function check_one_decl(ctx, decl, hdecls, __ring_ev_fail) {
  ...
  __ring_match59: {
    const __ring_m59 = hd;
    if (__ring_m59._tag === "Fn") {
      const name = __ring_m59.name; const effects = __ring_m59.effects;
  ...
  __ring_match60: {
    const __ring_m60 = decl;
    if (__ring_m60._tag === "Impl") {
      const target_type = __ring_m60.target_type; const type_params = __ring_m60.type_params; const methods = __ring_m60.methods; const span = __ring_m60.span;
      for (const m of methods) {
        __ring_match61: {
          const __ring_m61 = m;
          if (__ring_m61._tag === "Delegate") {
            const field = __ring_m61.field; const trait_names = __ring_m61.trait_names; const dspan = __ring_m61.span;
