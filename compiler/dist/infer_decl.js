...
function check_decl(ctx, decl, __ring_ev_fail) {
  ...
  __match_fail(__ring_m);
    ...
    if (__ring_m0._tag === "ModBlock") {
      const name = __ring_m0.name; const uses = __ring_m0.uses; const decls = __ring_m0.decls; const is_pub = __ring_m0.is_pub; const span = __ring_m0.span;
      return check_mod_decl(ctx, name, uses, decls, is_pub, span, __ring_ev_fail);
      break __ring_match0;
    }
...
function check_mod_decl(ctx, mod_name, uses, decls, is_pub, span, __ring_ev_fail) {
  ...
  for (const decl of decls) {
    ...
    __ring_match6: {
      const __ring_m6 = result;
      if (__ring_m6._tag === "some") {
        const hd = __ring_m6._0;
        List_push(hdecls, hd);
        break __ring_match6;
      }
...
function check_sig_decl(ctx, name, members, is_pub, span) {
  let hmembers = [];
  __ring_match7: {
    const __ring_m7 = _Map_get(ctx.env.types.sigs, name);
    if (__ring_m7._tag === "some") {
      const sig_def = __ring_m7._0;
      for (const m of members) {
        __ring_match8: {
          const __ring_m8 = _Map_get(sig_def.members, m.name);
          if (__ring_m8._tag === "some") {
            const scheme = __ring_m8._0;
  ...
  for (const use_decl of uses) {
    ...
    __ring_match9: {
      ...
      if (__ring_m9._tag === "some") {
        ...
        __ring_match10: {
          const __ring_m10 = use_decl.imports;
          if (__ring_m10._tag === "NamedItems") {
            const names = __ring_m10.names;
  ...
  __match_fail(__ring_m);
          ...
          if (__ring_m10._tag === "Module") {
            if ((name_start_idx < List_len(segments))) {
              ...
              __ring_match12: {
                const __ring_m12 = env$TypeEnv_lookup(ctx.env, qualified_name);
...
function check_const_decl(ctx, name, type_annotation, init, is_pub, span, __ring_ev_fail) {
  ...
  __ring_match14: {
    const __ring_m14 = expected_ty;
    if (__ring_m14._tag === "some") {
      const ann_ty = __ring_m14._0;
  ...
  for (const method of methods) {
    __ring_match15: {
      ...
      if (__ring_m15._tag === "ExternFn") {
        const name = __ring_m15.name; const mtps = __ring_m15.type_params; const params = __ring_m15.params; const return_type = __ring_m15.return_type; const declared_effects = __ring_m15.declared_effects; const is_pub = __ring_m15.is_pub; const mspan = __ring_m15.span;
      ...
      if (__ring_m15._tag === "Fn") {
        const name = __ring_m15.name; const mtps = __ring_m15.type_params; const params = __ring_m15.params; const return_type = __ring_m15.return_type; const declared_effects = __ring_m15.declared_effects; const body = __ring_m15.body; const is_pub = __ring_m15.is_pub; const mspan = __ring_m15.span;
        List_push(hmethods, check_fn_decl(ctx, name, mtps, params, return_type, declared_effects, body, is_pub, mspan, Option_some(impl_self_type), __ring_ev_fail));
        break __ring_match15;
      }
      break __ring_match15;
    }
...
function check_trait_decl(ctx, name, type_params, ast_methods, is_pub, span, __ring_ev_fail) {
  ...
  if ((List_len(trait_def.methods) > 0)) {
    __ring_match16: {
      const __ring_m16 = List_first(trait_def.methods);
      if (__ring_m16._tag === "some") {
        const first_method = __ring_m16._0;
        __ring_match17: {
          const __ring_m17 = first_method.ty;
          if (__ring_m17._tag === "FnType") {
            const fps = __ring_m17.params;
            if ((List_len(fps) > 0)) {
              __ring_match18: {
                const __ring_m18 = List_first(fps);
                if (__ring_m18._tag === "some") {
                  const fp = __ring_m18._0;
                  self_var = fp;
  ...
  __match_fail(__ring_m);
    ...
    if (m.has_default) {
      __ring_match19: {
        const __ring_m19 = ast_method;
        if (__ring_m19._tag === "some") {
          const am = __ring_m19._0;
          __ring_match20: {
            const __ring_m20 = am;
            if (__ring_m20._tag === "Fn") {
              const abody = __ring_m20.body;
...
function check_trait_default_body(ctx, trait_name, self_var, hparams, body) {
  ...
  for (const p of hparams) {
    ...
    if (p.is_mutable) {
      __ring_match22: {
        const __ring_m22 = env$TypeEnv_lookup(ctx.env, p.name);
        if (__ring_m22._tag === "some") {
          const ps = __ring_m22._0;
          __ring_match23: {
            const __ring_m23 = ps.def_id;
...
function effects_match_kind(a, b) {
  __ring_match29: {
    ...
    if (__ring_m29._tag === "MutEffect") {
      __ring_match31: {
        const __ring_m31 = b;
        if (__ring_m31._tag === "MutEffect") {
          return true;
    ...
    if (__ring_m29._tag === "CustomEffect") {
      const na = __ring_m29.name;
      __ring_match33: {
        const __ring_m33 = b;
        if (__ring_m33._tag === "CustomEffect") {
          const nb = __ring_m33.name;
          return (na === nb);
