...
function check_effects_capability(ctx, name, effects, cap, span) {
  ...
  __ring_match16: {
    const __ring_m16 = effects.tail;
    if (__ring_m16._tag === "some") {
      const _ = infer_ctx$type_error(ctx.sink, codes$E0408, `'${name}' has polymorphic effects (open effect row) which cannot be verified against mod requires constraints`, span, diagnostics$DiagnosticContext_OtherContext(Option_some("capability violation: open effect row")));
...
function check_const_decl(ctx, name, type_annotation, init, is_pub, span, __ring_ev_fail) {
  ...
  __ring_match23: {
    const __ring_m23 = type_annotation;
    if (__ring_m23._tag === "some") {
      const texpr = __ring_m23._0;
  ...
  __ring_match24: {
    const __ring_m24 = expected_ty;
    if (__ring_m24._tag === "some") {
      const ann_ty = __ring_m24._0;
...
function check_effect_decl(ctx, name, type_params, ast_ops, is_pub, span, __ring_ev_fail) {
  ...
  __match_fail(__ring_m);
    ...
    __ring_match25: {
      const __ring_m25 = ast_op_opt;
      if (__ring_m25._tag === "some") {
        const ast_op = __ring_m25._0;
        __ring_match26: {
          const __ring_m26 = ast_op.body;
          if (__ring_m26._tag === "some") {
            const body_expr = __ring_m26._0;
  ...
  if ((all_defaults && (List_len(def.ops) > 0))) {
    ...
    for (const hop of hops) {
      __ring_match27: {
        const __ring_m27 = hop.default_body;
        if (__ring_m27._tag === "some") {
          const body = __ring_m27._0;
...
function update_impl_method_effects(ctx, target_type, method_name, effects) {
  __ring_match29: {
    const __ring_m29 = _Map_get(ctx.env.trait_reg.impl_methods, target_type);
    if (__ring_m29._tag === "some") {
      const methods_map = __ring_m29._0;
      __ring_match30: {
        const __ring_m30 = _Map_get(methods_map, method_name);
        if (__ring_m30._tag === "some") {
          const scheme = __ring_m30._0;
          __ring_match31: {
            const __ring_m31 = scheme.ty;
            if (__ring_m31._tag === "FnType") {
              const params = __ring_m31.params; const return_type = __ring_m31.return_type;
...
function check_impl_decl(ctx, target_type, type_params, trait_name, methods, span, __ring_ev_fail) {
  ...
  __ring_match33: {
    const __ring_m33 = _Map_get(ctx.env.types.structs, target_type);
    ...
    if (__ring_m33._tag === "none") {
      __ring_match34: {
        const __ring_m34 = _Map_get(ctx.env.types.enums, target_type);
        if (__ring_m34._tag === "some") {
          const def = __ring_m34._0;
  ...
  for (const tp of type_params) {
    __ring_match35: {
      const __ring_m35 = _Map_get(ctx.type_param_scope, tp.name);
      if (__ring_m35._tag === "some") {
        const tv = __ring_m35._0;
        __ring_match36: {
          const __ring_m36 = tv;
...
function expand_delegate_impls(ctx, target_type, type_params, field, trait_names, span) {
  let result = [];
  __ring_match39: {
    const __ring_m39 = _Map_get(ctx.env.types.structs, target_type);
    if (__ring_m39._tag === "none") {
      return result;
      break __ring_match39;
    }
    if (__ring_m39._tag === "some") {
      const struct_def = __ring_m39._0;
  ...
  __ring_match42: {
    const __ring_m42 = _Map_get(ctx.env.types.structs, target_type);
    ...
    if (__ring_m42._tag === "none") {
      __ring_match43: {
        const __ring_m43 = _Map_get(ctx.env.types.enums, target_type);
            ...
            __ring_match44: {
              ...
              if (__ring_m44._tag === "some") {
                ...
                for (const tm of trait_def.methods) {
                  __ring_match45: {
                    const __ring_m45 = tm.ty;
                    if (__ring_m45._tag === "FnType") {
                      const trait_params = __ring_m45.params; const ret_ty = __ring_m45.return_type; const eff = __ring_m45.effects;
                      let hparams = [];
  ...
  return Option_none;
                        ...
                        __ring_match46: {
                          const __ring_m46 = ftn;
                          if (__ring_m46._tag === "some") {
                            const field_tn = __ring_m46._0;
                            let has_explicit = false;
                            __ring_match47: {
                              const __ring_m47 = _Map_get(ctx.env.trait_reg.impl_methods, field_tn);
                              if (__ring_m47._tag === "some") {
                                const methods_map = __ring_m47._0;
...
function check_trait_decl(ctx, name, type_params, ast_methods, is_pub, span, __ring_ev_fail) {
  ...
  if ((List_len(trait_def.methods) > 0)) {
    __ring_match48: {
      const __ring_m48 = List_first(trait_def.methods);
      if (__ring_m48._tag === "some") {
        const first_method = __ring_m48._0;
        __ring_match49: {
          const __ring_m49 = first_method.ty;
          if (__ring_m49._tag === "FnType") {
            const fps = __ring_m49.params;
            if ((List_len(fps) > 0)) {
              __ring_match50: {
                const __ring_m50 = List_first(fps);
                if (__ring_m50._tag === "some") {
                  const fp = __ring_m50._0;
                  self_var = fp;
  ...
  __match_fail(__ring_m);
    ...
    if (m.has_default) {
      __ring_match51: {
        const __ring_m51 = ast_method;
        if (__ring_m51._tag === "some") {
          const am = __ring_m51._0;
          __ring_match52: {
            const __ring_m52 = am;
            if (__ring_m52._tag === "Fn") {
              const abody = __ring_m52.body;
...
function check_trait_default_body(ctx, trait_name, self_var, hparams, body) {
  ...
  for (const p of hparams) {
    ...
    if (p.is_mutable) {
      __ring_match54: {
        const __ring_m54 = env$TypeEnv_lookup(ctx.env, p.name);
        if (__ring_m54._tag === "some") {
          const ps = __ring_m54._0;
          __ring_match55: {
            const __ring_m55 = ps.def_id;
...
function is_value_type(t) {
  __ring_match61: {
    ...
    if (__ring_m61._tag === "BoolType") {
      return true;
      break __ring_match61;
    }
    if (__ring_m61._tag === "StrType") {
      return true;
      break __ring_match61;
    }
    return false;
...
function check_fn_decl(ctx, name, type_params, params, return_type, declared_effects, body, is_pub, span, self_type, __ring_ev_fail) {
  ...
  for (const tp of type_params) {
    __ring_match62: {
      const __ring_m62 = _Map_get(ctx.type_param_scope, tp.name);
      if (__ring_m62._tag === "some") {
        const tv = __ring_m62._0;
        __ring_match63: {
          const __ring_m63 = tv;
  ...
  __match_fail(__ring_m);
    ...
    __ring_match64: {
      const __ring_m64 = param_scheme;
      if (__ring_m64._tag === "some") {
        const ps = __ring_m64._0;
        __ring_match65: {
          const __ring_m65 = ps.def_id;
          if (__ring_m65._tag === "some") {
            const did = __ring_m65._0;
  ...
  for (const inferred_eff of inferred_effects.effects) {
    ...
    for (const declared_eff of declared_row.effects) {
      if (types$effects_match_kind(inferred_eff, declared_eff)) {
        found = true;
        __ring_match66: {
          const __ring_m66 = [inferred_eff, declared_eff];
          if (Array.isArray(__ring_m66) && __ring_m66.length === 2 && __ring_m66[0]._tag === "FailEffect" && __ring_m66[1]._tag === "FailEffect") {
            const ie = __ring_m66[0].error_type; const de = __ring_m66[1].error_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, ie, de, ctx.subst, span);
            break __ring_match66;
          }
          if (Array.isArray(__ring_m66) && __ring_m66.length === 2 && __ring_m66[0]._tag === "MutEffect" && __ring_m66[1]._tag === "MutEffect") {
            const is = __ring_m66[0].state_type; const ds = __ring_m66[1].state_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, is, ds, ctx.subst, span);
            break __ring_match66;
          }
          if (Array.isArray(__ring_m66) && __ring_m66.length === 2 && __ring_m66[0]._tag === "CustomEffect" && __ring_m66[1]._tag === "CustomEffect") {
            const ia = __ring_m66[0].type_args; const da = __ring_m66[1].type_args;
            let i = 0;
  ...
  if ((name === "main")) {
    for (const eff of final_effects.effects) {
      __ring_match67: {
        const __ring_m67 = eff;
        if (__ring_m67._tag === "CustomEffect") {
          const eff_name = __ring_m67.name;
          let skip = false;
          __ring_match68: {
            const __ring_m68 = _Map_get(ctx.env.types.effects, eff_name);
  ...
  __ring_match69: {
    const __ring_m69 = fn_def_id;
    if (__ring_m69._tag === "some") {
      const did = __ring_m69._0;
  ...
  for (const p of params) {
    if (((p.name === "self") || (!p.is_mutable))) {
      ...
      __ring_match70: {
        const __ring_m70 = List_get(final_params, fi);
        if (__ring_m70._tag === "some") {
          const fp = __ring_m70._0;
...
function check_one_decl(ctx, decl, hdecls, __ring_ev_fail) {
  ...
  __ring_match71: {
    const __ring_m71 = hd;
    if (__ring_m71._tag === "Fn") {
      const name = __ring_m71.name; const effects = __ring_m71.effects;
      if ((List_len(effects.effects) > 0)) {
        infer_ctx$update_fn_effects(ctx.env, name, effects);
      }
  ...
  __ring_match72: {
    const __ring_m72 = decl;
    if (__ring_m72._tag === "Impl") {
      const target_type = __ring_m72.target_type; const type_params = __ring_m72.type_params; const methods = __ring_m72.methods; const span = __ring_m72.span;
      for (const m of methods) {
        __ring_match73: {
          const __ring_m73 = m;
          if (__ring_m73._tag === "Delegate") {
            const field = __ring_m73.field; const trait_names = __ring_m73.trait_names; const dspan = __ring_m73.span;
...
function collect_effect_spans(decls, spans) {
  for (const decl of decls) {
    __ring_match74: {
      ...
      if (__ring_m74._tag === "Effect") {
        const name = __ring_m74.name; const span = __ring_m74.span;
        _Map_insert(spans, name, span);
        break __ring_match74;
      }
      if (__ring_m74._tag === "ModBlock") {
        const mod_decls = __ring_m74.decls;
...
function dfs_detect_cycle(ctx, name, state, path, effect_spans) {
  ...
  List_push(path, name);
  __ring_match75: {
    const __ring_m75 = _Map_get(ctx.effect_default_deps, name);
    if (__ring_m75._tag === "some") {
      const deps = __ring_m75._0;
      for (const dep of deps) {
        __ring_match76: {
          const __ring_m76 = _Map_get(state, dep);
