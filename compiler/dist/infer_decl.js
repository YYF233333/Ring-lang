...
function check_trait_decl(ctx, name, type_params, ast_methods, is_pub, span, __ring_ev_fail) {
  ...
  if ((List_len(trait_def.methods) > 0)) {
    __ring_match31: {
      const __ring_m31 = List_first(trait_def.methods);
      if (__ring_m31._tag === "some") {
        const first_method = __ring_m31._0;
        __ring_match32: {
          const __ring_m32 = first_method.ty;
          if (__ring_m32._tag === "FnType") {
            const fps = __ring_m32.params;
            if ((List_len(fps) > 0)) {
              __ring_match33: {
                const __ring_m33 = List_first(fps);
                if (__ring_m33._tag === "some") {
                  const fp = __ring_m33._0;
                  self_var = fp;
  ...
  __match_fail(__ring_m);
    ...
    if (m.has_default) {
      __ring_match34: {
        const __ring_m34 = ast_method;
        if (__ring_m34._tag === "some") {
          const am = __ring_m34._0;
          __ring_match35: {
            const __ring_m35 = am;
            if (__ring_m35._tag === "Fn") {
              const abody = __ring_m35.body;
...
function check_trait_default_body(ctx, trait_name, self_var, hparams, body) {
  ...
  for (const p of hparams) {
    ...
    if (p.is_mutable) {
      __ring_match37: {
        const __ring_m37 = env$TypeEnv_lookup(ctx.env, p.name);
        if (__ring_m37._tag === "some") {
          const ps = __ring_m37._0;
          __ring_match38: {
            const __ring_m38 = ps.def_id;
      ...
      if (__ring_m39._tag === "some") {
        ...
        __ring_match40: {
          ...
          if (__ring_m40._tag === "TypeVar") {
            ...
            __ring_match41: {
              const __ring_m41 = resolved;
              if (__ring_m41._tag === "TypeVar") {
                const rid = __ring_m41.id;
...
function check_fn_decl(ctx, name, type_params, params, return_type, declared_effects, body, is_pub, span, self_type, __ring_ev_fail) {
  ...
  __match_fail(__ring_m);
    ...
    __ring_match46: {
      const __ring_m46 = param_scheme;
      if (__ring_m46._tag === "some") {
        const ps = __ring_m46._0;
        __ring_match47: {
          const __ring_m47 = ps.def_id;
  ...
  for (const inferred_eff of inferred_effects.effects) {
    ...
    for (const declared_eff of declared_row.effects) {
      if (types$effects_match_kind(inferred_eff, declared_eff)) {
        found = true;
        __ring_match48: {
          const __ring_m48 = [inferred_eff, declared_eff];
          if (Array.isArray(__ring_m48) && __ring_m48.length === 2 && __ring_m48[0]._tag === "FailEffect" && __ring_m48[1]._tag === "FailEffect") {
            const ie = __ring_m48[0].error_type; const de = __ring_m48[1].error_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, ie, de, ctx.subst, span);
            break __ring_match48;
          }
          if (Array.isArray(__ring_m48) && __ring_m48.length === 2 && __ring_m48[0]._tag === "MutEffect" && __ring_m48[1]._tag === "MutEffect") {
            const is = __ring_m48[0].state_type; const ds = __ring_m48[1].state_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, is, ds, ctx.subst, span);
            break __ring_match48;
          }
          if (Array.isArray(__ring_m48) && __ring_m48.length === 2 && __ring_m48[0]._tag === "CustomEffect" && __ring_m48[1]._tag === "CustomEffect") {
            const ia = __ring_m48[0].type_args; const da = __ring_m48[1].type_args;
            let i = 0;
  ...
  if ((name === "main")) {
    for (const eff of final_effects.effects) {
      __ring_match49: {
        const __ring_m49 = eff;
        if (__ring_m49._tag === "CustomEffect") {
          const eff_name = __ring_m49.name;
          let skip = false;
          __ring_match50: {
            const __ring_m50 = _Map_get(ctx.env.types.effects, eff_name);
            if (__ring_m50._tag === "some") {
              const edef = __ring_m50._0;
...
function check_one_decl(ctx, decl, hdecls, __ring_ev_fail) {
  ...
  __ring_match52: {
    ...
    if (__ring_m52._tag === "Fn") {
      const name = __ring_m52.name; const effects = __ring_m52.effects;
