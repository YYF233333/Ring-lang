...
function check_trait_default_body(ctx, trait_name, self_var, hparams, body) {
  ...
  for (const p of hparams) {
    ...
    if (p.is_mutable) {
      __ring_match57: {
        const __ring_m57 = env$TypeEnv_lookup(ctx.env, p.name);
        if (__ring_m57._tag === "some") {
          const ps = __ring_m57._0;
          __ring_match58: {
            const __ring_m58 = ps.def_id;
...
function check_fn_decl(ctx, name, type_params, params, return_type, declared_effects, body, is_pub, span, self_type, __ring_ev_fail) {
  ...
  for (const tp of type_params) {
    __ring_match65: {
      const __ring_m65 = _Map_get(ctx.type_param_scope, tp.name);
      if (__ring_m65._tag === "some") {
        const tv = __ring_m65._0;
        __ring_match66: {
          const __ring_m66 = tv;
  ...
  __match_fail(__ring_m);
    ...
    __ring_match67: {
      const __ring_m67 = param_scheme;
      if (__ring_m67._tag === "some") {
        const ps = __ring_m67._0;
        __ring_match68: {
          const __ring_m68 = ps.def_id;
  ...
  for (const inferred_eff of inferred_effects.effects) {
    ...
    for (const declared_eff of declared_row.effects) {
      if (types$effects_match_kind(inferred_eff, declared_eff)) {
        found = true;
        __ring_match69: {
          const __ring_m69 = [inferred_eff, declared_eff];
          if (Array.isArray(__ring_m69) && __ring_m69.length === 2 && __ring_m69[0]._tag === "FailEffect" && __ring_m69[1]._tag === "FailEffect") {
            const ie = __ring_m69[0].error_type; const de = __ring_m69[1].error_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, ie, de, ctx.subst, span);
            break __ring_match69;
          }
          if (Array.isArray(__ring_m69) && __ring_m69.length === 2 && __ring_m69[0]._tag === "MutEffect" && __ring_m69[1]._tag === "MutEffect") {
            const is = __ring_m69[0].state_type; const ds = __ring_m69[1].state_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, is, ds, ctx.subst, span);
            break __ring_match69;
          }
          if (Array.isArray(__ring_m69) && __ring_m69.length === 2 && __ring_m69[0]._tag === "CustomEffect" && __ring_m69[1]._tag === "CustomEffect") {
            const ia = __ring_m69[0].type_args; const da = __ring_m69[1].type_args;
            let i = 0;
  ...
  for (const p of params) {
    if (((p.name === "self") || (!p.is_mutable))) {
      ...
      __ring_match73: {
        const __ring_m73 = List_get(final_params, fi);
        if (__ring_m73._tag === "some") {
          const fp = __ring_m73._0;
...
function check_one_decl(ctx, decl, hdecls, __ring_ev_fail) {
  ...
  __ring_match74: {
    ...
    if (__ring_m74._tag === "Fn") {
      const name = __ring_m74.name; const effects = __ring_m74.effects;
      if ((List_len(effects.effects) > 0)) {
        infer_ctx$update_fn_effects(ctx.env, name, effects);
      }
  ...
  __ring_match75: {
    ...
    if (__ring_m75._tag === "Impl") {
      const target_type = __ring_m75.target_type; const type_params = __ring_m75.type_params; const methods = __ring_m75.methods; const span = __ring_m75.span;
      for (const m of methods) {
        __ring_match76: {
          const __ring_m76 = m;
          if (__ring_m76._tag === "Delegate") {
            const field = __ring_m76.field; const trait_names = __ring_m76.trait_names; const dspan = __ring_m76.span;
  ...
  for (const decl of decls) {
    __ring_match77: {
      ...
      if (__ring_m77._tag === "Effect") {
        const name = __ring_m77.name; const span = __ring_m77.span;
...
function dfs_detect_cycle(ctx, name, state, path, effect_spans) {
  ...
  List_push(path, name);
  __ring_match78: {
    const __ring_m78 = _Map_get(ctx.effect_default_deps, name);
    if (__ring_m78._tag === "some") {
      const deps = __ring_m78._0;
      for (const dep of deps) {
        __ring_match79: {
          const __ring_m79 = _Map_get(state, dep);
...
function check(ctx, program, __ring_ev_fail) {
  ...
  for (const decl of program.decls) {
    __ring_match80: {
      const __ring_m80 = decl;
      if (__ring_m80._tag === "Impl") {
        const target_type = __ring_m80.target_type; const type_params = __ring_m80.type_params; const trait_name = __ring_m80.trait_name; const methods = __ring_m80.methods; const span = __ring_m80.span;
