...
function check_effect_decl(ctx, name, type_params, ast_ops, is_pub, span, __ring_ev_fail) {
  ...
  __match_fail(__ring_m);
    ...
    __ring_match24: {
      const __ring_m24 = ast_op.body;
      if (__ring_m24._tag === "some") {
        const body_expr = __ring_m24._0;
...
function update_impl_method_effects(ctx, target_type, method_name, effects) {
  __ring_match25: {
    ...
    if (__ring_m25._tag === "some") {
      const methods_map = __ring_m25._0;
      __ring_match26: {
        const __ring_m26 = _Map_get(methods_map, method_name);
        if (__ring_m26._tag === "some") {
          const scheme = __ring_m26._0;
          __ring_match27: {
            const __ring_m27 = scheme.ty;
            if (__ring_m27._tag === "FnType") {
              const params = __ring_m27.params; const return_type = __ring_m27.return_type;
...
function check_impl_decl(ctx, target_type, type_params, trait_name, methods, span, __ring_ev_fail) {
  ...
  for (const tp of type_params) {
    __ring_match28: {
      const __ring_m28 = _Map_get(ctx.type_param_scope, tp.name);
      if (__ring_m28._tag === "some") {
        const tv = __ring_m28._0;
  ...
  __ring_match29: {
    const __ring_m29 = _Map_get(ctx.env.types.structs, target_type);
    if (__ring_m29._tag === "some") {
      const def = __ring_m29._0;
      return types$Type_StructType(def.name, impl_tp_types, def.fields);
      break __ring_match29;
    }
    if (__ring_m29._tag === "none") {
      __ring_match30: {
        const __ring_m30 = _Map_get(ctx.env.types.enums, target_type);
  ...
  for (const method of methods) {
    __ring_match33: {
      const __ring_m33 = method;
      if (__ring_m33._tag === "ExternFn") {
        const name = __ring_m33.name; const mtps = __ring_m33.type_params; const params = __ring_m33.params; const return_type = __ring_m33.return_type; const declared_effects = __ring_m33.declared_effects; const is_pub = __ring_m33.is_pub; const mspan = __ring_m33.span;
      ...
      if (__ring_m33._tag === "Fn") {
        const name = __ring_m33.name; const mtps = __ring_m33.type_params; const params = __ring_m33.params; const return_type = __ring_m33.return_type; const declared_effects = __ring_m33.declared_effects; const body = __ring_m33.body; const is_pub = __ring_m33.is_pub; const mspan = __ring_m33.span;
        ...
        __ring_match34: {
          const __ring_m34 = hdecl;
          if (__ring_m34._tag === "Fn") {
            const mname = __ring_m34.name; const inferred_effects = __ring_m34.effects;
...
function expand_delegate_impls(ctx, target_type, type_params, field, trait_names, span) {
  ...
  __ring_match35: {
    ...
    if (__ring_m35._tag === "some") {
      const struct_def = __ring_m35._0;
  ...
  __ring_match38: {
    ...
    if (__ring_m38._tag === "none") {
      __ring_match39: {
        const __ring_m39 = _Map_get(ctx.env.types.enums, target_type);
    ...
    __match_fail(__ring_m38);
          ...
          for (const tname of trait_names) {
            __ring_match40: {
              const __ring_m40 = _Map_get(ctx.env.trait_reg.traits, tname);
              if (__ring_m40._tag === "none") {
                break __ring_match40;
              }
              if (__ring_m40._tag === "some") {
                const trait_def = __ring_m40._0;
                let trait_hmethods = [];
                for (const tm of trait_def.methods) {
                  __ring_match41: {
                    const __ring_m41 = tm.ty;
                    if (__ring_m41._tag === "FnType") {
                      const trait_params = __ring_m41.params; const ret_ty = __ring_m41.return_type; const eff = __ring_m41.effects;
                      let hparams = [];
  ...
  return Option_none;
                        ...
                        __ring_match42: {
                          const __ring_m42 = ftn;
                          if (__ring_m42._tag === "some") {
                            const field_tn = __ring_m42._0;
                            let has_explicit = false;
                            __ring_match43: {
                              const __ring_m43 = _Map_get(ctx.env.trait_reg.impl_methods, field_tn);
                              if (__ring_m43._tag === "some") {
                                const methods_map = __ring_m43._0;
...
function check_fn_decl(ctx, name, type_params, params, return_type, declared_effects, body, is_pub, span, self_type, __ring_ev_fail) {
  ...
  for (const inferred_eff of inferred_effects.effects) {
    ...
    for (const declared_eff of declared_row.effects) {
      if (types$effects_match_kind(inferred_eff, declared_eff)) {
        found = true;
        __ring_match61: {
          const __ring_m61 = [inferred_eff, declared_eff];
          if (Array.isArray(__ring_m61) && __ring_m61.length === 2 && __ring_m61[0]._tag === "FailEffect" && __ring_m61[1]._tag === "FailEffect") {
            const ie = __ring_m61[0].error_type; const de = __ring_m61[1].error_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, ie, de, ctx.subst, span);
            break __ring_match61;
          }
          if (Array.isArray(__ring_m61) && __ring_m61.length === 2 && __ring_m61[0]._tag === "MutEffect" && __ring_m61[1]._tag === "MutEffect") {
            const is = __ring_m61[0].state_type; const ds = __ring_m61[1].state_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, is, ds, ctx.subst, span);
            break __ring_match61;
          }
          if (Array.isArray(__ring_m61) && __ring_m61.length === 2 && __ring_m61[0]._tag === "CustomEffect" && __ring_m61[1]._tag === "CustomEffect") {
            const ia = __ring_m61[0].type_args; const da = __ring_m61[1].type_args;
            let i = 0;
  ...
  if ((name === "main")) {
    for (const eff of final_effects.effects) {
      __ring_match62: {
        const __ring_m62 = eff;
        if (__ring_m62._tag === "CustomEffect") {
          const eff_name = __ring_m62.name;
          let skip = false;
          __ring_match63: {
            const __ring_m63 = _Map_get(ctx.env.types.effects, eff_name);
...
function check_one_decl(ctx, decl, hdecls, __ring_ev_fail) {
  ...
  __ring_match65: {
    ...
    if (__ring_m65._tag === "Fn") {
      const name = __ring_m65.name; const effects = __ring_m65.effects;
      if ((List_len(effects.effects) > 0)) {
        infer_ctx$update_fn_effects(ctx.env, name, effects);
      }
  ...
  __ring_match66: {
    ...
    if (__ring_m66._tag === "Impl") {
      const target_type = __ring_m66.target_type; const type_params = __ring_m66.type_params; const methods = __ring_m66.methods; const span = __ring_m66.span;
      for (const m of methods) {
        __ring_match67: {
          const __ring_m67 = m;
          if (__ring_m67._tag === "Delegate") {
            const field = __ring_m67.field; const trait_names = __ring_m67.trait_names; const dspan = __ring_m67.span;
...
function check(ctx, program, __ring_ev_fail) {
  ...
  for (const decl of program.decls) {
    __ring_match68: {
      const __ring_m68 = decl;
      if (__ring_m68._tag === "Impl") {
        const target_type = __ring_m68.target_type; const type_params = __ring_m68.type_params; const trait_name = __ring_m68.trait_name; const methods = __ring_m68.methods; const span = __ring_m68.span;
