...
function update_impl_method_effects(ctx, target_type, method_name, effects) {
  __ring_match20: {
    const __ring_m20 = _Map_get(ctx.env.trait_reg.impl_methods, target_type);
    if (__ring_m20._tag === "some") {
      const methods_map = __ring_m20._0;
      __ring_match21: {
        const __ring_m21 = _Map_get(methods_map, method_name);
...
function check_impl_decl(ctx, target_type, type_params, trait_name, methods, span, __ring_ev_fail) {
  ...
  __ring_match24: {
    const __ring_m24 = _Map_get(ctx.env.types.structs, target_type);
    ...
    if (__ring_m24._tag === "none") {
      __ring_match25: {
        const __ring_m25 = _Map_get(ctx.env.types.enums, target_type);
  ...
  for (const tp of type_params) {
    __ring_match26: {
      const __ring_m26 = _Map_get(ctx.type_param_scope, tp.name);
      if (__ring_m26._tag === "some") {
        const tv = __ring_m26._0;
        __ring_match27: {
          const __ring_m27 = tv;
  ...
  for (const method of methods) {
    __ring_match28: {
      ...
      if (__ring_m28._tag === "ExternFn") {
        const name = __ring_m28.name; const mtps = __ring_m28.type_params; const params = __ring_m28.params; const return_type = __ring_m28.return_type; const declared_effects = __ring_m28.declared_effects; const is_pub = __ring_m28.is_pub; const mspan = __ring_m28.span;
      ...
      if (__ring_m28._tag === "Fn") {
        const name = __ring_m28.name; const mtps = __ring_m28.type_params; const params = __ring_m28.params; const return_type = __ring_m28.return_type; const declared_effects = __ring_m28.declared_effects; const body = __ring_m28.body; const is_pub = __ring_m28.is_pub; const mspan = __ring_m28.span;
        ...
        __ring_match29: {
          const __ring_m29 = hdecl;
          if (__ring_m29._tag === "Fn") {
            const mname = __ring_m29.name; const inferred_effects = __ring_m29.effects;
...
function check_trait_decl(ctx, name, type_params, ast_methods, is_pub, span, __ring_ev_fail) {
  ...
  if ((List_len(trait_def.methods) > 0)) {
    __ring_match30: {
      const __ring_m30 = List_first(trait_def.methods);
      if (__ring_m30._tag === "some") {
        const first_method = __ring_m30._0;
        __ring_match31: {
          const __ring_m31 = first_method.ty;
          if (__ring_m31._tag === "FnType") {
            const fps = __ring_m31.params;
            if ((List_len(fps) > 0)) {
              __ring_match32: {
                const __ring_m32 = List_first(fps);
  ...
  __match_fail(__ring_m);
    ...
    if (m.has_default) {
      __ring_match33: {
        const __ring_m33 = ast_method;
        if (__ring_m33._tag === "some") {
          const am = __ring_m33._0;
          __ring_match34: {
            const __ring_m34 = am;
            if (__ring_m34._tag === "Fn") {
              const abody = __ring_m34.body;
...
function check_trait_default_body(ctx, trait_name, self_var, hparams, body) {
  ...
  for (const p of hparams) {
    ...
    if (p.is_mutable) {
      __ring_match36: {
        const __ring_m36 = env$TypeEnv_lookup(ctx.env, p.name);
...
function check_fn_decl(ctx, name, type_params, params, return_type, declared_effects, body, is_pub, span, self_type, __ring_ev_fail) {
  ...
  for (const tp of type_params) {
    __ring_match43: {
      const __ring_m43 = _Map_get(ctx.type_param_scope, tp.name);
      if (__ring_m43._tag === "some") {
        const tv = __ring_m43._0;
        __ring_match44: {
          const __ring_m44 = tv;
  ...
  __match_fail(__ring_m);
    ...
    __ring_match45: {
      const __ring_m45 = param_scheme;
      if (__ring_m45._tag === "some") {
        const ps = __ring_m45._0;
        __ring_match46: {
          const __ring_m46 = ps.def_id;
  ...
  for (const inferred_eff of inferred_effects.effects) {
    ...
    for (const declared_eff of declared_row.effects) {
      if (types$effects_match_kind(inferred_eff, declared_eff)) {
        found = true;
        __ring_match47: {
          const __ring_m47 = [inferred_eff, declared_eff];
          if (Array.isArray(__ring_m47) && __ring_m47.length === 2 && __ring_m47[0]._tag === "FailEffect" && __ring_m47[1]._tag === "FailEffect") {
            const ie = __ring_m47[0].error_type; const de = __ring_m47[1].error_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, ie, de, ctx.subst, span);
            break __ring_match47;
          }
          if (Array.isArray(__ring_m47) && __ring_m47.length === 2 && __ring_m47[0]._tag === "MutEffect" && __ring_m47[1]._tag === "MutEffect") {
            const is = __ring_m47[0].state_type; const ds = __ring_m47[1].state_type;
            ctx.subst = infer_ctx$unify_at(ctx.sink, ctx.env, is, ds, ctx.subst, span);
            break __ring_match47;
          }
          if (Array.isArray(__ring_m47) && __ring_m47.length === 2 && __ring_m47[0]._tag === "CustomEffect" && __ring_m47[1]._tag === "CustomEffect") {
            const ia = __ring_m47[0].type_args; const da = __ring_m47[1].type_args;
            let i = 0;
  ...
  if ((name === "main")) {
    for (const eff of final_effects.effects) {
      __ring_match48: {
        const __ring_m48 = eff;
        if (__ring_m48._tag === "CustomEffect") {
          const eff_name = __ring_m48.name;
...
function check_one_decl(ctx, decl, hdecls, __ring_ev_fail) {
  ...
  __ring_match50: {
    ...
    if (__ring_m50._tag === "Fn") {
      const name = __ring_m50.name; const effects = __ring_m50.effects;
