...
function Parser_parse_return_expr(self) {
  const start = Parser_current_span_start(self);
  Parser_expect(self, lexer$TokenKind_TkReturn);
  let value = Option_none;
    ...
    if (Parser_check(self, lexer$TokenKind_TkRBrace)) {
      __ring_match10: {
        ...
        if (__ring_m10._tag === "ExprStmt") {
          const e = __ring_m10.expr; const hs = __ring_m10.has_semi;
          if ((!hs)) {
            tail = Option_some(e);
          } else {
...
function Parser_parse_impl_decl(self) {
  ...
  const type_params = Parser_parse_type_params(self);
  const first_name = Parser_parse_qualified_ident(self);
...
function Parser_parse_assoc_type_decl(self, is_pub) {
  const start = Parser_current_span_start(self);
  Parser_advance(self);
  const name = Parser_expect(self, lexer$TokenKind_TkIdent).value;
  let bounds = [];
  ...
  if (Parser_try_consume(self, lexer$TokenKind_TkEq)) {
    value = Option_some(Parser_parse_type_expr(self));
  }
  const end = Parser_current_span_start(self);
...
function Parser_parse_match_arm(self) {
  ...
  const body = (Parser_check(self, lexer$TokenKind_TkReturn) ? Parser_parse_return_expr(self) : Parser_parse_expr(self));
...
function Parser_parse_qualified_ident(self) {
  const name = Parser_expect(self, lexer$TokenKind_TkIdent).value;
  if ((!Parser_check(self, lexer$TokenKind_TkColonColon))) {
    return name;
  }
  ...
  while (Parser_check(self, lexer$TokenKind_TkColonColon)) {
    Parser_advance(self);
    const segment = Parser_expect(self, lexer$TokenKind_TkIdent).value;
...
function Parser_validate_target_type_args(self, type_params) {
  ...
  while (((!Parser_check(self, lexer$TokenKind_TkGt)) ? (!Parser_at_end(self)) : false)) {
    ...
    if (lexer$__TokenKind_Eq.eq(arg_tok.kind, lexer$TokenKind_TkIdent)) {
      ...
      if ((!found)) {
        Parser_report_error(self, codes$E0105, `type argument '${arg_name}' in target type is not a type parameter declared on the impl; expected one of the impl's type parameters`, Option_some(arg_tok.span));
      }
      Parser_advance(self);
    } else {
      Parser_report_error(self, codes$E0105, `expected type parameter name in target type arguments, got '${arg_tok.value}'`, Option_some(arg_tok.span));
      Parser_advance(self);
    }
    Parser_try_consume(self, lexer$TokenKind_TkComma);
  }
  ...
  if (Parser_check(self, lexer$TokenKind_TkLt)) {
    ...
    while (((!Parser_check(self, lexer$TokenKind_TkGt)) ? (!Parser_at_end(self)) : false)) {
      if ((Parser_check(self, lexer$TokenKind_TkIdent) ? lexer$__TokenKind_Eq.eq(Parser_peek_at(self, 1).kind, lexer$TokenKind_TkEq) : false)) {
        ...
        Parser_advance(self);
        const ac_ty = Parser_parse_type_expr(self);
...
function Parser_parse_params(self) {
  ...
  while (true) {
    ...
    __ring_match16: {
      const __ring_m16 = p.default_value;
...
function Parser_parse_param(self) {
  ...
  if (Parser_try_consume(self, lexer$TokenKind_TkEq)) {
    default_value = Option_some(Parser_parse_expr(self, PREC_NONE));
  }
  const end = Parser_current_span_start(self);
...
function is_decl_start(k) {
  __ring_match18: {
    const __ring_m18 = k;
    if (__ring_m18._tag === "TkFn") {
      return true;
      break __ring_match18;
    }
    if (__ring_m18._tag === "TkStruct") {
      return true;
      break __ring_match18;
    }
    if (__ring_m18._tag === "TkEnum") {
      return true;
      break __ring_match18;
    }
    if (__ring_m18._tag === "TkEffect") {
      return true;
      break __ring_match18;
    }
    if (__ring_m18._tag === "TkTrait") {
      return true;
      break __ring_match18;
    }
    if (__ring_m18._tag === "TkImpl") {
      return true;
      break __ring_match18;
    }
    if (__ring_m18._tag === "TkExtern") {
      return true;
      break __ring_match18;
    }
    if (__ring_m18._tag === "TkUse") {
      return true;
      break __ring_match18;
    }
    if (__ring_m18._tag === "TkPub") {
      return true;
      break __ring_match18;
    }
    if (__ring_m18._tag === "TkTest") {
      return true;
      break __ring_match18;
    }
    if (__ring_m18._tag === "TkConst") {
      return true;
      break __ring_match18;
    }
    if (__ring_m18._tag === "TkMod") {
      return true;
      break __ring_match18;
    }
    if (__ring_m18._tag === "TkSig") {
      return true;
      break __ring_match18;
    }
    return false;
...
function type_expr_span(te) {
  __ring_match19: {
    const __ring_m19 = te;
    if (__ring_m19._tag === "Named") {
      const span = __ring_m19.span;
      return span;
...
function infix_precedence(kind) {
  __ring_match20: {
    const __ring_m20 = kind;
    if (__ring_m20._tag === "TkCatch") {
      return PREC_CATCH;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkPipePipe") {
      return PREC_LOGIC_OR;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkAmpAmp") {
      return PREC_LOGIC_AND;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkEqEq") {
      return PREC_EQUALITY;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkBangEq") {
      return PREC_EQUALITY;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkLt") {
      return PREC_COMPARE;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkGt") {
      return PREC_COMPARE;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkLtEq") {
      return PREC_COMPARE;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkGtEq") {
      return PREC_COMPARE;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkDotDot") {
      return PREC_RANGE;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkDotDotEq") {
      return PREC_RANGE;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkPlus") {
      return PREC_ADD_SUB;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkMinus") {
      return PREC_ADD_SUB;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkStar") {
      return PREC_MUL_DIV;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkSlash") {
      return PREC_MUL_DIV;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkPercent") {
      return PREC_MUL_DIV;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkDot") {
      return PREC_POSTFIX;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkLParen") {
      return PREC_POSTFIX;
      break __ring_match20;
    }
    if (__ring_m20._tag === "TkLBracket") {
      return PREC_POSTFIX;
      break __ring_match20;
    }
    return PREC_NONE;
