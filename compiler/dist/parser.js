...
function Parser_parse_pattern(self, __ring_ev_fail) {
  ...
  if (Parser_check(self, lexer$TokenKind_TkMinus)) {
    const next = Parser_peek_at(self, 1);
    if (lexer$__TokenKind_Eq.eq(next.kind, lexer$TokenKind_TkIntLit)) {
      Parser_advance(self);
      Parser_advance(self);
      return ast$Pattern_Literal(ast$LiteralValue_IntVal((0 - Option_unwrap_or(parse_int(next.value), 0))), Parser_make_span(self, start, next.span.end));
    }
    if (lexer$__TokenKind_Eq.eq(next.kind, lexer$TokenKind_TkFloatLit)) {
      Parser_advance(self);
      Parser_advance(self);
      return ast$Pattern_Literal(ast$LiteralValue_FloatVal((0 - Option_unwrap_or(parse_float(next.value), 0))), Parser_make_span(self, start, next.span.end));
    }
  }
