...
function parse(source, file) {
  let sink = diagnostics$new_collecting_sink();
...
function Parser_parse_program(self) {
  ...
  while ((!Parser_at_end(self))) {
    ...
    if (Parser_check(self, lexer$TokenKind_TkUse)) {
      ...
      List_push(uses, Parser_parse_use_decl(self, false));
      continue;
    }
    if (Parser_check(self, lexer$TokenKind_TkPub)) {
      ...
      if (Parser_check(self, lexer$TokenKind_TkUse)) {
        ...
        List_push(uses, Parser_parse_use_decl(self, true));
        continue;
      }
      self.pos = save_pos;
    ...
    decls_started = true;
    __ring_match3: {
      const __ring_m3 = Parser_parse_decl(self);
      if (__ring_m3._tag === "some") {
        const decl = __ring_m3._0;
...
function Parser_parse_fn_decl(self, is_pub, body_optional) {
  ...
  const end = Parser_current_span_start(self);
  return ast$Decl_Fn(name, type_params, params, return_type, body, is_pub, is_abstract_val, Parser_make_span(self, start, end));
}
...
function Parser_parse_extern_fn_decl_body(self, is_pub, start) {
  ...
  const end = Parser_current_span_start(self);
  return ast$Decl_ExternFn(name, type_params, params, return_type, is_pub, Parser_make_span(self, start, end));
}
...
function Parser_parse_expr_bp(self, min_prec) {
  ...
  while (true) {
    ...
    if (Parser_check(self, lexer$TokenKind_TkOr)) {
      left = Parser_parse_or_expr(self, left);
      ...
      if (Parser_check(self, lexer$TokenKind_TkCatch)) {
        ...
        if (Parser_check(self, lexer$TokenKind_TkDot)) {
          ...
          if (Parser_check(self, lexer$TokenKind_TkLParen)) {
            ...
            if (Parser_check(self, lexer$TokenKind_TkQuestion)) {
              const q_tok = Parser_advance(self);
              left = ast$Expr_OptionUnwrap(left, Parser_make_span(self, expr_span(left).start, q_tok.span.end));
              last_was_comparison = false;
            } else {
...
function Parser_parse_prefix(self) {
  ...
  if (Parser_check(self, lexer$TokenKind_TkTry)) {
    Parser_advance(self);
    const body = Parser_parse_block_expr(self);
    return ast$Expr_TryBlock(body, Parser_make_span(self, start, expr_span(body).end));
  }
...
function Parser_parse_or_expr(self, left) {
  Parser_advance(self);
  const default_value = Parser_parse_expr_bp(self, PREC_OR());
  const span = Parser_make_span(self, expr_span(left).start, expr_span(default_value).end);
  return ast$Expr_OrExpr(left, default_value, span);
}
