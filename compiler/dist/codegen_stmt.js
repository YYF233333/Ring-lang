...
function emit_stmt(ctx, stmt) {
  __ring_match30: {
    ...
    if (__ring_m30._tag === "While") {
      const condition = __ring_m30.condition; const body = __ring_m30.body;
      const cond = codegen_expr$gen_expr(ctx, condition);
    ...
    if (__ring_m30._tag === "ForIn") {
      const binding = __ring_m30.binding; const destructure = __ring_m30.destructure; const iterable = __ring_m30.iterable; const body = __ring_m30.body; const iterable_type_name = __ring_m30.iterable_type_name; const iter_type_name = __ring_m30.iter_type_name;
      __ring_match32: {
        const __ring_m32 = iterable;
        if (__ring_m32._tag === "RangeExpr") {
          const start = __ring_m32.start; const end = __ring_m32.end; const inclusive = __ring_m32.inclusive;
  ...
  return false;
    ...
    if (__ring_m30._tag === "LetDestructure") {
      const bindings = __ring_m30.bindings; const init = __ring_m30.init;
    ...
    if (__ring_m30._tag === "IfLet") {
      const pattern = __ring_m30.pattern; const expr = __ring_m30.expr; const then_block = __ring_m30.then_block; const else_block = __ring_m30.else_block;
      codegen_ctx$emit(ctx, "{");
      codegen_ctx$push_indent(ctx);
    ...
    if (__ring_m30._tag === "Var") {
      const name = __ring_m30.name; const def_id = __ring_m30.def_id; const init = __ring_m30.init;
      const sname = codegen_ctx$safe_ident(name);
      const init_js = codegen_expr$gen_expr(ctx, init);
  ...
  __match_fail(__ring_m);
    ...
    if (__ring_m30._tag === "Assign") {
      const target = __ring_m30.target; const value = __ring_m30.value;
