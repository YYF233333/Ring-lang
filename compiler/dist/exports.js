...
function extract_exports(module_key, module_prefix, program, hprogram, env) {
  ...
  for (const decl of program.decls) {
    __ring_match0: {
      ...
      if (__ring_m0._tag === "Struct") {
        ...
        if (is_pub) {
          __ring_match2: {
            const __ring_m2 = _Map_get(env.structs, name);
      ...
      if (__ring_m0._tag === "Enum") {
        ...
        if (is_pub) {
          __ring_match3: {
            const __ring_m3 = _Map_get(env.enums, name);
      ...
      if (__ring_m0._tag === "Effect") {
        ...
        if (is_pub) {
          __ring_match5: {
            const __ring_m5 = _Map_get(env.effects, name);
      ...
      if (__ring_m0._tag === "Trait") {
        ...
        if (is_pub) {
          __ring_match6: {
            const __ring_m6 = _Map_get(env.traits, name);
      ...
      if (__ring_m0._tag === "Impl") {
        ...
        __ring_match7: {
          const __ring_m7 = _Map_get(env.impl_methods, target_type);
      ...
      if (__ring_m0._tag === "ExternType") {
        ...
        if (is_pub) {
          __ring_match13: {
            const __ring_m13 = _Map_get(env.structs, name);
  ...
  let trait_impls = empty_impl_entries();
  for (const impl_ of env.trait_impls) {
  ...
  for (const use_decl of program.uses) {
    if (use_decl.is_pub) {
      __ring_match14: {
        const __ring_m14 = use_decl.imports;
        if (__ring_m14._tag === "NamedItems") {
          const names = __ring_m14.names;
  ...
  __match_fail(__ring_m);
            ...
            __ring_match15: {
              const __ring_m15 = env$TypeEnv_lookup(env, local_name);
            ...
            __ring_match16: {
              const __ring_m16 = _Map_get(env.structs, local_name);
            ...
            __ring_match17: {
              const __ring_m17 = _Map_get(env.enums, local_name);
            ...
            __ring_match18: {
              const __ring_m18 = _Map_get(env.effects, local_name);
              if (__ring_m18._tag === "some") {
                const effdef = __ring_m18._0;
            ...
            __ring_match19: {
              const __ring_m19 = _Map_get(env.traits, local_name);
              if (__ring_m19._tag === "some") {
                const tdef = __ring_m19._0;
