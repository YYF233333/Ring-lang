...
function __ring_tuple_eq(a, b) { if (a.length !== b.length) return false; for (var i = 0; i < a.length; i++) { if (Array.isArray(a[i]) && Array.isArray(b[i])) { if (!__ring_tuple_eq(a[i], b[i])) return false; } else if (a[i] !== b[i]) return false; } return true; }
