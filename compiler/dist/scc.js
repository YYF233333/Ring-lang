  ...
  __match_fail(__ring_m);
      ...
      if (__ring_m6._tag === "Impl") {
        ...
        while (true) {
          ...
          const method = __ring_next_3._0;
          __ring_match7: {
            const __ring_m7 = method;
            if (__ring_m7._tag === "Fn") {
              const mname = __ring_m7.name;
              const full_name = (function() {
  const __ring_m = mod_prefix;
...
function collect_stmt_callees(stmt, registered_fns, callees) {
  __ring_match8: {
    ...
    if (__ring_m8._tag === "Let") {
      const init = __ring_m8.init;
      return collect_expr_callees(init, registered_fns, callees);
      break __ring_match8;
    }
    if (__ring_m8._tag === "Var") {
      const init = __ring_m8.init;
      return collect_expr_callees(init, registered_fns, callees);
      break __ring_match8;
    }
    if (__ring_m8._tag === "Assign") {
      const target = __ring_m8.target; const value = __ring_m8.value;
    ...
    if (__ring_m8._tag === "While") {
      const condition = __ring_m8.condition; const body = __ring_m8.body;
      collect_expr_callees(condition, registered_fns, callees);
      return collect_expr_callees(body, registered_fns, callees);
      break __ring_match8;
    }
    if (__ring_m8._tag === "ForIn") {
      const iterable = __ring_m8.iterable; const body = __ring_m8.body;
      collect_expr_callees(iterable, registered_fns, callees);
      return collect_expr_callees(body, registered_fns, callees);
      break __ring_match8;
    }
    if (__ring_m8._tag === "LetDestructure") {
      const init = __ring_m8.init;
      return collect_expr_callees(init, registered_fns, callees);
      break __ring_match8;
    }
    if (__ring_m8._tag === "IfLet") {
      const expr = __ring_m8.expr; const then_block = __ring_m8.then_block; const else_block = __ring_m8.else_block;
      collect_expr_callees(expr, registered_fns, callees);
      collect_expr_callees(then_block, registered_fns, callees);
      __ring_match10: {
        const __ring_m10 = else_block;
...
function collect_expr_callees(expr, registered_fns, callees) {
  __ring_match11: {
    ...
    if (__ring_m11._tag === "Call") {
      const callee = __ring_m11.callee; const args = __ring_m11.args;
      __ring_match12: {
    ...
    if (__ring_m11._tag === "MethodCall") {
      const receiver = __ring_m11.receiver; const args = __ring_m11.args;
      collect_expr_callees(receiver, registered_fns, callees);
      const __ring_iter_5 = __List_Iterable.iter(args);
      while (true) {
        const __ring_next_5 = __ListIterator_Iterator.next(__ring_iter_5);
        if (__ring_next_5._tag === "none") break;
        const arg = __ring_next_5._0;
        collect_expr_callees(arg, registered_fns, callees);
      }
      break __ring_match11;
    }
    ...
    if (__ring_m11._tag === "Lambda") {
      const body = __ring_m11.body;
      return collect_expr_callees(body, registered_fns, callees);
      break __ring_match11;
    }
    if (__ring_m11._tag === "BinOp") {
      const left = __ring_m11.left; const right = __ring_m11.right;
    ...
    if (__ring_m11._tag === "UnaryOp") {
      const operand = __ring_m11.operand;
    ...
    if (__ring_m11._tag === "StructLit") {
      const fields = __ring_m11.fields; const spread = __ring_m11.spread;
    ...
    if (__ring_m11._tag === "CatchExpr") {
      const inner = __ring_m11.expr; const arms = __ring_m11.arms;
    ...
    if (__ring_m11._tag === "HandleExpr") {
      ...
      while (true) {
        ...
        const handler = __ring_next_10._0;
    ...
    if (__ring_m11._tag === "StringInterp") {
      ...
      while (true) {
        ...
        __ring_match18: {
          const __ring_m18 = part;
          if (__ring_m18._tag === "ExprPart") {
            const e = __ring_m18._0;
            collect_expr_callees(e, registered_fns, callees);
            break __ring_match18;
          }
          if (__ring_m18._tag === "LitPart") {
            break __ring_match18;
          }
    ...
    if (__ring_m11._tag === "Range") {
      const start = __ring_m11.start; const end = __ring_m11.end;
...
function prefix_mod_decl(mod_name, decl) {
  __ring_match19: {
    ...
    if (__ring_m19._tag === "Fn") {
      const name = __ring_m19.name; const type_params = __ring_m19.type_params; const params = __ring_m19.params; const return_type = __ring_m19.return_type; const declared_effects = __ring_m19.declared_effects; const body = __ring_m19.body; const is_pub = __ring_m19.is_pub; const is_abstract = __ring_m19.is_abstract; const span = __ring_m19.span;
    ...
    if (__ring_m19._tag === "Impl") {
      const target_type = __ring_m19.target_type; const type_params = __ring_m19.type_params; const trait_name = __ring_m19.trait_name; const methods = __ring_m19.methods; const span = __ring_m19.span;
  ...
  __match_fail(__ring_m);
      ...
      __ring_match21: {
        ...
        if (__ring_m21._tag === "some") {
          ...
          const __ring_iter_15 = ___Set_Iterable.iter(edges);
          while (true) {
            const __ring_next_15 = __SetIterator_Iterator.next(__ring_iter_15);
        ...
        if (__ring_m21._tag === "none") {
          let edge_list = [];
          const __ring_iter_16 = ___Set_Iterable.iter(edges);
          while (true) {
            const __ring_next_16 = __SetIterator_Iterator.next(__ring_iter_16);
...
function build_call_graph(decls, registered_fns) {
  let graph = map_new();
  const __ring_iter_19 = ___Set_Iterable.iter(registered_fns);
  while (true) {
...
function tarjan_scc(graph) {
  ...
  const __ring_iter_22 = __List_Iterable.iter(_Map_entries(graph));
  while (true) {
