  ...
  __ring_match7: {
    ...
    if (__ring_m7._tag === "EnumType") {
      ...
      for (const v of variants) {
        let sub_patterns_for_variant = empty_pattern_rows();
        for (const p of patterns) {
          __ring_match8: {
            const __ring_m8 = p;
        ...
        if ((_Set_contains(covered, v.name) === false)) {
          ...
          if ((List_len(v.fields) > 0)) {
            ...
            let normalized = empty_pattern_rows();
...
function finite_type_ctors(ty) {
  __ring_match15: {
    ...
    if (__ring_m15._tag === "BoolType") {
      let result = empty_ctors();
      List_push(result, new Ctor("true", 0, empty_types(), Option_none, false));
      List_push(result, new Ctor("false", 0, empty_types(), Option_none, false));
    ...
    if (__ring_m15._tag === "UnitType") {
      let result = empty_ctors();
      List_push(result, new Ctor("()", 0, empty_types(), Option_none, false));
      return Option_some(result);
      break __ring_match15;
    }
...
function empty_pattern_rows() {
  const x = [0];
  List_clear(x);
  return x.map((function(i) { return panic("unreachable"); }));
}

function empty_ctors() {
  const x = [0];
  List_clear(x);
  return x.map((function(i) { return panic("unreachable"); }));
}
  ...
  __ring_match18: {
    ...
    if (__ring_m18._tag === "some") {
      ...
      for (const ctor of ctor_list) {
        let specialized = empty_pattern_rows();
    ...
    if (__ring_m18._tag === "none") {
      let defaults = empty_pattern_rows();
      for (const row of rows) {
        const first = pat_at(row, 0);
        __ring_match21: {
          const __ring_m21 = first;
          if (__ring_m21._tag === "Wildcard") {
            let tail = empty_patterns();
