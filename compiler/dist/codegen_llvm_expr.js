...
function gen_extern_LLVMGetTargetFromTriple(ctx, arg_vals, info) {
  ...
  const is_err = LLVMBuildICmp(ctx.builder, 33, result, zero_i32, codegen_llvm_ctx$fresh_name(ctx, "gtt_err"));
  const current_fn = LLVMGetBasicBlockParent(LLVMGetInsertBlock(ctx.builder));
        ...
        if (__ring_m178._tag === "some") {
          ...
          __ring_match179: {
            ...
            if (__ring_m179._tag === "some") {
              ...
              const cmp = LLVMBuildICmp(ctx.builder, 32, tag_val, expected, codegen_llvm_ctx$fresh_name(ctx, "ntc"));
