...
function gen_llvm_expr(ctx, expr) {
  __ring_match8: {
    ...
    if (__ring_m8._tag === "StructLit") {
      const name = __ring_m8.name; const fields = __ring_m8.fields;
...
function gen_string_interp(ctx, parts) {
  ...
  while (true) {
    ...
    __ring_match61: {
      const __ring_m61 = part;
      if (__ring_m61._tag === "Literal") {
      ...
      if (__ring_m61._tag === "Expression") {
        const e = __ring_m61._0;
...
function gen_match_expr(ctx, scrutinee, arms, result_ty) {
  ...
  __ring_match62: {
    ...
    if (__ring_m62._tag === "some") {
      const ename = __ring_m62._0;
      __ring_match63: {
        const __ring_m63 = _Map_get(ctx.enum_types, ename);
        if (__ring_m63._tag === "some") {
          const enum_info = __ring_m63._0;
...
function gen_match_arm_enum(ctx, arm, scrut_val, enum_name, enum_info, switch_val, merge_bb, current_fn, phi_vals, phi_bbs) {
  __ring_match64: {
    const __ring_m64 = arm.pattern;
    if (__ring_m64._tag === "Constructor") {
      const name = __ring_m64.name; const fields = __ring_m64.fields;
      __ring_match65: {
        const __ring_m65 = _Map_get(enum_info.variants, name);
        if (__ring_m65._tag === "some") {
          const vi = __ring_m65._0;
          ...
          for (let i = 0; i < __ring_end23; i++) {
            __ring_match66: {
              const __ring_m66 = List_get(fields, i);
              if (__ring_m66._tag === "some") {
                const field_pat = __ring_m66._0;
                __ring_match67: {
                  const __ring_m67 = field_pat;
                  if (__ring_m67._tag === "Binding") {
                    const bname = __ring_m67.name;
    ...
    if (__ring_m64._tag === "NamedConstructor") {
      const name = __ring_m64.name; const named_fields = __ring_m64.fields;
      __ring_match68: {
        const __ring_m68 = _Map_get(enum_info.variants, name);
        if (__ring_m68._tag === "some") {
          const vi = __ring_m68._0;
          const arm_bb = LLVMAppendBasicBlockInContext(ctx.context, current_fn, `match.arm.${name}`);
          LLVMAddCase(switch_val, LLVMConstInt(ctx.i64_type, vi.tag, 0), arm_bb);
          LLVMPositionBuilderAtEnd(ctx.builder, arm_bb);
          ...
          for (let i = 0; i < __ring_end24; i++) {
            __ring_match69: {
              const __ring_m69 = List_get(named_fields, i);
              if (__ring_m69._tag === "some") {
                const nf = __ring_m69._0;
                let field_idx = i;
                ...
                __ring_match70: {
                  const __ring_m70 = nf.pattern;
                  if (__ring_m70._tag === "Binding") {
                    const bname = __ring_m70.name;
    ...
    if (__ring_m64._tag === "Wildcard") {
      break __ring_match64;
    }
    if (__ring_m64._tag === "Binding") {
      const bname = __ring_m64.name;
      break __ring_match64;
    }
    break __ring_match64;
  }
...
function bind_nested_pattern(ctx, val, pat) {
  __ring_match72: {
    ...
    if (__ring_m72._tag === "TuplePattern") {
      const elements = __ring_m72.elements;
      const get_fn = codegen_llvm_ctx$get_or_declare_runtime_fn(ctx, "ring_list_get", [ctx.ptr_type, ctx.i64_type], ctx.ptr_type);
      const get_ty = codegen_llvm_ctx$get_rt_fn_type(ctx, "ring_list_get");
      const __ring_end27 = List_len(elements);
      for (let i = 0; i < __ring_end27; i++) {
        __ring_match75: {
          const __ring_m75 = List_get(elements, i);
          if (__ring_m75._tag === "some") {
            const ep = __ring_m75._0;
...
function gen_match_if_else(ctx, scrut_val, scrut_ty, arms, merge_bb, default_bb, current_fn) {
  ...
  const __ring_end29 = total;
  for (let i = 0; i < __ring_end29; i++) {
    __ring_match78: {
      const __ring_m78 = List_get(arms, i);
      if (__ring_m78._tag === "some") {
        const arm = __ring_m78._0;
        const is_last = (i === (total - 1));
        __ring_match79: {
          const __ring_m79 = arm.pattern;
          ...
          if (__ring_m79._tag === "Literal") {
            const value = __ring_m79.value;
          ...
          if (__ring_m79._tag === "TuplePattern") {
            ...
            for (let j = 0; j < __ring_end30; j++) {
              __ring_match80: {
                const __ring_m80 = List_get(elements, j);
                if (__ring_m80._tag === "some") {
                  const elem_pat = __ring_m80._0;
                  __ring_match81: {
                    const __ring_m81 = elem_pat;
...
function find_enum_by_variant(ctx, variant_name, qualifier) {
  __ring_match82: {
    const __ring_m82 = qualifier;
    if (__ring_m82._tag === "some") {
      const q = __ring_m82._0;
      __ring_match83: {
        const __ring_m83 = _Map_get(ctx.enum_types, q);
  ...
  __ring_match84: {
    const __ring_m84 = _Map_get(ctx.enum_types, variant_name);
  ...
  const __ring_iter_31 = __List_Iterable.iter(_Map_entries(ctx.enum_types));
  while (true) {
    ...
    __ring_match85: {
      const __ring_m85 = _Map_get(einfo.variants, variant_name);
...
function gen_literal_pattern_cond(ctx, scrut_val, scrut_ty, value) {
  __ring_match86: {
    const __ring_m86 = value;
    if (__ring_m86._tag === "IntVal") {
      const n = __ring_m86._0;
      const unbox_fn = codegen_llvm_ctx$get_or_declare_runtime_fn(ctx, "ring_unbox_int", [ctx.ptr_type], ctx.i64_type);
      const unbox_ty = codegen_llvm_ctx$get_rt_fn_type(ctx, "ring_unbox_int");
    ...
    if (__ring_m86._tag === "BoolVal") {
      const b = __ring_m86._0;
      const unbox_fn = codegen_llvm_ctx$get_or_declare_runtime_fn(ctx, "ring_unbox_bool", [ctx.ptr_type], ctx.i64_type);
      const unbox_ty = codegen_llvm_ctx$get_rt_fn_type(ctx, "ring_unbox_bool");
...
function gen_named_variant_construct(ctx, enum_name, variant_name, fields) {
  __ring_match87: {
    const __ring_m87 = _Map_get(ctx.enum_types, enum_name);
    if (__ring_m87._tag === "some") {
      const enum_info = __ring_m87._0;
      __ring_match88: {
        const __ring_m88 = _Map_get(enum_info.variants, variant_name);
        if (__ring_m88._tag === "some") {
          ...
          const __ring_end33 = List_len(fields);
          for (let i = 0; i < __ring_end33; i++) {
            __ring_match89: {
              const __ring_m89 = List_get(fields, i);
              if (__ring_m89._tag === "some") {
                const f = __ring_m89._0;
                const val = gen_llvm_expr(ctx, f.value);
                let field_idx = i;
                const __ring_end34 = List_len(vi.field_names);
    ...
    if (__ring_m87._tag === "none") {
      ...
      __ring_match90: {
        const __ring_m90 = _Map_get(ctx.functions, ctor_name);
...
function gen_lambda(ctx, params, return_type, body, ty) {
  ...
  const __ring_end39 = List_len(captures);
  for (let i = 0; i < __ring_end39; i++) {
    __ring_match91: {
      const __ring_m91 = List_get(captures, i);
      if (__ring_m91._tag === "some") {
        const cap_name = __ring_m91._0;
  ...
  const __ring_end40 = List_len(params);
  for (let i = 0; i < __ring_end40; i++) {
    __ring_match92: {
      const __ring_m92 = List_get(params, i);
      if (__ring_m92._tag === "some") {
        const p = __ring_m92._0;
  ...
  for (let i = 0; i < __ring_end41; i++) {
    __ring_match93: {
      const __ring_m93 = List_get(captures, i);
      if (__ring_m93._tag === "some") {
        const cap_name = __ring_m93._0;
...
function collect_captures(ctx, expr, params, captures) {
  __ring_match94: {
    const __ring_m94 = expr;
    if (__ring_m94._tag === "Ident") {
      const name = __ring_m94.name; const resolved_name = __ring_m94.resolved_name;
      const lookup_name = (function() {
  const __ring_m = resolved_name;
  if (__ring_m._tag === "some") { const rn = __ring_m._0; return rn; }
  if (__ring_m._tag === "none") { return name; }
  ...
  __ring_match95: {
    const __ring_m95 = _Map_get(ctx.functions, mangled2);
    ...
    if (__ring_m95._tag === "none") {
      ...
      __ring_match96: {
        const __ring_m96 = _Map_get(ctx.functions, mangled3);
  ...
  __match_fail(__ring_m);
    ...
    if (__ring_m94._tag === "BinOp") {
      const left = __ring_m94.left; const right = __ring_m94.right;
      collect_captures(ctx, left, params, captures);
      return collect_captures(ctx, right, params, captures);
      break __ring_match94;
    }
    if (__ring_m94._tag === "UnaryOp") {
      const operand = __ring_m94.operand;
      return collect_captures(ctx, operand, params, captures);
      break __ring_match94;
    }
    if (__ring_m94._tag === "Call") {
      const callee = __ring_m94.callee; const args = __ring_m94.args;
      collect_captures(ctx, callee, params, captures);
      const __ring_iter_44 = __List_Iterable.iter(args);
      while (true) {
        const __ring_next_44 = __ListIterator_Iterator.next(__ring_iter_44);
        if (__ring_next_44._tag === "none") break;
        const a = __ring_next_44._0;
        collect_captures(ctx, a, params, captures);
      }
      break __ring_match94;
    }
    if (__ring_m94._tag === "FieldAccess") {
      const receiver = __ring_m94.receiver;
      return collect_captures(ctx, receiver, params, captures);
      break __ring_match94;
    }
    if (__ring_m94._tag === "Block") {
      const stmts = __ring_m94.stmts; const tail = __ring_m94.tail;
      ...
      __ring_match97: {
        const __ring_m97 = tail;
        if (__ring_m97._tag === "some") {
          const t = __ring_m97._0;
    ...
    if (__ring_m94._tag === "IfExpr") {
      const condition = __ring_m94.condition; const then_branch = __ring_m94.then_branch; const else_branch = __ring_m94.else_branch;
      collect_captures(ctx, condition, params, captures);
      collect_captures(ctx, then_branch, params, captures);
      __ring_match98: {
        const __ring_m98 = else_branch;
        if (__ring_m98._tag === "some") {
          const eb = __ring_m98._0;
    ...
    if (__ring_m94._tag === "MatchExpr") {
      const scrutinee = __ring_m94.scrutinee; const arms = __ring_m94.arms;
    ...
    if (__ring_m94._tag === "StringInterp") {
      const parts = __ring_m94.parts;
      const __ring_iter_47 = __List_Iterable.iter(parts);
      while (true) {
        const __ring_next_47 = __ListIterator_Iterator.next(__ring_iter_47);
        if (__ring_next_47._tag === "none") break;
        const part = __ring_next_47._0;
        __ring_match99: {
          const __ring_m99 = part;
          if (__ring_m99._tag === "Expression") {
            const e = __ring_m99._0;
    ...
    if (__ring_m94._tag === "StructLit") {
      const fields = __ring_m94.fields;
      const __ring_iter_48 = __List_Iterable.iter(fields);
      while (true) {
        const __ring_next_48 = __ListIterator_Iterator.next(__ring_iter_48);
        if (__ring_next_48._tag === "none") break;
        const f = __ring_next_48._0;
        collect_captures(ctx, f.value, params, captures);
      }
      break __ring_match94;
    }
    if (__ring_m94._tag === "ListLit") {
      const elements = __ring_m94.elements;
    ...
    if (__ring_m94._tag === "IndexExpr") {
      const receiver = __ring_m94.receiver; const index = __ring_m94.index;
      collect_captures(ctx, receiver, params, captures);
      return collect_captures(ctx, index, params, captures);
      break __ring_match94;
    }
    if (__ring_m94._tag === "Lambda") {
      const lb = __ring_m94.body;
...
function collect_captures_stmt(ctx, stmt, params, captures) {
  __ring_match100: {
    const __ring_m100 = stmt;
    if (__ring_m100._tag === "Let") {
      const init = __ring_m100.init;
      return collect_captures(ctx, init, params, captures);
      break __ring_match100;
    }
    if (__ring_m100._tag === "Var") {
      const init = __ring_m100.init;
      return collect_captures(ctx, init, params, captures);
      break __ring_match100;
    }
    if (__ring_m100._tag === "Assign") {
      const target = __ring_m100.target; const value = __ring_m100.value;
      collect_captures(ctx, target, params, captures);
      return collect_captures(ctx, value, params, captures);
      break __ring_match100;
    }
    if (__ring_m100._tag === "ExprStmt") {
      const expr = __ring_m100.expr;
      return collect_captures(ctx, expr, params, captures);
      break __ring_match100;
    }
    if (__ring_m100._tag === "Return") {
      const value = __ring_m100.value;
      __ring_match101: {
    ...
    if (__ring_m100._tag === "While") {
      const condition = __ring_m100.condition; const body = __ring_m100.body;
      collect_captures(ctx, condition, params, captures);
      return collect_captures(ctx, body, params, captures);
      break __ring_match100;
    }
    if (__ring_m100._tag === "ForIn") {
      const iterable = __ring_m100.iterable; const body = __ring_m100.body;
...
function gen_catch_arms(ctx, error_val, arms) {
  ...
  while (true) {
    ...
    __ring_match102: {
      ...
      if (__ring_m102._tag === "Constructor") {
        const name = __ring_m102.name; const fields = __ring_m102.fields;
...
function gen_handle_expr(ctx, body, handlers) {
  ...
  const __ring_iter_52 = __List_Iterable.iter(handlers);
  while (true) {
    const __ring_next_52 = __ListIterator_Iterator.next(__ring_iter_52);
    if (__ring_next_52._tag === "none") break;
    const h = __ring_next_52._0;
    __ring_match103: {
      const __ring_m103 = _Map_get(by_effect, h.effect_name);
      if (__ring_m103._tag === "some") {
        const existing = __ring_m103._0;
  ...
  const __ring_iter_53 = __List_Iterable.iter(_Map_entries(by_effect));
  while (true) {
    ...
    const __ring_iter_54 = __List_Iterable.iter(hs);
    while (true) {
...
function gen_effect_op(ctx, effect_name, op_name, args) {
  ...
  __match_fail(__ring_m);
    ...
    __ring_match104: {
      const __ring_m104 = _Map_get(ctx.named_values, ev_name);
      if (__ring_m104._tag === "some") {
        const ev_alloca = __ring_m104._0;
