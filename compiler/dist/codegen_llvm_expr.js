...
function find_fn_by_prefix_enumeration(ctx, name) {
  ...
  const __ring_iter_2 = __List_Iterable.iter(_Map_entries(ctx.imports_map));
  while (true) {
    ...
    __ring_match10: {
      const __ring_m10 = maybe_idx;
      if (__ring_m10._tag === "some") {
        const sep_idx = __ring_m10._0;
...
function find_fn_by_suffix(ctx, name) {
  const suffix = `$$_${name}`;
  const __ring_iter_3 = __List_Iterable.iter(_Map_entries(ctx.functions));
  while (true) {
...
function find_fn_precise(ctx, name) {
  ...
  __ring_match13: {
    const __ring_m13 = step2;
    if (__ring_m13._tag === "some") {
      const fn_val = __ring_m13._0;
...
function find_function_in_ctx(ctx, mangled, name) {
  __ring_match15: {
    const __ring_m15 = _Map_get(ctx.functions, mangled);
    ...
    if (__ring_m15._tag === "none") {
      ...
      __ring_match16: {
        const __ring_m16 = _Map_get(ctx.functions, bare);
...
function get_or_create_dict_global(ctx, name) {
  __ring_match17: {
    const __ring_m17 = _Map_get(ctx.dict_singletons, name);
    if (__ring_m17._tag === "some") {
      const g = __ring_m17._0;
      return g;
...
function emit_wrapped_method_thunk(ctx, mangled, method_fn, method_name, dispatch_arity, inner_count) {
  ...
  __ring_match18: {
    const __ring_m18 = _Map_get(ctx.functions, thunk_name);
    if (__ring_m18._tag === "some") {
      const existing = __ring_m18._0;
      return existing;
  ...
  const __ring_end4 = dispatch_arity;
  ...
  const __ring_end5 = inner_count;
  for (let j = 0; j < __ring_end5; j++) {
  ...
  let mp = [];
  const __ring_end6 = (dispatch_arity + inner_count);
...
function build_wrapped_dict_typed(ctx, dict_name, trait_name, inner_dicts, dict_tid) {
  ...
  const __ring_end10 = method_count;
  ...
  const __ring_end11 = method_count;
  for (let i = 0; i < __ring_end11; i++) {
    __ring_match19: {
      const __ring_m19 = List_get(method_order, i);
      if (__ring_m19._tag === "some") {
        const method_name = __ring_m19._0;
        const mangled = codegen_llvm_ctx$llvm_mangle_method(target_type, method_name);
        __ring_match20: {
          const __ring_m20 = _Map_get(ctx.functions, mangled);
          if (__ring_m20._tag === "some") {
            const method_fn = __ring_m20._0;
            ...
            let env_elem_types = [ctx.i64_type];
            const __ring_end12 = inner_count;
            for (let j = 0; j < __ring_end12; j++) {
              List_push(env_elem_types, ctx.ptr_type);
            }
            const env_ty = LLVMStructTypeInContext(ctx.context, env_elem_types, 0);
...
function get_or_create_static_dict_getter(ctx, name) {
  ...
  __ring_match21: {
    const __ring_m21 = _Map_get(ctx.functions, fname);
...
function resolve_static_dict_by_name(ctx, name) {
  ...
  __ring_match22: {
    const __ring_m22 = _Map_get(ctx.functions, init_fn_name);
    if (__ring_m22._tag === "some") {
      const init_fn = __ring_m22._0;
  ...
  __match_fail(__ring_m);
    ...
    if (__ring_m22._tag === "none") {
      __ring_match23: {
        const __ring_m23 = _Map_get(ctx.dict_globals, name);
        if (__ring_m23._tag === "some") {
          const init_fn = __ring_m23._0;
...
function resolve_dict_ref(ctx, dr) {
  __ring_match24: {
    const __ring_m24 = dr;
    if (__ring_m24._tag === "Simple") {
      const name = __ring_m24._0;
      __ring_match25: {
        const __ring_m25 = _Map_get(ctx.named_values, name);
        if (__ring_m25._tag === "some") {
          const alloca = __ring_m25._0;
    ...
    if (__ring_m24._tag === "Static") {
      const name = __ring_m24._0;
      return resolve_static_dict_by_name(ctx, name);
      break __ring_match24;
    }
    if (__ring_m24._tag === "Wrapped") {
      const dict = __ring_m24.dict; const trait_name = __ring_m24.trait_name; const inner_dicts = __ring_m24.inner_dicts;
...
function lookup_evidence(ctx, ev_param_name) {
  __ring_match26: {
    const __ring_m26 = _Map_get(ctx.named_values, ev_param_name);
    if (__ring_m26._tag === "some") {
      const alloca = __ring_m26._0;
...
function gen_dict_closure_wrapper(ctx, lookup_name, name, dict_names, ty) {
  ...
  let env_elem_types = [ctx.i64_type];
  const __ring_end17 = captured_count;
  ...
  let thunk_param_types = [ctx.ptr_type];
  const __ring_end18 = param_count;
...
function is_boxed_def(ctx, def_id) {
  __ring_match28: {
    const __ring_m28 = def_id;
    if (__ring_m28._tag === "some") {
      const did = __ring_m28._0;
...
function call_zero_arg_or_return(ctx, fn_val, mangled) {
  __ring_match29: {
    const __ring_m29 = _Map_get(ctx.fn_types, mangled);
    if (__ring_m29._tag === "some") {
      const fn_ty = __ring_m29._0;
...
function gen_ident(ctx, name, resolved_name, def_id, dict_closure_dicts, ty) {
  __ring_match30: {
    const __ring_m30 = dict_closure_dicts;
    if (__ring_m30._tag === "some") {
      const dicts = __ring_m30._0;
      if ((List_len(dicts) > 0)) {
        const lk = (function() {
  ...
  __ring_match31: {
    const __ring_m31 = _Map_get(ctx.named_values, lookup_name);
    if (__ring_m31._tag === "some") {
      const alloca = __ring_m31._0;
    ...
    if (__ring_m31._tag === "none") {
      __ring_match32: {
        const __ring_m32 = _Map_get(ctx.named_values, name);
        if (__ring_m32._tag === "some") {
          const alloca = __ring_m32._0;
        ...
        if (__ring_m32._tag === "none") {
          ...
          __ring_match33: {
            const __ring_m33 = _Map_get(ctx.functions, mangled_resolved);
            if (__ring_m33._tag === "some") {
              const fn_val = __ring_m33._0;
            ...
            if (__ring_m33._tag === "none") {
              ...
              __ring_match34: {
                const __ring_m34 = _Map_get(ctx.functions, mangled_bare);
                if (__ring_m34._tag === "some") {
                  const fn_val = __ring_m34._0;
                ...
                if (__ring_m34._tag === "none") {
                  ...
                  __ring_match35: {
                    const __ring_m35 = _Map_get(ctx.functions, mangled_name_resolved);
                    if (__ring_m35._tag === "some") {
                      const fn_val = __ring_m35._0;
                    ...
                    if (__ring_m35._tag === "none") {
                      ...
                      __ring_match36: {
                        const __ring_m36 = _Map_get(ctx.functions, mangled_lu);
                        if (__ring_m36._tag === "some") {
                          const fn_val = __ring_m36._0;
                        ...
                        if (__ring_m36._tag === "none") {
                          ...
                          __ring_match37: {
                            const __ring_m37 = found;
                            if (__ring_m37._tag === "some") {
                              const fi = __ring_m37._0;
                            ...
                            if (__ring_m37._tag === "none") {
                              ...
                              __ring_match38: {
                                const __ring_m38 = found2;
                                if (__ring_m38._tag === "some") {
                                  const fi2 = __ring_m38._0;
...
function resolve_dispatch_dict(ctx, dispatch, trait_name_hint) {
  __ring_match39: {
    ...
    if (__ring_m39._tag === "Dict") {
      const param = __ring_m39.param;
      __ring_match40: {
        const __ring_m40 = _Map_get(ctx.named_values, param);
    ...
    if (__ring_m39._tag === "Direct") {
      const dict = __ring_m39.dict; const extra_dicts = __ring_m39.extra_dicts;
      if ((List_len(extra_dicts) === 0)) {
        return resolve_dict_ref(ctx, hir$DictRef_Simple(dict));
      } else {
        __ring_match41: {
          const __ring_m41 = trait_name_hint;
          if (__ring_m41._tag === "some") {
            const tn = __ring_m41._0;
...
function gen_closure_call(ctx, closure_ptr, arg_vals) {
  ...
  const __ring_iter_23 = __List_Iterable.iter(arg_vals);
  while (true) {
...
function trait_name_from_dict_param(dict_param) {
  ...
  __ring_match50: {
    const __ring_m50 = Str_index_of(rest, "_");
    if (__ring_m50._tag === "some") {
      const us = __ring_m50._0;
...
function get_trait_method_index(ctx, dict_param, method) {
  __ring_match51: {
    const __ring_m51 = trait_name_from_dict_param(dict_param);
    if (__ring_m51._tag === "some") {
      const trait_name = __ring_m51._0;
      __ring_match52: {
        const __ring_m52 = _Map_get(ctx.trait_method_order, trait_name);
        if (__ring_m52._tag === "some") {
          const order = __ring_m52._0;
          let idx = 0;
          const __ring_iter_25 = __List_Iterable.iter(order);
          while (true) {
...
function lookup_call_mut_flags(ctx, callee) {
  __ring_match54: {
    const __ring_m54 = callee;
    if (__ring_m54._tag === "Ident") {
      const name = __ring_m54.name; const resolved_name = __ring_m54.resolved_name;
  ...
  __match_fail(__ring_m);
    ...
    if (__ring_m54._tag === "FieldAccess") {
      const receiver = __ring_m54.receiver; const field = __ring_m54.field;
  ...
  __match_fail(__ring_m);
      ...
      if ((type_name === "")) {
        ...
        __ring_match55: {
          const __ring_m55 = _Map_get(ctx.fn_mut_params, ufcs_name);
          if (__ring_m55._tag === "some") {
            const flags = __ring_m55._0;
            let shifted = [];
            let i = 1;
            while ((i < List_len(flags))) {
              __ring_match56: {
                const __ring_m56 = List_get(flags, i);
...
function gen_runtime_call(ctx, name, args) {
  __ring_match57: {
    const __ring_m57 = _Map_get(ctx.rt_fns, name);
    if (__ring_m57._tag === "some") {
      const fn_val = __ring_m57._0;
...
function gen_direct_call(ctx, name, arg_vals, dict_vals) {
  ...
  __ring_match58: {
    const __ring_m58 = rt_name;
    if (__ring_m58._tag === "some") {
      const rtn = __ring_m58._0;
  ...
  __ring_match59: {
    const __ring_m59 = found_fn;
    if (__ring_m59._tag === "some") {
      const fn_info = __ring_m59._0;
      ...
      __ring_match60: {
        const __ring_m60 = _Map_get(ctx.fn_evidence_params, fn_info.fn_mangled);
        if (__ring_m60._tag === "some") {
          const ev_params = __ring_m60._0;
  ...
  __match_fail(__ring_m);
    ...
    if (__ring_m59._tag === "none") {
      __ring_match61: {
        const __ring_m61 = _Map_get(ctx.named_values, name);
        if (__ring_m61._tag === "some") {
          const alloca = __ring_m61._0;
        ...
        if (__ring_m61._tag === "none") {
          ...
          __ring_match62: {
            const __ring_m62 = _Map_get(ctx.rt_fns, rt_fallback);
...
function ensure_runtime_method(ctx, name, arg_count) {
  __ring_match63: {
    const __ring_m63 = _Map_get(ctx.rt_fns, name);
    if (__ring_m63._tag === "some") {
      const f = __ring_m63._0;
      return f;
...
function gen_method_call(ctx, recv, recv_type, method, args, dict_vals) {
  ...
  __ring_match64: {
    const __ring_m64 = rt_method;
    if (__ring_m64._tag === "some") {
      const base_rt_name = __ring_m64._0;
  ...
  return base_rt_name;
      ...
      let ai_idx = 0;
      const __ring_iter_30 = __List_Iterable.iter(args);
      while (true) {
    ...
    if (__ring_m64._tag === "none") {
      ...
      __ring_match65: {
        const __ring_m65 = _Map_get(ctx.functions, mangled);
        if (__ring_m65._tag === "some") {
          const fn_val = __ring_m65._0;
          const __ring_iter_32 = __List_Iterable.iter(dict_vals);
          while (true) {
            const __ring_next_32 = __ListIterator_Iterator.next(__ring_iter_32);
            if (__ring_next_32._tag === "none") break;
            const dv = __ring_next_32._0;
            List_push(call_args, dv);
          }
          __ring_match66: {
            const __ring_m66 = _Map_get(ctx.fn_evidence_params, mangled);
            if (__ring_m66._tag === "some") {
              const ev_params = __ring_m66._0;
...
function consider_capture_name(ctx, name, resolved_name, params, captures) {
  ...
  __ring_match67: {
    const __ring_m67 = _Map_get(ctx.functions, mangled2);
    ...
    if (__ring_m67._tag === "none") {
      ...
      __ring_match68: {
        const __ring_m68 = _Map_get(ctx.functions, mangled3);
...
function collect_dictref_names(ctx, dr, params, captures) {
  __ring_match69: {
    const __ring_m69 = dr;
    if (__ring_m69._tag === "Simple") {
      const name = __ring_m69._0;
      return consider_capture_name(ctx, name, Option_none, params, captures);
      break __ring_match69;
    }
    if (__ring_m69._tag === "Static") {
      break __ring_match69;
    }
    if (__ring_m69._tag === "Wrapped") {
      const dict = __ring_m69.dict; const inner_dicts = __ring_m69.inner_dicts;
      consider_capture_name(ctx, dict, Option_none, params, captures);
      const __ring_iter_36 = __List_Iterable.iter(inner_dicts);
      while (true) {
        const __ring_next_36 = __ListIterator_Iterator.next(__ring_iter_36);
        if (__ring_next_36._tag === "none") break;
        const inner = __ring_next_36._0;
...
function collect_dispatch_dict(ctx, dispatch, params, captures) {
  __ring_match70: {
    const __ring_m70 = dispatch;
    if (__ring_m70._tag === "some") {
      const d = __ring_m70._0;
      __ring_match71: {
        const __ring_m71 = d;
        if (__ring_m71._tag === "Dict") {
          const param = __ring_m71.param;
          return consider_capture_name(ctx, param, Option_none, params, captures);
          break __ring_match71;
        }
        if (__ring_m71._tag === "Direct") {
          const dict = __ring_m71.dict; const extra_dicts = __ring_m71.extra_dicts;
          consider_capture_name(ctx, dict, Option_none, params, captures);
          const __ring_iter_37 = __List_Iterable.iter(extra_dicts);
          while (true) {
            const __ring_next_37 = __ListIterator_Iterator.next(__ring_iter_37);
            if (__ring_next_37._tag === "none") break;
            const ed = __ring_next_37._0;
            collect_dictref_names(ctx, ed, params, captures);
          }
          break __ring_match71;
        }
        if (__ring_m71._tag === "Builtin") {
          break __ring_match71;
        }
...
function collect_captures_stmt(ctx, stmt, params, captures) {
  __ring_match72: {
    const __ring_m72 = stmt;
    if (__ring_m72._tag === "Let") {
      const init = __ring_m72.init;
      return collect_captures(ctx, init, params, captures);
      break __ring_match72;
    }
    if (__ring_m72._tag === "Var") {
      const init = __ring_m72.init;
      return collect_captures(ctx, init, params, captures);
      break __ring_match72;
    }
    if (__ring_m72._tag === "Assign") {
      const target = __ring_m72.target; const value = __ring_m72.value;
      collect_captures(ctx, target, params, captures);
      return collect_captures(ctx, value, params, captures);
      break __ring_match72;
    }
    if (__ring_m72._tag === "ExprStmt") {
      const expr = __ring_m72.expr;
      return collect_captures(ctx, expr, params, captures);
      break __ring_match72;
    }
    if (__ring_m72._tag === "Return") {
      const value = __ring_m72.value;
      __ring_match73: {
        const __ring_m73 = value;
        if (__ring_m73._tag === "some") {
          const v = __ring_m73._0;
    ...
    if (__ring_m72._tag === "While") {
      const condition = __ring_m72.condition; const body = __ring_m72.body;
      collect_captures(ctx, condition, params, captures);
      return collect_captures(ctx, body, params, captures);
      break __ring_match72;
    }
    if (__ring_m72._tag === "ForIn") {
      const iterable = __ring_m72.iterable; const body = __ring_m72.body;
      collect_captures(ctx, iterable, params, captures);
      return collect_captures(ctx, body, params, captures);
      break __ring_match72;
    }
    if (__ring_m72._tag === "LetDestructure") {
      const init = __ring_m72.init;
      return collect_captures(ctx, init, params, captures);
      break __ring_match72;
    }
    if (__ring_m72._tag === "IfLet") {
      const expr = __ring_m72.expr; const then_block = __ring_m72.then_block; const else_block = __ring_m72.else_block;
      collect_captures(ctx, expr, params, captures);
      collect_captures(ctx, then_block, params, captures);
      __ring_match74: {
        const __ring_m74 = else_block;
        if (__ring_m74._tag === "some") {
          const eb = __ring_m74._0;
    ...
    if (__ring_m72._tag === "Drop") {
      const name = __ring_m72.name;
      return consider_capture_name(ctx, name, Option_none, params, captures);
      break __ring_match72;
    }
    if (__ring_m72._tag === "Dup") {
      const name = __ring_m72.name;
      return consider_capture_name(ctx, name, Option_none, params, captures);
      break __ring_match72;
    }
...
function collect_captures(ctx, expr, params, captures) {
  __ring_match75: {
    ...
    if (__ring_m75._tag === "Ident") {
      const name = __ring_m75.name; const resolved_name = __ring_m75.resolved_name;
      return consider_capture_name(ctx, name, resolved_name, params, captures);
      break __ring_match75;
    }
    if (__ring_m75._tag === "BinOp") {
      const left = __ring_m75.left; const right = __ring_m75.right; const eq_dispatch = __ring_m75.eq_dispatch; const ord_dispatch = __ring_m75.ord_dispatch;
    ...
    if (__ring_m75._tag === "UnaryOp") {
      const operand = __ring_m75.operand;
      return collect_captures(ctx, operand, params, captures);
      break __ring_match75;
    }
    if (__ring_m75._tag === "Call") {
      const callee = __ring_m75.callee; const args = __ring_m75.args; const resolved_dicts = __ring_m75.resolved_dicts; const dict_dispatch = __ring_m75.dict_dispatch;
    ...
    if (__ring_m75._tag === "DictConstruct") {
      const inner = __ring_m75.inner;
      const __ring_iter_40 = __List_Iterable.iter(inner);
      while (true) {
        const __ring_next_40 = __ListIterator_Iterator.next(__ring_iter_40);
        if (__ring_next_40._tag === "none") break;
        const d = __ring_next_40._0;
        collect_dictref_names(ctx, d, params, captures);
      }
      break __ring_match75;
    }
    if (__ring_m75._tag === "FieldAccess") {
      const receiver = __ring_m75.receiver;
    ...
    if (__ring_m75._tag === "IfExpr") {
      const condition = __ring_m75.condition; const then_branch = __ring_m75.then_branch; const else_branch = __ring_m75.else_branch;
      collect_captures(ctx, condition, params, captures);
      collect_captures(ctx, then_branch, params, captures);
      __ring_match78: {
        const __ring_m78 = else_branch;
        if (__ring_m78._tag === "some") {
          const eb = __ring_m78._0;
          return collect_captures(ctx, eb, params, captures);
    ...
    if (__ring_m75._tag === "MatchExpr") {
      const scrutinee = __ring_m75.scrutinee; const arms = __ring_m75.arms;
    ...
    if (__ring_m75._tag === "StringInterp") {
      const parts = __ring_m75.parts;
      ...
      while (true) {
        ...
        __ring_match79: {
          const __ring_m79 = part;
          if (__ring_m79._tag === "Expression") {
            const e = __ring_m79._0;
    ...
    if (__ring_m75._tag === "StructLit") {
      const fields = __ring_m75.fields; const spread = __ring_m75.spread;
      const __ring_iter_44 = __List_Iterable.iter(fields);
      while (true) {
        const __ring_next_44 = __ListIterator_Iterator.next(__ring_iter_44);
        if (__ring_next_44._tag === "none") break;
        const f = __ring_next_44._0;
        collect_captures(ctx, f.value, params, captures);
      }
      __ring_match80: {
        const __ring_m80 = spread;
        if (__ring_m80._tag === "some") {
          const sp = __ring_m80._0;
    ...
    if (__ring_m75._tag === "ListLit") {
      const elements = __ring_m75.elements;
    ...
    if (__ring_m75._tag === "IndexExpr") {
      const receiver = __ring_m75.receiver; const index = __ring_m75.index;
      collect_captures(ctx, receiver, params, captures);
      return collect_captures(ctx, index, params, captures);
      break __ring_match75;
    }
    if (__ring_m75._tag === "Lambda") {
      const lb = __ring_m75.body;
      return collect_captures(ctx, lb, params, captures);
      break __ring_match75;
    }
    if (__ring_m75._tag === "NamedVariantConstruct") {
      const nvc_fields = __ring_m75.fields; const nvc_spread = __ring_m75.spread;
      const __ring_iter_47 = __List_Iterable.iter(nvc_fields);
      while (true) {
        const __ring_next_47 = __ListIterator_Iterator.next(__ring_iter_47);
        if (__ring_next_47._tag === "none") break;
        const f = __ring_next_47._0;
        collect_captures(ctx, f.value, params, captures);
      }
      __ring_match81: {
        const __ring_m81 = nvc_spread;
        if (__ring_m81._tag === "some") {
          const sp = __ring_m81._0;
          return collect_captures(ctx, sp, params, captures);
    ...
    if (__ring_m75._tag === "TryCatch") {
      const tc_body = __ring_m75.body; const tc_arms = __ring_m75.arms;
      collect_captures(ctx, tc_body, params, captures);
      const __ring_iter_48 = __List_Iterable.iter(tc_arms);
      while (true) {
    ...
    if (__ring_m75._tag === "HandleExpr") {
      const he_body = __ring_m75.body;
      return collect_captures(ctx, he_body, params, captures);
      break __ring_match75;
    }
    if (__ring_m75._tag === "EffectOp") {
      const eo_eff = __ring_m75.effect_name; const eo_args = __ring_m75.args;
      const __ring_iter_49 = __List_Iterable.iter(eo_args);
      while (true) {
    ...
    if (__ring_m75._tag === "RangeExpr") {
      const rs = __ring_m75.start; const re = __ring_m75.end;
      collect_captures(ctx, rs, params, captures);
      return collect_captures(ctx, re, params, captures);
      break __ring_match75;
    }
    if (__ring_m75._tag === "Clone") {
      const inner = __ring_m75.inner;
...
function gen_literal_pattern_cond(ctx, scrut_val, scrut_ty, value) {
  __ring_match82: {
    const __ring_m82 = value;
    if (__ring_m82._tag === "IntVal") {
      const n = __ring_m82._0;
    ...
    if (__ring_m82._tag === "BoolVal") {
      const b = __ring_m82._0;
      const raw = unbox_int(ctx, scrut_val);
      const lit = (b ? LLVMConstInt(ctx.i64_type, 1, 0) : LLVMConstInt(ctx.i64_type, 0, 0));
      return LLVMBuildICmp(ctx.builder, 32, raw, lit, codegen_llvm_ctx$fresh_name(ctx, "eq"));
      break __ring_match82;
    }
    if (__ring_m82._tag === "StrVal") {
      const s = __ring_m82._0;
    ...
    if (__ring_m82._tag === "FloatVal") {
      const f = __ring_m82._0;
      const unbox_fn = codegen_llvm_ctx$get_or_declare_runtime_fn(ctx, "ring_unbox_float", [ctx.ptr_type], ctx.double_type);
      const unbox_ty = codegen_llvm_ctx$get_rt_fn_type(ctx, "ring_unbox_float");
...
function find_enum_by_variant(ctx, variant_name, qualifier) {
  __ring_match83: {
    const __ring_m83 = qualifier;
    if (__ring_m83._tag === "some") {
      const q = __ring_m83._0;
      __ring_match84: {
        const __ring_m84 = _Map_get(ctx.enum_types, q);
        if (__ring_m84._tag === "some") {
          const ei = __ring_m84._0;
  ...
  __ring_match85: {
    const __ring_m85 = _Map_get(ctx.enum_types, variant_name);
    if (__ring_m85._tag === "some") {
      const ei = __ring_m85._0;
      return Option_some(ei);
  ...
  const __ring_iter_50 = __List_Iterable.iter(_Map_entries(ctx.enum_types));
  while (true) {
    ...
    const ename = __ring_dt2[0];
    const einfo = __ring_dt2[1];
    __ring_match86: {
      const __ring_m86 = _Map_get(einfo.variants, variant_name);
...
function gen_ctor_tag_test(ctx, scrut_val, cname, qualifier, match_bb, miss_bb, current_fn) {
  ...
  __ring_match87: {
    ...
    if (__ring_m87._tag === "some") {
      const enum_info = __ring_m87._0;
      __ring_match88: {
        const __ring_m88 = _Map_get(enum_info.variants, cname);
        if (__ring_m88._tag === "some") {
          const vi = __ring_m88._0;
...
function bind_nested_pattern(ctx, val, pat) {
  __ring_match89: {
    const __ring_m89 = pat;
    if (__ring_m89._tag === "Binding") {
      const name = __ring_m89.name;
    ...
    if (__ring_m89._tag === "Wildcard") {
      break __ring_match89;
    }
    if (__ring_m89._tag === "Constructor") {
      const name = __ring_m89.name; const qualifier = __ring_m89.qualifier; const fields = __ring_m89.fields;
      const enum_info = find_enum_by_variant(ctx, name, qualifier);
      __ring_match90: {
        const __ring_m90 = enum_info;
        if (__ring_m90._tag === "some") {
          const ei = __ring_m90._0;
          const __ring_end51 = List_len(fields);
          for (let i = 0; i < __ring_end51; i++) {
            __ring_match91: {
              const __ring_m91 = List_get(fields, i);
              if (__ring_m91._tag === "some") {
                const fp = __ring_m91._0;
    ...
    if (__ring_m89._tag === "TuplePattern") {
      const elements = __ring_m89.elements;
      ...
      const __ring_end52 = List_len(elements);
      for (let i = 0; i < __ring_end52; i++) {
        __ring_match92: {
          const __ring_m92 = List_get(elements, i);
          if (__ring_m92._tag === "some") {
            const ep = __ring_m92._0;
    ...
    if (__ring_m89._tag === "NamedConstructor") {
      const name = __ring_m89.name; const qualifier = __ring_m89.qualifier; const fields = __ring_m89.fields;
      const enum_info = find_enum_by_variant(ctx, name, qualifier);
      __ring_match93: {
        const __ring_m93 = enum_info;
        if (__ring_m93._tag === "some") {
          const ei = __ring_m93._0;
  ...
  __match_fail(__ring_m);
          ...
          for (let i = 0; i < __ring_end53; i++) {
            __ring_match94: {
              const __ring_m94 = List_get(fields, i);
              if (__ring_m94._tag === "some") {
                const nf = __ring_m94._0;
                let field_idx = i;
                const __ring_end54 = List_len(fnames);
                for (let fi = 0; fi < __ring_end54; fi++) {
...
function bind_constructor_fields(ctx, scrut_val, cname, qualifier, fields) {
  ...
  __ring_match95: {
    ...
    if (__ring_m95._tag === "some") {
      const enum_info = __ring_m95._0;
      const __ring_end55 = List_len(fields);
      for (let i = 0; i < __ring_end55; i++) {
        __ring_match96: {
          const __ring_m96 = List_get(fields, i);
          if (__ring_m96._tag === "some") {
            const field_pat = __ring_m96._0;
            __ring_match97: {
              const __ring_m97 = field_pat;
              if (__ring_m97._tag === "Binding") {
                const bname = __ring_m97.name;
...
function bind_named_constructor_fields(ctx, scrut_val, cname, qualifier, named_fields) {
  ...
  __ring_match98: {
    ...
    if (__ring_m98._tag === "some") {
      const enum_info = __ring_m98._0;
      __ring_match99: {
        const __ring_m99 = _Map_get(enum_info.variants, cname);
        if (__ring_m99._tag === "some") {
          const vi = __ring_m99._0;
          const __ring_end56 = List_len(named_fields);
          for (let i = 0; i < __ring_end56; i++) {
            __ring_match100: {
              ...
              if (__ring_m100._tag === "some") {
                ...
                let field_idx = i;
                const __ring_end57 = List_len(vi.field_names);
                for (let fi = 0; fi < __ring_end57; fi++) {
...
function tuple_element_type(ty, idx) {
  __ring_match102: {
    ...
    if (__ring_m102._tag === "TupleType") {
      const elements = __ring_m102.elements;
      __ring_match103: {
        const __ring_m103 = List_get(elements, idx);
...
function gen_handle_expr(ctx, body, handlers) {
  ...
  const __ring_iter_65 = __List_Iterable.iter(handlers);
  while (true) {
    const __ring_next_65 = __ListIterator_Iterator.next(__ring_iter_65);
    if (__ring_next_65._tag === "none") break;
    const h = __ring_next_65._0;
    __ring_match104: {
      const __ring_m104 = _Map_get(by_effect, h.effect_name);
      if (__ring_m104._tag === "some") {
        const existing = __ring_m104._0;
  ...
  const __ring_iter_66 = __List_Iterable.iter(_Map_entries(by_effect));
  while (true) {
...
function gen_catch_arms(ctx, error_val, arms) {
  ...
  const __ring_iter_68 = __List_Iterable.iter(arms);
  while (true) {
    const __ring_next_68 = __ListIterator_Iterator.next(__ring_iter_68);
    if (__ring_next_68._tag === "none") break;
    const arm = __ring_next_68._0;
    __ring_match105: {
      const __ring_m105 = arm.pattern;
      ...
      if (__ring_m105._tag === "Constructor") {
        const name = __ring_m105.name; const fields = __ring_m105.fields;
...
function gen_catch_closure(ctx, arms) {
  ...
  const __ring_end71 = List_len(captures);
  for (let i = 0; i < __ring_end71; i++) {
    __ring_match106: {
      const __ring_m106 = List_get(captures, i);
      if (__ring_m106._tag === "some") {
        const cap_name = __ring_m106._0;
  ...
  const __ring_end72 = List_len(captures);
  for (let i = 0; i < __ring_end72; i++) {
    __ring_match107: {
      const __ring_m107 = List_get(captures, i);
      if (__ring_m107._tag === "some") {
        const cap_name = __ring_m107._0;
...
function gen_named_variant_construct(ctx, enum_name, variant_name, fields) {
  __ring_match108: {
    const __ring_m108 = _Map_get(ctx.enum_types, enum_name);
    if (__ring_m108._tag === "some") {
      const enum_info = __ring_m108._0;
      __ring_match109: {
        const __ring_m109 = _Map_get(enum_info.variants, variant_name);
        if (__ring_m109._tag === "some") {
          const vi = __ring_m109._0;
    ...
    if (__ring_m108._tag === "none") {
      ...
      __ring_match111: {
        const __ring_m111 = _Map_get(ctx.functions, ctor_name);
        if (__ring_m111._tag === "some") {
          const fn_val = __ring_m111._0;
          let args = [];
...
function gen_match_arm_enum(ctx, arm, scrut_val, enum_name, enum_info, switch_val, merge_bb, current_fn, phi_vals, phi_bbs) {
  __ring_match112: {
    const __ring_m112 = arm.pattern;
    if (__ring_m112._tag === "Constructor") {
      const name = __ring_m112.name; const fields = __ring_m112.fields;
      __ring_match113: {
        const __ring_m113 = _Map_get(enum_info.variants, name);
        if (__ring_m113._tag === "some") {
          const vi = __ring_m113._0;
          ...
          for (let i = 0; i < __ring_end76; i++) {
            __ring_match114: {
              const __ring_m114 = List_get(fields, i);
              if (__ring_m114._tag === "some") {
                const field_pat = __ring_m114._0;
                __ring_match115: {
                  const __ring_m115 = field_pat;
                  if (__ring_m115._tag === "Binding") {
                    const bname = __ring_m115.name;
                    const field_ptr = LLVMBuildStructGEP2(ctx.builder, enum_info.llvm_type, scrut_val, (i + 1), codegen_llvm_ctx$fresh_name(ctx, "ef"));
                    const field_val = LLVMBuildLoad2(ctx.builder, ctx.ptr_type, field_ptr, codegen_llvm_ctx$fresh_name(ctx, bname));
                    const alloca = codegen_llvm_ctx$build_entry_alloca(ctx, ctx.ptr_type, bname);
                    discard(LLVMBuildStore(ctx.builder, field_val, alloca));
    ...
    if (__ring_m112._tag === "NamedConstructor") {
      const name = __ring_m112.name; const named_fields = __ring_m112.fields;
      __ring_match116: {
        const __ring_m116 = _Map_get(enum_info.variants, name);
        if (__ring_m116._tag === "some") {
          const vi = __ring_m116._0;
          const arm_bb = LLVMAppendBasicBlockInContext(ctx.context, current_fn, `match.arm.${name}`);
          LLVMAddCase(switch_val, LLVMConstInt(ctx.i64_type, vi.tag, 0), arm_bb);
          LLVMPositionBuilderAtEnd(ctx.builder, arm_bb);
          ...
          for (let i = 0; i < __ring_end77; i++) {
            __ring_match117: {
              const __ring_m117 = List_get(named_fields, i);
              if (__ring_m117._tag === "some") {
                const nf = __ring_m117._0;
                let field_idx = i;
                ...
                __ring_match118: {
                  const __ring_m118 = nf.pattern;
                  if (__ring_m118._tag === "Binding") {
                    const bname = __ring_m118.name;
                    const field_ptr = LLVMBuildStructGEP2(ctx.builder, enum_info.llvm_type, scrut_val, (field_idx + 1), codegen_llvm_ctx$fresh_name(ctx, "ef"));
                    const field_val = LLVMBuildLoad2(ctx.builder, ctx.ptr_type, field_ptr, codegen_llvm_ctx$fresh_name(ctx, bname));
                    const alloca = codegen_llvm_ctx$build_entry_alloca(ctx, ctx.ptr_type, bname);
                    discard(LLVMBuildStore(ctx.builder, field_val, alloca));
    ...
    if (__ring_m112._tag === "Wildcard") {
      break __ring_match112;
    }
    if (__ring_m112._tag === "Binding") {
      const bname = __ring_m112.name;
      break __ring_match112;
    }
    if (__ring_m112._tag === "OrPattern") {
      const patterns = __ring_m112.patterns;
      const arm_bb = LLVMAppendBasicBlockInContext(ctx.context, current_fn, "match.arm.or");
      const __ring_iter_79 = __List_Iterable.iter(patterns);
      while (true) {
        const __ring_next_79 = __ListIterator_Iterator.next(__ring_iter_79);
        if (__ring_next_79._tag === "none") break;
        const alt = __ring_next_79._0;
  ...
  return Option_none;
        ...
        __ring_match119: {
          const __ring_m119 = alt_name;
          if (__ring_m119._tag === "some") {
            const an = __ring_m119._0;
            __ring_match120: {
              const __ring_m120 = _Map_get(enum_info.variants, an);
              if (__ring_m120._tag === "some") {
                const vi = __ring_m120._0;
                LLVMAddCase(switch_val, LLVMConstInt(ctx.i64_type, vi.tag, 0), arm_bb);
...
function gen_match_arm_wildcard(ctx, arm, scrut_val, default_bb, merge_bb, phi_vals, phi_bbs) {
  ...
  __ring_match121: {
    const __ring_m121 = arm.pattern;
    if (__ring_m121._tag === "Binding") {
      const bname = __ring_m121.name;
      const alloca = codegen_llvm_ctx$build_entry_alloca(ctx, ctx.ptr_type, bname);
...
function gen_match_if_else(ctx, scrut_val, scrut_ty, arms, merge_bb, default_bb, current_fn) {
  ...
  const __ring_end80 = total;
  for (let i = 0; i < __ring_end80; i++) {
    __ring_match123: {
      const __ring_m123 = List_get(arms, i);
      if (__ring_m123._tag === "some") {
        const arm = __ring_m123._0;
  ...
  __match_fail(__ring_m);
        ...
        __ring_match124: {
          const __ring_m124 = arm.pattern;
          if (__ring_m124._tag === "Wildcard") {
            open_block = false;
          ...
          if (__ring_m124._tag === "Binding") {
            const bname = __ring_m124.name;
            open_block = false;
            const next_bb = ((has_guard ? (is_last === false) : false) ? LLVMAppendBasicBlockInContext(ctx.context, current_fn, "match.next") : default_bb);
          ...
          if (__ring_m124._tag === "Literal") {
            const value = __ring_m124.value;
          ...
          if (__ring_m124._tag === "Constructor") {
            const cname = __ring_m124.name; const qualifier = __ring_m124.qualifier; const fields = __ring_m124.fields;
            const next_bb = (is_last ? default_bb : LLVMAppendBasicBlockInContext(ctx.context, current_fn, "match.next"));
          ...
          if (__ring_m124._tag === "NamedConstructor") {
            const cname = __ring_m124.name; const qualifier = __ring_m124.qualifier; const nfields = __ring_m124.fields;
            const next_bb = (is_last ? default_bb : LLVMAppendBasicBlockInContext(ctx.context, current_fn, "match.next"));
            const arm_bb = LLVMAppendBasicBlockInContext(ctx.context, current_fn, `match.ctor.${cname}`);
            gen_ctor_tag_test(ctx, scrut_val, cname, qualifier, arm_bb, next_bb, current_fn);
            LLVMPositionBuilderAtEnd(ctx.builder, arm_bb);
          ...
          if (__ring_m124._tag === "OrPattern") {
            const patterns = __ring_m124.patterns;
            const next_bb = (is_last ? default_bb : LLVMAppendBasicBlockInContext(ctx.context, current_fn, "match.next"));
            const arm_bb = LLVMAppendBasicBlockInContext(ctx.context, current_fn, "match.or");
            const palts = patterns;
            const nalts = List_len(palts);
            const __ring_end81 = nalts;
            for (let k = 0; k < __ring_end81; k++) {
              __ring_match125: {
                const __ring_m125 = List_get(palts, k);
                if (__ring_m125._tag === "some") {
                  const alt = __ring_m125._0;
  ...
  return Option_none;
                  ...
                  __ring_match126: {
                    const __ring_m126 = alt_ref;
                    if (__ring_m126._tag === "some") {
                      const ar = __ring_m126._0;
                      const __ring_dt4 = ar;
          ...
          if (__ring_m124._tag === "TuplePattern") {
            const elements = __ring_m124.elements;
            const get_fn = codegen_llvm_ctx$get_or_declare_runtime_fn(ctx, "ring_list_get", [ctx.ptr_type, ctx.i64_type], ctx.ptr_type);
            const get_ty = codegen_llvm_ctx$get_rt_fn_type(ctx, "ring_list_get");
            const next_bb = (is_last ? default_bb : LLVMAppendBasicBlockInContext(ctx.context, current_fn, "match.next"));
            const __ring_end82 = List_len(elements);
            for (let j = 0; j < __ring_end82; j++) {
              __ring_match127: {
                const __ring_m127 = List_get(elements, j);
                if (__ring_m127._tag === "some") {
                  const elem_pat = __ring_m127._0;
  ...
  return Option_none;
                  ...
                  __ring_match128: {
                    const __ring_m128 = ctor_ref;
                    if (__ring_m128._tag === "some") {
                      const cref = __ring_m128._0;
                    ...
                    if (__ring_m128._tag === "none") {
                      __ring_match131: {
                        const __ring_m131 = elem_pat;
                        if (__ring_m131._tag === "Literal") {
                          const value = __ring_m131.value;
                          const idx = LLVMConstInt(ctx.i64_type, j, 0);
            ...
            const __ring_end83 = List_len(elements);
            for (let j = 0; j < __ring_end83; j++) {
              __ring_match132: {
                const __ring_m132 = List_get(elements, j);
                if (__ring_m132._tag === "some") {
                  const elem_pat = __ring_m132._0;
                  __ring_match133: {
                    const __ring_m133 = elem_pat;
                    if (__ring_m133._tag === "Binding") {
                      const bname = __ring_m133.name;
                      const idx = LLVMConstInt(ctx.i64_type, j, 0);
...
function gen_match_expr(ctx, scrutinee, arms, result_ty) {
  ...
  __ring_match135: {
    const __ring_m135 = enum_name;
    if (__ring_m135._tag === "some") {
      const ename = __ring_m135._0;
      __ring_match136: {
        const __ring_m136 = _Map_get(ctx.enum_types, ename);
...
function gen_lambda(ctx, params, return_type, body, ty) {
  ...
  const __ring_end88 = List_len(captures);
  for (let i = 0; i < __ring_end88; i++) {
    __ring_match137: {
      const __ring_m137 = List_get(captures, i);
      if (__ring_m137._tag === "some") {
        const cap_name = __ring_m137._0;
  ...
  const __ring_end89 = List_len(params);
  for (let i = 0; i < __ring_end89; i++) {
    __ring_match138: {
      const __ring_m138 = List_get(params, i);
      if (__ring_m138._tag === "some") {
        const p = __ring_m138._0;
  ...
  for (let i = 0; i < __ring_end90; i++) {
    __ring_match139: {
      const __ring_m139 = List_get(captures, i);
      if (__ring_m139._tag === "some") {
        const cap_name = __ring_m139._0;
        const cap_val = (function() {
  const __ring_m = _Map_get(ctx.named_values, cap_name);
  if (__ring_m._tag === "some") { const alloca = __ring_m._0; return LLVMBuildLoad2(ctx.builder, ctx.ptr_type, alloca, codegen_llvm_ctx$fresh_name(ctx, "cv")); }
  if (__ring_m._tag === "none") { return LLVMConstPointerNull(ctx.ptr_type); }
...
function gen_string_interp(ctx, parts) {
  ...
  while (true) {
    ...
    __ring_match140: {
      const __ring_m140 = part;
      if (__ring_m140._tag === "Literal") {
        const s = __ring_m140._0;
      ...
      if (__ring_m140._tag === "Expression") {
        const e = __ring_m140._0;
...
function gen_struct_lit(ctx, name, fields, spread) {
  __ring_match142: {
    const __ring_m142 = _Map_get(ctx.struct_types, name);
    if (__ring_m142._tag === "some") {
      const info = __ring_m142._0;
      ...
      __ring_match143: {
        const __ring_m143 = spread;
        if (__ring_m143._tag === "some") {
          const spread_expr = __ring_m143._0;
...
function gen_field_access(ctx, receiver, field, ty) {
  ...
  __ring_match145: {
    const __ring_m145 = _Map_get(ctx.struct_types, type_name);
    if (__ring_m145._tag === "some") {
      const info = __ring_m145._0;
      let field_idx = (-1);
...
function gen_mut_arg_llvm(ctx, arg) {
  __ring_match146: {
    const __ring_m146 = arg;
    if (__ring_m146._tag === "Ident") {
      const name = __ring_m146.name; const resolved_name = __ring_m146.resolved_name; const def_id = __ring_m146.def_id;
  ...
  __match_fail(__ring_m);
        ...
        __ring_match147: {
          const __ring_m147 = _Map_get(ctx.named_values, lookup_name);
          if (__ring_m147._tag === "some") {
            const alloca = __ring_m147._0;
          ...
          if (__ring_m147._tag === "none") {
            __ring_match148: {
              const __ring_m148 = _Map_get(ctx.named_values, name);
              if (__ring_m148._tag === "some") {
                const alloca = __ring_m148._0;
                return LLVMBuildLoad2(ctx.builder, ctx.ptr_type, alloca, codegen_llvm_ctx$fresh_name(ctx, "mcell"));
...
function gen_dict_dispatch_call(ctx, callee, args, dd) {
  ...
  let call_args = [];
  __ring_match151: {
    const __ring_m151 = recv_val;
    if (__ring_m151._tag === "some") {
      const rv = __ring_m151._0;
  ...
  const __ring_end98 = List_len(args);
  for (let i = other_arg_start; i < __ring_end98; i++) {
    __ring_match152: {
      const __ring_m152 = List_get(args, i);
      if (__ring_m152._tag === "some") {
        const a = __ring_m152._0;
  ...
  const dict_ptr = (function() {
  const __ring_m = _Map_get(ctx.named_values, dd.dict_param);
  if (__ring_m._tag === "some") { const dict_alloca = __ring_m._0; return LLVMBuildLoad2(ctx.builder, ctx.ptr_type, dict_alloca, codegen_llvm_ctx$fresh_name(ctx, "dp")); }
  if (__ring_m._tag === "none") { return resolve_static_dict_by_name(ctx, dd.dict_param); }
  __match_fail(__ring_m);
})();
...
function gen_call(ctx, callee, args, resolved_dicts, dict_dispatch, result_ty, effects) {
  __ring_match153: {
    const __ring_m153 = dict_dispatch;
    if (__ring_m153._tag === "some") {
      const dd = __ring_m153._0;
  ...
  let __ring_blk0;
  __ring_match154: {
    const __ring_m154 = callee;
    if (__ring_m154._tag === "Ident") {
      const name = __ring_m154.name; const resolved_name = __ring_m154.resolved_name;
      const call_name = (function() {
  const __ring_m = resolved_name;
  if (__ring_m._tag === "some") { const rn = __ring_m._0; return rn; }
  if (__ring_m._tag === "none") { return name; }
  __match_fail(__ring_m);
      ...
      __ring_blk0 = gen_direct_call(ctx, final_name, arg_vals, dict_vals);
      break __ring_match154;
    }
    if (__ring_m154._tag === "FieldAccess") {
      const receiver = __ring_m154.receiver; const field = __ring_m154.field;
      const recv_val = gen_llvm_expr(ctx, receiver);
      const recv_type = hir$hexpr_type(receiver);
      __ring_blk0 = gen_method_call(ctx, recv_val, recv_type, field, arg_vals, dict_vals);
      break __ring_match154;
    }
    const closure_val = gen_llvm_expr(ctx, callee);
    __ring_blk0 = gen_closure_call(ctx, closure_val, arg_vals);
    break __ring_match154;
  }
  const raw = __ring_blk0;
  if (is_unit_type(result_ty)) {
    return LLVMConstPointerNull(ctx.ptr_type);
  }
  return raw;
...
function gen_eq_dispatch_llvm(ctx, op, left, right, dispatch) {
  ...
  __ring_match158: {
    const __ring_m158 = op;
    if (__ring_m158._tag === "Neq") {
...
function gen_binop(ctx, op, left, right, eq_dispatch, ord_dispatch, result_ty) {
  ...
  if (is_eq_op) {
    __ring_match160: {
      const __ring_m160 = eq_dispatch;
      if (__ring_m160._tag === "some") {
        const d = __ring_m160._0;
        __ring_match161: {
          const __ring_m161 = d;
          if (__ring_m161._tag === "Builtin") {
  ...
  if (is_ord_op) {
    __ring_match162: {
      const __ring_m162 = ord_dispatch;
      if (__ring_m162._tag === "some") {
        const d = __ring_m162._0;
        __ring_match163: {
          const __ring_m163 = d;
          if (__ring_m163._tag === "Builtin") {
...
function gen_llvm_expr(ctx, expr) {
  __ring_match164: {
    const __ring_m164 = expr;
    if (__ring_m164._tag === "IntLit") {
      const value = __ring_m164.value;
      return gen_int_lit(ctx, value);
      break __ring_match164;
    }
    if (__ring_m164._tag === "FloatLit") {
      const value = __ring_m164.value;
      return gen_float_lit(ctx, value);
      break __ring_match164;
    }
    if (__ring_m164._tag === "StrLit") {
      const value = __ring_m164.value;
      return gen_str_lit(ctx, value);
      break __ring_match164;
    }
    if (__ring_m164._tag === "BoolLit") {
      const value = __ring_m164.value;
      return gen_bool_lit(ctx, value);
      break __ring_match164;
    }
    if (__ring_m164._tag === "Ident") {
      const name = __ring_m164.name; const resolved_name = __ring_m164.resolved_name; const def_id = __ring_m164.def_id; const dict_closure_dicts = __ring_m164.dict_closure_dicts; const ty = __ring_m164.ty;
      return gen_ident(ctx, name, resolved_name, def_id, dict_closure_dicts, ty);
      break __ring_match164;
    }
    if (__ring_m164._tag === "BinOp") {
      const op = __ring_m164.op; const left = __ring_m164.left; const right = __ring_m164.right; const eq_dispatch = __ring_m164.eq_dispatch; const ord_dispatch = __ring_m164.ord_dispatch; const ty = __ring_m164.ty;
      return gen_binop(ctx, op, left, right, eq_dispatch, ord_dispatch, ty);
      break __ring_match164;
    }
    if (__ring_m164._tag === "UnaryOp") {
      const op = __ring_m164.op; const operand = __ring_m164.operand; const ty = __ring_m164.ty;
      return gen_unaryop(ctx, op, operand, ty);
      break __ring_match164;
    }
    if (__ring_m164._tag === "Call") {
      const callee = __ring_m164.callee; const args = __ring_m164.args; const resolved_dicts = __ring_m164.resolved_dicts; const dict_dispatch = __ring_m164.dict_dispatch; const ty = __ring_m164.ty; const effects = __ring_m164.effects;
      return gen_call(ctx, callee, args, resolved_dicts, dict_dispatch, ty, effects);
      break __ring_match164;
    }
    if (__ring_m164._tag === "DictConstruct") {
      const base_dict = __ring_m164.base_dict; const trait_name = __ring_m164.trait_name; const inner = __ring_m164.inner;
      return build_wrapped_dict(ctx, base_dict, trait_name, inner);
      break __ring_match164;
    }
    if (__ring_m164._tag === "FieldAccess") {
      const receiver = __ring_m164.receiver; const field = __ring_m164.field; const ty = __ring_m164.ty;
      return gen_field_access(ctx, receiver, field, ty);
      break __ring_match164;
    }
    if (__ring_m164._tag === "StructLit") {
      const name = __ring_m164.name; const fields = __ring_m164.fields; const spread = __ring_m164.spread;
      return gen_struct_lit(ctx, name, fields, spread);
      break __ring_match164;
    }
    if (__ring_m164._tag === "Block") {
      const stmts = __ring_m164.stmts; const tail = __ring_m164.tail;
      return gen_block(ctx, stmts, tail);
      break __ring_match164;
    }
    if (__ring_m164._tag === "IfExpr") {
      const condition = __ring_m164.condition; const then_branch = __ring_m164.then_branch; const else_branch = __ring_m164.else_branch;
      return gen_if_expr(ctx, condition, then_branch, else_branch);
      break __ring_match164;
    }
    if (__ring_m164._tag === "StringInterp") {
      const parts = __ring_m164.parts;
      return gen_string_interp(ctx, parts);
      break __ring_match164;
    }
    if (__ring_m164._tag === "Lambda") {
      const params = __ring_m164.params; const return_type = __ring_m164.return_type; const body = __ring_m164.body; const ty = __ring_m164.ty;
      return gen_lambda(ctx, params, return_type, body, ty);
      break __ring_match164;
    }
    if (__ring_m164._tag === "MatchExpr") {
      const scrutinee = __ring_m164.scrutinee; const arms = __ring_m164.arms; const ty = __ring_m164.ty;
      return gen_match_expr(ctx, scrutinee, arms, ty);
      break __ring_match164;
    }
    if (__ring_m164._tag === "NamedVariantConstruct") {
      const enum_name = __ring_m164.enum_name; const variant_name = __ring_m164.variant_name; const fields = __ring_m164.fields;
      return gen_named_variant_construct(ctx, enum_name, variant_name, fields);
      break __ring_match164;
    }
    if (__ring_m164._tag === "TryCatch") {
      const body = __ring_m164.body; const arms = __ring_m164.arms;
      return gen_try_catch(ctx, body, arms);
      break __ring_match164;
    }
    if (__ring_m164._tag === "HandleExpr") {
      const body = __ring_m164.body; const handlers = __ring_m164.handlers;
      return gen_handle_expr(ctx, body, handlers);
      break __ring_match164;
    }
    if (__ring_m164._tag === "EffectOp") {
      const effect_name = __ring_m164.effect_name; const op_name = __ring_m164.op_name; const args = __ring_m164.args;
      return gen_effect_op(ctx, effect_name, op_name, args);
      break __ring_match164;
    }
    if (__ring_m164._tag === "RangeExpr") {
      const start = __ring_m164.start; const end = __ring_m164.end; const inclusive = __ring_m164.inclusive;
      return gen_range_expr(ctx, start, end, inclusive);
      break __ring_match164;
    }
    if (__ring_m164._tag === "ListLit") {
      const elements = __ring_m164.elements;
      return gen_list_lit(ctx, elements);
      break __ring_match164;
    }
    if (__ring_m164._tag === "TupleLit") {
      const elements = __ring_m164.elements;
      return gen_tuple_lit(ctx, elements);
      break __ring_match164;
    }
    if (__ring_m164._tag === "IndexExpr") {
      const receiver = __ring_m164.receiver; const index = __ring_m164.index; const ty = __ring_m164.ty;
      return gen_index_expr(ctx, receiver, index, ty);
      break __ring_match164;
    }
    if (__ring_m164._tag === "Clone") {
      const inner = __ring_m164.inner;
...
function emit_memoised_dict_getter(ctx, name, build_fn, build_fn_ty) {
  const fname = `ring_dict_init_${name}`;
  __ring_match165: {
    const __ring_m165 = _Map_get(ctx.functions, fname);
    if (__ring_m165._tag === "some") {
      const existing = __ring_m165._0;
      return existing;
