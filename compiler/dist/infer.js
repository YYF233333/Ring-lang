...
function check_assign_target_mutable(ctx, target) {
  __ring_match33: {
    ...
    if (__ring_m33._tag === "IndexExpr") {
      ...
      const root = find_root_expr(receiver);
      __ring_match39: {
        const __ring_m39 = root;
        if (__ring_m39._tag === "Ident") {
          const name = __ring_m39.name; const rspan = __ring_m39.span;
          const scheme = env$TypeEnv_lookup(ctx.env, name);
          __ring_match40: {
...
function infer_expr(ctx, expr, subst, __ring_ev_fail) {
  __ring_match43: {
    ...
    if (__ring_m43._tag === "IntLit") {
      const value = __ring_m43.value; const span = __ring_m43.span;
    ...
    if (__ring_m43._tag === "Ident") {
      const name = __ring_m43.name; const qualifier = __ring_m43.qualifier; const span = __ring_m43.span;
      return infer_ident(ctx, name, span, subst, qualifier);
      break __ring_match43;
    }
    if (__ring_m43._tag === "BinOp") {
      const op = __ring_m43.op; const left = __ring_m43.left; const right = __ring_m43.right; const span = __ring_m43.span;
      return infer_bin_op(ctx, op, left, right, span, subst, __ring_ev_fail);
      break __ring_match43;
    }
    if (__ring_m43._tag === "UnaryOp") {
      const op = __ring_m43.op; const operand = __ring_m43.operand; const span = __ring_m43.span;
      return infer_unary_op(ctx, op, operand, span, subst, __ring_ev_fail);
      break __ring_match43;
    }
    if (__ring_m43._tag === "Call") {
      const callee = __ring_m43.callee; const args = __ring_m43.args; const span = __ring_m43.span;
      return infer_call(ctx, callee, args, span, subst, __ring_ev_fail);
      break __ring_match43;
    }
    if (__ring_m43._tag === "MethodCall") {
      const receiver = __ring_m43.receiver; const method = __ring_m43.method; const args = __ring_m43.args; const span = __ring_m43.span;
      return infer_method_call(ctx, receiver, method, args, span, subst, __ring_ev_fail);
      break __ring_match43;
    }
    if (__ring_m43._tag === "FieldAccess") {
      const receiver = __ring_m43.receiver; const field = __ring_m43.field; const span = __ring_m43.span;
      return infer_field_access(ctx, receiver, field, span, subst, __ring_ev_fail);
      break __ring_match43;
    }
    if (__ring_m43._tag === "StructLit") {
      const name = __ring_m43.name; const qualifier = __ring_m43.qualifier; const fields = __ring_m43.fields; const spread = __ring_m43.spread; const span = __ring_m43.span;
      return infer_struct_lit(ctx, name, fields, spread, span, subst, qualifier, __ring_ev_fail);
      break __ring_match43;
    }
    if (__ring_m43._tag === "MatchExpr") {
      const scrutinee = __ring_m43.scrutinee; const arms = __ring_m43.arms; const span = __ring_m43.span;
    ...
    if (__ring_m43._tag === "IfExpr") {
      const condition = __ring_m43.condition; const then_branch = __ring_m43.then_branch; const else_branch = __ring_m43.else_branch; const span = __ring_m43.span;
      return infer_if(ctx, condition, then_branch, else_branch, span, subst, __ring_ev_fail);
      break __ring_match43;
    }
    if (__ring_m43._tag === "StringInterp") {
      const parts = __ring_m43.parts; const span = __ring_m43.span;
      return infer_string_interp(ctx, parts, span, subst, __ring_ev_fail);
      break __ring_match43;
    }
    if (__ring_m43._tag === "CatchExpr") {
      const catch_expr = __ring_m43.expr; const arms = __ring_m43.arms; const span = __ring_m43.span;
      return infer_catch(ctx, catch_expr, arms, span, subst, __ring_ev_fail);
      break __ring_match43;
    }
    if (__ring_m43._tag === "HandleExpr") {
      const body = __ring_m43.body; const handlers = __ring_m43.handlers; const span = __ring_m43.span;
      return infer_handle(ctx, body, handlers, span, subst, __ring_ev_fail);
      break __ring_match43;
    }
    if (__ring_m43._tag === "Lambda") {
      const params = __ring_m43.params; const body = __ring_m43.body; const span = __ring_m43.span;
      return infer_lambda(ctx, params, body, span, subst, Option_none, __ring_ev_fail);
      break __ring_match43;
    }
    if (__ring_m43._tag === "ListLit") {
      const elements = __ring_m43.elements; const span = __ring_m43.span;
    ...
    if (__ring_m43._tag === "Range") {
      const start = __ring_m43.start; const end = __ring_m43.end; const inclusive = __ring_m43.inclusive; const span = __ring_m43.span;
    ...
    if (__ring_m43._tag === "IndexExpr") {
      const receiver = __ring_m43.receiver; const index = __ring_m43.index; const span = __ring_m43.span;
...
function infer_index_expr(ctx, receiver, index, span, subst, __ring_ev_fail) {
  ...
  __ring_match44: {
    ...
    if (__ring_m44._tag === "StrType") {
      s = infer_ctx$unify_at(ctx.sink, ctx.env, idx_type, types$INT, s, span);
      result_ty = types$STR;
      break __ring_match44;
    }
    if (__ring_m44._tag === "ErrorType") {
      result_ty = types$Type_ErrorType;
      break __ring_match44;
    }
    const _ = infer_ctx$type_error(ctx.sink, codes$E0306, `Type '${types$type_to_string(recv_type)}' does not support indexing`, span, diagnostics$DiagnosticContext_OtherContext(Option_some("only List, Map, and Str support subscript operator []")));
...
function infer_ident(ctx, name, span, subst, qualifier) {
  ...
  __ring_match45: {
    ...
    if (__ring_m45._tag === "some") {
      ...
      if (((q === "self") || Str_starts_with(q, "super"))) {
        __ring_match46: {
          const __ring_m46 = infer_ctx$resolve_relative_qualifier(q, ctx.mod_path_stack);
          if (__ring_m46._tag === "some") {
            const prefix = __ring_m46._0;
            if ((prefix === "")) {
  ...
  __ring_match47: {
    ...
    if (__ring_m47._tag === "some") {
      ...
      __ring_match48: {
        const __ring_m48 = mod_scheme;
        if (__ring_m48._tag === "some") {
          const ms = __ring_m48._0;
        ...
        if (__ring_m48._tag === "none") {
          if ((List_len(ctx.mod_path_stack) > 0)) {
            ...
            __ring_match49: {
              const __ring_m49 = full_scheme;
              if (__ring_m49._tag === "some") {
                const fs = __ring_m49._0;
  ...
  __ring_match50: {
    ...
    if (__ring_m50._tag === "none") {
      __ring_match51: {
        const __ring_m51 = resolved_qualifier;
  ...
  __match_fail(__ring_m);
      ...
      __ring_match52: {
        ...
        if (__ring_m52._tag === "some") {
          ...
          __ring_match53: {
            const __ring_m53 = _Map_get(ctx.env.types.enums, q);
            if (__ring_m53._tag === "some") {
              const enum_def = __ring_m53._0;
      ...
      __ring_match54: {
        const __ring_m54 = enum_name;
        if (__ring_m54._tag === "some") {
          const en = __ring_m54._0;
...
function infer_bin_op(ctx, op, left, right, span, subst, __ring_ev_fail) {
  ...
  __ring_match55: {
    ...
    if (__ring_m55._tag === "Sub") {
      result_type = infer_numeric_op(ctx, lr.hexpr, rr.hexpr, s, span, "-");
      s = infer_ctx$unify_at(ctx.sink, ctx.env, hir$hexpr_type(lr.hexpr), hir$hexpr_type(rr.hexpr), s, span);
      break __ring_match55;
    }
    if (__ring_m55._tag === "Mul") {
      result_type = infer_numeric_op(ctx, lr.hexpr, rr.hexpr, s, span, "*");
      s = infer_ctx$unify_at(ctx.sink, ctx.env, hir$hexpr_type(lr.hexpr), hir$hexpr_type(rr.hexpr), s, span);
      break __ring_match55;
    }
    if (__ring_m55._tag === "Div") {
      result_type = infer_numeric_op(ctx, lr.hexpr, rr.hexpr, s, span, "/");
      s = infer_ctx$unify_at(ctx.sink, ctx.env, hir$hexpr_type(lr.hexpr), hir$hexpr_type(rr.hexpr), s, span);
      break __ring_match55;
    }
    if (__ring_m55._tag === "Mod") {
      result_type = infer_numeric_op(ctx, lr.hexpr, rr.hexpr, s, span, "%");
      s = infer_ctx$unify_at(ctx.sink, ctx.env, hir$hexpr_type(lr.hexpr), hir$hexpr_type(rr.hexpr), s, span);
      break __ring_match55;
    }
    if (__ring_m55._tag === "Eq") {
      s = infer_ctx$unify_at(ctx.sink, ctx.env, hir$hexpr_type(lr.hexpr), hir$hexpr_type(rr.hexpr), s, span);
...
function is_primitive_eq(t) {
  __ring_match57: {
    ...
    if (__ring_m57._tag === "UnitType") {
      return true;
      break __ring_match57;
    }
    if (__ring_m57._tag === "NeverType") {
      return true;
      break __ring_match57;
    }
    if (__ring_m57._tag === "AnyType") {
      return true;
      break __ring_match57;
    }
    return false;
...
function is_primitive_ord(t) {
  __ring_match58: {
    ...
    if (__ring_m58._tag === "StrType") {
      return true;
      break __ring_match58;
    }
    if (__ring_m58._tag === "BoolType") {
      return true;
      break __ring_match58;
    }
    return false;
...
function resolve_trait_dispatch(ctx, resolved, trait_name, error_code, subst, span, op, is_builtin) {
  ...
  __ring_match61: {
    const __ring_m61 = bound_resolved;
    if (__ring_m61._tag === "TypeVar") {
      const bid = __ring_m61.id;
      return (bid === id);
    ...
    break __ring_match61;
      ...
      __ring_match63: {
        const __ring_m63 = _Map_get(ctx.env.scope.var_bounds, id);
        if (__ring_m63._tag === "some") {
          const var_bounds = __ring_m63._0;
    ...
    if (__ring_m60._tag === "StructType") {
      const name = __ring_m60.name; const type_params = __ring_m60.type_params;
...
function resolve_trait_extra_dicts(ctx, type_args, subst, trait_name) {
  ...
  for (const arg of type_args) {
    ...
    __ring_match64: {
      const __ring_m64 = dict;
      if (__ring_m64._tag === "some") {
        const d = __ring_m64._0;
        List_push(dicts, d);
...
function resolve_type_to_trait_dict(ctx, t, trait_name) {
  __ring_match65: {
    const __ring_m65 = types$type_to_builtin_name(t);
    if (__ring_m65._tag === "some") {
      const builtin_name = __ring_m65._0;
      __ring_match66: {
        const __ring_m66 = t;
...
function infer_call(ctx, callee, args, span, subst, __ring_ev_fail) {
  ...
  __ring_match71: {
    const __ring_m71 = expected;
    if (__ring_m71._tag === "FnType") {
      const exp_params = __ring_m71.params;
  ...
  __ring_match72: {
    const __ring_m72 = resolved_callee_type;
    if (__ring_m72._tag === "FnType") {
      const fn_effects = __ring_m72.effects;
  ...
  let resolved_dicts = [];
  __ring_match73: {
    const __ring_m73 = callee;
    if (__ring_m73._tag === "Ident") {
      const callee_name = __ring_m73.name;
      __ring_match74: {
        const __ring_m74 = env$TypeEnv_lookup(ctx.env, callee_name);
        if (__ring_m74._tag === "some") {
          const callee_scheme = __ring_m74._0;
...
function resolve_arg_dict_closure(ctx, harg, s) {
  __ring_match75: {
    const __ring_m75 = harg;
    if (__ring_m75._tag === "Ident") {
      const name = __ring_m75.name; const resolved_name = __ring_m75.resolved_name; const def_id = __ring_m75.def_id; const ty = __ring_m75.ty; const effects = __ring_m75.effects; const span = __ring_m75.span;
      const arg_scheme = env$TypeEnv_lookup(ctx.env, name);
      __ring_match76: {
        const __ring_m76 = arg_scheme;
        if (__ring_m76._tag === "some") {
          const as_ = __ring_m76._0;
          ...
          for (const bound of as_.bounds) {
            __ring_match77: {
              const __ring_m77 = _Map_get(var_map, bound.type_var);
              if (__ring_m77._tag === "some") {
                const fresh_var = __ring_m77._0;
...
function resolve_arg_bound_dict(ctx, concrete, trait_name, dicts) {
  __ring_match78: {
    ...
    if (__ring_m78._tag === "TypeVar") {
      ...
      __ring_match79: {
        const __ring_m79 = matching;
        if (__ring_m79._tag === "some") {
          const fb = __ring_m79._0;
    ...
    __ring_match80: {
      const __ring_m80 = types$type_to_builtin_name(concrete);
      if (__ring_m80._tag === "some") {
        const prim_name = __ring_m80._0;
...
function check_expr_is_let_def(ctx, expr) {
  __ring_match81: {
    const __ring_m81 = expr;
    if (__ring_m81._tag === "Ident") {
      const name = __ring_m81.name;
      __ring_match82: {
        const __ring_m82 = env$TypeEnv_lookup(ctx.env, name);
        if (__ring_m82._tag === "some") {
          const s = __ring_m82._0;
          __ring_match83: {
            const __ring_m83 = s.def_id;
            if (__ring_m83._tag === "some") {
              const did = __ring_m83._0;
    ...
    if (__ring_m81._tag === "FieldAccess") {
      const inner = __ring_m81.receiver;
...
function check_receiver_mutability(ctx, receiver, recv_type, method, span) {
  ...
  __ring_match86: {
    const __ring_m86 = type_name;
    if (__ring_m86._tag === "some") {
      const tname = __ring_m86._0;
      __ring_match87: {
        const __ring_m87 = _Map_get(ctx.env.trait_reg.mut_methods, tname);
        if (__ring_m87._tag === "some") {
          const mut_set = __ring_m87._0;
...
function infer_method_call(ctx, receiver, method, args, span, subst, __ring_ev_fail) {
  __ring_match88: {
    const __ring_m88 = receiver;
    if (__ring_m88._tag === "Ident") {
      const recv_name = __ring_m88.name;
      __ring_match89: {
        const __ring_m89 = _Map_get(ctx.env.types.effects, recv_name);
  ...
  if (Option_is_none(method_type)) {
    __ring_match92: {
      const __ring_m92 = types$type_to_builtin_name(recv_type);
      if (__ring_m92._tag === "some") {
        const type_name = __ring_m92._0;
  ...
  if (Option_is_none(method_type)) {
    __ring_match93: {
      const __ring_m93 = recv_var_id;
      if (__ring_m93._tag === "some") {
        const rvid = __ring_m93._0;
        for (const fb of ctx.current_fn_bounds) {
          if ((resolve_var_id(fb.type_param_var_id, s) === rvid)) {
            __ring_match94: {
              const __ring_m94 = _Map_get(ctx.env.trait_reg.traits, fb.trait_name);
              if (__ring_m94._tag === "some") {
                const trait_def = __ring_m94._0;
                ...
                __ring_match95: {
                  ...
                  if (__ring_m95._tag === "some") {
                    const found_method = __ring_m95._0;
    ...
    if (__ring_m96._tag === "some") {
      ...
      __ring_match97: {
        ...
        if (__ring_m97._tag === "FnType") {
          const mt_params = __ring_m97.params;
          if ((List_len(mt_params) > 0)) {
            __ring_match98: {
              const __ring_m98 = List_first(mt_params);
              if (__ring_m98._tag === "some") {
                const first_param = __ring_m98._0;
  ...
  const expected = env$apply_subst(s, expected_raw);
  __ring_match99: {
    const __ring_m99 = expected;
    if (__ring_m99._tag === "FnType") {
      const exp_params = __ring_m99.params;
      return infer_lambda(ctx, lparams, lbody, lspan, s, Option_some(exp_params), __ring_ev_fail);
  ...
  __ring_match100: {
    ...
    if (__ring_m100._tag === "some") {
      ...
      __ring_match101: {
        const __ring_m101 = mt;
        if (__ring_m101._tag === "FnType") {
          const mt_params = __ring_m101.params; const mt_ret = __ring_m101.return_type; const mt_effects = __ring_m101.effects;
          let i = 0;
          for (const harg of hargs) {
            if (((i + 1) < List_len(mt_params))) {
              __ring_match102: {
                const __ring_m102 = List_get(mt_params, (i + 1));
                if (__ring_m102._tag === "some") {
                  const expected_param = __ring_m102._0;
    ...
    if (__ring_m100._tag === "none") {
      __ring_match104: {
        const __ring_m104 = recv_type;
        if (__ring_m104._tag === "TypeVar") {
  ...
  let resolved_dicts = [];
  __ring_match105: {
    const __ring_m105 = method_scheme;
    if (__ring_m105._tag === "some") {
      const ms = __ring_m105._0;
...
function lookup_impl_method(ctx, type_name, method) {
  __ring_match107: {
    const __ring_m107 = _Map_get(ctx.env.trait_reg.impl_methods, type_name);
    if (__ring_m107._tag === "some") {
      const impl_methods = __ring_m107._0;
      __ring_match108: {
        const __ring_m108 = _Map_get(impl_methods, method);
...
function lookup_trait_method(ctx, type_name, method, span) {
  ...
  for (const impl_entry of ctx.env.trait_reg.trait_impls) {
    if ((impl_entry.target_type_name === type_name)) {
      __ring_match109: {
        const __ring_m109 = _Map_get(ctx.env.trait_reg.traits, impl_entry.trait_name);
        if (__ring_m109._tag === "some") {
          const trait_def = __ring_m109._0;
          const tm = ((__a) => { const __i = __a.findIndex((function(m) { return (m.name === method); })); return __i >= 0 ? { _tag: "some", _0: __a[__i] } : { _tag: "none" }; })(trait_def.methods);
          __ring_match110: {
            const __ring_m110 = tm;
            if (__ring_m110._tag === "some") {
              const found_method = __ring_m110._0;
              __ring_match111: {
                const __ring_m111 = found_trait_name;
                if (__ring_m111._tag === "some") {
                  const prev_trait = __ring_m111._0;
...
function infer_effect_op(ctx, effect_name, op_name, args, span, subst, __ring_ev_fail) {
  ...
  __ring_match112: {
    const __ring_m112 = effect_def_opt;
...
function infer_field_access(ctx, receiver, field, span, subst, __ring_ev_fail) {
  ...
  __ring_match117: {
    ...
    if (__ring_m117._tag === "StructType") {
      const name = __ring_m117.name; const type_params = __ring_m117.type_params;
      __ring_match118: {
        const __ring_m118 = _Map_get(ctx.env.types.structs, name);
        if (__ring_m118._tag === "some") {
          const struct_def = __ring_m118._0;
          ...
          __ring_match119: {
            ...
            if (__ring_m119._tag === "some") {
              const found_field = __ring_m119._0;
              let inst_map = map_new();
              let fi = 0;
              while (((fi < List_len(struct_def.type_param_vars)) && (fi < List_len(type_params)))) {
                __ring_match120: {
                  const __ring_m120 = [List_get(struct_def.type_param_vars, fi), List_get(type_params, fi)];
                  if (Array.isArray(__ring_m120) && __ring_m120.length === 2 && __ring_m120[0]._tag === "some" && __ring_m120[1]._tag === "some") {
                    const var_id = __ring_m120[0]._0; const tp = __ring_m120[1]._0;
    ...
    if (__ring_m117._tag === "RecordType") {
      const rec_fields = __ring_m117.fields; const tail = __ring_m117.tail;
    ...
    if (__ring_m117._tag === "TupleType") {
      const elements = __ring_m117.elements;
      __ring_match123: {
        const __ring_m123 = parse_int(field);
        ...
        if (__ring_m123._tag === "some") {
          ...
          __ring_match124: {
            const __ring_m124 = List_get(elements, i);
...
function infer_struct_lit(ctx, name, fields, spread, span, subst, qualifier, __ring_ev_fail) {
  let resolved_qualifier = qualifier;
  __ring_match125: {
    const __ring_m125 = qualifier;
    if (__ring_m125._tag === "some") {
      const q = __ring_m125._0;
      if (((q === "self") || Str_starts_with(q, "super"))) {
        __ring_match126: {
          const __ring_m126 = infer_ctx$resolve_relative_qualifier(q, ctx.mod_path_stack);
          if (__ring_m126._tag === "some") {
            const prefix = __ring_m126._0;
            if ((prefix === "")) {
              resolved_qualifier = Option_none;
            } else {
              resolved_qualifier = Option_some(prefix);
  ...
  __ring_match130: {
    const __ring_m130 = resolved_qualifier;
    if (__ring_m130._tag === "some") {
      const q = __ring_m130._0;
      __ring_match131: {
        const __ring_m131 = _Map_get(ctx.env.types.enums, q);
        if (__ring_m131._tag === "some") {
          const enum_def = __ring_m131._0;
          if (enum_def.variants.some((function(v) { return (v.name === name); }))) {
        ...
        if (__ring_m131._tag === "none") {
          if ((List_len(ctx.mod_path_stack) > 0)) {
            ...
            __ring_match132: {
              const __ring_m132 = _Map_get(ctx.env.types.enums, full_q);
              if (__ring_m132._tag === "some") {
                const enum_def = __ring_m132._0;
                if (enum_def.variants.some((function(v) { return (v.name === name); }))) {
                  variant_enum = Option_some(enum_def.name);
                }
  ...
  __ring_match134: {
    const __ring_m134 = variant_enum;
    if (__ring_m134._tag === "some") {
      const ve = __ring_m134._0;
      __ring_match135: {
        const __ring_m135 = _Map_get(ctx.env.types.enums, ve);
        if (__ring_m135._tag === "some") {
          const enum_def = __ring_m135._0;
          ...
          __ring_match136: {
            const __ring_m136 = variant;
            if (__ring_m136._tag === "some") {
              const v = __ring_m136._0;
              __ring_match137: {
                const __ring_m137 = v.field_names;
  ...
  __ring_match138: {
    const __ring_m138 = struct_def_opt;
  ...
  while ((tpi < List_len(struct_def.type_param_vars))) {
    __ring_match139: {
      const __ring_m139 = List_get(struct_def.type_param_vars, tpi);
      if (__ring_m139._tag === "some") {
        const var_id = __ring_m139._0;
  ...
  for (const field of fields) {
    ...
    __ring_match141: {
      const __ring_m141 = def_field;
      if (__ring_m141._tag === "some") {
        const df = __ring_m141._0;
...
function infer_named_variant_construct(ctx, enum_name, variant_name, variant, enum_def, fields, spread, span, subst, __ring_ev_fail) {
  ...
  while ((tpi < List_len(enum_def.type_param_vars))) {
    __ring_match142: {
      const __ring_m142 = List_get(enum_def.type_param_vars, tpi);
      if (__ring_m142._tag === "some") {
        const var_id = __ring_m142._0;
        const tv = env$TypeEnv_fresh_var(ctx.env);
        _Map_insert(inst_map, var_id, tv);
        List_push(type_param_types, tv);
  ...
  for (const field of fields) {
    ...
    __ring_match144: {
      const __ring_m144 = field_idx;
      if (__ring_m144._tag === "some") {
        const idx = __ring_m144._0;
        __ring_match145: {
          const __ring_m145 = List_get(variant.fields, idx);
          if (__ring_m145._tag === "some") {
            const ftype = __ring_m145._0;
...
function rewrite_bare_enum_bindings(env, pattern) {
  __ring_match146: {
    const __ring_m146 = pattern;
    if (__ring_m146._tag === "Binding") {
      const name = __ring_m146.name; const span = __ring_m146.span;
      __ring_match147: {
        const __ring_m147 = _Map_get(env.types.variant_to_enum, name);
        if (__ring_m147._tag === "some") {
          const ve = __ring_m147._0;
          __ring_match148: {
            const __ring_m148 = _Map_get(env.types.enums, ve);
            if (__ring_m148._tag === "some") {
              const edef = __ring_m148._0;
    ...
    if (__ring_m146._tag === "TuplePattern") {
      const elements = __ring_m146.elements; const span = __ring_m146.span;
      let new_elems = [];
    ...
    if (__ring_m146._tag === "Constructor") {
      const name = __ring_m146.name; const qualifier = __ring_m146.qualifier; const fields = __ring_m146.fields; const span = __ring_m146.span;
      let new_fields = [];
      for (const f of fields) {
    ...
    if (__ring_m146._tag === "NamedConstructor") {
      const name = __ring_m146.name; const qualifier = __ring_m146.qualifier; const fields = __ring_m146.fields; const rest = __ring_m146.rest; const span = __ring_m146.span;
      let new_fields = [];
      for (const f of fields) {
...
function infer_match(ctx, scrutinee, arms, span, subst, __ring_ev_fail) {
  ...
  __ring_match150: {
    const __ring_m150 = arm.guard;
    if (__ring_m150._tag === "some") {
      const g = __ring_m150._0;
  ...
  __ring_match152: {
    const __ring_m152 = missing;
    if (__ring_m152._tag === "some") {
      const m = __ring_m152._0;
...
function infer_if(ctx, condition, then_branch, else_branch, span, subst, __ring_ev_fail) {
  ...
  let result_type = types$UNIT;
  __ring_match153: {
    const __ring_m153 = else_branch;
    if (__ring_m153._tag === "some") {
      const eb = __ring_m153._0;
      __ring_match154: {
        const __ring_m154 = eb;
        if (__ring_m154._tag === "Block") {
        ...
        if (__ring_m154._tag === "IfExpr") {
          const ec = __ring_m154.condition; const etb = __ring_m154.then_branch; const eeb = __ring_m154.else_branch; const espan = __ring_m154.span;
...
function infer_string_interp(ctx, parts, span, subst, __ring_ev_fail) {
  ...
  for (const part of parts) {
    __ring_match155: {
      ...
      if (__ring_m155._tag === "LitPart") {
        const str_val = __ring_m155._0;
        List_push(hparts, hir$HStringInterpPart_Literal(str_val));
        break __ring_match155;
      }
      if (__ring_m155._tag === "ExprPart") {
        const expr = __ring_m155._0;
...
function infer_catch(ctx, expr, arms, span, subst, __ring_ev_fail) {
  ...
  for (const eff of effects.effects) {
    __ring_match156: {
      const __ring_m156 = eff;
      if (__ring_m156._tag === "FailEffect") {
        const et = __ring_m156.error_type;
        if (found_fail) {
  ...
  let guard_hexpr = Option_none;
  __ring_match157: {
    const __ring_m157 = arm.guard;
    if (__ring_m157._tag === "some") {
      const g = __ring_m157._0;
      const gr = infer_expr(ctx, g, s, __ring_ev_fail);
      s = gr.subst;
      s = infer_ctx$unify_at(ctx.sink, ctx.env, hir$hexpr_type(gr.hexpr), types$BOOL, s, arm.span);
      const me = infer_ctx$merge_effects(ctx.env, effects, gr.effects, s, __ring_ev_fail);
  ...
  __ring_match159: {
    const __ring_m159 = missing;
    if (__ring_m159._tag === "some") {
      const m = __ring_m159._0;
...
function infer_handle(ctx, body, handlers, span, subst, __ring_ev_fail) {
  ...
  for (const handler of handlers) {
    ...
    __ring_match160: {
      const __ring_m160 = effect_def;
      if (__ring_m160._tag === "some") {
        const ed = __ring_m160._0;
    ...
    __ring_match161: {
      const __ring_m161 = effect_def;
      if (__ring_m161._tag === "some") {
        const ed = __ring_m161._0;
  ...
  __match_fail(__ring_m);
    ...
    __ring_match162: {
      const __ring_m162 = handler.resume_name;
  ...
  __match_fail(__ring_m);
    ...
    env$TypeEnv_pop_scope(ctx.env);
    __ring_match163: {
      const __ring_m163 = handler_body_result;
      if (__ring_m163._tag === "some") {
        const hbr = __ring_m163._0;
        s = hbr.subst;
...
function infer_lambda(ctx, params, body, span, subst, expected_param_types, __ring_ev_fail) {
  ...
  __match_fail(__ring_m);
    ...
    __ring_match164: {
      const __ring_m164 = expected_param_types;
      if (__ring_m164._tag === "some") {
        const epts = __ring_m164._0;
        if (Option_is_none(p.type_annotation)) {
          __ring_match165: {
            const __ring_m165 = List_get(epts, pi);
            if (__ring_m165._tag === "some") {
              const expected_t = __ring_m165._0;
    ...
    __ring_match166: {
      const __ring_m166 = lam_scheme;
      if (__ring_m166._tag === "some") {
        const ls = __ring_m166._0;
        __ring_match167: {
          const __ring_m167 = ls.def_id;
  ...
  env$TypeEnv_pop_scope(ctx.env);
  __ring_match168: {
    const __ring_m168 = body_result;
    if (__ring_m168._tag === "some") {
      const body_r = __ring_m168._0;
      s = body_r.subst;
      let applied_params = [];
