...
function emit_return(ctx, value) {
  ...
  while ((ci >= 0)) {
    __ring_match44: {
      const __ring_m44 = List_get(ctx.handle_cleanup_stack, ci);
      if (__ring_m44._tag === "some") {
        const cleanup = __ring_m44._0;
...
function emit_while(ctx, condition, body) {
  let __ring_blk15;
  __ring_match47: {
    const __ring_m47 = ctx.current_fn;
...
function emit_llvm_stmt(ctx, stmt) {
  __ring_match48: {
    const __ring_m48 = stmt;
    if (__ring_m48._tag === "Let") {
    ...
    if (__ring_m48._tag === "Var") {
      const name = __ring_m48.name; const def_id = __ring_m48.def_id; const init = __ring_m48.init;
      const val = codegen_llvm_expr$gen_llvm_expr(ctx, init);
    ...
    if (__ring_m48._tag === "Assign") {
      const target = __ring_m48.target; const value = __ring_m48.value;
    ...
    if (__ring_m48._tag === "Return") {
      const value = __ring_m48.value;
      return emit_return(ctx, value);
      break __ring_match48;
    }
    if (__ring_m48._tag === "While") {
      const condition = __ring_m48.condition; const body = __ring_m48.body;
      return emit_while(ctx, condition, body);
      break __ring_match48;
    }
    if (__ring_m48._tag === "ForIn") {
      const binding = __ring_m48.binding; const destructure = __ring_m48.destructure; const iterable = __ring_m48.iterable; const body = __ring_m48.body; const iterable_type_name = __ring_m48.iterable_type_name; const iter_type_name = __ring_m48.iter_type_name;
      return emit_for_in(ctx, binding, destructure, iterable, body, iterable_type_name, iter_type_name);
      break __ring_match48;
    }
    if (__ring_m48._tag === "Break") {
      return emit_break(ctx);
      break __ring_match48;
    }
    if (__ring_m48._tag === "Continue") {
      return emit_continue(ctx);
      break __ring_match48;
    }
    if (__ring_m48._tag === "LetDestructure") {
      const pattern = __ring_m48.pattern; const bindings = __ring_m48.bindings; const init = __ring_m48.init;
      return emit_let_destructure(ctx, bindings, init);
      break __ring_match48;
    }
    if (__ring_m48._tag === "IfLet") {
      const pattern = __ring_m48.pattern; const expr = __ring_m48.expr; const then_block = __ring_m48.then_block; const else_block = __ring_m48.else_block;
      return emit_if_let(ctx, pattern, expr, then_block, else_block);
      break __ring_match48;
    }
    if (__ring_m48._tag === "Drop") {
