...
function emit_assign(ctx, target, value) {
  ...
  __ring_match7: {
    const __ring_m7 = target;
    if (__ring_m7._tag === "Ident") {
      const name = __ring_m7.name; const resolved_name = __ring_m7.resolved_name;
  ...
  __match_fail(__ring_m);
    ...
    if (__ring_m7._tag === "FieldAccess") {
      const receiver = __ring_m7.receiver; const field = __ring_m7.field;
  ...
  return panic("LLVM codegen: field assign on non-struct type");
      ...
      __ring_match10: {
        const __ring_m10 = _Map_get(ctx.struct_types, type_name);
        if (__ring_m10._tag === "some") {
          const info = __ring_m10._0;
          let field_idx = (-1);
...
function emit_return(ctx, value) {
  ...
  __ring_match12: {
    const __ring_m12 = ctx.current_fn;
    if (__ring_m12._tag === "some") {
      const f = __ring_m12._0;
...
function emit_for_in(ctx, binding, destructure, iterable, body, iterable_type_name, iter_type_name) {
  ...
  __ring_match13: {
    const __ring_m13 = iterable;
    if (__ring_m13._tag === "RangeExpr") {
      const start = __ring_m13.start; const end = __ring_m13.end; const inclusive = __ring_m13.inclusive;
...
function emit_for_in_range_direct(ctx, binding, start, end, inclusive, body) {
  ...
  const counter_alloca = LLVMBuildAlloca(ctx.builder, ctx.i64_type, codegen_llvm_ctx$fresh_name(ctx, "i"));
  ...
  const binding_alloca = LLVMBuildAlloca(ctx.builder, ctx.ptr_type, binding);
...
function emit_for_in_range_var(ctx, binding, iterable, body) {
  ...
  const range_ty = LLVMBuildAlloca(ctx.builder, ctx.ptr_type, codegen_llvm_ctx$fresh_name(ctx, "rng"));
  const struct_ty = LLVMBuildAlloca(ctx.builder, ctx.ptr_type, codegen_llvm_ctx$fresh_name(ctx, "dummy"));
...
function emit_for_in_list(ctx, binding, destructure, iterable, body) {
  ...
  const counter_alloca = LLVMBuildAlloca(ctx.builder, ctx.i64_type, codegen_llvm_ctx$fresh_name(ctx, "idx"));
  ...
  __ring_match14: {
    ...
    if (__ring_m14._tag === "some") {
      ...
      if ((List_len(ds) > 0)) {
        ...
        for (let i = 0; i < __ring_end3; i++) {
          __ring_match15: {
            ...
            if (__ring_m15._tag === "some") {
              ...
              const alloca = LLVMBuildAlloca(ctx.builder, ctx.ptr_type, d.name);
        ...
        const alloca = LLVMBuildAlloca(ctx.builder, ctx.ptr_type, binding);
...
function emit_break(ctx) {
  __ring_match16: {
    const __ring_m16 = ctx.loop_break_bb;
...
function emit_continue(ctx) {
  __ring_match17: {
    const __ring_m17 = ctx.loop_continue_bb;
    if (__ring_m17._tag === "some") {
      const bb = __ring_m17._0;
      discard(LLVMBuildBr(ctx.builder, bb));
      const current_fn_val = (function() {
  const __ring_m = ctx.current_fn;
  if (__ring_m._tag === "some") { const f = __ring_m._0; return f; }
    ...
    __ring_match18: {
      ...
      if (__ring_m18._tag === "some") {
        ...
        if ((b.name !== "_")) {
          ...
          const alloca = LLVMBuildAlloca(ctx.builder, ctx.ptr_type, b.name);
          discard(LLVMBuildStore(ctx.builder, elem, alloca));
...
function emit_if_let(ctx, pattern, expr, then_block, else_block) {
  ...
  __ring_match19: {
    const __ring_m19 = pattern;
    if (__ring_m19._tag === "Constructor") {
  ...
  return "Option";
      ...
      __ring_match20: {
        const __ring_m20 = _Map_get(ctx.enum_types, enum_name);
        if (__ring_m20._tag === "some") {
          const enum_info = __ring_m20._0;
          __ring_match21: {
            const __ring_m21 = _Map_get(enum_info.variants, name);
