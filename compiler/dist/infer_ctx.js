...
function infer_suggestion(code, message, context) {
  ...
  if ((code === "E0201")) {
    __ring_match0: {
      const __ring_m0 = context;
      if (__ring_m0._tag === "UndefinedVariable") {
        const name = __ring_m0.name; const scope_locals = __ring_m0.scope_locals;
        __ring_match1: {
          const __ring_m1 = scope_locals;
          if (__ring_m1._tag === "some") {
            const locals = __ring_m1._0;
            const similar = find_similar_name(name, locals);
            __ring_match2: {
              const __ring_m2 = similar;
              if (__ring_m2._tag === "some") {
                const suggestion = __ring_m2._0;
  ...
  if ((code === "E0203")) {
    __ring_match3: {
      const __ring_m3 = context;
      if (__ring_m3._tag === "MissingField") {
        const field = __ring_m3.field; const available = __ring_m3.available;
        __ring_match4: {
          const __ring_m4 = available;
          if (__ring_m4._tag === "some") {
            const avail = __ring_m4._0;
            const similar = find_similar_name(field, avail);
            __ring_match5: {
              const __ring_m5 = similar;
              if (__ring_m5._tag === "some") {
                const suggestion = __ring_m5._0;
                List_push(suggestions, new diagnostics$Suggestion(`Did you mean '${suggestion}'?`, Option_some(suggestion), Option_none));
                break __ring_match5;
              }
...
function merge_effects(env, a, b, s, __ring_ev_fail) {
  ...
  let result_s = s;
  __ring_match6: {
    const __ring_m6 = m.tails_to_unify;
    if (__ring_m6._tag === "some") {
      const pair = __ring_m6._0;
...
function collect_free_vars(t, result) {
  __ring_match7: {
    ...
    if (__ring_m7._tag === "FnType") {
      ...
      __ring_match8: {
        const __ring_m8 = effects.tail;
        if (__ring_m8._tag === "some") {
          const tail_id = __ring_m8._0;
    ...
    if (__ring_m7._tag === "StructType") {
      const type_params = __ring_m7.type_params;
    ...
    if (__ring_m7._tag === "RecordType") {
      const fields = __ring_m7.fields; const tail = __ring_m7.tail;
      ...
      for (const e of effects) {
        __ring_match12: {
          ...
          if (__ring_m12._tag === "CustomEffect") {
            const type_args = __ring_m12.type_args;
            for (const a of type_args) {
              collect_free_vars(a, result);
            }
            break __ring_match12;
...
function generalize(env, t, subst) {
  ...
  for (const tv of type_vars) {
    __ring_match14: {
      const __ring_m14 = _Map_get(env.scope.var_bounds, tv);
      if (__ring_m14._tag === "some") {
        const traits = __ring_m14._0;
...
function build_scheme_var_map(scheme, instantiated_type) {
  ...
  __ring_match17: {
    const __ring_m17 = [scheme.ty, instantiated_type];
    if (Array.isArray(__ring_m17) && __ring_m17.length === 2 && __ring_m17[0]._tag === "FnType" && __ring_m17[1]._tag === "FnType") {
      const sp = __ring_m17[0].params; const sr = __ring_m17[0].return_type; const ip = __ring_m17[1].params; const ir = __ring_m17[1].return_type;
      let i = 0;
      const limit = ((List_len(sp) < List_len(ip)) ? List_len(sp) : List_len(ip));
      while ((i < limit)) {
        __ring_match18: {
          const __ring_m18 = [List_get(sp, i), List_get(ip, i)];
          if (Array.isArray(__ring_m18) && __ring_m18.length === 2 && __ring_m18[0]._tag === "some" && __ring_m18[1]._tag === "some") {
            const s_param = __ring_m18[0]._0; const i_param = __ring_m18[1]._0;
...
function collect_var_mappings(scheme_type, inst_type, type_vars, result) {
  __ring_match19: {
    ...
    if (__ring_m19._tag === "StructType") {
      const sn = __ring_m19.name; const stp = __ring_m19.type_params;
      __ring_match20: {
        const __ring_m20 = inst_type;
        if (__ring_m20._tag === "StructType") {
          const in_ = __ring_m20.name; const itp = __ring_m20.type_params;
          if ((sn === in_)) {
            let i = 0;
            const limit = ((List_len(stp) < List_len(itp)) ? List_len(stp) : List_len(itp));
            while ((i < limit)) {
              __ring_match21: {
                const __ring_m21 = [List_get(stp, i), List_get(itp, i)];
                if (Array.isArray(__ring_m21) && __ring_m21.length === 2 && __ring_m21[0]._tag === "some" && __ring_m21[1]._tag === "some") {
                  const s = __ring_m21[0]._0; const inst = __ring_m21[1]._0;
    ...
    if (__ring_m19._tag === "EnumType") {
      const sn = __ring_m19.name; const stp = __ring_m19.type_params;
      __ring_match22: {
        const __ring_m22 = inst_type;
        if (__ring_m22._tag === "EnumType") {
          const in_ = __ring_m22.name; const itp = __ring_m22.type_params;
          if ((sn === in_)) {
            let i = 0;
            const limit = ((List_len(stp) < List_len(itp)) ? List_len(stp) : List_len(itp));
            while ((i < limit)) {
              __ring_match23: {
                const __ring_m23 = [List_get(stp, i), List_get(itp, i)];
                if (Array.isArray(__ring_m23) && __ring_m23.length === 2 && __ring_m23[0]._tag === "some" && __ring_m23[1]._tag === "some") {
                  const s = __ring_m23[0]._0; const inst = __ring_m23[1]._0;
                  collect_var_mappings(s, inst, type_vars, result);
...
function resolve_dicts_from_scheme(sink, env, current_fn_bounds, scheme, callee_type, s, span) {
  ...
  for (const bound of scheme.bounds) {
    let found = false;
    __ring_match24: {
      const __ring_m24 = _Map_get(var_map, bound.type_var);
      if (__ring_m24._tag === "some") {
        const fresh_var = __ring_m24._0;
        const concrete = env$apply_subst(s, fresh_var);
        __ring_match25: {
          const __ring_m25 = concrete;
        ...
        if ((!found)) {
          __ring_match27: {
            const __ring_m27 = types$type_to_builtin_name(concrete);
            if (__ring_m27._tag === "some") {
              const prim_name = __ring_m27._0;
...
function resolve_type_expr(ctx, texpr) {
  __ring_match28: {
    const __ring_m28 = texpr;
    if (__ring_m28._tag === "Named") {
      const name = __ring_m28.name; const type_args = __ring_m28.type_args; const span = __ring_m28.span;
    ...
    if (__ring_m28._tag === "TupleType") {
      const elements = __ring_m28.elements;
      let resolved_elems = [];
      for (const e of elements) {
...
function resolve_named_type(ctx, name, type_args, span) {
  ...
  __ring_match31: {
    const __ring_m31 = _Map_get(ctx.type_param_scope, name);
    if (__ring_m31._tag === "some") {
      const tp = __ring_m31._0;
      return tp;
  ...
  if (((name === hir$BUILTIN_OPTION) && (List_len(type_args) === 1))) {
    __ring_match32: {
      const __ring_m32 = List_get(type_args, 0);
      if (__ring_m32._tag === "some") {
        const arg = __ring_m32._0;
  ...
  if (_Map_contains_key(ctx.env.types.structs, name)) {
    __ring_match33: {
      const __ring_m33 = _Map_get(ctx.env.types.structs, name);
  ...
  if (_Map_contains_key(ctx.env.types.enums, name)) {
    __ring_match34: {
      const __ring_m34 = _Map_get(ctx.env.types.enums, name);
  ...
  __ring_match35: {
    const __ring_m35 = _Map_get(ctx.env.types.type_aliases, name);
    if (__ring_m35._tag === "some") {
      const alias = __ring_m35._0;
      ...
      while ((i < limit)) {
        __ring_match36: {
          const __ring_m36 = [List_get(alias.type_param_vars, i), List_get(resolved_args, i)];
          if (Array.isArray(__ring_m36) && __ring_m36.length === 2 && __ring_m36[0]._tag === "some" && __ring_m36[1]._tag === "some") {
            const var_id = __ring_m36[0]._0; const arg = __ring_m36[1]._0;
...
function bind_named_constructor_pattern(ctx, name, qualifier, fields, expected_type, subst, span) {
  ...
  __ring_match47: {
    ...
    if (__ring_m47._tag === "some") {
      ...
      __ring_match48: {
        const __ring_m48 = _Map_get(ctx.env.types.enums, ename);
        if (__ring_m48._tag === "some") {
          const enum_def = __ring_m48._0;
          const variant = ((__a) => { const __i = __a.findIndex((function(v) { return (v.name === name); })); return __i >= 0 ? { _tag: "some", _0: __a[__i] } : { _tag: "none" }; })(enum_def.variants);
          __ring_match49: {
            const __ring_m49 = variant;
            if (__ring_m49._tag === "some") {
              const v = __ring_m49._0;
              __ring_match50: {
                const __ring_m50 = v.field_names;
                if (__ring_m50._tag === "some") {
                  const vfield_names = __ring_m50._0;
                  const resolved_expected = env$apply_subst(subst, expected_type);
                  __ring_match51: {
                    const __ring_m51 = resolved_expected;
                    if (__ring_m51._tag === "EnumType") {
                      const rname = __ring_m51.name;
                      if ((rname !== ename)) {
                        const _ = type_error(ctx.sink, codes$E0301, `variant '${name}' belongs to enum '${ename}', not '${rname}'`, span, diagnostics$DiagnosticContext_TypeMismatch(rname, ename, Option_none));
                      }
                  ...
                  for (const field of fields) {
                    ...
                    __ring_match52: {
                      const __ring_m52 = field_idx;
                      if (__ring_m52._tag === "some") {
                        const idx = __ring_m52._0;
                        __ring_match53: {
                          const __ring_m53 = List_get(v.fields, idx);
                          if (__ring_m53._tag === "some") {
                            const ftype = __ring_m53._0;
                            const field_type = ((_Map_len(inst_map) > 0) ? env$apply_subst_map(inst_map, ftype) : ftype);
...
function build_instantiation_map(type_param_vars, resolved_expected) {
  ...
  __ring_match56: {
    const __ring_m56 = resolved_expected;
    if (__ring_m56._tag === "EnumType") {
      const type_params = __ring_m56.type_params;
      let i = 0;
      while (((i < List_len(type_param_vars)) && (i < List_len(type_params)))) {
        __ring_match57: {
          const __ring_m57 = [List_get(type_param_vars, i), List_get(type_params, i)];
          if (Array.isArray(__ring_m57) && __ring_m57.length === 2 && __ring_m57[0]._tag === "some" && __ring_m57[1]._tag === "some") {
            const var_id = __ring_m57[0]._0; const tp = __ring_m57[1]._0;
