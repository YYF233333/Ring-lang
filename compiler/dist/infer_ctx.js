...
function resolve_dicts_from_scheme(sink, env, current_fn_bounds, scheme, callee_type, s, span) {
  ...
  for (const bound of scheme.bounds) {
    let found = false;
    __ring_match30: {
      const __ring_m30 = _Map_get(var_map, bound.type_var);
      if (__ring_m30._tag === "some") {
        const fresh_var = __ring_m30._0;
...
function resolve_concrete_type_to_dict_ref(env, current_fn_bounds, t, s, trait_name) {
  __ring_match34: {
    const __ring_m34 = types$type_to_builtin_name(t);
    if (__ring_m34._tag === "some") {
      const builtin_name = __ring_m34._0;
      __ring_match35: {
        const __ring_m35 = t;
  ...
  __ring_match36: {
    ...
    if (__ring_m36._tag === "TypeVar") {
      ...
      __ring_match37: {
        const __ring_m37 = bound;
        if (__ring_m37._tag === "some") {
          const b = __ring_m37._0;
...
function resolve_type_expr(ctx, texpr) {
  __ring_match38: {
    const __ring_m38 = texpr;
    if (__ring_m38._tag === "Named") {
      const name = __ring_m38.name; const qualifier = __ring_m38.qualifier; const type_args = __ring_m38.type_args; const span = __ring_m38.span;
      __ring_match39: {
        const __ring_m39 = qualifier;
        if (__ring_m39._tag === "some") {
          const q = __ring_m39._0;
          let resolved_q = q;
          if (((q === "self") || Str_starts_with(q, "super"))) {
            __ring_match40: {
              const __ring_m40 = resolve_relative_qualifier(q, ctx.mod_path_stack);
              if (__ring_m40._tag === "some") {
                const prefix = __ring_m40._0;
                resolved_q = prefix;
    ...
    if (__ring_m38._tag === "FnType") {
      const params = __ring_m38.params; const return_type = __ring_m38.return_type; const effects = __ring_m38.effects;
      let resolved_params = [];
      for (const p of params) {
...
function resolve_named_type(ctx, name, type_args, span) {
  ...
  __ring_match43: {
    const __ring_m43 = _Map_get(ctx.type_param_scope, name);
    if (__ring_m43._tag === "some") {
      const tp = __ring_m43._0;
      return tp;
  ...
  if (((name === hir$BUILTIN_OPTION) && (List_len(type_args) === 1))) {
    __ring_match44: {
      const __ring_m44 = List_get(type_args, 0);
      if (__ring_m44._tag === "some") {
        const arg = __ring_m44._0;
  ...
  if (_Map_contains_key(ctx.env.types.structs, name)) {
    __ring_match45: {
      const __ring_m45 = _Map_get(ctx.env.types.structs, name);
  ...
  if (_Map_contains_key(ctx.env.types.enums, name)) {
    __ring_match46: {
      const __ring_m46 = _Map_get(ctx.env.types.enums, name);
  ...
  __ring_match47: {
    const __ring_m47 = _Map_get(ctx.env.types.type_aliases, name);
    if (__ring_m47._tag === "some") {
      const alias = __ring_m47._0;
      ...
      while ((i < limit)) {
        __ring_match48: {
          const __ring_m48 = [List_get(alias.type_param_vars, i), List_get(resolved_args, i)];
          if (Array.isArray(__ring_m48) && __ring_m48.length === 2 && __ring_m48[0]._tag === "some" && __ring_m48[1]._tag === "some") {
            const var_id = __ring_m48[0]._0; const arg = __ring_m48[1]._0;
...
function bind_pattern(ctx, pattern, expected_type, subst) {
  __ring_match49: {
    const __ring_m49 = pattern;
    if (__ring_m49._tag === "Wildcard") {
      break __ring_match49;
    }
    if (__ring_m49._tag === "Binding") {
      const name = __ring_m49.name; const span = __ring_m49.span;
      ...
      __ring_match50: {
        ...
        if (__ring_m50._tag === "some") {
          ...
          __ring_match51: {
            const __ring_m51 = scheme.def_id;
    ...
    if (__ring_m49._tag === "Constructor") {
      const name = __ring_m49.name; const qualifier = __ring_m49.qualifier; const fields = __ring_m49.fields; const span = __ring_m49.span;
    ...
    if (__ring_m49._tag === "TuplePattern") {
      ...
      __ring_match52: {
        ...
        if (__ring_m52._tag === "TupleType") {
          const type_elems = __ring_m52.elements;
          ...
          while ((i < List_len(elements))) {
            __ring_match53: {
              const __ring_m53 = [List_get(elements, i), List_get(type_elems, i)];
              if (Array.isArray(__ring_m53) && __ring_m53.length === 2 && __ring_m53[0]._tag === "some" && __ring_m53[1]._tag === "some") {
                const pat = __ring_m53[0]._0; const ty = __ring_m53[1]._0;
...
function bind_constructor_pattern(ctx, name, qualifier, fields, expected_type, subst, span) {
  ...
  __ring_match54: {
    ...
    if (__ring_m54._tag === "some") {
      ...
      __ring_match55: {
        const __ring_m55 = _Map_get(ctx.env.types.enums, ename);
          ...
          __ring_match56: {
            ...
            if (__ring_m56._tag === "some") {
              ...
              while (((i < List_len(fields)) && (i < List_len(v.fields)))) {
                __ring_match58: {
                  const __ring_m58 = [List_get(fields, i), List_get(v.fields, i)];
                  if (Array.isArray(__ring_m58) && __ring_m58.length === 2 && __ring_m58[0]._tag === "some" && __ring_m58[1]._tag === "some") {
                    const fpat = __ring_m58[0]._0; const ftype = __ring_m58[1]._0;
...
function bind_named_constructor_pattern(ctx, name, qualifier, fields, expected_type, subst, span) {
  ...
  __ring_match59: {
    ...
    if (__ring_m59._tag === "some") {
      ...
      __ring_match60: {
        const __ring_m60 = _Map_get(ctx.env.types.enums, ename);
        if (__ring_m60._tag === "some") {
          const enum_def = __ring_m60._0;
          const variant = ((__a) => { const __i = __a.findIndex((function(v) { return (v.name === name); })); return __i >= 0 ? { _tag: "some", _0: __a[__i] } : { _tag: "none" }; })(enum_def.variants);
          __ring_match61: {
            const __ring_m61 = variant;
            if (__ring_m61._tag === "some") {
              const v = __ring_m61._0;
              __ring_match62: {
                const __ring_m62 = v.field_names;
                if (__ring_m62._tag === "some") {
                  const vfield_names = __ring_m62._0;
                  const resolved_expected = env$apply_subst(subst, expected_type);
                  __ring_match63: {
                    const __ring_m63 = resolved_expected;
                    if (__ring_m63._tag === "EnumType") {
                      const rname = __ring_m63.name;
                      if ((rname !== ename)) {
                        const _ = type_error(ctx.sink, codes$E0301, `variant '${name}' belongs to enum '${ename}', not '${rname}'`, span, diagnostics$DiagnosticContext_TypeMismatch(rname, ename, Option_none));
                      }
                  ...
                  for (const field of fields) {
                    ...
                    __ring_match64: {
                      const __ring_m64 = field_idx;
                      if (__ring_m64._tag === "some") {
                        const idx = __ring_m64._0;
                        __ring_match65: {
                          const __ring_m65 = List_get(v.fields, idx);
                          if (__ring_m65._tag === "some") {
                            const ftype = __ring_m65._0;
                            const field_type = ((_Map_len(inst_map) > 0) ? env$apply_subst_map(inst_map, ftype) : ftype);
...
function bind_struct_pattern_fields(ctx, struct_name, display_name, fields, expected_type, subst, span) {
  __ring_match66: {
    const __ring_m66 = _Map_get(ctx.env.types.structs, struct_name);
    if (__ring_m66._tag === "some") {
      ...
      for (const field of fields) {
        ...
        __ring_match67: {
          const __ring_m67 = found;
          if (__ring_m67._tag === "some") {
            const sf = __ring_m67._0;
    ...
    if (__ring_m66._tag === "none") {
      if ((List_len(ctx.mod_path_stack) > 0)) {
        ...
        __ring_match68: {
          const __ring_m68 = _Map_get(ctx.env.types.structs, full_name);
          if (__ring_m68._tag === "some") {
            const sdef = __ring_m68._0;
            const resolved_expected = env$apply_subst(subst, expected_type);
            ...
            for (const field of fields) {
              ...
              __ring_match69: {
                const __ring_m69 = found;
                if (__ring_m69._tag === "some") {
                  const sf = __ring_m69._0;
                  const field_type = ((_Map_len(inst_map) > 0) ? env$apply_subst_map(inst_map, sf.ty) : sf.ty);
                  bind_pattern(ctx, field.pattern, field_type, subst);
...
function resolve_pattern_enum(ctx, variant_name, qualifier, span) {
  __ring_match70: {
    const __ring_m70 = qualifier;
    if (__ring_m70._tag === "some") {
      const q = __ring_m70._0;
      const direct = _Map_get(ctx.env.types.enums, q);
      __ring_match71: {
        const __ring_m71 = direct;
      ...
      if ((List_len(ctx.mod_path_stack) > 0)) {
        ...
        __ring_match72: {
          const __ring_m72 = fallback;
          if (__ring_m72._tag === "some") {
            const enum_def2 = __ring_m72._0;
...
function build_instantiation_map(type_param_vars, resolved_expected) {
  ...
  __ring_match73: {
    ...
    if (__ring_m73._tag === "EnumType") {
      const type_params = __ring_m73.type_params;
      let i = 0;
      while (((i < List_len(type_param_vars)) && (i < List_len(type_params)))) {
        __ring_match74: {
          const __ring_m74 = [List_get(type_param_vars, i), List_get(type_params, i)];
          if (Array.isArray(__ring_m74) && __ring_m74.length === 2 && __ring_m74[0]._tag === "some" && __ring_m74[1]._tag === "some") {
            const var_id = __ring_m74[0]._0; const tp = __ring_m74[1]._0;
    ...
    if (__ring_m73._tag === "StructType") {
      ...
      while (((i < List_len(type_param_vars)) && (i < List_len(type_params)))) {
        __ring_match75: {
          const __ring_m75 = [List_get(type_param_vars, i), List_get(type_params, i)];
          if (Array.isArray(__ring_m75) && __ring_m75.length === 2 && __ring_m75[0]._tag === "some" && __ring_m75[1]._tag === "some") {
            const var_id = __ring_m75[0]._0; const tp = __ring_m75[1]._0;
            _Map_insert(inst_map, var_id, tp);
