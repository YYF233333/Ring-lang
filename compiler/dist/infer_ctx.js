...
function generalize(env, t, subst) {
  ...
  for (const tv of type_vars) {
    __ring_match8: {
      const __ring_m8 = _Map_get(env.scope.var_bounds, tv);
...
function resolve_named_type(ctx, name, type_args, span) {
  ...
  __ring_match29: {
    const __ring_m29 = _Map_get(ctx.env.types.type_aliases, name);
...
function bind_constructor_pattern(ctx, name, qualifier, fields, expected_type, subst, span) {
  ...
  __ring_match36: {
    ...
    if (__ring_m36._tag === "some") {
      ...
      __ring_match37: {
        const __ring_m37 = _Map_get(ctx.env.types.enums, ename);
...
function bind_named_constructor_pattern(ctx, name, qualifier, fields, expected_type, subst, span) {
  ...
  __ring_match41: {
    ...
    if (__ring_m41._tag === "some") {
      ...
      __ring_match42: {
        const __ring_m42 = _Map_get(ctx.env.types.enums, ename);
