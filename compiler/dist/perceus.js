...
function is_fresh_owned_returning_call(callee) {
  __ring_match9: {
    const __ring_m9 = callee;
    return false;
    break __ring_match9;
  }
}
...
function rc_block_inner(stmts, tail, escape, owned, boxed, gensym) {
  ...
  const inner_owned = List_concat(owned, block_locals);
  let new_stmts = [];
  ...
  while (true) {
    ...
    const __ring_iter_4 = __List_Iterable.iter(rc_stmt(s, inner_owned, boxed, gensym));
    while (true) {
  ...
  const __ring_m = tail;
  if (__ring_m._tag === "some") { const t = __ring_m._0; return Option_some(rc_escape_or_value(t, escape, inner_owned, boxed, gensym)); }
  if (__ring_m._tag === "none") { return Option_none; }
  __match_fail(__ring_m);
})();
  if ((List_len(block_locals) === 0)) {
    return [new_stmts, new_tail];
  } else {
    if (block_diverges(new_stmts, new_tail)) {
      return [new_stmts, new_tail];
    } else {
      const drops = drops_for(block_locals);
      __ring_match11: {
        const __ring_m11 = new_tail;
        if (__ring_m11._tag === "some") {
          ...
          const __ring_iter_5 = __List_Iterable.iter(drops);
          while (true) {
        ...
        if (__ring_m11._tag === "none") {
          const __ring_iter_6 = __List_Iterable.iter(drops);
          while (true) {
  ...
  while (true) {
    ...
    __ring_match12: {
      ...
      if (__ring_m12._tag === "Let") {
        const name = __ring_m12.name; const init = __ring_m12.init;
...
function is_droppable_init(init) {
  __ring_match13: {
    ...
    if (__ring_m13._tag === "Call") {
      const callee = __ring_m13.callee;
      return (is_borrow_returning_call(callee) || is_fresh_owned_returning_call(callee));
      break __ring_match13;
    }
    return false;
    break __ring_match13;
...
function drops_for(names) {
  let out = [];
  const __ring_iter_8 = __List_Iterable.iter(names);
  while (true) {
    ...
    if (__ring_m15._tag === "Return") {
      ...
      __ring_match16: {
        ...
        if (__ring_m16._tag === "some") {
          ...
          const __ring_iter_10 = __List_Iterable.iter(drops_for(owned));
          while (true) {
        ...
        if (__ring_m16._tag === "none") {
          let out = [];
          const __ring_iter_11 = __List_Iterable.iter(drops_for(owned));
          while (true) {
  ...
  __ring_match17: {
    ...
    if (__ring_m17._tag === "Call") {
      ...
      while (true) {
        ...
        const new_a = (list_contains_int(sink, i) ? rc_escape(a, owned, boxed, gensym) : rc_expr(a, false, owned, boxed, gensym));
...
function list_contains_int(xs, x) {
  const __ring_iter_22 = __List_Iterable.iter(xs);
  while (true) {
...
function expr_diverges(expr) {
  __ring_match21: {
    ...
    if (__ring_m21._tag === "Block") {
      const stmts = __ring_m21.stmts; const tail = __ring_m21.tail;
      let any = false;
    ...
    if (__ring_m21._tag === "IfExpr") {
      const then_branch = __ring_m21.then_branch; const else_branch = __ring_m21.else_branch;
      __ring_match23: {
    ...
    if (__ring_m21._tag === "MatchExpr") {
      const arms = __ring_m21.arms;
