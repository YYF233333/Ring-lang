    ...
    for (const decl of program.decls) {
      __ring_match4: {
        ...
        if (__ring_m4._tag === "Fn") {
          const name = __ring_m4.name; const body = __ring_m4.body;
          let callees = set_new();
    ...
    while (changed) {
      ...
      for (const entry of _Map_entries(fn_callees)) {
        ...
        for (const callee of _Set_to_list(callees)) {
          __ring_match5: {
            const __ring_m5 = _Map_get(ctx.local_fn_effects, callee);
            if (__ring_m5._tag === "some") {
              const callee_effects = __ring_m5._0;
              __ring_match6: {
                const __ring_m6 = _Map_get(ctx.local_fn_effects, name);
  ...
  for (const decl of program.decls) {
    __ring_match7: {
      ...
      if (__ring_m7._tag === "Impl") {
        const target_type = __ring_m7.target_type; const trait_name = __ring_m7.trait_name; const methods = __ring_m7.methods;
      ...
      if (__ring_m7._tag === "Trait") {
        const name = __ring_m7.name; const methods = __ring_m7.methods;
...
function collect_local_calls(expr, local_names, out) {
  __ring_match16: {
    ...
    if (__ring_m16._tag === "Lambda") {
      const body = __ring_m16.body;
      return collect_local_calls(body, local_names, out);
      break __ring_match16;
    }
    if (__ring_m16._tag === "TryBlock") {
      const body = __ring_m16.body;
      return collect_local_calls(body, local_names, out);
      break __ring_match16;
    }
    if (__ring_m16._tag === "OptionUnwrap") {
      const expr = __ring_m16.expr;
      return collect_local_calls(expr, local_names, out);
      break __ring_match16;
    }
    if (__ring_m16._tag === "OptionOr") {
      const expr = __ring_m16.expr; const default_value = __ring_m16.default_value;
      collect_local_calls(expr, local_names, out);
      return collect_local_calls(default_value, local_names, out);
      break __ring_match16;
    }
...
function collect_local_calls_stmt(stmt, local_names, out) {
  __ring_match24: {
    ...
    if (__ring_m24._tag === "While") {
      const condition = __ring_m24.condition; const body = __ring_m24.body;
      collect_local_calls(condition, local_names, out);
      return collect_local_calls(body, local_names, out);
      break __ring_match24;
    }
    if (__ring_m24._tag === "ForIn") {
      const iterable = __ring_m24.iterable; const body = __ring_m24.body;
      collect_local_calls(iterable, local_names, out);
      return collect_local_calls(body, local_names, out);
      break __ring_match24;
    }
    if (__ring_m24._tag === "LetDestructure") {
      const init = __ring_m24.init;
      return collect_local_calls(init, local_names, out);
      break __ring_match24;
    }
    if (__ring_m24._tag === "IfLet") {
      const expr = __ring_m24.expr; const then_block = __ring_m24.then_block; const else_block = __ring_m24.else_block;
      collect_local_calls(expr, local_names, out);
