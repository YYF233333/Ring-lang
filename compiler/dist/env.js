...
function TypeEnv_bind(self, name, scheme) {
  ...
  __ring_match6: {
    ...
    if (__ring_m6._tag === "none") {
      __ring_blk0 = new TypeScheme(scheme.ty, scheme.type_vars, scheme.bounds, Option_some(TypeEnv_fresh_def_id(self)));
      break __ring_match6;
    }
...
function add_impl(reg, entry) {
  __ring_match13: {
    const __ring_m13 = _Map_get(reg.trait_impls, entry.target_type_name);
...
function apply_subst_row(subst, row) {
  ...
  __ring_match15: {
    ...
    if (__ring_m15._tag === "some") {
      ...
      __ring_match16: {
        const __ring_m16 = union_find$uf_lookup(subst, root_id);
    ...
    if (__ring_m18._tag === "RecordType") {
      ...
      __ring_match20: {
        ...
        if (__ring_m20._tag === "some") {
          ...
          const root_id = union_find$uf_find(subst, t_id);
          __ring_match21: {
            const __ring_m21 = union_find$uf_lookup(subst, root_id);
            if (__ring_m21._tag === "some") {
              ...
              __ring_match22: {
                ...
                if (__ring_m22._tag === "TypeVar") {
                  const new_id = __ring_m22.id; const new_name = __ring_m22.name;
                  return types$Type_RecordType(mapped_fields, Option_some(new_id), new_name);
                  break __ring_match22;
                }
                if (__ring_m22._tag === "RecordType") {
                  const extra_fields = __ring_m22.fields; const extra_tail = __ring_m22.tail; const extra_tn = __ring_m22.tail_name;
    ...
    if (__ring_m28._tag === "RecordType") {
      ...
      __ring_match29: {
        ...
        if (__ring_m29._tag === "some") {
          ...
          __ring_match30: {
            const __ring_m30 = _Map_get(subst, t_id);
            if (__ring_m30._tag === "some") {
              ...
              __ring_match31: {
                ...
                if (__ring_m31._tag === "TypeVar") {
                  const new_id = __ring_m31.id; const new_name = __ring_m31.name;
                  return types$Type_RecordType(mapped_fields, Option_some(new_id), new_name);
                  break __ring_match31;
                }
                if (__ring_m31._tag === "RecordType") {
                  const extra_fields = __ring_m31.fields; const extra_tail = __ring_m31.tail; const extra_tn = __ring_m31.tail_name;
                  let all_fields = list_clone(mapped_fields);
...
function find_impl(reg, type_name, trait_name) {
  __ring_match33: {
    const __ring_m33 = _Map_get(reg.trait_impls, type_name);
    if (__ring_m33._tag === "some") {
      const impls = __ring_m33._0;
...
function has_impl(reg, type_name, trait_name) {
  __ring_match34: {
    const __ring_m34 = _Map_get(reg.trait_impls, type_name);
    if (__ring_m34._tag === "some") {
      const impls = __ring_m34._0;
...
function lookup_variant(def, name) {
  __ring_match35: {
    const __ring_m35 = _Map_get(def.variant_index, name);
    if (__ring_m35._tag === "some") {
      const idx = __ring_m35._0;
