...
function mono(ty) {
  return new TypeScheme(ty, empty_ints(), empty_bounds(), Option_none);
}

function new_type_env() {
  const initial_scope = new Scope(map_new());
  return new TypeEnv(0, 0, [initial_scope], map_new(), map_new(), map_new(), map_new(), map_new(), map_new(), empty_impl_entries(), map_new(), map_new(), map_new(), set_new(), map_new());
}
