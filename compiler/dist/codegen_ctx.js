...
function qualify(ctx, name) {
  __ring_match1: {
    const __ring_m1 = ctx.imports_map;
    if (__ring_m1._tag === "some") {
      const imap = __ring_m1._0;
      if ((!_Set_contains(ctx.local_names, name, __Str_Eq))) {
        __ring_match2: {
          const __ring_m2 = _Map_get(imap, name);
          if (__ring_m2._tag === "some") {
            const qualified = __ring_m2._0;
            return qualified;
            break __ring_match2;
          }
  ...
  __ring_match3: {
    const __ring_m3 = ctx.module_prefix;
    if (__ring_m3._tag === "some") {
      const prefix = __ring_m3._0;
      if (_Set_contains(ctx.local_names, name, __Str_Eq)) {
