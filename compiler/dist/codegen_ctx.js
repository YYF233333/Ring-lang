...
function safe_ident(name) {
  if (_Set_contains(JS_RESERVED(), name)) {
    return `_${name}`;
  } else {
    return name;
  }
...
class CodegenCtx {
  constructor(lines, indent_level, impl_methods, struct_field_order, trait_decls, match_counter, dt_counter, loop_counter, module_prefix, imports_map, skip_preamble, skip_main_call, local_names, local_fn_effects, current_fn_effects, in_try_fail, module_imports, module_exports) {
    this.lines = lines;
...
function new_codegen_ctx(skip_preamble, skip_main_call) {
  return new CodegenCtx([], 0, map_new(), map_new(), map_new(), 0, 0, 0, Option_none, Option_none, skip_preamble, skip_main_call, set_new(), map_new(), Option_none, false, Option_none, Option_none);
}

function indent_str(level) {
  let result = "";
  const __ring_end0 = level;
...
function qualify(ctx, name) {
  __ring_match0: {
    const __ring_m0 = ctx.imports_map;
    if (__ring_m0._tag === "some") {
      const imap = __ring_m0._0;
      if ((!_Set_contains(ctx.local_names, name))) {
        __ring_match1: {
          const __ring_m1 = _Map_get(imap, name);
          if (__ring_m1._tag === "some") {
            const qualified = __ring_m1._0;
            return qualified;
            break __ring_match1;
          }
...
function extract_effect_names(effects) {
  ...
  for (const e of effects.effects) {
    ...
    if ((List_contains(names, n) === false)) {
      List_push(names, n);
    }
  }
  List_sort(names);
