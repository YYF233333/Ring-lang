...
function compile_phases(entry_file) {
  __ring_match1: {
    ...
    if (__ring_m1._tag === "some") {
      ...
      for (const key of graph.topo_order) {
        if (check_ok) {
          __ring_match3: {
            const __ring_m3 = _Map_get(module_asts, key);
            if (__ring_m3._tag === "some") {
              const ast = __ring_m3._0;
  ...
  __match_fail(__ring_m);
              ...
              for (const dk of deps) {
                __ring_match4: {
                  const __ring_m4 = _Map_get(module_exports_map, dk);
  ...
  __match_fail(__ring_m);
                ...
                __ring_match5: {
                  const __ring_m5 = _Map_get(graph.modules, key);
                  if (__ring_m5._tag === "some") {
                    const mod_ = __ring_m5._0;
  ...
  __ring_match6: {
    ...
    if (__ring_m6._tag === "some") {
      ...
      for (const key of phases.graph.topo_order) {
        __ring_match7: {
          const __ring_m7 = [_Map_get(phases.graph.modules, key), _Map_get(phases.module_hirs, key)];
          if (Array.isArray(__ring_m7) && __ring_m7.length === 2 && __ring_m7[0]._tag === "some" && __ring_m7[1]._tag === "some") {
            const mod_ = __ring_m7[0]._0; const hir = __ring_m7[1]._0;
            const prefix = resolver$module_prefix(mod_.path_segments);
...
function compile_project_esm(entry_file, out_dir) {
  __ring_match8: {
    const __ring_m8 = compile_phases(entry_file);
    if (__ring_m8._tag === "none") {
      return new EsmCompileResult(false, "");
      break __ring_match8;
    }
    if (__ring_m8._tag === "some") {
      const phases = __ring_m8._0;
      const entry_key = resolver$module_key(phases.graph.entry.path_segments);
      ...
      for (const key of phases.graph.topo_order) {
        __ring_match9: {
          const __ring_m9 = [_Map_get(phases.graph.modules, key), _Map_get(phases.module_hirs, key), _Map_get(phases.module_asts, key)];
          if (Array.isArray(__ring_m9) && __ring_m9.length === 3 && __ring_m9[0]._tag === "some" && __ring_m9[1]._tag === "some" && __ring_m9[2]._tag === "some") {
            const mod_ = __ring_m9[0]._0; const hir = __ring_m9[1]._0; const ast = __ring_m9[2]._0;
  ...
  __match_fail(__ring_m);
            ...
            for (const dk of deps) {
              __ring_match10: {
                const __ring_m10 = [_Map_get(phases.module_exports_map, dk), _Map_get(phases.graph.modules, dk)];
                if (Array.isArray(__ring_m10) && __ring_m10.length === 2 && __ring_m10[0]._tag === "some" && __ring_m10[1]._tag === "some") {
                  const dep_exports = __ring_m10[0]._0; const dep_mod = __ring_m10[1]._0;
            ...
            for (const use_decl of ast.uses) {
              __ring_match13: {
                const __ring_m13 = use_decl.imports;
                if (__ring_m13._tag === "NamedItems") {
                  const names = __ring_m13.names;
                  for (const item of names) {
                    __ring_match14: {
                      const __ring_m14 = item.alias;
                      if (__ring_m14._tag === "some") {
                        const alias = __ring_m14._0;
                        __ring_match15: {
                          const __ring_m15 = _Map_get(imports_map, item.name);
                  ...
                  if ((!_Map_contains_key(imports_map, name))) {
                    for (const eentry of _Map_entries(phases.module_exports_map)) {
                      ...
                      if ((((other_key !== key) && _Map_contains_key(other_exports.values, name)) && (!_Set_contains(other_exports.extern_values, name, __Str_Eq)))) {
                        __ring_match17: {
                          const __ring_m17 = _Map_get(phases.graph.modules, other_key);
                          if (__ring_m17._tag === "some") {
                            const other_mod = __ring_m17._0;
            ...
            for (const decl of ast.decls) {
              __ring_match18: {
                ...
                if (__ring_m18._tag === "Fn") {
                  const name = __ring_m18.name; const is_pub = __ring_m18.is_pub;
                  if (is_pub) {
                ...
                if (__ring_m18._tag === "Enum") {
                  const name = __ring_m18.name; const is_pub = __ring_m18.is_pub; const variants = __ring_m18.variants;
                  if (is_pub) {
                ...
                if (__ring_m18._tag === "ModBlock") {
                  const mod_name = __ring_m18.name; const mod_decls = __ring_m18.decls; const mpub = __ring_m18.is_pub;
                  if (mpub) {
                    for (const subdecl of mod_decls) {
                      const prefixed = infer_register$prefix_decl_name(mod_name, subdecl);
                      __ring_match19: {
                        const __ring_m19 = prefixed;
                        if (__ring_m19._tag === "Fn") {
                          const fname = __ring_m19.name; const fpub = __ring_m19.is_pub;
                          if (fpub) {
                        ...
                        if (__ring_m19._tag === "Struct") {
                          const sname = __ring_m19.name; const spub = __ring_m19.is_pub;
                          if (spub) {
                        ...
                        if (__ring_m19._tag === "Enum") {
                          const ename = __ring_m19.name; const epub = __ring_m19.is_pub; const variants = __ring_m19.variants;
                          if (epub) {
                            for (const v of variants) {
                        ...
                        if (__ring_m19._tag === "Const") {
                          const cname = __ring_m19.name; const cpub = __ring_m19.is_pub;
                          if (cpub) {
                        ...
                        if (__ring_m19._tag === "ModBlock") {
                          const sub_mod_name = __ring_m19.name; const sub_mod_decls = __ring_m19.decls; const sub_mpub = __ring_m19.is_pub;
                          if (sub_mpub) {
                            for (const sub_subdecl of sub_mod_decls) {
                              const sub_prefixed = infer_register$prefix_decl_name(sub_mod_name, sub_subdecl);
                              __ring_match20: {
                                const __ring_m20 = sub_prefixed;
                                if (__ring_m20._tag === "Fn") {
                                  const fname2 = __ring_m20.name; const fpub2 = __ring_m20.is_pub;
                                  if (fpub2) {
                                    List_push(export_names, codegen_ctx$safe_ident(fname2));
                                  }
                                  break __ring_match20;
                                }
                                if (__ring_m20._tag === "Struct") {
                                  const sname2 = __ring_m20.name; const spub2 = __ring_m20.is_pub;
                                  if (spub2) {
                                    List_push(export_names, codegen_ctx$safe_ident(sname2));
                                  }
                                  break __ring_match20;
                                }
                                if (__ring_m20._tag === "Enum") {
                                  const ename2 = __ring_m20.name; const epub2 = __ring_m20.is_pub; const vars2 = __ring_m20.variants;
                                  if (epub2) {
                                    for (const v of vars2) {
                                      const sn2 = codegen_ctx$safe_ident(ename2);
                                      List_push(export_names, `${sn2}_${v.name}`);
                                    }
                                  }
                                  break __ring_match20;
                                }
                                if (__ring_m20._tag === "Const") {
                                  const cname2 = __ring_m20.name; const cpub2 = __ring_m20.is_pub;
                                  if (cpub2) {
                                    List_push(export_names, codegen_ctx$safe_ident(cname2));
                                  }
                                  break __ring_match20;
                                }
                                break __ring_match20;
            ...
            for (const decl of ast.decls) {
              __ring_match21: {
                ...
                if (__ring_m21._tag === "Impl") {
                  const target_type = __ring_m21.target_type; const impl_trait = __ring_m21.trait_name; const methods = __ring_m21.methods;
                  let is_pub_type = false;
                  for (const d of ast.decls) {
                    __ring_match22: {
                      const __ring_m22 = d;
                      if (__ring_m22._tag === "Struct") {
                        const dn = __ring_m22.name; const dp = __ring_m22.is_pub;
            ...
            for (const use_decl of ast.uses) {
              if (use_decl.is_pub) {
                ...
                __ring_match26: {
                  const __ring_m26 = _Map_get(phases.module_exports_map, src_key);
                  if (__ring_m26._tag === "some") {
                    const src_exports = __ring_m26._0;
                    const src_prefix = src_exports.module_prefix;
                    __ring_match27: {
                      const __ring_m27 = use_decl.imports;
                      if (__ring_m27._tag === "NamedItems") {
                        const names = __ring_m27.names;
                        for (const item of names) {
  ...
  __match_fail(__ring_m);
                      ...
                      if (__ring_m27._tag === "Module") {
                        ...
                        for (const tentry of _Map_entries(src_exports.types)) {
                          ...
                          List_push(export_names, local_js);
                          __ring_match30: {
                            const __ring_m30 = tdef;
                            if (__ring_m30._tag === "EnumDef_") {
...
function build_imports_map(graph, exports_map, key) {
  ...
  __ring_match31: {
    const __ring_m31 = _Map_get(graph.dependencies, key);
    if (__ring_m31._tag === "some") {
      const deps = __ring_m31._0;
      for (const dk of deps) {
        __ring_match32: {
          const __ring_m32 = _Map_get(exports_map, dk);
          if (__ring_m32._tag === "some") {
            const dep_exports = __ring_m32._0;
            const dep_prefix = dep_exports.module_prefix;
            let bare_variants = set_new();
            for (const tentry of _Map_entries(dep_exports.types)) {
...
function build_external_impl_methods(graph, exports_map, key) {
  let result = map_new();
  __ring_match37: {
    const __ring_m37 = _Map_get(graph.dependencies, key);
    if (__ring_m37._tag === "some") {
      const deps = __ring_m37._0;
      for (const dk of deps) {
        __ring_match38: {
          const __ring_m38 = _Map_get(exports_map, dk);
          if (__ring_m38._tag === "some") {
            const dep_exports = __ring_m38._0;
            const dep_prefix = dep_exports.module_prefix;
