        ...
        if (parse_ok) {
          __ring_match1: {
            ...
            if (__ring_m1._tag === "some") {
              ...
              const ast = parser$parse(source, mod_.file_path);
  ...
  __match_fail(__ring_m);
            ...
            for (const decl of ast.decls) {
              __ring_match18: {
                ...
                if (__ring_m18._tag === "Impl") {
                  const target_type = __ring_m18.target_type; const impl_trait = __ring_m18.trait_name; const methods = __ring_m18.methods;
                  let is_pub_type = false;
                  for (const d of ast.decls) {
                    __ring_match19: {
                      const __ring_m19 = d;
                      if (__ring_m19._tag === "Struct") {
                        const dn = __ring_m19.name; const dp = __ring_m19.is_pub;
            ...
            for (const use_decl of ast.uses) {
              if (use_decl.is_pub) {
                ...
                __ring_match23: {
                  const __ring_m23 = _Map_get(phases.module_exports_map, src_key);
                  if (__ring_m23._tag === "some") {
                    const src_exports = __ring_m23._0;
                    const src_prefix = src_exports.module_prefix;
                    __ring_match24: {
                      const __ring_m24 = use_decl.imports;
...
function build_imports_map(graph, exports_map, key) {
  ...
  __ring_match28: {
    const __ring_m28 = _Map_get(graph.dependencies, key);
    if (__ring_m28._tag === "some") {
      const deps = __ring_m28._0;
      for (const dk of deps) {
        __ring_match29: {
          const __ring_m29 = _Map_get(exports_map, dk);
          if (__ring_m29._tag === "some") {
            const dep_exports = __ring_m29._0;
            const dep_prefix = dep_exports.module_prefix;
            let bare_variants = set_new();
            for (const tentry of _Map_entries(dep_exports.types)) {
...
function build_external_struct_fields(graph, exports_map, key) {
  let result = map_new();
  __ring_match32: {
    const __ring_m32 = _Map_get(graph.dependencies, key);
    if (__ring_m32._tag === "some") {
      const deps = __ring_m32._0;
      for (const dk of deps) {
        __ring_match33: {
          const __ring_m33 = _Map_get(exports_map, dk);
          if (__ring_m33._tag === "some") {
            const dep_exports = __ring_m33._0;
            const dep_prefix = dep_exports.module_prefix;
