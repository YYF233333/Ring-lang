...
function gen_expr(ctx, expr) {
  ...
  __match_fail(__ring_m);
    ...
    if (__ring_m0._tag === "IndexExpr") {
      const receiver = __ring_m0.receiver; const index = __ring_m0.index;
      const r = gen_expr(ctx, receiver);
      const i = gen_expr(ctx, index);
      const recv_ty = hir$hexpr_type(receiver);
      __ring_match3: {
        const __ring_m3 = recv_ty;
...
function gen_binop(ctx, op, left, right, eq_dispatch, ord_dispatch) {
  __ring_match4: {
    const __ring_m4 = try_eq_dispatch(ctx, op, left, right, eq_dispatch);
    if (__ring_m4._tag === "some") {
      const result = __ring_m4._0;
      return result;
      break __ring_match4;
    }
  ...
  __ring_match5: {
    const __ring_m5 = try_ord_dispatch(ctx, op, left, right, ord_dispatch);
        ...
        if (__ring_m16._tag === "some") {
          ...
          if ((List_len(actual_effects.effects) > 0)) {
            ...
            for (const e of actual_effects.effects) {
              if (_Set_contains(caller_effect_names, types$effect_kind_name(e))) {
                List_push(needed, e);
              }
            }
...
function has_fail_effect(expr) {
  __ring_match46: {
    ...
    if (__ring_m46._tag === "IntLit") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "FloatLit") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "StrLit") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "BoolLit") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    ...
    if (__ring_m46._tag === "BinOp") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "UnaryOp") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "Call") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "FieldAccess") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "StructLit") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "NamedVariantConstruct") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "MatchExpr") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    ...
    if (__ring_m46._tag === "IfExpr") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "StringInterp") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "TryCatch") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "HandleExpr") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "Lambda") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "EffectOp") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "RangeExpr") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "ListLit") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "TupleLit") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
    if (__ring_m46._tag === "IndexExpr") {
      const effects = __ring_m46.effects;
      return check_fail(effects);
      break __ring_match46;
    }
...
function gen_handle(ctx, body, handlers) {
  ...
  for (const h of handlers) {
    __ring_match48: {
      const __ring_m48 = _Map_get(by_effect, h.effect_name);
      if (__ring_m48._tag === "some") {
        const existing = __ring_m48._0;
...
function gen_lambda(ctx, params, body, ty) {
  ...
  __ring_match51: {
    ...
    if (__ring_m51._tag === "FnType") {
      const effects = __ring_m51.effects;
...
function gen_lambda_capture_evidence(ctx, args, idx) {
  __ring_match52: {
    const __ring_m52 = List_get(args, idx);
    if (__ring_m52._tag === "some") {
      const arg = __ring_m52._0;
      __ring_match53: {
        const __ring_m53 = arg;
        if (__ring_m53._tag === "Lambda") {
          const params = __ring_m53.params; const body = __ring_m53.body;
          let p_names = [""];
          List_clear(p_names);
          for (const p of params) {
            List_push(p_names, codegen_ctx$safe_ident(p.name));
        ...
        __ring_match54: {
          const __ring_m54 = arg_type;
          if (__ring_m54._tag === "FnType") {
            const params = __ring_m54.params;
