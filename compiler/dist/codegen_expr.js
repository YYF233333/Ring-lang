...
function has_fail_effect(expr) {
  __ring_match45: {
    ...
    if (__ring_m45._tag === "IntLit") {
      const effects = __ring_m45.effects;
...
function gen_handle_body(ctx, expr, ev_params) {
  __ring_match48: {
    ...
    if (__ring_m48._tag === "Block") {
      const stmts = __ring_m48.stmts; const tail = __ring_m48.tail;
      __ring_match49: {
...
function gen_lambda(ctx, params, body, ty) {
  ...
  __ring_match50: {
    ...
    if (__ring_m50._tag === "FnType") {
      const effects = __ring_m50.effects;
...
function gen_lambda_capture_evidence(ctx, args, idx) {
  __ring_match51: {
    const __ring_m51 = List_get(args, idx);
    if (__ring_m51._tag === "some") {
      const arg = __ring_m51._0;
      __ring_match52: {
        const __ring_m52 = arg;
        if (__ring_m52._tag === "Lambda") {
          const params = __ring_m52.params; const body = __ring_m52.body;
          let p_names = [""];
          List_clear(p_names);
          for (const p of params) {
            List_push(p_names, codegen_ctx$safe_ident(p.name));
        ...
        __ring_match53: {
          const __ring_m53 = arg_type;
          if (__ring_m53._tag === "FnType") {
            const params = __ring_m53.params;
