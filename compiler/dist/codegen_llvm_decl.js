...
class DerivedFnScaffold {
  constructor(fn_val, self_val, saved_fn, saved_bb) {
    this.fn_val = fn_val;
    this.self_val = self_val;
    this.saved_fn = saved_fn;
    this.saved_bb = saved_bb;
  }
}

function collect_trait_dict_params(bounds, trait_name) {
  let params = [];
  const __ring_iter_2 = __List_Iterable.iter(bounds);
  while (true) {
...
function begin_derived_fn(ctx, type_name, method_name, trait_name, is_binary, bounds) {
  const mangled = codegen_llvm_ctx$llvm_mangle_method(type_name, method_name);
  __ring_match6: {
    const __ring_m6 = _Map_get(ctx.functions, mangled);
  ...
  const dict_offset = (is_binary ? 2 : 1);
  setup_derived_dict_params(ctx, fn_val, dict_params, dict_offset);
  const self_val = LLVMGetParam(fn_val, 0);
  const scaffold = new DerivedFnScaffold(fn_val, self_val, saved_fn, saved_bb);
  return Option_some([scaffold, dict_params]);
}
...
function collect_all_supertraits_llvm(ctx, trait_name) {
  ...
  let stack = [];
  __ring_match7: {
    const __ring_m7 = _Map_get(ctx.trait_supertraits, trait_name);
    if (__ring_m7._tag === "some") {
      const supers = __ring_m7._0;
      const __ring_iter_5 = __List_Iterable.iter(supers);
      while (true) {
        const __ring_next_5 = __ListIterator_Iterator.next(__ring_iter_5);
        if (__ring_next_5._tag === "none") break;
        const st = __ring_next_5._0;
        List_push(stack, st);
      }
      break __ring_match7;
    }
  ...
  while ((List_len(stack) > 0)) {
    ...
    __ring_match8: {
      const __ring_m8 = _Map_get(ctx.trait_supertraits, current);
      if (__ring_m8._tag === "some") {
        const parent_supers = __ring_m8._0;
        const __ring_iter_6 = __List_Iterable.iter(parent_supers);
        while (true) {
          const __ring_next_6 = __ListIterator_Iterator.next(__ring_iter_6);
          if (__ring_next_6._tag === "none") break;
          const ps = __ring_next_6._0;
          List_push(stack, ps);
        }
        break __ring_match8;
      }
...
function end_derived_fn(ctx, scaffold) {
  ctx.current_fn = scaffold.saved_fn;
  return LLVMPositionBuilderAtEnd(ctx.builder, scaffold.saved_bb);
}

function emit_clone_fn(ctx, type_name) {
  const empty_bounds = [];
  const scaffold_result = begin_derived_fn(ctx, type_name, "clone", "Clone", false, empty_bounds);
  if (Option_is_none(scaffold_result)) {
    return;
  }
  const __ring_dt0 = Option_unwrap(scaffold_result);
  const scaffold = __ring_dt0[0];
  const _dict_params = __ring_dt0[1];
  const self_val = scaffold.self_val;
  ...
  codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, dup_ty, dup_fn, [self_val], ""));
  LLVMBuildRet(ctx.builder, self_val);
  return end_derived_fn(ctx, scaffold);
}
...
function emit_default_method_thunk(ctx, default_fn_name, default_fn, target_type, trait_name) {
  ...
  __ring_match9: {
    const __ring_m9 = _Map_get(ctx.functions, thunk_name);
    if (__ring_m9._tag === "some") {
      const existing = __ring_m9._0;
      return existing;
      break __ring_match9;
    }
  ...
  let __ring_blk0;
  __ring_match10: {
    const __ring_m10 = _Map_get(ctx.fn_types, default_fn_name);
    ...
    if (__ring_m10._tag === "none") {
      let pts = [];
      const __ring_end8 = default_arity;
      for (let i = 0; i < __ring_end8; i++) {
        List_push(pts, ctx.ptr_type);
      }
      __ring_blk0 = LLVMFunctionType(ctx.ptr_type, pts, 0);
      break __ring_match10;
    }
    __match_fail(__ring_m10);
  }
  const default_ty = __ring_blk0;
  ...
  const __ring_end10 = thunk_arity;
...
function emit_dict_method_thunk(ctx, mangled, method_fn) {
  ...
  __ring_match11: {
    const __ring_m11 = _Map_get(ctx.functions, thunk_name);
  ...
  let __ring_blk1;
  __ring_match12: {
    const __ring_m12 = _Map_get(ctx.fn_types, mangled);
    ...
    if (__ring_m12._tag === "none") {
      ...
      const __ring_end12 = method_arity;
      for (let i = 0; i < __ring_end12; i++) {
        List_push(method_param_types, ctx.ptr_type);
      }
      __ring_blk1 = LLVMFunctionType(ctx.ptr_type, method_param_types, 0);
      break __ring_match12;
    }
    __match_fail(__ring_m12);
  }
  const method_ty = __ring_blk1;
  const saved_block = LLVMGetInsertBlock(ctx.builder);
  const entry = LLVMAppendBasicBlockInContext(ctx.context, thunk_fn, "entry");
  LLVMPositionBuilderAtEnd(ctx.builder, entry);
  let fwd_args = [];
  const __ring_end13 = method_arity;
...
function emit_dict_method_slot(ctx, target_type, trait_name, method_name, dict_struct_ty, dict_ptr, closure_ty, closure_size, alloc_fn, alloc_ty, slot_idx) {
  ...
  __ring_match13: {
    const __ring_m13 = _Map_get(ctx.functions, mangled);
    if (__ring_m13._tag === "some") {
      const method_fn = __ring_m13._0;
    ...
    if (__ring_m13._tag === "none") {
      ...
      __ring_match14: {
        const __ring_m14 = _Map_get(ctx.functions, default_fn_name);
        if (__ring_m14._tag === "some") {
          const default_fn = __ring_m14._0;
...
function emit_derived_trait_dict(ctx, target_type, trait_name) {
  ...
  let __ring_blk2;
  __ring_match15: {
    const __ring_m15 = _Map_get(ctx.trait_method_order, trait_name);
    if (__ring_m15._tag === "some") {
      const order = __ring_m15._0;
      __ring_blk2 = order;
      break __ring_match15;
    }
    if (__ring_m15._tag === "none") {
      __ring_blk2 = panic("unreachable");
      break __ring_match15;
    }
    __match_fail(__ring_m15);
  }
  const method_order = __ring_blk2;
  ...
  __ring_match16: {
    const __ring_m16 = _Map_get(ctx.functions, build_fn_name);
  ...
  for (let i = 0; i < __ring_end15; i++) {
    __ring_match17: {
      const __ring_m17 = List_get(method_order, i);
      if (__ring_m17._tag === "some") {
        const method_name = __ring_m17._0;
...
function resolve_dict_for_derived(ctx, name) {
  __ring_match18: {
    const __ring_m18 = _Map_get(ctx.named_values, name);
    if (__ring_m18._tag === "some") {
      const alloca = __ring_m18._0;
  ...
  __ring_match19: {
    const __ring_m19 = _Map_get(ctx.functions, init_fn_name);
    if (__ring_m19._tag === "some") {
      const init_fn = __ring_m19._0;
      let __ring_blk3;
      __ring_match20: {
        const __ring_m20 = _Map_get(ctx.fn_types, init_fn_name);
        ...
        if (__ring_m20._tag === "none") {
          __ring_blk3 = LLVMFunctionType(ctx.ptr_type, [], 0);
          break __ring_match20;
        }
        __match_fail(__ring_m20);
      }
      const init_fn_ty = __ring_blk3;
      return LLVMBuildCall2(ctx.builder, init_fn_ty, init_fn, [], codegen_llvm_ctx$fresh_name(ctx, "dict"));
      break __ring_match19;
    }
    if (__ring_m19._tag === "none") {
      __ring_match21: {
        const __ring_m21 = _Map_get(ctx.dict_globals, name);
        if (__ring_m21._tag === "some") {
          const getter_fn = __ring_m21._0;
...
function emit_identity_to_debug_str(ctx, val) {
  ...
  __ring_match22: {
    ...
    if (__ring_m22._tag === "none") {
      __ring_blk4 = panic("LLVM codegen: emit_identity_to_debug_str outside function");
      break __ring_match22;
    }
  ...
  const is_tagged = LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_EQ, tag_bit, one, codegen_llvm_ctx$fresh_name(ctx, "tag"));
  ...
  codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, is_tagged, tagged_bb, heap_bb));
  ...
  const dup_fn = codegen_llvm_ctx$get_or_declare_runtime_fn(ctx, "ring_dup", [ctx.ptr_type], ctx.void_type);
  const dup_ty = codegen_llvm_ctx$get_rt_fn_type(ctx, "ring_dup");
  codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, dup_ty, dup_fn, [val], ""));
...
function emit_tuple_debug_str(ctx, val, element_actions) {
  ...
  codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, sb_add_ty, sb_add_fn, [sb, open_paren], codegen_llvm_ctx$fresh_name(ctx, "sba")));
  codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, drop_ty, drop_fn, [open_paren], ""));
  const __ring_end17 = List_len(element_actions);
  for (let i = 0; i < __ring_end17; i++) {
    if ((i > 0)) {
      const sep = gen_str_lit_simple(ctx, ", ");
      codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, sb_add_ty, sb_add_fn, [sb, sep], codegen_llvm_ctx$fresh_name(ctx, "sba")));
      codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, drop_ty, drop_fn, [sep], ""));
    }
    ...
    codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, sb_add_ty, sb_add_fn, [sb, elem_str], codegen_llvm_ctx$fresh_name(ctx, "sba")));
    codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, drop_ty, drop_fn, [elem_str], ""));
  }
  const close_paren = gen_str_lit_simple(ctx, ")");
  codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, sb_add_ty, sb_add_fn, [sb, close_paren], codegen_llvm_ctx$fresh_name(ctx, "sba")));
  codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, drop_ty, drop_fn, [close_paren], ""));
  ...
  codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, drop_ty, drop_fn, [sb], ""));
  return result;
}
...
function emit_debug_field_to_str(ctx, val, action) {
  __ring_match23: {
    ...
    if (__ring_m23._tag === "FloatIdentity") {
    ...
    if (__ring_m23._tag === "BoolIdentity") {
      const raw = codegen_llvm_expr$unbox_int(ctx, val);
    ...
    if (__ring_m23._tag === "Tuple") {
      const element_actions = __ring_m23.element_actions;
      return emit_tuple_debug_str(ctx, val, element_actions);
      break __ring_match23;
    }
    if (__ring_m23._tag === "FnLiteral") {
...
function emit_enum_variant_debug_str(ctx, self_val, type_name, variant, enum_info) {
  ...
  if (variant.has_named_fields) {
    ...
    codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, sb_add_ty, sb_add_fn, [sb, prefix], codegen_llvm_ctx$fresh_name(ctx, "sba")));
    codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, drop_ty, drop_fn, [prefix], ""));
  } else {
    const prefix = gen_str_lit_simple(ctx, `${variant.name}(`);
    codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, sb_add_ty, sb_add_fn, [sb, prefix], codegen_llvm_ctx$fresh_name(ctx, "sba")));
    codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, drop_ty, drop_fn, [prefix], ""));
  ...
  const __ring_end18 = List_len(variant.fields);
  for (let fi = 0; fi < __ring_end18; fi++) {
    ...
    if (variant.has_named_fields) {
      ...
      codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, sb_add_ty, sb_add_fn, [sb, label], codegen_llvm_ctx$fresh_name(ctx, "sba")));
      codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, drop_ty, drop_fn, [label], ""));
    }
    ...
    codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, sb_add_ty, sb_add_fn, [sb, str_val], codegen_llvm_ctx$fresh_name(ctx, "sba")));
    codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, drop_ty, drop_fn, [str_val], ""));
  }
  const suffix = (variant.has_named_fields ? gen_str_lit_simple(ctx, " }") : gen_str_lit_simple(ctx, ")"));
  codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, sb_add_ty, sb_add_fn, [sb, suffix], codegen_llvm_ctx$fresh_name(ctx, "sba")));
  codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, drop_ty, drop_fn, [suffix], ""));
  const sb_to_str_fn = codegen_llvm_ctx$get_or_declare_runtime_fn(ctx, "ring_sb_to_str", [ctx.ptr_type], ctx.ptr_type);
  const sb_to_str_ty = codegen_llvm_ctx$get_rt_fn_type(ctx, "ring_sb_to_str");
  const result = LLVMBuildCall2(ctx.builder, sb_to_str_ty, sb_to_str_fn, [sb], codegen_llvm_ctx$fresh_name(ctx, "dbg"));
  codegen_llvm_expr$discard(LLVMBuildCall2(ctx.builder, drop_ty, drop_fn, [sb], ""));
...
function load_enum_tag(ctx, self_val) {
  ...
  return LLVMBuildLoad2(ctx.builder, ctx.i64_type, tag_ptr, codegen_llvm_ctx$fresh_name(ctx, "tv"));
}

function emit_enum_debug_fn(ctx, type_name, variants, bounds) {
  const scaffold_result = begin_derived_fn(ctx, type_name, "debug", "Debug", false, bounds);
  if (Option_is_none(scaffold_result)) {
    return;
  }
  const __ring_dt1 = Option_unwrap(scaffold_result);
  const scaffold = __ring_dt1[0];
  const _dict_params = __ring_dt1[1];
  const fn_val = scaffold.fn_val;
  const self_val = scaffold.self_val;
  ...
  let __ring_blk5;
  __ring_match24: {
    const __ring_m24 = _Map_get(ctx.enum_types, type_name);
    if (__ring_m24._tag === "some") {
      const info = __ring_m24._0;
      __ring_blk5 = info;
      break __ring_match24;
    }
    if (__ring_m24._tag === "none") {
      __ring_blk5 = panic("unreachable");
      break __ring_match24;
    }
    __match_fail(__ring_m24);
  }
  const enum_info = __ring_blk5;
  const tag_val = load_enum_tag(ctx, self_val);
  ...
  let incoming_vals = [];
  let incoming_bbs = [];
  const __ring_end19 = List_len(variants);
  for (let vi = 0; vi < __ring_end19; vi++) {
    const variant = __ring_index(variants, vi);
    let __ring_blk6;
    __ring_match25: {
      const __ring_m25 = _Map_get(enum_info.variants, variant.name);
      if (__ring_m25._tag === "some") {
        const vinfo = __ring_m25._0;
        __ring_blk6 = vinfo.tag;
...
function find_field_index(field_names, target) {
  const __ring_end20 = List_len(field_names);
...
function emit_struct_debug_fn(ctx, type_name, fields, bounds) {
  ...
  const __ring_dt2 = Option_unwrap(scaffold_result);
  const scaffold = __ring_dt2[0];
  const _dict_params = __ring_dt2[1];
  const self_val = scaffold.self_val;
  ...
  let __ring_blk7;
  __ring_match26: {
    const __ring_m26 = _Map_get(ctx.struct_types, type_name);
    if (__ring_m26._tag === "some") {
      const info = __ring_m26._0;
      __ring_blk7 = info;
      break __ring_match26;
    }
    if (__ring_m26._tag === "none") {
      __ring_blk7 = panic("unreachable");
      break __ring_match26;
    }
    __match_fail(__ring_m26);
  }
  const struct_info = __ring_blk7;
  const __ring_end21 = List_len(fields);
...
function emit_derived_debug_llvm(ctx, di) {
  ...
  let emitted = false;
  __ring_match27: {
    const __ring_m27 = di.type_kind;
    if (__ring_m27._tag === "StructKind") {
      __ring_match28: {
        const __ring_m28 = di.struct_fields;
        if (__ring_m28._tag === "some") {
          const fields = __ring_m28._0;
    ...
    if (__ring_m27._tag === "EnumKind") {
      __ring_match29: {
        const __ring_m29 = di.enum_variants;
        if (__ring_m29._tag === "some") {
          const variants = __ring_m29._0;
...
function emit_dict_eq_call(ctx, lhs, rhs, dict_name, extra_dicts) {
  ...
  return LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_NE, raw, LLVMConstInt(ctx.i64_type, 0, 0), codegen_llvm_ctx$fresh_name(ctx, "di1"));
}
...
function emit_float_identity_eq_cmp(ctx, lhs, rhs) {
  ...
  return LLVMBuildFCmp(ctx.builder, codegen_llvm_ctx$LLVM_REAL_OEQ, lhs_raw, rhs_raw, codegen_llvm_ctx$fresh_name(ctx, "feq"));
}

function emit_identity_eq_cmp(ctx, lhs, rhs) {
  let __ring_blk8;
  __ring_match30: {
    const __ring_m30 = ctx.current_fn;
    ...
    if (__ring_m30._tag === "none") {
      __ring_blk8 = panic("LLVM codegen: emit_identity_eq_cmp outside function");
      break __ring_match30;
    }
  ...
  const is_tagged = LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_EQ, lhs_tag, one, codegen_llvm_ctx$fresh_name(ctx, "tag"));
  ...
  codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, is_tagged, tagged_bb, heap_bb));
  LLVMPositionBuilderAtEnd(ctx.builder, tagged_bb);
  const tagged_eq = LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_EQ, lhs_int, rhs_int, codegen_llvm_ctx$fresh_name(ctx, "teq"));
  const tagged_end = LLVMGetInsertBlock(ctx.builder);
  codegen_llvm_expr$discard(LLVMBuildBr(ctx.builder, merge_bb));
  LLVMPositionBuilderAtEnd(ctx.builder, heap_bb);
  ...
  const heap_eq = LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_NE, heap_result, LLVMConstInt(ctx.i64_type, 0, 0), codegen_llvm_ctx$fresh_name(ctx, "hne"));
  const heap_end = LLVMGetInsertBlock(ctx.builder);
  codegen_llvm_expr$discard(LLVMBuildBr(ctx.builder, merge_bb));
  LLVMPositionBuilderAtEnd(ctx.builder, merge_bb);
...
function emit_tuple_eq_cmp(ctx, lhs, rhs, element_actions) {
  if ((List_len(element_actions) === 0)) {
    return LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_EQ, LLVMConstInt(ctx.i64_type, 0, 0), LLVMConstInt(ctx.i64_type, 0, 0), codegen_llvm_ctx$fresh_name(ctx, "ttrue"));
  }
  let __ring_blk9;
  __ring_match31: {
    const __ring_m31 = ctx.current_fn;
    ...
    if (__ring_m31._tag === "none") {
      __ring_blk9 = panic("LLVM codegen: emit_tuple_eq_cmp outside function");
      break __ring_match31;
    }
    __match_fail(__ring_m31);
  }
  const current_fn = __ring_blk9;
  const get_fn = codegen_llvm_ctx$get_or_declare_runtime_fn(ctx, "ring_list_get", [ctx.ptr_type, ctx.i64_type], ctx.ptr_type);
  const get_ty = codegen_llvm_ctx$get_rt_fn_type(ctx, "ring_list_get");
  ...
  const __ring_end23 = List_len(element_actions);
  for (let i = 0; i < __ring_end23; i++) {
    ...
    codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, elem_eq, next_bb, ret_false_bb));
  ...
  const true_i1 = LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_EQ, LLVMConstInt(ctx.i64_type, 0, 0), LLVMConstInt(ctx.i64_type, 0, 0), codegen_llvm_ctx$fresh_name(ctx, "t"));
  const false_i1 = LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_NE, LLVMConstInt(ctx.i64_type, 0, 0), LLVMConstInt(ctx.i64_type, 0, 0), codegen_llvm_ctx$fresh_name(ctx, "f"));
...
function emit_field_eq_cmp(ctx, lhs, rhs, action) {
  __ring_match32: {
    const __ring_m32 = action;
    if (__ring_m32._tag === "Identity") {
      return emit_identity_eq_cmp(ctx, lhs, rhs);
      break __ring_match32;
    }
    if (__ring_m32._tag === "FloatIdentity") {
      return emit_float_identity_eq_cmp(ctx, lhs, rhs);
      break __ring_match32;
    }
    if (__ring_m32._tag === "BoolIdentity") {
      const lhs_int = llvm_ptrtoint(ctx, lhs);
      const rhs_int = llvm_ptrtoint(ctx, rhs);
      return LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_EQ, lhs_int, rhs_int, codegen_llvm_ctx$fresh_name(ctx, "beq"));
      break __ring_match32;
    }
    if (__ring_m32._tag === "Call") {
      const dict_name = __ring_m32.dict_name; const extra_dicts = __ring_m32.extra_dicts;
      return emit_dict_eq_call(ctx, lhs, rhs, dict_name, extra_dicts);
      break __ring_match32;
    }
    if (__ring_m32._tag === "Tuple") {
      const element_actions = __ring_m32.element_actions;
      return emit_tuple_eq_cmp(ctx, lhs, rhs, element_actions);
      break __ring_match32;
    }
    if (__ring_m32._tag === "FnLiteral") {
      return LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_EQ, LLVMConstInt(ctx.i64_type, 0, 0), LLVMConstInt(ctx.i64_type, 0, 0), codegen_llvm_ctx$fresh_name(ctx, "ftrue"));
      break __ring_match32;
    }
...
function load_enum_tags(ctx, self_val, other_val) {
  const tag_ty = LLVMStructTypeInContext(ctx.context, [ctx.i64_type], 0);
...
function emit_enum_eq_fn(ctx, type_name, variants, bounds) {
  const scaffold_result = begin_derived_fn(ctx, type_name, "eq", "Eq", true, bounds);
  if (Option_is_none(scaffold_result)) {
    return;
  }
  const __ring_dt3 = Option_unwrap(scaffold_result);
  const scaffold = __ring_dt3[0];
  const _dict_params = __ring_dt3[1];
  const fn_val = scaffold.fn_val;
  const self_val = scaffold.self_val;
  ...
  let __ring_blk10;
  __ring_match33: {
    const __ring_m33 = _Map_get(ctx.enum_types, type_name);
    if (__ring_m33._tag === "some") {
      const info = __ring_m33._0;
      __ring_blk10 = info;
      break __ring_match33;
    }
    if (__ring_m33._tag === "none") {
      __ring_blk10 = panic("unreachable");
      break __ring_match33;
    }
    __match_fail(__ring_m33);
  }
  const enum_info = __ring_blk10;
  const __ring_dt4 = load_enum_tags(ctx, self_val, other_val);
  const self_tag = __ring_dt4[0];
  const other_tag = __ring_dt4[1];
  const tags_eq = LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_EQ, self_tag, other_tag, codegen_llvm_ctx$fresh_name(ctx, "teq"));
  let any_fields = false;
  const __ring_iter_24 = __List_Iterable.iter(variants);
  while (true) {
  ...
  if ((!any_fields)) {
    ...
    codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, tags_eq, true_bb, false_bb));
    ...
    codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, tags_eq, tags_same_bb, tags_diff_bb));
    ...
    for (let vi = 0; vi < __ring_end25; vi++) {
      ...
      let __ring_blk11;
      __ring_match34: {
        const __ring_m34 = _Map_get(enum_info.variants, variant.name);
        if (__ring_m34._tag === "some") {
          const vinfo = __ring_m34._0;
          __ring_blk11 = vinfo.tag;
      ...
      const var_tag = __ring_blk11;
      const case_bb = LLVMAppendBasicBlockInContext(ctx.context, fn_val, `eq.v.${variant.name}`);
      LLVMAddCase(switch_val, LLVMConstInt(ctx.i64_type, var_tag, 0), case_bb);
      LLVMPositionBuilderAtEnd(ctx.builder, case_bb);
      if ((List_len(variant.fields) === 0)) {
        codegen_llvm_expr$discard(LLVMBuildBr(ctx.builder, ret_true_bb));
      } else {
        const __ring_end26 = List_len(variant.fields);
        for (let fi = 0; fi < __ring_end26; fi++) {
          ...
          codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, eq_i1, next_bb, ret_false_bb));
          LLVMPositionBuilderAtEnd(ctx.builder, next_bb);
        }
      }
    }
...
function emit_struct_eq_fn(ctx, type_name, fields, bounds) {
  ...
  const __ring_dt5 = Option_unwrap(scaffold_result);
  const scaffold = __ring_dt5[0];
  const _dict_params = __ring_dt5[1];
  const fn_val = scaffold.fn_val;
  const self_val = scaffold.self_val;
  const other_val = LLVMGetParam(fn_val, 1);
  if ((List_len(fields) === 0)) {
  ...
  let __ring_blk12;
  __ring_match35: {
    const __ring_m35 = _Map_get(ctx.struct_types, type_name);
    if (__ring_m35._tag === "some") {
      const info = __ring_m35._0;
      __ring_blk12 = info;
      break __ring_match35;
    }
    if (__ring_m35._tag === "none") {
      __ring_blk12 = panic("unreachable");
      break __ring_match35;
    }
    __match_fail(__ring_m35);
  }
  const struct_info = __ring_blk12;
  ...
  const __ring_end27 = List_len(fields);
  for (let fi = 0; fi < __ring_end27; fi++) {
    const field = __ring_index(fields, fi);
    const field_idx = find_field_index(struct_info.field_names, field.name);
    if ((field_idx < 0)) {
      continue;
...
function emit_struct_ne_fn(ctx, type_name) {
  ...
  __ring_match36: {
    const __ring_m36 = _Map_get(ctx.functions, mangled_ne);
  ...
  let __ring_blk13;
  __ring_match37: {
    const __ring_m37 = _Map_get(ctx.functions, mangled_eq);
    ...
    if (__ring_m37._tag === "none") {
      __ring_blk13 = panic("unreachable");
      break __ring_match37;
    }
    __match_fail(__ring_m37);
  }
  const eq_fn = __ring_blk13;
  let __ring_blk14;
  __ring_match38: {
    const __ring_m38 = _Map_get(ctx.fn_types, mangled_eq);
    ...
    if (__ring_m38._tag === "none") {
      __ring_blk14 = LLVMFunctionType(ctx.ptr_type, [ctx.ptr_type, ctx.ptr_type], 0);
      break __ring_match38;
    }
    __match_fail(__ring_m38);
  }
  const eq_fn_ty = __ring_blk14;
  const eq_arity = LLVMCountParams(eq_fn);
  let ne_param_types = [];
  const __ring_end28 = eq_arity;
  ...
  const scaffold = new DerivedFnScaffold(fn_val, LLVMGetParam(fn_val, 0), ctx.current_fn, LLVMGetInsertBlock(ctx.builder));
  ctx.current_fn = Option_some(fn_val);
  const entry = LLVMAppendBasicBlockInContext(ctx.context, fn_val, "entry");
  LLVMPositionBuilderAtEnd(ctx.builder, entry);
  let eq_args = [];
  const __ring_end29 = eq_arity;
...
function emit_derived_eq_llvm(ctx, di) {
  const type_name = di.type_name;
  __ring_match39: {
    const __ring_m39 = di.type_kind;
    if (__ring_m39._tag === "StructKind") {
      __ring_match40: {
        const __ring_m40 = di.struct_fields;
        if (__ring_m40._tag === "some") {
          const fields = __ring_m40._0;
    ...
    if (__ring_m39._tag === "EnumKind") {
      __ring_match41: {
        const __ring_m41 = di.enum_variants;
        if (__ring_m41._tag === "some") {
          const variants = __ring_m41._0;
...
function emit_float_identity_cmp(ctx, lhs, rhs) {
  ...
  __ring_match42: {
    ...
    if (__ring_m42._tag === "none") {
      __ring_blk15 = panic("LLVM codegen: emit_float_identity_cmp outside function");
      break __ring_match42;
    }
    __match_fail(__ring_m42);
  }
  const current_fn = __ring_blk15;
  const unbox_fn = codegen_llvm_ctx$get_or_declare_runtime_fn(ctx, "ring_unbox_float", [ctx.ptr_type], ctx.double_type);
  const unbox_ty = codegen_llvm_ctx$get_rt_fn_type(ctx, "ring_unbox_float");
  const lhs_raw = LLVMBuildCall2(ctx.builder, unbox_ty, unbox_fn, [lhs], codegen_llvm_ctx$fresh_name(ctx, "lf"));
  const rhs_raw = LLVMBuildCall2(ctx.builder, unbox_ty, unbox_fn, [rhs], codegen_llvm_ctx$fresh_name(ctx, "rf"));
  const is_lt = LLVMBuildFCmp(ctx.builder, codegen_llvm_ctx$LLVM_REAL_OLT, lhs_raw, rhs_raw, codegen_llvm_ctx$fresh_name(ctx, "flt"));
  const is_gt = LLVMBuildFCmp(ctx.builder, codegen_llvm_ctx$LLVM_REAL_OGT, lhs_raw, rhs_raw, codegen_llvm_ctx$fresh_name(ctx, "fgt"));
  ...
  codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, is_lt, lt_bb, gt_check_bb));
  ...
  codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, is_gt, gt_bb, eq_bb));
...
function emit_identity_cmp(ctx, lhs, rhs) {
  let __ring_blk16;
  __ring_match43: {
    const __ring_m43 = ctx.current_fn;
    if (__ring_m43._tag === "some") {
      const f = __ring_m43._0;
      __ring_blk16 = f;
      break __ring_match43;
    }
    if (__ring_m43._tag === "none") {
      __ring_blk16 = panic("LLVM codegen: emit_identity_cmp outside function");
      break __ring_match43;
    }
    __match_fail(__ring_m43);
  }
  const current_fn = __ring_blk16;
  const lhs_int = llvm_ptrtoint(ctx, lhs);
  const rhs_int = llvm_ptrtoint(ctx, rhs);
  const one = LLVMConstInt(ctx.i64_type, 1, 0);
  const lhs_tag = llvm_and(ctx, lhs_int, one);
  ...
  const is_lt = LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_SLT, lhs_unboxed, rhs_unboxed, codegen_llvm_ctx$fresh_name(ctx, "lt"));
  const is_gt = LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_SGT, lhs_unboxed, rhs_unboxed, codegen_llvm_ctx$fresh_name(ctx, "gt"));
  ...
  codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, is_lt, t_lt_bb, t_gt_check_bb));
  ...
  codegen_llvm_expr$discard(LLVMBuildBr(ctx.builder, t_merge_bb));
  LLVMPositionBuilderAtEnd(ctx.builder, t_gt_check_bb);
  codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, is_gt, t_gt_bb, t_eq_bb));
...
function emit_tuple_cmp(ctx, lhs, rhs, element_actions) {
  ...
  let __ring_blk17;
  __ring_match44: {
    const __ring_m44 = ctx.current_fn;
    if (__ring_m44._tag === "some") {
      const f = __ring_m44._0;
      __ring_blk17 = f;
      break __ring_match44;
    }
    if (__ring_m44._tag === "none") {
      __ring_blk17 = panic("LLVM codegen: emit_tuple_cmp outside function");
      break __ring_match44;
    }
    __match_fail(__ring_m44);
  }
  const current_fn = __ring_blk17;
  ...
  const __ring_end31 = List_len(element_actions);
  for (let i = 0; i < __ring_end31; i++) {
    ...
    if ((i < (List_len(element_actions) - 1))) {
      ...
      const is_zero = LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_EQ, raw, LLVMConstInt(ctx.i64_type, 0, 0), codegen_llvm_ctx$fresh_name(ctx, "iz"));
      const next_bb = LLVMAppendBasicBlockInContext(ctx.context, current_fn, `tc.next.${i}`);
      codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, is_zero, next_bb, exit_bb));
      LLVMPositionBuilderAtEnd(ctx.builder, next_bb);
    } else {
      codegen_llvm_expr$discard(LLVMBuildBr(ctx.builder, exit_bb));
    }
  }
...
function emit_field_cmp(ctx, lhs, rhs, action) {
  __ring_match45: {
    const __ring_m45 = action;
    if (__ring_m45._tag === "Identity") {
      return emit_identity_cmp(ctx, lhs, rhs);
      break __ring_match45;
    }
    if (__ring_m45._tag === "FloatIdentity") {
      return emit_float_identity_cmp(ctx, lhs, rhs);
      break __ring_match45;
    }
    if (__ring_m45._tag === "BoolIdentity") {
      return emit_identity_cmp(ctx, lhs, rhs);
      break __ring_match45;
    }
    if (__ring_m45._tag === "Call") {
      const dict_name = __ring_m45.dict_name; const extra_dicts = __ring_m45.extra_dicts;
      return emit_dict_cmp_call(ctx, lhs, rhs, dict_name, extra_dicts);
      break __ring_match45;
    }
    if (__ring_m45._tag === "Tuple") {
      const element_actions = __ring_m45.element_actions;
      return emit_tuple_cmp(ctx, lhs, rhs, element_actions);
      break __ring_match45;
    }
    if (__ring_m45._tag === "FnLiteral") {
      return codegen_llvm_expr$box_int(ctx, LLVMConstInt(ctx.i64_type, 0, 0));
      break __ring_match45;
    }
...
function emit_enum_cmp_fn(ctx, type_name, variants, bounds) {
  const scaffold_result = begin_derived_fn(ctx, type_name, "cmp", "Ord", true, bounds);
  if (Option_is_none(scaffold_result)) {
    return;
  }
  const __ring_dt6 = Option_unwrap(scaffold_result);
  const scaffold = __ring_dt6[0];
  const _dict_params = __ring_dt6[1];
  const fn_val = scaffold.fn_val;
  const self_val = scaffold.self_val;
  const other_val = LLVMGetParam(fn_val, 1);
  if (Option_is_none(_Map_get(ctx.enum_types, type_name))) {
  ...
  let __ring_blk18;
  __ring_match46: {
    const __ring_m46 = _Map_get(ctx.enum_types, type_name);
    if (__ring_m46._tag === "some") {
      const info = __ring_m46._0;
      __ring_blk18 = info;
      break __ring_match46;
    }
    if (__ring_m46._tag === "none") {
      __ring_blk18 = panic("unreachable");
      break __ring_match46;
    }
    __match_fail(__ring_m46);
  }
  const enum_info = __ring_blk18;
  const __ring_dt7 = load_enum_tags(ctx, self_val, other_val);
  const self_tag = __ring_dt7[0];
  const other_tag = __ring_dt7[1];
  const tags_eq = LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_EQ, self_tag, other_tag, codegen_llvm_ctx$fresh_name(ctx, "teq"));
  ...
  codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, tags_eq, tags_same_bb, tags_diff_bb));
  LLVMPositionBuilderAtEnd(ctx.builder, tags_diff_bb);
  const self_lt = LLVMBuildICmp(ctx.builder, codegen_llvm_ctx$LLVM_INT_SLT, self_tag, other_tag, codegen_llvm_ctx$fresh_name(ctx, "slt"));
  ...
  codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, self_lt, lt_bb, gt_bb));
  LLVMPositionBuilderAtEnd(ctx.builder, lt_bb);
  const neg_one = LLVMConstInt(ctx.i64_type, (0 - 1), 1);
  const lt_val = codegen_llvm_expr$box_int(ctx, neg_one);
  const lt_end = LLVMGetInsertBlock(ctx.builder);
  codegen_llvm_expr$discard(LLVMBuildBr(ctx.builder, tag_merge));
  LLVMPositionBuilderAtEnd(ctx.builder, gt_bb);
  const gt_val = codegen_llvm_expr$box_int(ctx, LLVMConstInt(ctx.i64_type, 1, 0));
  const gt_end = LLVMGetInsertBlock(ctx.builder);
  codegen_llvm_expr$discard(LLVMBuildBr(ctx.builder, tag_merge));
  ...
  const __ring_end32 = List_len(variants);
  for (let vi = 0; vi < __ring_end32; vi++) {
    const variant = __ring_index(variants, vi);
    let __ring_blk19;
    __ring_match47: {
      const __ring_m47 = _Map_get(enum_info.variants, variant.name);
      if (__ring_m47._tag === "some") {
        const vinfo = __ring_m47._0;
        __ring_blk19 = vinfo.tag;
    ...
    LLVMPositionBuilderAtEnd(ctx.builder, case_bb);
    if ((List_len(variant.fields) === 0)) {
      codegen_llvm_expr$discard(LLVMBuildRet(ctx.builder, codegen_llvm_expr$box_int(ctx, LLVMConstInt(ctx.i64_type, 0, 0))));
    } else {
      const __ring_end33 = List_len(variant.fields);
      for (let fi = 0; fi < __ring_end33; fi++) {
        const field = __ring_index(variant.fields, fi);
        const self_fp = LLVMBuildStructGEP2(ctx.builder, enum_info.llvm_type, self_val, (fi + 1), codegen_llvm_ctx$fresh_name(ctx, "sf"));
        const self_fv = LLVMBuildLoad2(ctx.builder, ctx.ptr_type, self_fp, codegen_llvm_ctx$fresh_name(ctx, "sv"));
        const other_fp = LLVMBuildStructGEP2(ctx.builder, enum_info.llvm_type, other_val, (fi + 1), codegen_llvm_ctx$fresh_name(ctx, "of"));
        ...
        if ((fi < (List_len(variant.fields) - 1))) {
          ...
          codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, is_zero, next_bb, ret_nz_bb));
          LLVMPositionBuilderAtEnd(ctx.builder, ret_nz_bb);
          codegen_llvm_expr$discard(LLVMBuildRet(ctx.builder, cmp_val));
          LLVMPositionBuilderAtEnd(ctx.builder, next_bb);
        } else {
          codegen_llvm_expr$discard(LLVMBuildRet(ctx.builder, cmp_val));
        }
...
function emit_struct_cmp_fn(ctx, type_name, fields, bounds) {
  ...
  const __ring_dt8 = Option_unwrap(scaffold_result);
  const scaffold = __ring_dt8[0];
  const _dict_params = __ring_dt8[1];
  const fn_val = scaffold.fn_val;
  const self_val = scaffold.self_val;
  const other_val = LLVMGetParam(fn_val, 1);
  if ((List_len(fields) === 0)) {
  ...
  __ring_match48: {
    ...
    if (__ring_m48._tag === "none") {
      __ring_blk20 = panic("unreachable");
      break __ring_match48;
    }
    __match_fail(__ring_m48);
  }
  const struct_info = __ring_blk20;
  const __ring_end34 = List_len(fields);
  for (let fi = 0; fi < __ring_end34; fi++) {
    ...
    if ((fi < (List_len(fields) - 1))) {
      ...
      codegen_llvm_expr$discard(LLVMBuildCondBr(ctx.builder, is_zero, next_bb, ret_bb));
...
function emit_derived_ord_llvm(ctx, di) {
  const type_name = di.type_name;
  __ring_match49: {
    const __ring_m49 = di.type_kind;
    if (__ring_m49._tag === "StructKind") {
      __ring_match50: {
        const __ring_m50 = di.struct_fields;
        if (__ring_m50._tag === "some") {
          const fields = __ring_m50._0;
    ...
    if (__ring_m49._tag === "EnumKind") {
      __ring_match51: {
        const __ring_m51 = di.enum_variants;
        if (__ring_m51._tag === "some") {
          const variants = __ring_m51._0;
...
function option_none_variant() {
  return new hir$DerivedVariant("none", [], false);
}

function option_some_variant(dict_name) {
  return new hir$DerivedVariant("some", [new hir$DerivedField("_0", Option_some(0), hir$FieldAction_Call(dict_name, []))], false);
}

function emit_builtin_derived_impls(ctx) {
  const option_eq = new hir$DerivedImpl("Option", "Eq", ["T"], [new hir$TraitBound("T", "Eq")], hir$TypeKind_EnumKind, Option_none, Option_some([option_some_variant(hir$trait_bound_param_name("T", "Eq")), option_none_variant()]));
  emit_derived_eq_llvm(ctx, option_eq);
  const option_debug = new hir$DerivedImpl("Option", "Debug", ["T"], [new hir$TraitBound("T", "Debug")], hir$TypeKind_EnumKind, Option_none, Option_some([option_some_variant(hir$trait_bound_param_name("T", "Debug")), option_none_variant()]));
  emit_derived_debug_llvm(ctx, option_debug);
  const option_ord = new hir$DerivedImpl("Option", "Ord", ["T"], [new hir$TraitBound("T", "Ord")], hir$TypeKind_EnumKind, Option_none, Option_some([option_some_variant(hir$trait_bound_param_name("T", "Ord")), option_none_variant()]));
  emit_derived_ord_llvm(ctx, option_ord);
  const option_clone = new hir$DerivedImpl("Option", "Clone", ["T"], [], hir$TypeKind_EnumKind, Option_none, Option_none);
  return emit_derived_clone_llvm(ctx, option_clone);
}

function is_enum_const_type(ty) {
  __ring_match52: {
    const __ring_m52 = ty;
    if (__ring_m52._tag === "EnumType") {
      return true;
...
function emit_const_body(ctx, name, init) {
  let __ring_blk21;
  __ring_match53: {
    const __ring_m53 = ctx.module_prefix;
    if (__ring_m53._tag === "some") {
      const prefix = __ring_m53._0;
      __ring_blk21 = codegen_llvm_ctx$llvm_mangle_fn_with_prefix(prefix, name);
      break __ring_match53;
    }
    if (__ring_m53._tag === "none") {
      __ring_blk21 = codegen_llvm_ctx$llvm_mangle_fn(name);
      break __ring_match53;
    }
    __match_fail(__ring_m53);
  }
  const const_fn_name = __ring_blk21;
  __ring_match54: {
    const __ring_m54 = _Map_get(ctx.functions, const_fn_name);
    if (__ring_m54._tag === "some") {
      const fn_val = __ring_m54._0;
      let __ring_blk22;
      __ring_match55: {
        const __ring_m55 = hir$hexpr_type(init);
      ...
      const is_str_const = __ring_blk22;
...
function emit_default_method_stubs(ctx, target_type, trait_name, impl_methods) {
  ...
  const method_order = __ring_blk23;
  const all_supers = collect_all_supertraits_llvm(ctx, trait_name);
  const super_count = List_len(all_supers);
  const __ring_iter_36 = __List_Iterable.iter(method_order);
  while (true) {
    const __ring_next_36 = __ListIterator_Iterator.next(__ring_iter_36);
    if (__ring_next_36._tag === "none") break;
    const method_name = __ring_next_36._0;
    if (_Set_contains(impl_method_names, method_name, __Str_Eq)) {
      continue;
    }
    const default_fn_name = `__${trait_name}_${method_name}`;
    __ring_match58: {
      const __ring_m58 = _Map_get(ctx.functions, default_fn_name);
      if (__ring_m58._tag === "some") {
        const default_fn = __ring_m58._0;
        const mangled = codegen_llvm_ctx$llvm_mangle_method(target_type, method_name);
        ...
        __ring_match59: {
          const __ring_m59 = _Map_get(ctx.fn_evidence_params, default_fn_name);
          if (__ring_m59._tag === "some") {
            const ev_params = __ring_m59._0;
        ...
        let __ring_blk24;
        __ring_match60: {
          const __ring_m60 = _Map_get(ctx.fn_types, default_fn_name);
          ...
          if (__ring_m60._tag === "none") {
            let pts = [];
            const __ring_end38 = default_arity;
            for (let i = 0; i < __ring_end38; i++) {
              List_push(pts, ctx.ptr_type);
            }
            __ring_blk24 = LLVMFunctionType(ctx.ptr_type, pts, 0);
            break __ring_match60;
          }
          __match_fail(__ring_m60);
        }
        const default_fn_ty = __ring_blk24;
        let call_args = [dict_ptr];
        const __ring_iter_39 = __List_Iterable.iter(all_supers);
        while (true) {
          const __ring_next_39 = __ListIterator_Iterator.next(__ring_iter_39);
          if (__ring_next_39._tag === "none") break;
          const st = __ring_next_39._0;
          const st_dict_name = hir$trait_dict_name(target_type, st);
          const st_dict_ptr = codegen_llvm_expr$resolve_static_dict_by_name(ctx, st_dict_name);
          List_push(call_args, st_dict_ptr);
        }
        const __ring_end40 = stub_arity;
...
function emit_derived_impls_llvm(ctx, derived_impls) {
  const __ring_iter_41 = __List_Iterable.iter(derived_impls);
  while (true) {
    const __ring_next_41 = __ListIterator_Iterator.next(__ring_iter_41);
    if (__ring_next_41._tag === "none") break;
    const di = __ring_next_41._0;
    __ring_match61: {
      const __ring_m61 = di.trait_name;
      if (__ring_m61 === "Eq") {
...
function emit_enum_constructors(ctx, name, variants) {
  let __ring_blk25;
  __ring_match62: {
    const __ring_m62 = _Map_get(ctx.enum_types, name);
    if (__ring_m62._tag === "some") {
      const info = __ring_m62._0;
      __ring_blk25 = info;
  ...
  const __ring_iter_42 = __List_Iterable.iter(variants);
  while (true) {
    ...
    let __ring_blk26;
    __ring_match63: {
      const __ring_m63 = _Map_get(enum_info.variants, v.name);
      if (__ring_m63._tag === "some") {
        const vi = __ring_m63._0;
        __ring_blk26 = vi;
    ...
    let __ring_blk27;
    __ring_match64: {
      const __ring_m64 = _Map_get(ctx.functions, ctor_name);
      if (__ring_m64._tag === "some") {
        const fv = __ring_m64._0;
        __ring_blk27 = fv;
    ...
    const __ring_end43 = List_len(v.fields);
...
function emit_fn_body(ctx, name, params, effects, body, trait_bounds, impl_type) {
  ...
  __ring_match65: {
    ...
    if (__ring_m65._tag === "none") {
      let __ring_blk29;
      __ring_match66: {
        const __ring_m66 = ctx.module_prefix;
        if (__ring_m66._tag === "some") {
          const prefix = __ring_m66._0;
  ...
  let __ring_blk30;
  __ring_match67: {
    const __ring_m67 = _Map_get(ctx.functions, mangled);
    if (__ring_m67._tag === "some") {
      const f = __ring_m67._0;
      __ring_blk30 = f;
  ...
  let __ring_blk31;
  __ring_match68: {
    const __ring_m68 = impl_type;
    if (__ring_m68._tag === "some") {
      const t = __ring_m68._0;
      __ring_blk31 = `${t}_${name}`;
  ...
  const effect_key = __ring_blk31;
  let __ring_blk32;
  __ring_match69: {
    const __ring_m69 = _Map_get(ctx.local_fn_effects, effect_key);
    if (__ring_m69._tag === "some") {
      const eff = __ring_m69._0;
      __ring_blk32 = eff;
      break __ring_match69;
    }
    if (__ring_m69._tag === "none") {
      __ring_blk32 = effects;
      break __ring_match69;
    }
    __match_fail(__ring_m69);
  }
  const effective_effects = __ring_blk32;
  const ev_names = effect_analysis$extract_effect_names(effective_effects);
  const __ring_iter_46 = __List_Iterable.iter(ev_names);
  while (true) {
    const __ring_next_46 = __ListIterator_Iterator.next(__ring_iter_46);
    if (__ring_next_46._tag === "none") break;
    const en = __ring_next_46._0;
...
function emit_struct_constructor(ctx, name, fields) {
  ...
  let param_types = [];
  const __ring_iter_47 = __List_Iterable.iter(fields);
  while (true) {
  ...
  let __ring_blk33;
  __ring_match71: {
    const __ring_m71 = _Map_get(ctx.struct_types, name);
    if (__ring_m71._tag === "some") {
      const info = __ring_m71._0;
      __ring_blk33 = info;
      break __ring_match71;
    }
    if (__ring_m71._tag === "none") {
      __ring_blk33 = panic(`LLVM codegen: struct '${name}' not registered`);
      break __ring_match71;
    }
    __match_fail(__ring_m71);
  }
  const struct_info = __ring_blk33;
...
function emit_one_default_method(ctx, fn_val, default_fn_name, trait_name, method, body) {
  ...
  const __ring_iter_50 = __List_Iterable.iter(method.params);
  while (true) {
    const __ring_next_50 = __ListIterator_Iterator.next(__ring_iter_50);
    if (__ring_next_50._tag === "none") break;
    const p = __ring_next_50._0;
    const alloca = LLVMBuildAlloca(ctx.builder, ctx.ptr_type, p.name);
    LLVMBuildStore(ctx.builder, LLVMGetParam(fn_val, param_idx), alloca);
    _Map_insert(ctx.named_values, p.name, alloca);
    param_idx = (param_idx + 1);
  }
  const ev_names = effect_analysis$extract_effect_names(method.effects);
...
function emit_trait_default_methods(ctx, trait_name, methods) {
  ...
  while (true) {
    ...
    const method = __ring_next_52._0;
    if ((!method.has_default)) {
      continue;
    }
    __ring_match72: {
      const __ring_m72 = method.body;
      if (__ring_m72._tag === "some") {
        const body = __ring_m72._0;
        const default_fn_name = `__${trait_name}_${method.name}`;
        __ring_match73: {
          const __ring_m73 = _Map_get(ctx.functions, default_fn_name);
...
function emit_trait_dict(ctx, target_type, trait_name, methods) {
  ...
  __ring_match75: {
    ...
    if (__ring_m75._tag === "none") {
      __ring_blk34 = impl_methods;
      break __ring_match75;
    }
    __match_fail(__ring_m75);
  }
  const method_order = __ring_blk34;
  const method_count = List_len(method_order);
  if ((method_count === 0)) {
    return;
  }
  ...
  LLVMPositionBuilderAtEnd(ctx.builder, entry);
  let dict_elem_types = [ctx.i64_type];
  const __ring_end54 = method_count;
  ...
  const closure_ty = LLVMStructTypeInContext(ctx.context, [ctx.ptr_type, ctx.ptr_type], 0);
  const closure_size = LLVMSizeOf(closure_ty);
  const __ring_end55 = method_count;
...
function emit_llvm_decl(ctx, decl) {
  __ring_match77: {
    ...
    if (__ring_m77._tag === "Fn") {
      const name = __ring_m77.name; const params = __ring_m77.params; const effects = __ring_m77.effects; const body = __ring_m77.body; const trait_bounds = __ring_m77.trait_bounds;
      return emit_fn_body(ctx, name, params, effects, body, trait_bounds, Option_none);
      break __ring_match77;
    }
    if (__ring_m77._tag === "Struct") {
      const name = __ring_m77.name; const fields = __ring_m77.fields;
      return emit_struct_constructor(ctx, name, fields);
      break __ring_match77;
    }
    if (__ring_m77._tag === "Enum") {
      const name = __ring_m77.name; const variants = __ring_m77.variants;
      return emit_enum_constructors(ctx, name, variants);
      break __ring_match77;
    }
    if (__ring_m77._tag === "Impl") {
      const target_type = __ring_m77.target_type; const trait_name = __ring_m77.trait_name; const methods = __ring_m77.methods;
      ...
      while (true) {
        ...
        __ring_match78: {
          ...
          if (__ring_m78._tag === "Fn") {
            const mn = __ring_m78.name; const mp = __ring_m78.params; const me = __ring_m78.effects; const mb = __ring_m78.body; const mtb = __ring_m78.trait_bounds;
      ...
      __ring_match79: {
        const __ring_m79 = trait_name;
        if (__ring_m79._tag === "some") {
          const tn = __ring_m79._0;
    ...
    if (__ring_m77._tag === "Effect") {
      break __ring_match77;
    }
    if (__ring_m77._tag === "Test") {
      break __ring_match77;
    }
    if (__ring_m77._tag === "Trait") {
      const trait_name = __ring_m77.name; const trait_methods = __ring_m77.methods;
      return emit_trait_default_methods(ctx, trait_name, trait_methods);
      break __ring_match77;
    }
    if (__ring_m77._tag === "ExternFn") {
      break __ring_match77;
    }
    if (__ring_m77._tag === "ExternType") {
      break __ring_match77;
    }
    if (__ring_m77._tag === "TypeAlias") {
      break __ring_match77;
    }
    if (__ring_m77._tag === "Const") {
      const name = __ring_m77.name; const init = __ring_m77.init;
      return emit_const_body(ctx, name, init);
      break __ring_match77;
    }
    if (__ring_m77._tag === "ModBlock") {
      const mod_decls = __ring_m77.decls;
      const __ring_iter_57 = __List_Iterable.iter(mod_decls);
      while (true) {
        const __ring_next_57 = __ListIterator_Iterator.next(__ring_iter_57);
        if (__ring_next_57._tag === "none") break;
        const subdecl = __ring_next_57._0;
        emit_llvm_decl(ctx, subdecl);
      }
      break __ring_match77;
    }
    if (__ring_m77._tag === "Sig") {
      break __ring_match77;
    }
  ...
  while (true) {
    ...
    __ring_match80: {
      ...
      if (__ring_m80._tag === "some") {
        const names = __ring_m80._0;
        __ring_blk35 = names;
        break __ring_match80;
      }
      if (__ring_m80._tag === "none") {
        let ns = [];
        const __ring_end59 = fc;
        for (let j = 0; j < __ring_end59; j++) {
          List_push(ns, "");
        }
        __ring_blk35 = ns;
        break __ring_match80;
    ...
    const fnames = __ring_blk35;
    let frs = [];
    const __ring_iter_60 = __List_Iterable.iter(v.fields);
    while (true) {
      const __ring_next_60 = __ListIterator_Iterator.next(__ring_iter_60);
      if (__ring_next_60._tag === "none") break;
      const ft = __ring_next_60._0;
...
export { emit_llvm_decl, register_struct_info, register_enum_info, emit_derived_impls_llvm, emit_builtin_derived_impls };