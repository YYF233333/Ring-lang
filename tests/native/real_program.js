...
function build_records() {
  let rs = [];
  List_push(rs, new Record("ada", 90));
  List_push(rs, new Record("bob", 75));
  List_push(rs, new Record("cleo", 88));
  return rs;
}

function names(rs) {
  let out = [];
  const __ring_iter_2 = __List_Iterable.iter(rs);
  while (true) {
    const __ring_next_2 = __ListIterator_Iterator.next(__ring_iter_2);
    if (__ring_next_2._tag === "none") break;
    const r = __ring_next_2._0;
    const nm = r.name;
    List_push(out, nm);
  }
  return out;
...
function score_lookup(rs) {
  let m = map_new();
  const __ring_iter_3 = __List_Iterable.iter(rs);
  while (true) {
    const __ring_next_3 = __ListIterator_Iterator.next(__ring_iter_3);
    if (__ring_next_3._tag === "none") break;
    const r = __ring_next_3._0;
    _Map_insert(m, r.name, r.score);
  }
  const direct = __ring_map_index(m, "bob");
  const safe = (function() {
  const __ring_m = _Map_get(m, "cleo");
...
function name_endpoints(ns) {
  const f = Option_unwrap_or(List_first(ns), "?");
  const l = Option_unwrap_or(List_last(ns), "?");
  return `${f}..${l}`;
}

function main(__ring_ev_io) {
  const rs = build_records();
  print(`count=${List_len(rs)}`, __ring_ev_io);
  const ns = names(rs);
  print(List_join(ns, ","), __ring_ev_io);
  print(`ends=${name_endpoints(ns)}`, __ring_ev_io);
  const top = __ring_index(rs, 0);
  print(`top=${top.name} (${top.score})`, __ring_ev_io);
  print(`sum=${score_lookup(rs)}`, __ring_ev_io);
  const word = "ring";
  print(`first=${__ring_str_index(word, 0)} len=${Str_len(word)}`, __ring_ev_io);
  let total = 0;
  const __ring_iter_4 = __List_Iterable.iter(rs);
  while (true) {
    const __ring_next_4 = __ListIterator_Iterator.next(__ring_iter_4);
    if (__ring_next_4._tag === "none") break;
    const r = __ring_next_4._0;
    total = (total + r.score);
  }
  return print(`total=${total}`, __ring_ev_io);
}

function __Record_Eq_eq(self, other) {
  return (self.name === other.name) && (self.score === other.score);
}
const __Record_Eq = { eq: __Record_Eq_eq, ne: function(self, other) { return !__Record_Eq_eq(self, other); } };
...
function __Record_Clone_clone(self) {
  return new Record(self.name, self.score);
}
const __Record_Clone = { clone: __Record_Clone_clone };
...
function __Record_Ord_cmp(self, other) {
  var c;
  c = (self.name < other.name ? -1 : self.name > other.name ? 1 : 0);
  if (c !== 0) return c;
  return (self.score < other.score ? -1 : self.score > other.score ? 1 : 0);
}
const __Record_Ord = { cmp: __Record_Ord_cmp };
...
function __Record_Debug_debug(self) {
  return "Record { " + "name: " + String(self.name) + ", " + "score: " + String(self.score) + " }";
}
const __Record_Debug = { debug: __Record_Debug_debug };
