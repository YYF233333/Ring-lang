...
**当前**：B-104 完整 Perceus RC（D1-D9 全部 ✅，leak 88%→1.2%@2.382B）。Native 自编译跑通（exit 0，~10.42B allocs，peak ~10.6GB）。G-a 三门中 ②③ 已达、① 门① refined 判据待 Discussion 拍板（D9 后 SB→0 / leak% 1.2% 递减有界）。B-080 tagged pointer ✅（290s vs 492s = 41% 加速，tid0/tid2 归零）。待攻坚：B-122 SCC 拓扑序（checker 正确性）。
