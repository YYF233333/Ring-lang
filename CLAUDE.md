...
- **度量语义覆盖**，不度量代码行覆盖
- **LLVM 差分回归测试**（`tests/cases/llvm/*.ring` + `tests/llvm_diff.test.mjs`，`npm run test:llvm`）：每个用例用 JS 和 LLVM 两个后端编译运行，断言输出一致（JS 后端是 oracle）。锁定 LLVM codegen 修复（tuple-match tag、OR-pattern、Set 迭代、泛型 Eq/Ord dict 派发、List.contains、fail/catch via ring_try、闭包捕获 RC（mut-cell 写穿 + env owned-capture drop）、泛型 trait 方法 dict 派发、双后端 parity 收口（一等多态函数值 / wrapped dict / 闭包捕获 dict / mut 参数 writeback / print Int / tuple-literal pattern）等）。需本地 clang，无则自动 skip。
