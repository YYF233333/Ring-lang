...
- **Handler 只支持 tail-resumptive + abort**：非 abort effect 的 handler 返回值即 resume 值；`fail.raise` 为 abort，捕获后当前 handler/evidence 先失活，abort arm 恰好执行一次并以其结果替换整个 `handle`，arm 内 re-raise 逃向外层。开放 HOF fail row 会按 payload 做精确约束；无法安全还原的 owner-qualified associated type 关系 fail-closed。通用 tail-resumptive arm 的静态 result/effect 契约仍由 audit #258 跟踪。Full AE（post-resume / multi-resume）不计划实现
