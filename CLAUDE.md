...
**当前状态：Phase 1 核心完成** — 编译器可以编译并运行基本 Ring-lang 程序（struct、enum、pattern matching、HM 类型推断、effect 推断、or/catch 错误处理）。80 个单元测试全部通过。
...
│   │   ├── hir/index.ts             HIR（Typed IR）节点定义
│   │   ├── errors.ts                诊断/错误类型
│   │   ├── parser/
│   │   │   ├── lexer.ts             手写词法分析器
