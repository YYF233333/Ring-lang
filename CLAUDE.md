...
- **度量语义覆盖**，不度量代码行覆盖
- **LLVM 差分回归测试**（`tests/cases/llvm/*.ring` + `tests/llvm_diff.test.mjs`，`npm run test:llvm`）：每个用例用 JS 和 LLVM 两个后端编译运行，断言输出一致（JS 后端是 oracle）。锁定 LLVM codegen 修复（tuple-match tag、OR-pattern、Set 迭代、泛型 Eq/Ord dict 派发、List.contains、fail/catch via ring_try 等）。需本地 clang，无则自动 skip。
