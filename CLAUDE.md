...
- **度量语义覆盖**，不度量代码行覆盖
- **LLVM 差分回归测试**（`tests/cases/llvm/*.ring` + `tests/llvm_diff.test.mjs`，`npm run test:llvm`）：每个用例用 JS 和 LLVM 两个后端编译运行，断言输出一致（JS 后端是 oracle）。锁定 LLVM codegen 修复（tuple-match tag、OR-pattern、Set 迭代、泛型 Eq/Ord dict 派发、List.contains、fail/catch via ring_try、闭包捕获 RC（mut-cell 写穿 + env owned-capture drop）、泛型 trait 方法 dict 派发、双后端 parity 收口（一等多态函数值 / wrapped dict / 闭包捕获 dict / mut 参数 writeback / print Int / tuple-literal pattern）、自定义 effect handler（tail-resumptive evidence struct 派发 + handler-arm 捕获外层 local 的 Perceus RC dup/move，#133）等）。需本地 clang，无则自动 skip。
...
**当前**：L0 RC 基础设施（B-012，2026-06-01）+ **L1 借用引擎（B-098，2026-06-04）已落地**——`perceus.ring` 从 owned-everywhere 改为 clone-all-escape（读取 borrow、逃逸点 `HExpr::Clone`、scope-end-drop-once、删 branch-balancing），**从根消除 #134 系统性 move-analysis double-free 崩溃类**：native ring.exe 编 `a_empty.ring` EXIT 0、JS 731×3 + llvm_diff 49×3 全绿、dist double-bootstrap 字节一致。**推进中**：**B-104 完整 Perceus RC 里程碑（专门 session，G-a 内存墙真解）**——over-free 链已终结（B-101 A1→B-102 R-clean→B-103 borrow-builtin 分类），G-a 主因 = incomplete RC（clone-all-escape 只实现半套 Perceus：drop named 绑定 + 逃逸，**不 drop 中间临时**），非 over-free。分波回收临时（ANF + W1 Call-arg + W2 scrutinee + W3a control-flow + W4 scalar-reassign）+ 两个 range-loop 修复把 INT leak **47.8%→1.6%**——**证伪「标记指针 = G-a 真解」**：标记指针（B-080）只消 ~21%（BOOL+INT）、非墙驱动，墙主体 **74.5% 是非标量临时**（STR/OPTION/CLOSURE/TUPLE）。**2026-06-09 用户拍板：停 wave 式「哪里漏堵哪里」打地鼠，改做完整 Perceus RC**——**D1** total return-mode drop pass（drop 所有 fresh-owned 临时，地基 B-103）+ **D2** 静态 leak verifier（post-RC HIR 线性检查，编译期证明 0 泄露/0 UAF，挂 `npm test`）+ **D3** 0 泄露 = 无环 by-construction + 环用 `Weak<T>`（§7.9）。garbage-free by construction（Koka POPL'21 定理）。**B-080 标记指针降级为后续 peak/perf；B-109 非标量 wave 折入 B-104。详见 design.md §7.11。** 执行序：**B-103 ✅（2026-06-11 done）**——ring_runtime.cpp ~170 函数 return-mode 全量分类表落 `perceus.ring`（FRESH/BORROW/SCALAR/NULL-NEVER，逐函数证据）、`is_borrow_returning_call` += 9 个 receiver-returning mutator 字段名（pre-fix UAF 实证）、runtime dup-on-share 补全 ×9（filter/concat/slice/reverse/sort/sort_default/map_from/map_int_from/flat_map）→ **D1 total pass（下一步，内置规则①extern-handle 类型级 RC 排除（audit #139 硬前置）②Unit 类型级不 Clone/Drop ③IndexExpr 按 receiver 精化 ④runtime overwrite/remove drop，见 backlog B-104）** → D2 verifier → native re-measure。后续 native 终验 B-089（G-a/b/c）+ native LLVM-C 链接（B-099）。
