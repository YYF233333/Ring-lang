...
- **执行序**：**D1** total return-mode drop pass（推进中：**规则①② ✅ Stage 1 + 规则③/total pass 收编 ✅ Stage 2**，2026-06-11，git `bfd4fe6`..`98eb25b`——extern/Unit 类型级排除、Str-IndexExpr=FRESH、ANF 可达位置全收口 + 2 个新不变量，llvm_diff 57→65 例、自编译 leak 15.3%→14.1%@2.382B；剩规则④ runtime overwrite/remove drop = Stage 3，spec 见 backlog B-104）→ **D2** 静态 leak verifier（post-RC HIR 线性检查，编译期证明 0 泄露/0 UAF，挂 `npm test`）→ **D3** 0 泄露 = 无环 by-construction + 环用 `Weak<T>`（§7.9）→ native re-measure。实现模型见 design.md §7.11。
