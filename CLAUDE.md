...
└── tests/
    ├── cases/                          端到端测试用例（78 正向 + 14 负向 = 92 个 .ring 文件）
    └── e2e.test.ts                     端到端测试运行器（242 tests: run + check + build + negative + llm-format）
```
...
- **Map\<K,V\> 不可变键值集合**：StructType 表示，JS Map 底层。`map_from([(k,v)])` / `map_new()` 构造。13 个方法：读取 7（len/get/contains_key/is_empty/keys/values/entries）+ 变换 2（insert/remove 返回新 Map）+ HOF 4（map_values/filter/fold/any，effect 多态）。HOF 内联 IIFE 生成
- **Set\<T\> 不可变集合**：StructType 表示，JS Set 底层。`set_from(list)` / `set_new()` 构造。13 个方法：读取 4（len/contains/is_empty/to_list）+ 变换 5（insert/remove/union/intersect/difference）+ HOF 4（filter/fold/any/all，effect 多态）。for..in 迭代支持
...
- 无 `loop` 无限循环语法（`while true` 可替代），无 Range/List/Set 以外的自定义迭代器
- 无 CI 管线（lint + test 依赖手动执行）
- LSP server 为单文件编译（无跨文件引用，无增量编译）
- Checker 多错误恢复：declaration 级（同一函数内仍停于首错）
- E2E 测试通过 `npm run test:e2e` 运行（242 tests），`npm run test:all` 运行全部（227 单元 + 242 E2E）
