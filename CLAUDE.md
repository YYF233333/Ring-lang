...
**当前状态：Phase 2 Session 2 完成** — 编译器支持 evidence passing effect system + trait 系统。100 个单元测试 + 51 个端到端测试全部通过。详见 `docs/superpowers/specs/2026-05-17-phase2-session2-evidence-passing.md`。
...
- Effect row unification 不完整（只 unify fail error_type，io/mut/custom 及 row tail 被忽略；当前 effect 检查由 infer.ts row_merge 承担，实际危害有限，Session 3 前必须修复）
- Resume 仅支持隐式模式（handler body 返回值即 resume 值，不支持 resume 后继续执行额外代码）
- Refinement `where` 子句只解析不验证
- 不含 row polymorphism、模块系统、async/spawn
