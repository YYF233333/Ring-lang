...
- **LLVM 后端**：codegen_llvm*.ring（5 个模块，~6400 行 Ring），通过 N-API addon 桥接 LLVM-C 22 API。编译器自身已编译链接为 native ring.exe（前端自举打通、能编真实程序）；自编译内存墙已破（B-104 D7 后 native 自编译完整跑通，peak ~10.6GB）；G-a 余门① live plateau（残余 = STR/SB/Type 爬升类）由 B-104 收尾中
- **测试**：node:test，零外部依赖
...
- **已落地地基**：L0 RC（B-012）→ L1 clone-all-escape 借用引擎（B-098）→ over-free/UAF 链终结 + B-103 return-mode 全量分类表（B-101→B-102→B-103 ✅，明细见 git + design.md §7.11）。
- **方向（2026-06-09 拍板）**：G-a 主因 = incomplete RC（不 drop 中间临时，墙主体 74.5% 非标量临时）；停 wave 式打地鼠，做完整 Perceus（garbage-free by construction，Koka POPL'21 定理）。B-080 标记指针降级 peak/perf，B-109 折入。
- **执行序**：**D1 total drop pass ✅**（2026-06-11/12，Stage 1-3 + #150 收尾全清；顺带新发现 runtime HOF 内部泄漏类 → audit #152）→ **D2 静态 leak verifier ✅**（`verify_rc.ring` + `--verify-rc` 随 npm test 强制（820 全量），编译器 self-verify **0 errors + 1292 文档化豁免**（清单见 verify_rc.ring 头注）；上线即抓出 RC pass 3 个潜伏漏洞）→ **native re-measure ✅：leak 14.0%@2.382B——G-a 未达**，最大残留类 = #151 codegen 合成 dict（≈28-38% residual）→ **D4 dict evidence HIR 一等化 ✅**（2026-06-12，git `0411b4c`..`2635bcc`：`DictRef::Static` 单例借用 + `HExpr::DictConstruct` 动态局部 + 新 pass `dict_lower.ring` 两后端同源；LLVM lazy memoised getter；typeid 16 DICT_STATIC never-drop / 17 DICT_DYN；verifier #151 豁免类删除零新增；5-probe 固化 `dict_singleton_hotloop.ring`）→ **re-measure ✅：leak 9.2%@2.382B（−34.1%，#151 兑现正中预测）**、CLOSURE/TUPLE 退出 top-6、15GB kill 点 +53%（>3.99B）——live 仍未 plateau、自编译仍未跑完，**G-a 未达** → **D5 归因测量 ✅**（2026-06-12，box-profile/计数器全量归因 residual 84% = And/Or-cond ≈69M ＞ none 构造 ≈64M ＞ const-getter/字面量 ≈51M；#152 HOF 类自编译份额 0.008% 零杠杆降级用户面；拍板 = D6 单例化先行 + D7 And/Or lower 殿后）→ **D6 none + const Str 单例化 ✅**（2026-06-12，git `ae8ab01`（#155 manifest 先行，llvm_diff 基线 72/72 解锁）+ `17a4ad3`：`ring_Option_none` 改 runtime lazy 单例（typeid 18 OPTION_NONE never-drop，codegen 只声明，顺带对齐 JS none 指针恒等）+ Str const lazy memoised getter（`emit_memoised_const_body`，typeid 19 CONST_STATIC，`ring_const_intern` 重标）；HIR/perceus/verify_rc 零修改零新增豁免；probe 固化 `none_const_singleton_hotloop.ring`）→ **re-measure ✅：leak 5.4%@2.382B（live 128.6M，−41.6%）**——OPTION 64.2M→**0.04M**（some 类水平，兑现）、STR −29.4M（BUILTIN_* getter 份额，正中预测 −29.6M）、15GB kill 点 >6.677B（**+67%**）、两跑 71 检查点全字节一致——live 仍线性爬（BOOL 199M/STR 76M/SB 35M @6.68B），**G-a 未达** → **D7 And/Or lower 成 if-else ✅**（2026-06-13，git `2de19dc`/`3893997`：新 pass `andor_lower.ring`（checker 末端，两后端同源）`a && b`→`if a { b } else { false }`；And/Or RC/codegen 特判全退役（gen_and/gen_or 删除、v_andor + x-andor 豁免类退役→self-verify 0 errors + **892 豁免**（1292−400 零新增类）、W3a 递归改账 effect-value 尾、D2-#3 同族潜伏 UAF 顺带封死）+ anf_should_materialize 新 IfExpr 臂（全 fresh 臂 phi 物化 + drop；borrow 臂留 x-cf-value 保守）；probe `andor_lower_hotloop.ring` 双点 live=24 常量）→ **re-measure ✅：leak 2.3%@2.382B（live 55.6M，−56.7%）**——BOOL 73.2M→**~43.6K**（−99.94%，top-6 退出）、未触类持平、两跑 311 检查点全字节一致——**G-a 三门：③ 自编译首次完整跑通（exit 0，~10.42B allocs）✅ + ② peak ~10.6GB << 25.9GB ✅ + ① live plateau 未达**（55.6M@2.38B→185.2M@10.42B 亚线性缓爬，leak% 2.3%→1.8%；爬升类 = STR 字面量/interp 101.9M + SB 47.3M + Type 22.7M = D5 已记账 D7 范围外）→ **下一棒待拍板**（候选 = interp SB/sb_to_str + type_to_string 字面量收口 vs 判定门①口径；capstone 全强度 ASan 自编译仍挂账待用户在场）。**D3（0 泄露范围）**= 无环 by-construction + 环用 `Weak<T>`（§7.9）。完整明细 = backlog B-104 + design.md §7.11 + git。
