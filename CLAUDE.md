...
**当前状态：自举完成（2026-05-21）。** 编译器已从 TypeScript 完全迁移到 Ring 自举实现（31 个 .ring 源文件）。TS 编译器已归档于 git tag `ts-compiler-final`。编译器覆盖完整的类型系统（HM 推断 + effect system + trait + row polymorphism）、控制流、集合类型（List/Map/Set/Tuple）、模块系统、FFI、标准库、auto-derive traits（Eq/Clone/Debug/Ord）。318/324 E2E 测试通过（6 个失败为 Ring parser 缺少错误恢复导致 panic）。LSP 暂不可用（原 TS 实现未移植）。详见下方"已实现功能"摘要。
...
│   └── bootstrap-roadmap.md            自举路线图（已完成，归档参考）
├── compiler/
│   ├── ast.ring                        AST 节点类型定义
...
HM unification、effect inference、exhaustiveness checking 等核心算法值得单元测试。编译器现为 Ring 自举，单元测试需用 Ring 编写。
...
1. **这个测试在语言演进后还有用吗？** 语义层面（E2E + negative + property）→ 有用。实现层面（编译器内部单元）→ 需随编译器一起维护。
...
```
现在（自举完成）：E2E 大量写 + 负面测试大量写 + property-based 成为主力。TS 单元测试已随 TS 编译器归档消失。
...
### Phase 3b ✅ — 自举完成 (2026-05-20 ~ 2026-05-21)

TS 编译器逐模块翻译为 Ring（31 个 .ring 源文件，~14,260 行），完成 fixed point 验证（v1 = v2，32/32 文件 byte-identical）。TS 编译器归档于 git tag `ts-compiler-final` 并从仓库删除。测试运行器改为 in-process 调用 Ring 编译器 ESM 模块。318/324 E2E 测试通过（6 个失败为 Ring parser 缺少错误恢复——panic 代替 graceful recovery）。自举期间修复 6 个 TS 编译器 bug + 9 个 Ring 源码 bug + 3 个 effect system bug（effects_same_kind fail 去重、require->__require evidence 命名、effect row set 比较）。技术债 41 项记录于 `docs/audit-report.md`。
...
- Ring parser 无错误恢复（遇到语法错误直接 panic），6/324 负面测试因此失败
- E2E 测试通过 `cd compiler && npm test` 运行（318/324 通过）
