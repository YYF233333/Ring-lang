...
**当前**：**B-100 JS 退役 Phase 1 进行中**——P1.1 覆盖矩阵 ✅ → P1.2 gap 修复 ✅ → P1.3 对抗 review ✅（R4+R5，2026-06-26）→ **P1.4 大部完成**（llvm_diff ×3 全绿 209/209 ✅；#208 修复 ✅；native E2E ✅ test:native-only 4/4 + test:native 3/3；双 bootstrap ❌——#209 ring.exe 嵌套表达式 RC 崩溃阻塞）。里程碑：B-099 ✅ B-089 ✅ B-104 ✅ B-080 ✅ B-122 ✅ B-138 ✅。测试状态以 `npm test` / `npm run test:llvm` 实跑为准，不在此记录具体计数。
