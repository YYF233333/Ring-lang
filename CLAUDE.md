...
**当前状态：Phase 2 Session 1 完成** — 编译器支持 trait 系统 + 泛型约束 + dictionary passing codegen。87 个单元测试 + 33 个端到端测试全部通过。详见 `docs/specs/2026-05-16-phase2-design.md`。
...
│   │   ├── hir/index.ts             HIR 节点定义 + 共享约定（variant_js_name, trait_dict_name）
...
- Effect handler 仅覆盖简单场景（多次 resume、嵌套 handler 待 Session 2 重写）
