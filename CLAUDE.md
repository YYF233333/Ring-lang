...
- **执行序**：**D1 total drop pass ✅**（2026-06-11/12，Stage 1-3 + #150 收尾全清；顺带新发现 runtime HOF 内部泄漏类 → audit #152）→ **D2 静态 leak verifier ✅**（`verify_rc.ring` + `--verify-rc` 随 npm test 强制（818 = 744 e2e + 74 verify），编译器 self-verify **0 errors + 1289 文档化豁免**（清单见 verify_rc.ring 头注）；上线即抓出 RC pass 3 个潜伏漏洞）→ **native re-measure ✅：leak 14.0%@2.382B、live 无 plateau、15GB kill 自编译未跑完——G-a 未达**，最大残留类 = audit #151 codegen 合成 dict（≈28-38% residual）→ **D4 dict evidence HIR 一等化（当前步，2026-06-12 拍板 = 三案 (c) + 吸收 (b) 单例语义：静态 dict = HIR 模块级单例、动态 wrapped = 局部 evidence 值；spec 见 backlog B-104 D4）** → re-measure → SB（tid13 ≈12M）归因。**D3（0 泄露范围）**= 无环 by-construction + 环用 `Weak<T>`（§7.9）。完整明细 = backlog B-104 + design.md §7.11 + git。
