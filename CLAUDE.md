...
└── tests/
    ...
    └── e2e.test.ts                     端到端测试运行器（149 tests: run + negative + modules + llm-format，concurrency 3）
```
...
- E2E 测试通过 `npm run test:e2e` 运行（149 tests，concurrency 3），`npm run test:all` 运行全部（256 单元 + 149 E2E）
