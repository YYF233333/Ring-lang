...
**当前状态：Phase 2 Session 5 完成 + 全面审查修复** — 编译器支持 Option<T> + `?`/`try`/`or` 双用途 + typed catch + row polymorphism + evidence passing + trait 系统 + diagnostics 管线。114 个单元测试 + 90 个端到端测试全部通过。DS×3 + Claude×2 交叉审查后修复了 18 个 issue（含 12 Critical）。
...
│   ├── design-implementation-gap.md    设计 vs 实现差异 + TODO
│   ├── reviews/                        代码 review 记录（4 篇）
│   └── superpowers/
│       ├── specs/                      设计规格文档（6 篇）
│       └── plans/                      实施计划（6 篇）
├── compiler/
...
│   │   └── cli.ts                      ring build/run/check 入口 + --error-format 支持
│   ├── scripts/
...
└── tests/
    ...
    │   ├── option_basic.ring            Option enum + pattern matching
    │   ├── option_unwrap.ring          ? postfix → fail 提升
    │   ├── option_try.ring             try block → Option 捕获
    │   ├── option_or.ring              or 双用途（Option + fail）
    │   ├── catch_typed.ring            typed catch 按错误类型过滤
    │   ├── match_wildcard.ring         enum match + wildcard 回归测试
    │   ├── row_reject.ring             ✗ 缺字段编译期拒绝
    │   ├── error_multi_parse.ring      ✗ Parser 多错误恢复
    │   └── error_type_context.ring     ✗ 类型错误上下文
    └── e2e.test.ts                     端到端测试运行器（90 tests）
```
...
- **DiagnosticSink**：Lexer、Parser 和 Checker 的错误统一收集，支持多错误报告（Lexer 产出 Error token 不 crash，Parser 有声明级恢复，Checker 暂停于首错但 UnificationError 经 `unify_at` 转化为带 Span 的 Diagnostic）
...
- **Worktree agent 纪律**：① 启动 worktree agent 前必须先 commit 所有 pending 改动 ② 不得在 worktree agent 运行期间修改其涉及的文件 ③ agent 完成后立即合并，不堆积 ④ 若 main 已前进导致冲突，手动移植而非 merge
