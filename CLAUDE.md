...
**当前状态：Phase 2 全部完成 + Phase 3a Batch 1-7 完成 + 自举准备修复进行中** — 编译器支持 Option<T>（统一为 EnumType） + `?`/`try`/`or` 双用途 + typed catch + row polymorphism + evidence passing + trait 系统（含高阶 dict closure + TypeScheme bounds 传递） + diagnostics 管线 + declaration 级多错误恢复 + **LSP server（def_id 作用域感知 + 跨模块类型信息）+ VSCode 插件** + **let 不可变性强制（E0205）** + **while 循环 + break/continue（E0206）** + **`for x in 0..N` range 循环（Rust 风格 `..` / `..=` 运算符）** + **Str 内置方法（16 个）** + **List\<T\> 可变集合类型（23 个方法 + `[]` 字面量 + for..in 迭代 + HOF effect 多态）** + **Tuple 类型（`(T1, T2)` 字面量 + 模式匹配 + `let (a, b) = expr` 解构）** + **Map\<K,V\> 可变键值集合（14 个方法 + `map_from`/`map_new`/`map_clone` 构造 + HOF effect 多态）** + **Set\<T\> 可变集合（13 个方法 + 集合运算 + for..in 迭代）** + **if-let 语句（单 arm 模式匹配）** + **模块系统（`use` 导入 + `pub use` 再导出 + `pub` 可见性强制 + 多文件编译 + 单文件 JS bundle）** + **`extern fn` FFI 声明（JS 函数直接映射 + 跨模块导出）** + **标准库（`std/` Ring 源码 + `extern type` 声明 + prelude 自动加载）** + **Enum 命名字段（声明 + 构造 + 模式匹配 + field punning + partial match `..`）** + **递归类型（enum/struct 自引用，穷尽性检查正确处理递归）** + **`var` 参数 + `var self`（可变参数 + impl 方法可变 self + field assignment 不可变性检查）** + **struct update 语法（`{ ..base, field: val }` 浅克隆+覆盖，struct + enum 命名变体均支持）** + **optional field 语法糖（`field?: Type` 脱糖为 `Option<Type>`，构造时可省略默认 `none`，struct + enum 均支持）**。256 个编译器单元测试 + 282 个端到端测试全部通过。
...
│   │   │   └── diagnostics.test.ts     Diagnostics 测试（8 tests）
│   │   ├── lsp/
...
└── tests/
    ...
    └── e2e.test.ts                     端到端测试运行器（265 tests: run + negative + modules + llm-format + determinism，concurrency 3）
```
...
- **Str 内置方法**（16 个）：len, contains, starts_with, ends_with, slice, trim, to_upper, to_lower, replace（全替换 replaceAll）, split, char_at, index_of, byte_at, pad_start, repeat, char_code_at。全部纯函数，通过 `std/str.ring` extern fn 声明，codegen 生成 `Str_method(self, ...)` 运行时调用
...
- **Ring 标准库（`std/` 目录）**：6 个 Ring 源码文件声明内置类型和方法：
  ...
  - `std/str.ring`：`impl Str` 16 个方法
  - `std/num.ring`：`Int::to_str`、`Float::to_str`、`parse_int`、`parse_float`
...
### 自举 Blocker 修复 ✅ — var 参数 + var self (2026-05-20)

- **`var` 参数**：函数参数和 lambda 参数支持 `var` 修饰符。`fn foo(var x: Int)` 声明可变参数，允许函数体内重新赋值。AST `Param` 和 HIR `HParam` 新增 `is_mutable?: boolean` 字段
- **`var self` impl 方法**：`impl Foo { fn method(var self) { self.field = expr } }` 允许方法修改 struct 字段。JS 运行时无需改动（对象本身就是引用传递）
- **字段赋值不可变性检查**：`target.field = expr` 现在检查根变量的可变性。`let p = Point { x: 1 }; p.x = 2` 正确报 E0205 错误。支持嵌套字段链（`self.inner.value = x` 检查 `self` 的可变性）
- **全管线支持**：Parser 解析 `var` 关键字 → Checker 注册 `mutable_vars` + 检查 field_access 赋值 → Zonk 传播 → LSP hover 显示 `var` 前缀
- 1 个正向 e2e 测试 + 2 个负向测试（不可变 self 字段赋值 + let 绑定字段赋值）

### 自举 Blocker 修复 ✅ — struct update 语法 (2026-05-20)

- **struct update 语法**：`MyStruct { ..base, field: new_val }` 浅克隆+字段覆盖。`..base` 在字段列表开头（JS 风格）。不指定的字段自动从 base 取值
- **enum 命名变体支持**：`Variant { ..base, field: new_val }` 同样支持。类型检查要求 base 表达式与目标 enum 类型统一
- **全管线实现**：Parser 解析 `..expr`（复用 DotDot token）→ AST/HIR `spread?: Expr` 字段 → Checker 推断 spread 类型并统一 + 有 spread 时跳过缺字段检查 → Codegen 生成 `new Struct(base.f1, override_f2)` 或 IIFE 包装复杂表达式 → Zonk/LSP hir-visitor 全部支持
- **Codegen 优化**：简单表达式（标识符/字段访问）直接使用 `base.field`；复杂表达式（函数调用等）用 IIFE `((__su) => new S(__su.f1, val))(complex())` 避免重复求值
- 3 个正向 e2e 测试（基本 + enum 变体 + 泛型）+ 1 个负向测试（类型不匹配报 E0301）

### 自举 Blocker 修复 ✅ — optional field 语法糖 (2026-05-20)

- **`field?: Type` 语法糖**：struct 和 enum 命名变体字段支持 `?` 后缀标记。脱糖为 `field: Option<Type>`。构造时可省略该字段，checker 自动填充 `none` HIR 节点
- **Parser**：字段名后检测 `Question` token，设置 `is_optional` 标记。struct 字段和 enum 命名字段均支持
- **Checker register**：`register_struct` 和 `register_enum` 中，optional 字段的类型自动包装为 `make_option_type(resolved_type)`，并在 StructDef/EnumDef 中记录 `is_optional` / `optional_fields`
- **Checker infer**：构造 struct/enum 时，缺失的 optional 字段不报 E0203，而是自动插入 `none` HIdent（带 `resolved_name: "Option_none"`）到 hfields 列表
- 2 个正向 e2e 测试（struct optional + enum optional）
...
- E2E 测试通过 `npm run test:e2e` 运行（282 tests，concurrency 3），`npm run test:all` 运行全部（256 单元 + 282 E2E）
...
- ✅ **Batch 2**: 核心类型方法（Str 方法 16 个 + List\<T\> 18 个方法 + `[]` 字面量 + for..in 扩展）(2026-05-19)
