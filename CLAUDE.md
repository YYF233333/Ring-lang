...
**当前状态：Phase 2 全部完成 + Phase 3a Batch 1-3 完成** — 编译器支持 Option<T>（统一为 EnumType） + `?`/`try`/`or` 双用途 + typed catch + row polymorphism + evidence passing + trait 系统（含高阶 dict closure + TypeScheme bounds 传递） + diagnostics 管线 + declaration 级多错误恢复 + **LSP server（def_id 作用域感知）+ VSCode 插件** + **let 不可变性强制（E0205）** + **while 循环 + break/continue（E0206）** + **`for x in 0..N` range 循环（Rust 风格 `..` 运算符）** + **Str 内置方法（12 个）** + **List\<T\> 不可变集合类型（18 个方法 + `[]` 字面量 + for..in 迭代 + HOF effect 多态）** + **Tuple 类型（`(T1, T2, ...)` 字面量 + 模式匹配 + `let (a, b) = expr` 解构 + 函数参数/返回值类型注解）**。227 个编译器单元测试 + 212 个端到端测试（含 64 正向 + 14 负向 + 3 LLM 格式）全部通过。
...
└── tests/
    ├── cases/                          端到端测试用例（63 正向 + 14 负向 = 77 个 .ring 文件）
    └── e2e.test.ts                     端到端测试运行器（206 tests: run + check + build + negative + llm-format）
```
...
- **List\<T\> 内置类型**：不可变值语义，StructType 表示，JS Array 底层。方法返回新 List，可变需 `Cell<List<T>>`
...
### Phase 3a Batch 3 ✅ — Tuple 类型 (2026-05-19)

- **Tuple 类型**：`(T1, T2, ...)` 类型注解，类型系统中表示为 `TupleType { kind: "tuple", elements: Type[] }`
- **Tuple 字面量**：`(expr1, expr2, ...)` 表达式，编译为 JS 数组。单元素括号表达式 `(expr)` 不是 tuple
- **Tuple 模式匹配**：match arm 中 `(pat1, pat2, ...)` 模式，codegen 生成 `Array.isArray(t) && t.length === N && ...`
- **Tuple 解构**：`let (a, b) = expr` 语句（新增 `LetDestructureStmt` / `HLetDestructureStmt` AST/HIR 节点），binding 变量有 def_id（LSP 跳转/引用/重命名可用）
- **Tuple 函数签名**：函数参数和返回值可使用 tuple 类型注解（`fn f(t: (Int, Str)) -> (Str, Int)`）
- **Tuple 穷尽性检查**：match 时检查 tuple 各列是否穷尽（按列独立检查）
- **Tuple 统一化**：HM unification 支持 tuple 类型（元素数量 + 逐元素 unify）
- **Codegen**：tuple 字面量 → JS 数组 `[e1, e2, ...]`，`let_destructure` → `const __ring_dt = init; const a = __ring_dt[0]; const b = __ring_dt[1]`
- **LSP**：definition/references/hover/completion/rename 全部支持 `let_destructure` binding
- **Parser 换行感知修复**：Pratt 解析器 `(` 作为后缀运算符时检查是否与前缀表达式同行。跨行 `(` 不作为函数调用，避免 `let x = 42\n("y", 1)` 被解析为 `42("y", 1)` 的 bug
- AST + HIR 新增 `LetDestructureStmt` / `HLetDestructureStmt`（含 bindings + def_id）
- 2 个新正向 e2e 测试（tuple_basic.ring + tuple_fn.ring）
...
- `List.contains` / `List.find` 等使用 JS `===` 引用相等——对 primitive 正确，对 struct/enum 仅比较引用。待 Eq trait 实现后升级为结构相等
...
- 无 `loop` 无限循环语法（`while true` 可替代），无 Range/List 以外的自定义迭代器
- 无 CI 管线（lint + test 依赖手动执行）
- LSP server 为单文件编译（无跨文件引用，无增量编译）
- Checker 多错误恢复：declaration 级（同一函数内仍停于首错）
- E2E 测试通过 `npm run test:e2e` 运行（212 tests），`npm run test:all` 运行全部（227 单元 + 212 E2E）
...
- ✅ **Batch 3**: Tuple 类型（`(T1, T2)` 字面量 + 模式匹配 + `let (a, b) = expr` 解构 + 函数签名）(2026-05-19)
- **Batch 4**: 扩展集合（Map\<K,V\> + Set\<T\>）+ 类型窄化（match 窄化）
- **Batch 5**: 模块系统（import/export）
